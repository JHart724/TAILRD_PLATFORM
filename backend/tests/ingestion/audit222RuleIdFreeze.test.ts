...
describe('AUDIT-222 ruleId freeze', () => {
  ...
  it('every gaps.push carries a ruleId', () => {
    ...
    // gap-cad-051-ncs-timing) per the report's 2026-07-31 amendment; +1 gap-vhd-040-valve-cad-revasc (2026-08-18).
    expect(engineIds).toHaveLength(371);
  });
  ...
  it('the adopted/generated split is the recorded one (263 registry / 108 generated)', () => {
    const reg = reportRows.filter((r) => r.provenance === 'registry-binding').length;
    expect({ registry: reg, generated: reportRows.length - reg }).toEqual({ registry: 263, generated: 108 });
  });
