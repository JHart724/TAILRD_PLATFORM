...
// Type-safe global auth + MFA on all PHI-accessing /api/* routes.
// authenticateToken accepts Request base type (no 'as any' cast needed).
// Mounted AFTER auth, sso, smart, webhooks (which have their own auth chains).
// Per-route authenticateToken/requireMFA in individual routers are now redundant
// but left in place as documentation.
app.use('/api', authenticateToken, requireMFA);
...
app.use('/api/registry', require('./routes/registry').default); // AUDIT-148 Slice 2: registry-case read endpoint
