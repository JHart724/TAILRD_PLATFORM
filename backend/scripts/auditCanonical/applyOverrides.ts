...
export const OVERRIDES: Record<ModuleCode, Record<string, Override>> = {
  HF: {
    ...
    // removed; the cited evaluator block is now a marker comment). HF-37-FU + HF-38 are PERMANENT retires and
    // stay SPEC_ONLY. HF-74/HF-91 have NO crosswalk row (registry-only orphans, no override). See CLAUDE.md §20.
    // AUDIT-194-B1 (2026-07-01): HF-90's 6 rows were reverted OUT of SPEC_ONLY (its detector RESTORED - nt_probnp
    // threaded), so they revert to their reconcile-derived PARTIAL (no override needed - removed below). ---
    'GAP-HF-004': { classification: 'SPEC_ONLY', auditNote: 'AUDIT-194 Part A 2026-06-30: HF-37-FU (discharge-followup, bare hasHF with no discharge/followup signal, non-functional) RETIRED -> SPEC_ONLY; gaps.push removed. DET_OK -> SPEC_ONLY. PERMANENT retire.' },
    'GAP-HF-040': { classification: 'SPEC_ONLY', auditNote: 'AUDIT-194 Part A 2026-06-30: HF-38 (influenza, bare hasHF, Z23 never in conditions - IDENTICAL to already-retired CAD-INFLUENZA, the section-20 pattern-class-sweep precedent) RETIRED -> SPEC_ONLY; gaps.push removed. DET_OK -> SPEC_ONLY. PERMANENT retire.' },
    // GAP-HF-051/052/053/055/056/134 (HF-90 amyloid) RESTORED to PARTIAL_DETECTION 2026-07-01 (AUDIT-194-B1):
    // HF-90's runtime gaps.push is live again (nt_probnp LOINC 33762-6 threaded, no longer hollow), so these
    // revert from the AUDIT-194 interim SPEC_ONLY back to their pre-AUDIT-194 PARTIAL_DETECTION.
    'GAP-HF-051': { classification: 'PARTIAL_DETECTION', registryId: 'gap-hf-90-amyloid-biomarker', auditNote: 'AUDIT-194-B1 2026-07-01: HF-90 amyloid biomarker RESTORED (nt_probnp LOINC 33762-6 threaded) -> re-bind to gap-hf-90-amyloid-biomarker, SPEC_ONLY reverts to PARTIAL_DETECTION.' },
    'GAP-HF-052': { classification: 'PARTIAL_DETECTION', registryId: 'gap-hf-90-amyloid-biomarker', auditNote: 'AUDIT-194-B1 2026-07-01: HF-90 RESTORED (nt_probnp threaded) -> PARTIAL_DETECTION.' },
    'GAP-HF-053': { classification: 'PARTIAL_DETECTION', registryId: 'gap-hf-90-amyloid-biomarker', auditNote: 'AUDIT-194-B1 2026-07-01: HF-90 RESTORED (nt_probnp threaded) -> PARTIAL_DETECTION.' },
    'GAP-HF-055': { classification: 'PARTIAL_DETECTION', registryId: 'gap-hf-90-amyloid-biomarker', auditNote: 'AUDIT-194-B1 2026-07-01: HF-90 RESTORED (nt_probnp threaded) -> PARTIAL_DETECTION.' },
    'GAP-HF-056': { classification: 'PARTIAL_DETECTION', registryId: 'gap-hf-90-amyloid-biomarker', auditNote: 'AUDIT-194-B1 2026-07-01: HF-90 RESTORED (nt_probnp threaded) -> PARTIAL_DETECTION.' },
    'GAP-HF-134': { classification: 'PARTIAL_DETECTION', registryId: 'gap-hf-90-amyloid-biomarker', auditNote: 'AUDIT-194-B1 2026-07-01: HF-90 RESTORED (nt_probnp threaded) -> PARTIAL_DETECTION.' },
  ...
  CAD: {
    ...
    'GAP-CAD-063': { classification: 'SPEC_ONLY', auditNote: 'MANUAL OVERRIDE (classification CONFIRMED SPEC_ONLY with recorded reason) - DEFERRED 2026-07-31 (Tranche 3 Slice 1): DATA WALL - STEMI door-to-balloon needs first-medical-contact and device TIMESTAMPS; procedures carry a single procedureDate (no times). Permanent under the current ingest substrate.' },
    'GAP-CAD-064': { classification: 'SPEC_ONLY', auditNote: 'MANUAL OVERRIDE (classification CONFIRMED SPEC_ONLY with recorded reason) - DEFERRED 2026-07-31 (Tranche 3 Slice 1): DATA WALL - transfer D2B needs FMC/transfer/device timestamps; same single-date substrate wall as GAP-CAD-063.' },
    'GAP-CAD-075': { classification: 'SPEC_ONLY', auditNote: 'MANUAL OVERRIDE (classification CONFIRMED SPEC_ONLY with recorded reason) - DEFERRED 2026-07-31 (Tranche 3 Slice 1): DATA WALL - acute stent thrombosis needs a chest-pain/ST-changes event stream within 24h of PCI; no symptom/ECG event data and no intra-day granularity in the substrate.' },
    'GAP-CAD-076': { classification: 'SPEC_ONLY', auditNote: 'MANUAL OVERRIDE (classification CONFIRMED SPEC_ONLY with recorded reason) - DEFERRED 2026-07-31 (Tranche 3 Slice 1): DATA WALL - stent-thrombosis dx (T82.86*) count is ZERO in the substrate (measured live) and DAPT fill/adherence data is absent; the eligible denominator is 0, an evaluator could never fire.' },
    'GAP-CAD-053': { classification: 'SPEC_ONLY', auditNote: 'MANUAL OVERRIDE (classification CONFIRMED SPEC_ONLY with recorded reason) - DEFERRED 2026-07-31 (Tranche 3 Slice 1): DATA WALL - radial-vs-femoral access site is not in any ingested field.' },
    'GAP-CAD-054': { classification: 'SPEC_ONLY', auditNote: 'MANUAL OVERRIDE (classification CONFIRMED SPEC_ONLY with recorded reason) - DEFERRED 2026-07-31 (Tranche 3 Slice 1): DATA WALL - discharge timing needs encounters; the encounters table holds 0 rows for the tenant (measured live). Also flagged: same-day-discharge rests on a 2021 SCAI expert consensus, not an ACC/AHA/ESC COR-graded recommendation (section 8 grounding bar).' },
    'GAP-CAD-055': { classification: 'SPEC_ONLY', auditNote: 'MANUAL OVERRIDE (classification CONFIRMED SPEC_ONLY with recorded reason) - DEFERRED 2026-07-31 (Tranche 3 Slice 1): DATA WALL + REVERSE-HOLLOW - post-CABG follow-up needs encounters (0 rows for the tenant, measured live); an absence-of-follow-up rule would fire for 758/758 CABG patients (~100%), the AUDIT-194 hollow signature in reverse. Also flagged: no ACC/AHA COR-graded anchor (STS practice guidance).' },
    // --- Tranche 3 CANDIDATE A (stress testing) CLOSED data-insufficient 2026-07-31. The standing ruling was
    // conditional - thread ONLY if stress-test RESULT values exist in source; presence-only closes the
    // candidate (the AUDIT-197 lesson: CAD-ISCHEMIA-GUIDED was retired precisely because hasModerateIschemia
    // was bound to stress_test_months !== undefined, i.e. test-PRESENCE read as positive ischemia).
    // THREE-WAY PROBE, measured live on demo-synthea-threaded, all negative:
    //   (i)   observation types matching stress/METs/exercise/Duke/capacity/watts, tenant-wide: ZERO types;
    //   (ii)  stress procedures with ANY observation within +/-1 day: 3 of 412 (0.7%), and those 3 carry only
    //         the routine panel (alt/ast/bmi/creatinine/egfr/hba1c/ldl/bp/hr/hgb/triglycerides) - a
    //         coincidental same-day wellness draw, NOT stress results;
    //   (iii) no result-bearing structured field on the Procedure row (code/name/status/date/performedBy only).
    // The 412 rows are bare procedure occurrences. Threading on presence alone would rebuild the AUDIT-197
    // defect. All three gaps stay SPEC_ONLY with the measured record. ---
    // (The three Candidate A gaps already carry AUDIT-184 / AUDIT-197 override entries later in this same
    // block; the measured record is APPENDED to those entries rather than added as new keys - duplicate keys
    // in an object literal silently lose to the later one, and tsc does not flag it. See DRIFT-60.)
    'GAP-CAD-007': {
      classification: 'PARTIAL_DETECTION',
  ...
  VHD: {
    ...
    'GAP-VHD-040': {
      classification: 'PARTIAL_DETECTION',
      registryId: 'gap-vhd-040-valve-cad-revasc',
      auditNote:
        'BUILT 2026-06-21, LANDED 2026-08-18 (recovered from the unmerged feat/t3-valve-cpt-vhd040 branch during the AUDIT-321 stale-branch sweep): purpose-built evaluator - prosthetic-valve presence (Z95.2 mechanical / Z95.3 bioprosthetic / Z95.4 other) + CAD (I25.*) + NO coded revascularization (no Z95.1 aortocoronary-bypass status, no Z95.5 PCI status), hospice-excluded. 2020 ACC/AHA VHD + 2021 ACC/AHA Coronary Revascularization, COR 1 LOE C-LD. SPEC_ONLY -> PARTIAL_DETECTION, not DET_OK, per section 16 underclaim-governs: the spec target is CAD NEEDING REVASCULARIZATION and severity is not precisely coded, so I25.* presence is a Path-B proxy and the rule surfaces the population for review rather than asserting the indication (the limitation is stated in the rule body and its evidence note). The registryId pin is required because the registry id gap-vhd-040-valve-cad-revasc is a naming mismatch against the spec id GAP-VHD-040, so reconcile cannot pair them - the same mechanism used by the other naming-mismatch pins in this file. RUNTIME-INERT ON THE CURRENT SUBSTRATE AND COUNTED ANYWAY, DELIBERATELY: Z95* device-status conditions are 0 rows / 0 patients tenant-wide (INGESTION_SOURCE_UNLOCK_REQUIREMENTS.md), so this rule fires zero times today. It FAILS SAFE - it gates on POSITIVE presence of Z95.2/3/4, so an absent signal yields no fire, never a false fire; it is NOT the section 20 hollow-over-fire class. Coverage measures what is BUILT, which is the definition applied to all 312 prior gaps, so withholding the count here would make the number mean something different for this one rule. AUDIT-322 is the control that stops the built figure being read as capability, and this override is recorded there as its first instance.',
    },
  ...
  PV: {
    ...
    // --- AUDIT-199-B (2026-07-09): dose parsed from DESCRIPTION -> the dose-blind false-negative is CLOSED -> DET_OK ---
    'GAP-PV-008': {
      classification: 'DET_OK',
      registryId: 'gap-pv-1-pad-statin',
      auditNote:
        'AUDIT-199 2026-07-08: DET_OK -> PARTIAL_DETECTION (PV-1 is the un-propagated section-20 sibling of the AUDIT-184 CAD-EXT dose-unknown suppression - the gate now excludes agent_dose_unknown so the ~100%-of-PAD dose-unknown over-fire is gone, keeping only the genuine not_high_intensity gap; PARTIAL because the moderate-dose-should-intensify cohort was still invisible while dose was unparsed). AUDIT-199-B 2026-07-09: PARTIAL_DETECTION -> DET_OK. csvParser.parseDoseFromDescription now parses the mg out of the medications.csv DESCRIPTION into a persisted doseValue, so highIntensityStatinStatus discriminates on real dose: on_high_intensity (atorva>=40 / rosuva>=20) suppressed, the moderate-dose (atorva 10/20/30, rosuva 5/10) + no-agent cohorts fire. The dose-blind false-negative that held it at PARTIAL is CLOSED on the CSV (Synthea) audit path -> DET_OK is honest. Documented residuals (do NOT re-open PARTIAL): compound-statin combos (Caduet/Vytorin, statin listed second) parse to null -> agent_dose_unknown -> suppressed (rare edge); the Redox real-EHR path still sets no numeric doseValue (fhirResourceHandlers dose string only) -> tracked as AUDIT-199-B-REDOX, a threading-completeness follow-on not a rule defect. Statins are once-daily so tablet-mg == daily-dose (no BID ambiguity). Sibling GAP-CAD-001/002 were already DET_OK (now genuinely so). 2024 ACC/AHA PAD + 2018 AHA/ACC Cholesterol high-intensity tier. gaps.push unchanged.',
    },
