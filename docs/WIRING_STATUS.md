...
- EP, Coronary, Structural, Valvular, Peripheral module views (5 of 6 modules)
  ...
  - **CAD (Coronary Intervention) Executive tier - PARTIAL (AUDIT-304 CAD convergence, in PR `feat/cad-exec-convergence`, 2026-07-11):** the new `CADExecutiveSummary` reads the live CAD dashboard contract (`useModuleDashboard('coronary-intervention')` -> `CADDashboardData`: patient total / open gaps / device candidates) with honest loading/error/empty states, and the Gap Intelligence headline reads the live `totalOpenGaps` across the real `totalPatients` (killing the 2,330 category-sum mislabel; the `?? 26` default + the config-fallback-behind-live-field are retired). All revenue/financial surfaces (the single consolidated ROI waterfall, DRG/CMI, P-v-R, benchmarks, forward outlook) remain DEMO by design (no backend revenue source; reconciled to one derived `cadDemoFinancials` model at $11.2M gap-closure opportunity, each surface DemoDataBadge-marked; the two program-revenue waterfalls consolidated to one). Rest of CAD (Service Line, Care Team) still hardcoded mock. Endpoint already emits the wired fields (no backend change).
  - **VHD (Valvular Disease) Executive tier - PARTIAL (AUDIT-304 VHD convergence, in PR `feat/vhd-exec-convergence`, 2026-07-12):** the new `VHDExecutiveSummary` reads the live VHD dashboard contract (`useModuleDashboard('valvular-disease')` -> `VHDDashboardData`: patient total / open gaps / device candidates) with honest loading/error/empty states, and the Gap Intelligence headline reads the live `totalOpenGaps` across the real `totalPatients` (killing the 372 category-sum mislabel; the `?? 6` default + the config-fallback-behind-live-field are retired). All revenue/financial surfaces (the single re-scoped ROI waterfall, DRG/CMI, P-v-R, benchmarks, forward outlook) + the 5 detail modals remain DEMO by design (no backend revenue source; reconciled to one derived `vhdDemoFinancials` model at $5.3M gap-closure opportunity, each surface DemoDataBadge-marked; the waterfall total re-scoped to equal its category sum). Rest of VHD (Service Line, Care Team) still hardcoded mock. Endpoint already emits the wired fields (no backend change).
  - **PV (Peripheral Vascular) Executive tier - PARTIAL (AUDIT-304 PV convergence, in PR `feat/pv-exec-convergence`, 2026-07-15; the 6th and FINAL Exec view of the arc):** the new `PVExecutiveSummary` reads the live PV dashboard contract (`useModuleDashboard('peripheral-vascular')` -> `PVDashboardData`: patient total / open gaps / device candidates) with honest loading/error/empty states, and the Gap Intelligence headline reads the live `totalOpenGaps` across the real `totalPatients` (killing the 1,150 category-sum mislabel; the `?? 12` default + the config-fallback-behind-live-field are retired). PV's special case: the Revenue-Opportunity KPI card keeps its REGISTRY-derived $8.1M value (= summed `PV_CLINICAL_GAPS` dollar opportunity, what `config.kpiData.totalOpportunity` already rendered) behind a distinct Registry pill, set apart from the demo-model cards. All other revenue/financial surfaces (the single re-scoped ROI waterfall, DRG/CMI, P-v-R, benchmarks, forward outlook) + the 5 detail modals remain DEMO by design (no backend revenue source; reconciled to one derived `pvDemoFinancials` model at $8.1M gap-closure opportunity, each surface DemoDataBadge-marked; the waterfall total re-scoped to equal its category sum, killing the 42.3M-vs-40.8M break; the ~$42M program-revenue literals removed incl. the export CSV). Rest of PV (Service Line, Care Team) still hardcoded mock. Endpoint already emits the wired fields (no backend change). **With PV, all 6 Executive tiers are converged (AUDIT-099 non-HF core CLOSED).**
