...
Expected: `af4b969 fix(audit-016-pr3-step-1-7): spotcheck-decrypt reads canonical purpose 'phi-encryption' + author missing §10.7 override file (closes verification-tooling gap caught at attempt-4 §10.7 invocation; bundles DRIFT-35 + §17.1 catalog addition) (#278)`.
...
Expected: `TaskDef` ends `tailrd-backend:194`, `Status=PRIMARY`, `RolloutState=COMPLETED`, `DesiredCount=1`, `RunningCount=1`.
...
aws ecs run-task `
  --cluster tailrd-production-cluster `
  --task-definition tailrd-backend:194 `
  --launch-type FARGATE `
...
### 10.12 Production execution history (CloudWatch `auditLogger` evidence; service `tailrd-audit`)

The STEP 1.7 rekey production-execute history, reconstructed from the AUDIT-013 `auditLogger` CloudWatch transport (`/ecs/tailrd-production-backend`, independent of the AUDIT-109 stdout transport that landed 2026-06-05):

1. **2026-05-15 (Day 15) - `audit_logs.description` execute HALTED** (pre-cursor-fix infinite loop; see 10.11.1). 0 rekeyed; operator-halted at 2h45min. No corruption.
2. **2026-05-16T00:01-02:57Z - FULL SWEEP (`PHI_REKEY_RUN_START ... targets=82` -> `PHI_REKEY_RUN_COMPLETED mode=execute rekeyed=495362 skipped=150 failed=0`).** Ran post-cursor-fix (PR #276) but PRE-AUDIT-115-fix, so under the (defective) all-skip-per-batch abort. Per-target `PHI_REKEY_TARGET_COMPLETED` evidence reconciles exactly: **21 targets fully rekeyed to canonical** (their `rekeyed` counts sum to precisely 495,362; incl. `conditions.conditionName` 225,439 + `medications.medicationName` 220,552 + `patients.{lastName,dateOfBirth,phone,mrn,street,city,state,zipCode}` 6,147 each), **3 targets `PHI_REKEY_BATCH_ALL_SKIPPED`** (rekeyed=0; `skipped=150 = 3 x 50` batch-1 aborts), and **58 targets attempted=0** (no V2 rows that day): 21 + 3 + 58 = 82. [Correction 2026-06-07: the earlier "27 of 29 fully rekeyed" framing was an overcount; the run log shows 21 fully-rekeyed targets. The current 29-target V2>0 inventory = 24 V2-bearing targets touched 2026-05-16 (21 rekeyed + 3 abort-skipped) + 5 targets that gained V2 rows only AFTER 2026-05-16 (`recommendations.{title,description,evidence}` + `users.{firstName,lastName}`, post-rekey canonical-middleware writes); 24 + 5 = 29.] The **3 `PHI_REKEY_BATCH_ALL_SKIPPED`** targets: `patients.firstName`, `audit_logs.description`, `audit_logs.newValues` - the AUDIT-115 silent-skip victims. Of the three, only `patients.firstName` had a real legacy tail behind its canonical seed (surfaced ~3 weeks later as the AUDIT-113 HF-worklist 500).
3. **2026-06-06 - AUDIT-113 firstName remediation** (post-AUDIT-115-fix image, task def `:267`): `--execute --target patients.firstName` -> `rowsRekeyed=5780 skipped=367 failed=0`; pre-execute snapshot `audit-113-pre-rekey-firstname-20260606`.
4. **2026-06-06 - corrective full-inventory sweep closure** (post-AUDIT-115-fix; pre-sweep snapshot `audit-016-rekey-sweep-pre-20260606`): re-executed the two remaining abort-skipped targets - `audit_logs.description` (`attempted=7305 rekeyed=0 skipped=7305 failed=0`) + `audit_logs.newValues` (`attempted=7101 rekeyed=0 skipped=7101 failed=0`): **both already fully canonical** (no legacy tail; the 2026-05-16 abort fired on a genuinely all-canonical batch-1). Verify-by-rekey of new-since-2026-05-16 targets `recommendations.{title,description,evidence}` + `users.{firstName,lastName}` = `rekeyed=0/skipped=all`. Canonical-decrypt verification, two layers: (1) spotcheck across all 29 current-V2 targets, 1,521 samples / 0 failures; (2) **FULL canonical-context decrypt probe (2026-06-07, ECS task `76e6a35f`, read-only, zero DB writes, zero plaintext logged; evidence script `docs/audit/sweeps/step3-canonical-decrypt-probe.js`)** over EVERY V2 envelope of both abort-skipped audit_logs targets: `audit_logs.description` 7,305/7,305 ok + `audit_logs.newValues` 7,101/7,101 ok = 14,406 envelopes / **0 failures** (rules out a hidden third-purpose/poison envelope class invisible to the skip-canonical scan). STEP-4 census of `audit_logs.newValues` non-V2 rows (2026-06-07, ECS task `abd4a721`, read-only, no decrypt, no PHI; evidence script `docs/audit/sweeps/step4-newvalues-census.js`): total 7,305 / v2=7,101 / **v0=0 / v1=0** / non-v2=204 = 203 JSON-`null` literals + 1 SQL NULL / real-plaintext=0 (the 204 carry no encryptable content). **STEP 1.7 V2->V2 purpose rekey is COMPLETE across all 82 targets; §10.7 purpose-carryover fully resolved.**

**Known latent defect surfaced during closure (now numbered AUDIT-116):** the `kind: 'json'` discovery/count filters in `audit-016-pr3-v0v1-to-v2.ts` do not account for the JSON `"`-quote prefix (mis-classified `audit_logs.newValues` as `v2=0 plaintext=7304` vs the correct quote-aware STEP-4 census `v2=7101 / v0=0 / v1=0 / JSON-null=203 / SQL-null=1 / real-plaintext=0`). Moot for `audit_logs.newValues` (0 v0/v1), but was latent across all 28 JSON targets. Tracked as **AUDIT-116** in the findings register - **RESOLVED 2026-06-07**: the `kind:'json'` discovery/count filters are now quote-aware (`'"enc:%'` / `'"enc:v2:%'` + `#>>'{}'` extraction, mirroring this rekey script) with 3 tests; the 28-target JSON census (ECS task `0a1b286c`) confirmed **0 v0/v1 across all 28 targets**, so severity downgraded MEDIUM (P2) -> LOW (P3) and the realized impact was cosmetic count mis-bucketing only.
