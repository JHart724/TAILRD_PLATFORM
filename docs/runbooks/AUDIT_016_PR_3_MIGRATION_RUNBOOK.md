...
2. **2026-05-16T00:01-02:57Z - FULL SWEEP (`PHI_REKEY_RUN_START ... targets=82` -> `PHI_REKEY_RUN_COMPLETED mode=execute rekeyed=495362 skipped=150 failed=0`).** Ran post-cursor-fix (PR #276) but PRE-AUDIT-115-fix, so under the (defective) all-skip-per-batch abort. Per-target `PHI_REKEY_TARGET_COMPLETED` evidence: **27 of 29 non-zero-V2 targets fully rekeyed to canonical** (incl. `conditions.conditionName` 225,439 + `medications.medicationName` 220,552 + `patients.{lastName,dateOfBirth,phone,mrn,street,city,state,zipCode}` 6,147 each). **3 targets `PHI_REKEY_BATCH_ALL_SKIPPED` (rekeyed=0; `skipped=150 = 3 x 50` batch-1 aborts): `patients.firstName`, `audit_logs.description`, `audit_logs.newValues`** - the AUDIT-115 silent-skip victims. Of the three, only `patients.firstName` had a real legacy tail behind its canonical seed (surfaced ~3 weeks later as the AUDIT-113 HF-worklist 500).
...
4. **2026-06-06 - corrective full-inventory sweep closure** (post-AUDIT-115-fix; pre-sweep snapshot `audit-016-rekey-sweep-pre-20260606`): re-executed the two remaining abort-skipped targets - `audit_logs.description` (`attempted=7305 rekeyed=0 skipped=7305 failed=0`) + `audit_logs.newValues` (`attempted=7101 rekeyed=0 skipped=7101 failed=0`): **both already fully canonical** (no legacy tail; the 2026-05-16 abort fired on a genuinely all-canonical batch-1). Verify-by-rekey of new-since-2026-05-16 targets `recommendations.{title,description,evidence}` + `users.{firstName,lastName}` = `rekeyed=0/skipped=all`. Canonical-decrypt spotcheck across all 29 non-zero-V2 targets: 1,521 samples / 0 failures. **STEP 1.7 V2->V2 purpose rekey is COMPLETE across all 82 targets; §10.7 purpose-carryover fully resolved.**

**Known latent defect surfaced during closure (separate finding):** the `kind: 'json'` discovery/count filters in `audit-016-pr3-v0v1-to-v2.ts` do not account for the JSON `"`-quote prefix (mis-classified `audit_logs.newValues` as `v2=0 plaintext=7304` vs the correct `v2=7101 plaintext-json=203 null=1`). Moot for `audit_logs.newValues` (0 v0/v1), but latent across all 28 JSON targets. Tracked as `AUDIT-XXX-future-v0v1-json-discovery-filter` in the findings register.
