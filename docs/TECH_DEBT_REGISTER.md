...
### 32. Aurora schema drift during the migration window (Wave 2 Attempt 3 lessons) - **RESOLVED 2026-04-27**
- **Severity:** N/A (already addressed; recorded as a learning entry)
- **Discovered:** 2026-04-27 during Wave 2 Attempt 3 inventory
- **Original impact:** The original Wave 2 + Wave 3 segmentation assumed Wave 1 data (`hospitals`, `users` loaded April 20) persisted on Aurora. The Day 7 chaos test on 2026-04-21 TRUNCATE'd Aurora's `hospitals` + `users` tables; that erasure was never reverted. Compounding: subsequent Prisma migrations (`20260425000000_audit_log_hospital_nullable` and `20260427000000_add_performance_request_log`) applied to RDS only via the deploy workflow's container CMD - Aurora's schema fell behind. Three Wave 2 attempts each surfaced a distinct downstream symptom:
  - Attempt 1 (2026-04-23T23:17Z): leftover `awsdms_*` validator control tables on Aurora caused `42P07 duplicate_table` cascade
  - Attempt 2 (2026-04-24T05:35Z): `SupportLobs: false` dropped TEXT columns from CSV unload, causing positional COPY misalignment (booleans into enum slots)
  - Attempt 3 (2026-04-27T17:32Z): FK violations from missing parent rows on Aurora (`patients_hospitalId_fkey` rejected because `hospitals` was empty) plus `performance_request_logs` schema drift (today's PR #184 RDS-only table)
- **Resolution (2026-04-27):**
  - Combined Wave 2 + Wave 3 + 4 into a single DMS task covering all 21 NEEDS_MIGRATION tables (~2.1M rows)
  - Created `infrastructure/scripts/phase-2d/inventoryRdsAurora.js` - RDS-vs-Aurora cross-database table inventory; surfaces drift before each migration wave
  - Created `infrastructure/scripts/phase-2d/applyAuroraSchemaParity.js` - `prisma migrate deploy` against Aurora + DROP stale artifacts + `information_schema.columns` parity check; verdict CLEAN before each wave
  - Documented schema-parity-during-migration-window requirement in `docs/DMS_MIGRATION_PLAN.md` §13
  - Dropped stale `chaos_test_day7` artifact from Aurora
- **Verification:** Aurora `_prisma_migrations` table now matches RDS exactly (3 migrations: April 20 baseline, April 25 audit_log nullable, April 27 PerformanceRequestLog). Schema parity check across all 21 tables: 21/21 matches, 0 column-level mismatches. `chaos_test_day7` dropped. Aurora ready to receive Wave 2 combined data.
- **Lesson encoded:** pre-flight verification for any future migration wave must include (a) target table emptiness (existing check), (b) **parent table population check** (new - closes Attempt 3's FK gap), (c) **schema parity check** (new - closes Attempt 3's drift gap). All three are now run together via `applyAuroraSchemaParity.js` before each wave.

### 33. DMS parallel-load FK race condition (Wave 2 Attempt 4 sub-finding) - **RESOLVED 2026-04-27**
- **Severity:** N/A (already addressed; recorded as institutional memory)
- **Discovered:** 2026-04-27T18:33Z during Wave 2 Attempt 4 combined-scope full-load
- **Original impact:** DMS full-load runs tables in parallel (default `MaxFullLoadSubTasks=8`). Without `load-order` attributes on selection rules, DMS chooses an internal scheduling order. Result: 11 of 22 tables errored on FK violations because parent tables (hospitals, users, patients) hadn't yet committed when child loads attempted INSERT. Big tables (procedures 971k, encounters 353k, medications 220k) loaded fine because they happened to be sequenced after their parents in DMS's internal order; smaller children were unlucky.
- **Affected tables (11):** `alerts`, `allergy_intolerances`, `audit_logs`, `conditions`, `contraindication_assessments`, `device_implants`, `drug_titrations`, `error_logs`, `intervention_tracking`, `login_sessions`, `performance_request_logs`. All have FKs to `patients`, `users`, or `hospitals`.
- **Resolution:** `aws dms reload-tables` with the 11 errored tables (split into 2 batches due to the 10-tables-per-request API cap) at 2026-04-27T18:38:11Z. Parents had committed by then, so reload succeeded for all 11 within ~1 minute. Final state: 22 tables loaded with exact source-row-count match (1,819,605 rows total, 0 drift). DMS task continues running with CDC active (`yfggbh5lxrhdbhyd_..._a807_ef3e6876b3ba` slot, `active=true`, plugin `test_decoding`).
- **Lesson encoded:** for any future migration with FK-rich schemas, mapping JSON should include `load-order` attribute on `selection` rules. Parent tables get higher numbers (e.g., 100), child tables lower (e.g., 50). Forces serial parent-before-child ordering during full-load. Avoids the reload-tables recovery cycle.
