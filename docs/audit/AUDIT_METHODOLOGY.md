...
## 22. Coverage-of-an-Invariant Discipline (AUDIT-203 / AUDIT-204 / AUDIT-208 catalyst, 2026-07-18)

**Statement.** When a change establishes or relies on an invariant, the SAME change establishes the mechanism that measures the invariant's COVERAGE. An invariant without a coverage check is an assumption, and its violations are found by accident.

### 22.1 The evidence: three consecutive instances, all found by accident

Each was surfaced while reading code for an UNRELATED reason. None was found by the discipline that owned it.

- **AUDIT-203 (audit coverage).** `writeAuditLog` existed on most PHI-mutating routes, so the audit-trail invariant was believed held. Nobody audited WHICH routes. Four routes were silently unaudited - `referrals.ts`, `clinicalIntelligence.ts`, `phenotypes.ts`, `accountSecurity.ts` - including a contraindication OVERRIDE, the single highest-stakes clinical write on the platform. Found while reading `referrals.ts` as the precedent for AUDIT-148 Slice 3. **Durable fix:** the `auditTrailClass` suite, which asserts an audit write per mutation against the REAL `HIPAA_GRADE_ACTIONS` set (a stub would pass regardless; the real set makes the assertion load-bearing).
- **AUDIT-204 (type-check coverage).** TypeScript checking was on, so the type-safety invariant was believed held. Nobody audited WHICH files opted out via `@ts-nocheck`. Found during a 3-check scan on an unrelated PR. **Durable fix:** `backend/scripts/checkTsNocheck.ts` wired into CI, cross-checked against the section-14 sanctioned-exceptions list.
- **AUDIT-208 (contract coverage).** Backend endpoints existed and `api.ts` clients existed, so the frontend-backend contract was believed held. Nobody audited WHICH were CONSUMED. Approximately 3% of the API reaches a rendered surface; 16 clients exist uncalled; `useTrials` and `useRegistryCases` are dead hooks with zero component callers. Found during a pre-demo environment check. **Durable fix:** pending, recorded in the finding.

### 22.2 The rule

**Invariant and coverage-check ship together.** A coverage check that satisfies this section requires all three of:

- **(a) A PRECISE detector, not a coarse grep.** The AUDIT-204 FALSE POSITIVE is the cautionary instance: `grep -rl "@ts-nocheck"` matched the literal token inside JSDoc PROSE and produced a HIGH-P1 finding against a file that was fully type-checked. A detector must model the thing it claims to detect (there, TypeScript's actual rule: a leading `//` line directive before any code), not merely the string that usually accompanies it.
- **(b) An explicit sanctioned-exceptions list.** Legitimate exceptions exist (the two section-14 runners; genuinely server-only endpoints such as webhooks / cdsHooks / sso callbacks). Without an allowlist the gate is noisy and gets disabled, which is worse than no gate. The allowlist must live beside the detector AND in the governing doc, so adding to one without the other is itself visible drift.
- **(c) CI ENFORCEMENT rather than vigilance.** All three instances above were found by accident precisely because the invariant depended on someone remembering. A coverage check is only durable when a machine runs it on every PR.

### 22.3 Applying it

At PAUSE A, for any change that establishes or relies on an invariant, state the invariant AND name the mechanism that will measure its coverage. If no such mechanism exists or is being built, that is a scope decision to surface, not an omission to leave silent. Cross-references: section 10 (CI enforcement model - where a coverage check is wired); section 17 (PR acceptance criteria); section 18 (status-surface discipline); CLAUDE.md section 20 (pattern-class sweep - the sibling discipline for a defect already found; this section is its preventive counterpart).

## 23. Measurement-Validity Discipline (2026-07-18 reconciliation catalyst)

**Statement.** Measure the quantity that DRIVES THE DECISION, not the quantity that is easy to count. Verify a metric's construct validity - does it actually measure the thing the decision turns on - before acting on it.

### 23.1 The evidence: two instances from the 2026-07-18 reconciliation

- **Stale-PR risk measured as distance-behind.** The raw behind-count for PR #422 read `228 files changed, 7,769 insertions, 19,811 deletions`, which looks like a catastrophic revert. That number measures **DISTANCE BEHIND main**, not blast radius. The PR's OWN diff-from-merge-base is **2 files / 7 lines** (a task-def pointer bump). The decision-relevant metric is **own-diff-from-merge-base**, plus a check of **whether the content already landed on main by another path**. Measured correctly, the seven stale PRs resolved to five CLOSE (superseded content, e.g. Phase-0B addenda already regenerated on main) and two REBASE (genuinely unlanded content) - a completely different disposition than the raw counts implied.
- **Register OPEN count measured by token parse.** A naive scan for OPEN-ish tokens read 81-83. The true count was **80**. Six entries are indexed with an OPEN token while carrying a LATER resolution addendum (AUDIT-139, AUDIT-183, AUDIT-197, AUDIT-201, AUDIT-205, AUDIT-206) - the dated-correction-supersedes convention (section 18) means a single index line can legitimately contain both states. The decision-relevant metric is a **literal index read with addenda reconciliation**.

### 23.2 Corollary: reconcile an unexpected count change by ID-LEVEL SET DIFF, never by arithmetic

When a count changes unexpectedly, do not reason about the delta arithmetically ("it went up by 2, that is probably the two I added"). Compute **which IDs entered and which left**. Arithmetic can accidentally agree - two unrelated errors of equal magnitude and opposite sign reconcile perfectly and hide both. **A set diff cannot accidentally agree.**

Proven this pass: an open-count change from 78 to 80 was reconciled to `ADDED = [AUDIT-208, AUDIT-209]`, `REMOVED = []`. That set diff did more than confirm the delta - it identified the REAL error, which was that 78 was a PRE-filing count that had been written into a status row shipping WITH the filings. Arithmetic would have accepted "78 + 2 new findings = 80, fine" and left the wrong number in the row.

### 23.3 Verify landed scope against ORIGIN, never from the CLI tail

**Rule: post-merge scope is verified with `git show --stat` against `origin/main` - the authoritative landed commit - never from the `gh` CLI output.**

Mechanism and why it is a rule rather than a habit: in this session the `gh pr merge` tail printed an UNRELATED changeset **three times** - PR #474's files on the #475 merge, and PR #477's PV Executive frontend files on BOTH the #476 and #478 merges. Each time, trusting the tail would have produced a false scope conclusion; in one case it looked as though a docs-only merge had absorbed foreign frontend files, which would have triggered an unnecessary revert investigation. The CLI tail is a convenience rendering, not a statement about what landed on the branch.

The verification is: `git fetch origin main`, then `git show --stat origin/main` (or `git show --stat <mergeCommit>` resolved from `gh pr view --json mergeCommit`), and confirm the file list matches the intended footprint. This extends CLAUDE.md section 19.1 PAUSE F ("on merge run the on-main verification") with the explicit negative: the gh output does not satisfy PAUSE F.

### 23.4 Applying it

Before acting on any number that gates a decision, state what the number measures and confirm that is what the decision turns on. Where a cheaper proxy is used, say so and bound its error. Cross-references: section 18 (status-surface discipline - severity and status are register-literal, and this section is the measurement analogue); section 22 (coverage checks are themselves measurements and must satisfy this section); CLAUDE.md section 19.5 (canonical-grep-first: verify state, never infer it).
