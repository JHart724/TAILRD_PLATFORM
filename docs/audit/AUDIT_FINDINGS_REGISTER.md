...
- **Classification effect:** GAP-SH-008 proposed DET_OK -> PARTIAL_DETECTION (operator review) - the evaluator exists but detects the wrong concept entirely. GAP-SH-009 (BAV aortopathy, same evaluator) already PARTIAL; also affected. Driver is §16 code-correctness, NOT §16.5. See `docs/audit/SH_BATCH1_AS_BAV.md`.
...
- **Classification effect:** GAP-SH-013 proposed DET_OK -> PARTIAL_DETECTION (operator review). The Batch-2 findings doc's prior "minor / clinically-related, holds DET_OK" framing is CORRECTED here by the §16.6 spec-based test. See `docs/audit/SH_BATCH2_TAVR_AORTIC.md`.
...
- **Classification effect:** GAP-SH-026 proposed DET_OK -> PARTIAL_DETECTION (operator review). The rule PARTIALLY OVERLAPS the target (true cryptogenic I63.9 + PFO patients ARE caught), so PARTIAL not SPEC_ONLY per the §16.6(ii) overlap rule. See `docs/audit/SH_BATCH4_PFO_ACHD_HCM.md`.
...
- **Classification effect:** GAP-SH-027 proposed DET_OK -> PARTIAL_DETECTION (operator review). The rule PARTIALLY OVERLAPS (RV-failure ASD patients ARE caught), so PARTIAL not SPEC_ONLY. See `docs/audit/SH_BATCH4_PFO_ACHD_HCM.md`.
...
- **Classification effect:** GAP-SH-028 proposed PARTIAL_DETECTION -> **SPEC_ONLY** (operator-decided 2026-06-08). Per the §16.6(ii) overlap rule, a fully-disjoint mapping is SPEC_ONLY (PARTIAL would overclaim). Mirrors AUDIT-126. See `docs/audit/SH_BATCH5_PHTN_PE_IE_MASSES.md`.
