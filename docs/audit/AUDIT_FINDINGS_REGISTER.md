...
- **Impact:** (a) **Detection:** bioprosthetic/TAVR-targeted rules (post-TAVR surveillance, ViV, PVL) gate on `Z95.2` (mechanical) - they MISS real bioprosthetic patients (coded `Z95.3`) and FALSE-FIRE on mechanical-valve patients (for whom ViV is inapplicable). (b) **Clinical / safety-adjacent:** the VD-1 mechanical-valve warfarin rule (`gapRuleEngine.ts:4939`, `hasMechanicalValve = Z95.2||Z95.3||Z95.4`) requires lifelong warfarin for ALL prosthetic valves incl. bioprosthetic (`Z95.3`); per 2020 ACC/AHA VHD only MECHANICAL valves need lifelong warfarin (bioprosthetic = 3-6 months then antiplatelet), so the rule can recommend chronic warfarin to bioprosthetic patients - an over-treatment / bleeding-risk direction. Blast radius cross-module (SH + VHD), 81 references.
- **Classification effect:** GAP-SH-061 (ViV TAVR, SH-VALVE-IN-VALVE) and GAP-SH-011 (post-TAVR surveillance echo, SH-6) are §16.6-affected DET_OK whose detection rests on the inverted `Z95.2` semantics - **proposed DET_OK -> PARTIAL_DETECTION, but DEFERRED to operator adjudication**: the actual under/over-detection impact depends on how TAVR/bioprosthetic valves are coded in the ingested data (Z95.2 vs Z95.3 - the AUDIT-118 data-coupling pattern). GAP-SH-013 already flips via AUDIT-122. VHD-rule reclassification deferred to the VHD re-audit.
