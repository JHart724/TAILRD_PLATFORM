...
- **Status note (CAD-side remediation, lineage):** 2026-07-11 - the CAD (Coronary Intervention) Executive tier (one of the 3 remaining non-HF views) received the full convergence pass on branch `feat/cad-exec-convergence`: the dead `dashboardError` (destructured, never consumed) is now consumed via the new `CADExecutiveSummary`, which renders honest loading / "Live data unavailable" / empty states instead of a fabricated fallback (the `?? 26` gap default AND the `config.kpiData.totalPatients` fallback sitting behind the live `totalPatients` field are both retired - the latter was the AUDIT-099 core defect: a fabricated number behind a live field); live patients / open gaps (NEW) / device candidates (NEW, previously unused) read the real CAD dashboard contract (`CADDashboardData`, from `GET /coronary-intervention/dashboard`); the GapIntelligenceCard headline reads the live `totalOpenGaps` across the real `totalPatients` (killing the 2,330 category-sum-as-population mislabel), composition demo-badged; the mock-fed financial surfaces (ROI waterfall, DRG/CMI, P-v-R, benchmarks) carry a DemoDataBadge; revenue reconciled to one derived `cadDemoFinancials` model anchored at $11.2M (gap-closure opportunity), consolidating the two coexisting ROI waterfalls into one and removing the 70.9M/52.2M/50.9M program-revenue literals. **The CAD-side dead-`dashboardError` + fabricated-KPI-behind-live-field defect is thereby CLOSED**; the remaining non-HF core of AUDIT-099 is now 2 views (VHD / PV), which stay OPEN.
...
- **Note (CAD convergence, 2026-07-11):** the AUDIT-304 CAD Exec convergence (branch `feat/cad-exec-convergence`) built a fresh `CADExecutiveSummary` and surfaced THREE dead-code items on the CAD exec tier, all flagged for deletion in a separate cleanup PR (NOT deleted in the convergence batch, out of scope): (1) `src/ui/coronaryIntervention/components/executive/CADExecutiveKPICard.tsx` - the unimported AUDIT-152 KPI-card copy (grep: zero renders); (2) `src/ui/coronaryIntervention/components/executive/CADFinancialWaterfall.tsx` - NEWLY ORPHANED by the dual-waterfall consolidation (its only render, `CoronaryExecutiveView.tsx`, was removed; the file now has zero importers); (3) the dead DRG drill-down path in `CoronaryExecutiveView.tsx` - the `getDRGData` builder + the `selectedDRG` `BaseDetailModal` are unreachable (pre-existing: `setSelectedDRG` is only ever called with `null`; `SharedDRGPerformance` is display-only and passes no click handler), so the DRG modal never opens. All three are dead code, not defects; consolidate into the AUDIT-152 v2.0 deletion sweep.
...
- **CAD replication (IN PR, branch `feat/cad-exec-convergence`):** full single-PR convergence of the CAD (Coronary Intervention) Executive tier to the HF/EP/SH exemplar. CAD is the AUDIT-302 refactored variant (already renders SharedROIWaterfall / SharedBenchmarksPanel / SharedProjectedVsRealized / SharedDRGPerformance). New `CADExecutiveSummary` (built fresh, NOT the unrendered AUDIT-152 `CADExecutiveKPICard` copy) wires the live CAD contract (`CADDashboardData`: patients / open gaps / device candidates) with honest loading/error/empty states (closes the CAD-side AUDIT-099 dead-`dashboardError`; retires the `?? 26` default AND the `config.kpiData.totalPatients` fallback-behind-live-field); `GapIntelligenceCard` reuses the opt-in `totalPatients` + `compositionDemo` props (kills the 2,330 category-sum mislabel); new `cadDemoFinancials` single-source model anchored at **$11.2M gap-closure opportunity** per the operator ruling. **DUAL-WATERFALL CONSOLIDATION (the CAD-specific reconcile):** the tier rendered TWO ROI waterfalls - a $70.9M `SharedROIWaterfall` and a $50.9M `CADFinancialWaterfall` - plus a $52.2M realized figure and ~$95M-annualized monthly data. Per the operator ruling these program-revenue figures are a DIFFERENT quantity (total coronary program revenue) wearing the "revenue opportunity" label and do not belong on a gap-closure exec tier; the two waterfalls are consolidated into ONE (fed from the $11.2M model), and the 70.9M/52.2M/50.9M literals + the monthly array are removed. New `CADForwardOutlookPanel` consolidates the pipeline/at-risk/predictive trio (fixes the 5.6-vs-7.8M at-risk break). DemoDataBadge introduced across the CAD exec surfaces (SharedDRGPerformance gains an opt-in `demoBadge` prop, default off so VHD/PV render unchanged). IA reordered to the **7-panel** exec narrative (no facility slot - CAD has no facility panel; DRG/CMI promoted, Export folded into the tier header closing the CAD AUDIT-161 dimension). Uniquely, CAD has a KCCQ-equivalent (the SAQ PRO card) - dissolved off the exec tier to the CAD SL `saq-outcomes` tab (the `SAQOutcomesPanel`, which already carries the Seattle Angina + Spertus framing); Gap Response Rate + Trajectory Trends relocated to the CAD SL gap-detection + risk-heatmap tabs. **Both known cross-module latent defects were ABSENT in CAD** (no DRG-modal crash - SharedDRGPerformance is display-only + the BaseDetailModal DRG path is dead code; no facility modal). Frontend only; the CAD dashboard endpoint already emits the wired fields (no backend/contract change). 34-assertion mirror suite `cadexec.convergence.test.tsx`. Operator Chrome verify + separately-authorized push pending.
