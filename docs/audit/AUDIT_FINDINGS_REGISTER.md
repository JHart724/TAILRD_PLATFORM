...
- **AUDIT-107** - Post-deploy Login verification regressed on prod ~2026-05-07 (70 consecutive failures); the gate is also non-blocking decoration so deploys ship unverified (Operational maturity / production verification). 2026-06-03 mechanism resolved to HTTP 500 (NOT credential) - the detection-gap finding whose predicted harm materialized as AUDIT-108. **RESOLVED 2026-06-07** - the option-(B) decision is MADE + implemented as a staged ladder: rung 1 (B1 loud-fail-only, verified alert path via AUDIT-114) + rung 2 (deploy-time smoke-hold, the B3 block-next-deploy posture with fail-open, PR #365 / `b23a8d4`, live-evaluated + synthetically verified incl. false-green immunity). B2-literal (pre-merge required check) re-scoped as staging-cluster-dependent (AUDIT-111/025/024); B4 (auto-rollback) deferred post-DUA on migration-compat. (credential + green-run slices were already closed 2026-06-06, run 27068065320)
...
- **AUDIT-117** - EP `gap-ep-006` dabigatran SAFETY rule + `gap-ep-001` AF-anticoag use `RXNORM_DOACS.DABIGATRAN = '1037045'` = "dabigatran etexilate 150 MG Oral Capsule" (tty=SCD), NOT the ingredient (correct = `1037042`, RxNav-verified 2026-06-08). The EP-006 Class-3-Harm SAFETY rule (`gapRuleEngine.ts:4187`, exact-RxCUI match) fires only on the 150mg capsule, so dabigatran 75mg (the renal-impairment dose) + eGFR<30 - the precise contraindication - is a FALSE-NEGATIVE; EP-OAC reads dabigatran-75/110mg patients as not-on-OAC (false-positive). (Phase 0B EP clinical-code verification, **OPEN** 2026-06-08 - HIGH P1, missed-gap on a SAFETY rule, mitigated synthetic/pre-DUA; sister AUDIT-031/032 EP SAFETY + AUDIT-042/056/057 Cat A wrong-drug class - this is wrong-GRANULARITY; remediation separate; operator confirms severity per §18)
- **AUDIT-118** - EP (and all-module) gap rules match patient medications by EXACT-RxCUI membership with NO RxNorm ingredient->descendant expansion (`runGapDetectionForPatient.ts:53` + `gapDetectionRunner.ts:100`: `medCodes = patient.medications.map(m => m.rxNormCode)`; rules do `medCodes.some(c => SET.includes(c))`). Value sets are ingredient-level (IN) while ingested meds (`ingestSynthea.ts:309` stores FHIR `coding.code` verbatim; Synthea emits SCD/product-level) are product-coded, so ingredient value sets SYSTEMATICALLY UNDER-DETECT product-coded meds (silent missed-detection). Confirmed by the codebase's own workarounds (digoxin "ingredient + 3 formulations" `:4261`; AUDIT-101 statin SCD->ingredient resolver). Blast radius module-wide / all medication-presence rules, NOT DOAC-only. (Phase 0B EP clinical-code verification, **OPEN** 2026-06-08 - HIGH P1 candidate, silent cross-medication under-detection, mitigated synthetic/pre-DUA; remediation = ingredient-normalize at match OR enumerate descendants; operator confirms severity + scope per §18)
- **AUDIT-145** - vbcService CMS/HEDIS quality-measure service is structurally dead-on-arrival: the numerator filters `therapyGap.gapType` (14-value enum `TherapyGapType`, `schema.prisma:1502-1517`) against kebab-case rule-ids (`vbcService.ts:16-40`, e.g. `'cad-statin-high-intensity'`) that are NOT enum members and are NOT persisted on TherapyGap (no rule-id column), suppressed by `as any` (`:53`). Both branches wrong (DB not executable in-audit): Postgres rejects the invalid enum labels -> per-measure try/catch swallows -> NO qualityMeasure rows written -> read endpoint empty; OR if tolerated, zero matches -> nonCompliant=0 -> every measure reports 100% compliance. Compounding Axis-3 defects on the same path: denominator = ALL active patients (`:60-62`) not the measure-eligible cohort + numerator = complement (`:65`) so rates INFLATE toward 100% even if the query worked (60% true -> 96% computed; error grows with hospital size); no CMS/HEDIS exclusions; CMS122 stored higher-is-better though it is an inverse measure (eCQI/CMS-verified). Architectural root: dedup key `patientId+gapType+module` (`gapDetectionRunner.ts:43`) collapses ~263 rules into 14 categories, so no rule-level numerator is expressible without a schema change. Measure-ID audit (web-verified 2026-06-11): `ACO-13` is "Falls: Screening for Future Fall Risk" (NQF 0101 / CMS139), NOT the "Statin Therapy" the code labels it - MISASSIGNED; `HEDIS-AOB` matches no NCQA HEDIS abbreviation found - UNVERIFIED; SPC / CBP / CMS122 code<->name pairings CORRECT. (Service-line aggregation Axis 1+3 / §20; **OPEN** 2026-06-11 - HIGH P1, customer-facing VBC correctness, **MUST-FIX-BEFORE-ANY-VBC-CLAIM + ESCALATE-AT-DUA** (same class as GAP-VHD-099: the day a real health system views quality measures it is immediately visibly broken in the flattering direction); remediation v2.0; operator confirms severity per §18)
- **AUDIT-148** - Advertised-but-absent research / trial-matching capability (commercial-claim honesty; HEADLINE of the registry/trial audit): the platform advertises a clinical-trial-matching + registry-submission capability for which NO backend service, NO schema model (no `ClinicalTrial` / `TrialMatch` / `RegistrySubmission` table in `schema.prisma`), and NO matching logic exist - `featureFlags { clinicalTrials: true, registryAssist: true }` (`admin.ts:1182`), tier gate `'clinical-trials'` (`tierEnforcement.ts:61`), the "Clinical Research Assist" dashboard tile (`App.tsx:526`). CORE defect = the "requires Premium" framing: "Patient-level matching requires Premium - upgrade to identify eligible patients..." (`ClinicalTrialEnrollment.tsx:143`) + "Patient-level matching and submission workflow: Premium" (`RegistryEligibility.tsx:90`) frame an absent capability as PAYWALLED (implies it exists at a higher tier) when no matching/submission engine exists at ANY tier - NOT an honest not-built disclosure. Partial mitigation: a "Beta" badge on the research tile (`App.tsx:537`) that does NOT hedge the free-tier "requires Premium" framing. AUDIT-145 advertised-but-dead pattern at the commercial-claim layer; sibling AUDIT-146 (provenance-blind). (Registry/trial provenance audit, Angle B / §20 commercial-claim extension; **OPEN** 2026-06-11 - HIGH P1, commercial-claim honesty on the program's highest-value research feature, **MUST-ADDRESS-BEFORE-ADVERTISING-RESEARCH-CAPABILITY**, mitigated pre-DUA/synthetic; remediation v2.0; operator confirms severity per §18). **SLICE 1 DELIVERED 2026-07-07 (trials backend, real):** the absent capability now has a real backend - Prisma schema (ClinicalTrial + TrialMatch + enums TrialMatchStatus{ELIGIBLE|INELIGIBLE|INDETERMINATE}/TrialProvenance, DeviceEligibility-templated, hospitalId-scoped, migration authored) + the shared buildPatientEvalContext (STEP-1 behavior-neutral extraction from both gap runners, golden-cohort gap-identity-proven) + the honest matcher evaluateTrialMatch (NEVER asserts ELIGIBLE on an unthreaded/Synthea-absent signal -> INDETERMINATE with the signal named; the trial analog of never-fire-on-absence) + GET /trials + GET /trials/:trialId/eligible-patients (tenant-scoped from JWT, returns the extended honest shape matchStatus+criteriaResults+indeterminateSignals for ALL three states, INDETERMINATE not filtered) + a section-16-verified curated seed (4 trials: HFrEF-GDMT fully-threadable -> ELIGIBLE/INELIGIBLE; PH-PASP + ATTR-BNP force INDETERMINATE). The honest client-side CT.gov discovery feed (AUDIT-147) is PRESERVED - curated-structured + live-discovery coexist. LATER SLICES: frontend contract extension (UI-track; endpoint returns the extended shape, no view calls it yet so nothing breaks), registry-case endpoints, refer/approve. gaps.push unchanged 369; full suite green 116/2251; the 2 prisma.clinicalTrial tsc errors are the WSL stale-client class (RULE 6, resolve in Docker). See register AUDIT-148 Slice 1.
- **AUDIT-085** - Production migration execution environment gap: Prisma-driven migration scripts cannot reach VPC-isolated Aurora from the operator's local machine; architectural decision to Option A (ECS RunTask with command override), implementation pending (Operations / production-execute infra, **OPEN**) [index line added 2026-06-13 to reconcile a filing-discipline omission - the detail entry (HIGH P1) + the BUILD_STATE row predate this index row; DRIFT-47 class]
- **AUDIT-163** `[KB-COMPLETENESS]` - POAF (post-operative atrial fibrillation) prophylaxis absent from the KB - cardiac-surgery population. The KB has NO gap for peri-operative beta-blocker (COR I) / amiodarone (COR IIa, high-risk) prophylaxis to PREVENT new-onset post-op AF (incidence 30-50% post-CABG/valve); the 3 adjacent gaps (GAP-VHD-077 post-valve-AF anticoagulation / GAP-EP-073 concomitant Maze / GAP-CAD-055 sternal-wound surveillance) are NOT prevention. **NEW CLASS: KB-completeness / spec-completeness** (missed-and-unknown; structurally NOT catchable by the code-vs-KB Phase-0B audit, which verifies code against the KB, not the KB against clinical reality). Finding #1 of the surgical peri-operative KB-completeness audit (spot-check: 7 of 8 peri-op quality concerns absent). (KB-completeness; **OPEN** 2026-06-15 - HIGH (P1), FINALIZED 2026-06-15 (operator); remediation = add to KB + author the detection rule in the v3.0 surgical-completeness buildout tranche)
- **AUDIT-303** - False capability/provenance labels on the demo/pilot surface (AUDIT-300-class claims-honesty): four affirmatively-misleading UI strings. (1) HIGH `KCCQOutcomesPanel.tsx:42` "Auto-calculated from EHR data" on a static const (no EHR pipeline). (2) HIGH `ValvularServiceLineView.tsx:46` "AI-driven clinical gap detection" on a deterministic rule engine - false AND misrepresents the §8 FDA-CDS deterministic posture (highest-priority). (3) LOW (downgraded) `StructuralCareTeamView.tsx:90` + `EPCareTeamView.tsx:101` tab descriptions still say "Real-time ... hospital alerts" though the rendered components are already honest EmptyStates (AUDIT-300 P2) - label-only nit. (4) MEDIUM `ValvularCareTeamView.tsx:105/:294` "Valve Worklist" tab renders a network graph (ValvularSurgicalNetworkVisualization), not a worklist - re-attributed to VHD (PV is clean). (UI/frontend-track claims-honesty, AUDIT-300 class; **OPEN** 2026-06-26 - HIGH (P1) proposed; higher priority than AUDIT-302; disposition = honesty-edit (items 1-4) + Layer-3 wire (item 1); verdict CONFIRMED; operator confirms severity per §18) See detail ### AUDIT-303.
...
- **AUDIT-018** - `AuditLog.description` accepts arbitrary input, not encrypted (Phase 2B, **RESOLVED 2026-05-08 via PR #263** (squash-merge `48eac39`); index reconciled 2026-06-13 - detail entry + the AUDIT-075 sister-bundle note both RESOLVED, this index line was stale OPEN; DRIFT-47 filing-discipline class, superseded not deleted)
- **AUDIT-019** - `FailedFhirBundle` plaintext PHI fragments (Phase 2B, **RESOLVED 2026-05-08 via PR #263** (squash-merge `48eac39`); index reconciled 2026-06-13 - detail entry + the AUDIT-075 sister-bundle note both RESOLVED, this index line was stale OPEN; DRIFT-47 filing-discipline class, superseded not deleted)
...
- **AUDIT-110** - brittle absolute-line assertions in `tests/scripts/auditCanonical` (extractCode VD-PANNUS commentLine + extractSpec VHD-005 specLine) break on unrelated source line-shifts (catch-#89 class); local pre-push gate set omits the auditCanonical suite (Test-infra robustness, OPEN; step 2 clinical-verdict-pin sweep complete 2026-06-11)
...
- **AUDIT-119** - EP registry/evaluator/gaps.push counts do not reconcile (registry=48, evaluator=47, gaps.push=49 per `docs/audit/canonical/EP.code.json`): the registry-without-evaluator (48 vs 47) is `gap-ep-anticoag-interruption` (registry `gapRuleEngine.ts:2023`) - registered but with NO EP evaluator, so it silently never fires (= the already-filed **AUDIT-035** EP registry-only orphan; the only periprocedural-anticoag block, `CAD-HEPARIN-BRIDGE` `:8685`, is a CORONARY_INTERVENTION gap, not EP). The gaps.push (49) > registry (48) is >=1 `gaps.push` with no registry id (unguarded provenance, AUDIT-106 B-axis). NOTE: automated enumeration FAILED (registry `id` vs evaluator `name` share no join key; a fuzzy match false-flagged `EP-RC`/`EP-CARDIOVERSION-TIMING` which HAVE evaluators) - the registry-without-evaluator was only found via the AUDIT-035 cross-reference. (Phase 0B EP clinical-code verification, **OPEN** 2026-06-08 - MEDIUM P2; AUDIT-106 class; operator confirms per §18)
- **AUDIT-130** - Single Netlify site (`tailrddemo.netlify.app`, the only site referenced repo-wide via `secrets.NETLIFY_SITE_ID`) serves BOTH the frozen-demo role and the production-frontend deploy target: CI `deploy-frontend` (`.github/workflows/deploy.yml`) collides with the intentional demo lock (site deliberately locked as the frozen demo as of 2026-06-08; first CI collision on the 2026-06-10 merge `48b7aeb`, `Error: Deployments are "locked" for production context`). Interim: `deploy-frontend` gated behind repo variable `DEPLOY_FRONTEND_ENABLED` (default off). (Deploy infrastructure / frontend, **OPEN** 2026-06-10 - MEDIUM P2, deploy-pipeline role-collision, no clinical/PHI dimension; remediation = dedicated production-frontend site + CORS allowlist (`backend/src/server.ts:99`) + staging CFN (`infrastructure/cloudformation/tailrd-staging.yml:439`) updates + CSRF-on-refresh fix as precondition; deferred until a real-API frontend has a consumer (post-DUA); operator confirms severity per §18)
- **AUDIT-131** - Staging ECS deploy (`Build & Deploy to Staging`) fails health verification identically on the 4 most recent main merges (runs back to at least 2026-06-08): 20/20 `Staging health: unavailable` then `ERROR: Staging deploy did not stabilize in 5 minutes`. Failure is independent of code changes (identical signature across unrelated merges incl. docs-only); the staging environment is unreachable and needs infra investigation (likely continuation of the **AUDIT-111** staging-undeployable condition - Prisma P3009 on the staging `_prisma_migrations` row - but root cause NOT yet verified). (Operational maturity / staging environment, **OPEN** 2026-06-10 - MEDIUM P2, silent-red post-merge gate class (AUDIT-107/111/112 sisters), no clinical/PHI dimension; remediation = infra investigation of staging task health (P3009 row, task logs, ALB target health), then a loud gate per AUDIT-107 precedent; operator confirms severity per §18)
- **AUDIT-133** - T1 sub-therapeutic-INR detection absent engine-wide: GAP-VHD-001 needs an INR-to-target comparison, but VD-3 (`gap-vd-3-inr-monitoring`) fires on INR-ABSENT and the mechanical-valve anticoag rule fires on warfarin-ABSENT, so a mechanical-valve patient on warfarin with a sub-therapeutic INR fires neither (disjoint triggers); the 2026-05-04 baseline over-claimed PARTIAL. (Phase 0B VHD clinical-code verification, **RESOLVED 2026-06-17** - v3.0 VHD chunk 3 built the purpose-built `gap-vhd-001-subtherapeutic-inr` evaluator (mechanical valve + warfarin + inr < 2.0) reading the AUDIT-170 INR slug-fix; the INR-to-target rule now exists. Mitigated synthetic/pre-DUA.) Classification effect: GAP-VHD-001 SPEC_ONLY -> **DET_OK**.
- **AUDIT-134** - Echo existence-proxy (`labValues['lvef']/['inr'] === undefined`) substituted for a severity/interval gate -> DET_OK over-credit; first two confirmed sites VD-2 (`gap-vd-2-bioprosthetic-echo`, GAP-VHD-010) + VD-5 (`gap-vd-5-aortic-regurgitation`, GAP-VHD-103); the existence check fires independent of the severity/interval the gap targets. (Phase 0B VHD clinical-code verification, **RESOLVED 2026-06-17** - the DET_OK over-credit was corrected to SPEC_ONLY at the 2026-06-10 re-audit (no site is over-credited); v3.0 VHD chunk 1 then built the purpose-built `gap-vhd-103-severe-ar-surgical` evaluator with a genuine severity gate (severe AR helper: valve_severity>=5 / vena_contracta>=0.6 + LVEF<=55) -> VHD-103 SPEC_ONLY -> PARTIAL (LVEF arm; LVESD arm Path-B). The VD-2/VHD-010 bioprosthetic-echo site remains SPEC_ONLY (VD-2 still an existence-proxy, not in the VHD buildable scope). Mitigated synthetic/pre-DUA.) Classification effect: GAP-VHD-103 SPEC_ONLY -> **PARTIAL**; GAP-VHD-010 holds SPEC_ONLY.
- **AUDIT-135** - Medication-gap rule omits the therapy-ABSENT check so it over-fires on already-treated patients: VD-14 (`gap-vd-14-dental-prophylaxis`, GAP-VHD-063) fires on dx alone; sibling rules DO guard (VD-13 `gapRuleEngine.ts:6620`, VD-ANTIPLATELET-BIOPROSTHETIC `:11231`). (Phase 0B VHD clinical-code verification, **RESOLVED 2026-06-17** - v3.0 VHD chunk 3 added the therapy-absent guard to VD-14 (`onProphylaxisAbx_VD14 = medCodes.includes('723') || medCodes.includes('2582')` in the if-condition); the dental-prophylaxis rule no longer over-fires on patients already on amoxicillin/clindamycin, guarded by vhdChunk3.test.ts (AUDIT-135 gate). Mitigated synthetic/pre-DUA.) Classification effect: GAP-VHD-063 over-firing remediated (VD-14 now guards); classification holds PARTIAL (broad rule).
- **AUDIT-172** - Cross-module IE/AF-anticoagulation overlap at the VHD close: the granular VHD-083 (rheumatic-AF -> warfarin mandate), VHD-057/059 (IE surgical indications) overlapped the broader VD-12 (`gap-vd-12-af-valve-anticoag`, general AF+valve -> OAC), EP-008 (DOAC-in-MS), and SH-029 (lumped IE-surgery). **SAFETY driver:** a rheumatic-MS+AF patient with no OAC could fire VD-12's generic "may use DOAC" alongside VHD-083's warfarin mandate (DOAC is contraindicated/harmful in rheumatic MS per INVICTUS). (Cross-module reconciliation, **RESOLVED 2026-06-17** - VHD-083 OWNS rheumatic-AF anticoagulation: VD-12 excludes rheumatic mitral (I05.x), EP-008 narrowed to nonrheumatic MS (I34.2). SH-029 scope-narrowed (HF arm -> VHD-057; embolic + uncontrolled-infection retained). Guard tests confirm single-fire + no over-narrowing. Mitigated synthetic/pre-DUA.) Classification effect: none (conditions narrowed; tiers unchanged).
- **AUDIT-136** - Broad-rule consolidation onto gaps the rule's OWN exclusion criteria make it structurally unable to fire for: VD-10 (`gap-vd-10-pregnancy-risk`) carries `EXCLUSION_PREGNANCY` (`gapRuleEngine.ts:5748`) yet was credited for the PREGNANT mechanical-valve anticoag gaps VHD-099/100/101 (zero true-positive intersection). **SAFETY sub-note (UPDATED 2026-06-17):** GAP-VHD-099 (mech-valve-in-pregnancy anticoagulation SAFETY) is now BUILT as a dedicated SAFETY_ALERT evaluator (v3.0 VHD chunk 5) -> SPEC_ONLY -> PARTIAL, no longer ZERO coverage (warfarin-dose/T1-LMWH-transition specifics remain Path-B). (Phase 0B VHD clinical-code verification, **OPEN** 2026-06-10 - MEDIUM P2, under-detection + Tier-S SAFETY sub-note, mitigated synthetic/pre-DUA; **partially addressed 2026-06-17**: VHD-099 built, 100/101 hold SPEC_ONLY; remediation v2.0 = anti-Xa/delivery-plan protocols for 100/101; operator confirms severity per §18) Classification effect: GAP-VHD-099 PARTIAL (built); GAP-VHD-100/101 hold SPEC_ONLY; VHD-099 inferredSafetyTag STRUCTURAL_SAFETY.
- **AUDIT-137** - META: the 2026-05-04 VHD baseline override pass was systematically unsound - 4/4 baseline DET_OK overrides failed re-verify (VHD-010/080/103 -> SPEC_ONLY, VHD-063 -> PARTIAL) AND the "clean reconciliation, no EXTRA rules" claim was false (16 of 32 rules EXTRA); flags the pre-§16.6 baseline layer as an untrustworthy v2.0 starting point. (Phase 0B VHD clinical-code verification, **OPEN** 2026-06-10 - MEDIUM P2, audit-process trustworthiness meta-finding, no runtime clinical defect; remediation v2.0 = treat the 2026-06-10 crosswalk as source of truth, re-verify any pre-§16.6 baseline; operator confirms severity per §18)
- **AUDIT-138** - Post-deploy smoke "Wait for ECS stabilization" window (24 polls x 5s = 2 min) is shorter than the single-task service's real rollout-to-healthy, so the smoke gate false-fails on healthy deploys on a pure timing race: on the 2ae35c5 deploy the new ECS task came up healthy (HTTP 200, ~357s uptime when probed) but the 2-min window expired first, and the AUDIT-114 alert path auto-filed issue #375; a manual re-run of the same run went fully green. deploy.yml's own stabilization gate (20 polls x 15s = 5 min) passes on the same rollouts. (Operational maturity / production verification, **RESOLVED 2026-06-11** - this PR: smoke window widened to match deploy.yml's proven gate exactly (20 x 15s = 5 min); AUDIT-114 smoke-gate class, sister AUDIT-107 / AUDIT-112; operator confirms severity per §18)
- **AUDIT-139** - SH spec-side BAV mis-code: the GAP-SH-008 detectionLogic in `CLINICAL_KNOWLEDGE_BASE_v4.0.md:464` read "Q23.0 BAV" but ICD-10 `Q23.0` is "Congenital stenosis of aortic valve", not bicuspid; bicuspid aortic valve is `Q23.81` (a discrete FY2025 ICD-10-CM code). Pure spec-text correction (Q23.0 -> Q23.81) via KB regen (extractSpec re-read -> `SH.spec.json:296`); SH gap distribution UNCHANGED (0 DET_OK / 30 PARTIAL / 58 SPEC_ONLY) and GAP-SH-008 stays PARTIAL_DETECTION with its AUDIT-121 override intact - no detection-logic change. DISTINCT from the code-side sibling AUDIT-121 (engine `SH-BICUSPID` gates `Q23.1` = congenital aortic insufficiency; still OPEN); AUDIT-139 does NOT resolve it - GAP-SH-008 stays PARTIAL until the engine fix (Q23.1 -> Q23.81) lands. (Phase 0B SH clinical-code verification / spec source-of-truth, **RESOLVED 2026-06-11** - this PR; MEDIUM P2, spec-text correctness, no clinical/PHI runtime dimension, mitigated synthetic/pre-DUA; operator confirms severity per §18)
- **AUDIT-140** - Cross-module patient double-count (summed-not-unioned), platform-wide: three independent implementations of one root cause sum per-module patient counts instead of unioning patient-id sets - `platformTotals.ts:30` (frontend), `platform.ts:53` (backend, mixed-semantics: deduped within module, summed across), `godView.ts:292` (worst: a patient in HF+CAD counts twice, `coverage = managed/totalPatients` `:296` exceeds 1.0 and renders ">100% actively managed" to SUPER_ADMIN at `GodView.tsx:253`). No patient-id union exists in any of the three. Reference fix pattern EXISTS in-repo: `vbcService.ts:57` (`distinct: ['patientId']` across a cross-module gap-type set). (Service-line aggregation Axis 2 / §20; **OPEN** 2026-06-11 - MEDIUM P2, incorrect headline aggregation, no clinical/PHI dimension, mitigated synthetic/pre-DUA; remediation v2.0 = union patient-id sets per the vbcService pattern; operator confirms severity per §18)
- **AUDIT-143** - `platform.ts` <-> `platformTotals` mock/real parity break + cohort divergence: the real path computes a different quantity than the mock path under the same `totalPatients` label - real = sum of within-module DISTINCT patients (`platform.ts:39-55`, hospital-scoped), mock = sum of per-gap cohort estimates (`platformTotals.ts`, platform-wide) - a three-way divergence (multiplicity / scope / source). Plus `platform.ts:42,45` omit the `resolvedAt: null` filter (counts RESOLVED gaps + their patients, unlike open-only siblings `gaps.ts:37` / `godView.ts:254`), and `platformTotals.ts:48` casts the totals-less real payload `as typeof PLATFORM_TOTALS` so `.totalOpportunity` is `undefined` on the real path. (Service-line aggregation Axis 4 / §20; **OPEN** 2026-06-11 - MEDIUM P2, flip-to-live silently redefines the headline number, mitigated synthetic/pre-DUA / real path not yet wired; remediation v2.0 = align the two paths to one quantity + fix the cast; operator confirms severity per §18)
- **AUDIT-146** - Fabricated static-table executive surface + provenance-blind badge on the `/service-line` FreeTierDashboard - DISTINCT from AUDIT-099 (cross-reference, not a re-scope): (1) `data.ts` (entire file, lines 1-938) is hardcoded literals - every rate/dollar/margin/LOS/benchmark a constant presented to a CMO as quality/financial/operational metrics (`CMS_KPIS:46`, `QUALITY_BENCHMARKS:82-89`, `MODULE_DETAIL_DATA:610`); 17 of 19 rendered surfaces literal, 2 mock-derived; (2) the badge `hasUploadedFiles ? 'verified' : 'estimate'` (8 sections) with `hasUploadedFiles` hardcoded `false` (`FreeTierDashboard.tsx:44`) always renders "CMS Estimate", "Verified" unreachable - provenance-blind (keys off an upload flag, not data provenance) + LATENT mislabel-on-connect; plus Axis-4 internal inconsistency: two different HF patient counts on one page (computed banner `FreeTierDashboard.tsx:91` vs hardcoded `MODULE_TILES` 4250 `data.ts:136`). HONESTY NUANCE: this surface DOES disclose ("Demo data" `:79`, "CMS Estimate") - the defect is provenance-blind-indicator + unreachable-honest-state, NOT the undisclosed fabrication of AUDIT-099. (Service-line aggregation Axis 1+3+4 / §20; **OPEN** 2026-06-11 - MEDIUM P2, same fabricated-executive-KPI class as AUDIT-099 (HIGH P1) but mitigated by the Demo/CMS-Estimate disclosure AUDIT-099 lacks, mitigated synthetic/pre-DUA; remediation v2.0 = wire the badge to provenance + back the tables or badge each unbacked cell; operator confirms severity per §18) **[extended 2026-06-11: free-tier trial/registry tiles `ClinicalTrialEnrollment.tsx` + `RegistryEligibility.tsx` added as a no-Badge worse-disclosure sub-surface; severity unchanged; see detail]**
- **AUDIT-147** - Research-module fabricated trial/registry data on the `/research` ResearchServiceLineView: `curatedTrials` 14 inline literals with hardcoded `eligiblePatients` counts rendered beside real-looking inclusion/exclusion criteria evaluated against nothing (`ResearchServiceLineView.tsx:281-295`; tell-tale `T01.eligiblePatients = PLATFORM_TOTALS...hf.patients > 0 ? 127 : 0` at `:282` - a pseudo-computed literal, the 127 invented), plus mock registry cases (`registryCasesMap`/`caseFlags`) presented as pre-populated data and field-confidence "eligibility" arithmetic over hardcoded confidences (`:569-574`). Same fabricated-static class as AUDIT-146 but a DISTINCT surface (`/research`, not `/service-line`). HONEST COUNTER-EXAMPLE in the same surface (file IN the entry, v2.0 must preserve): the LIVE ClinicalTrials.gov feed (`:519-560`) is correctly built - honest live-vs-curated labeling (`:963`/`:496`), graceful "Live search temporarily unavailable - showing curated trials" fallback (`:909`), no patient data sent - the in-repo honest-external-data reference (contrast AUDIT-143's silent fallback). Duplicate-NCT risk: EARLY-TAVR `NCT03042104` is curated T14 AND can return from the live feed (same trial twice, divergent fields). (Registry/trial provenance audit, §20 Axis 1+3+4; **OPEN** 2026-06-11 - MEDIUM P2, fabricated-display-data, mitigated pre-DUA/synthetic; remediation v2.0; operator confirms severity per §18)
- **AUDIT-300** - Cross-module clinical-content leak (clinical-content-correctness class; a NEW audit dimension Phase 0C - styling-focused - missed entirely): one module's clinical framework + fabricated patient cohorts + affirmatively-FALSE clinical claims render on the WRONG modules, across all 6 clinical modules, on the demo/pilot-facing surface a cardiologist/CMO evaluates. ROOT (data-wiring = HARDCODED / module-blind, option a): the shared service-line components take ZERO props and self-generate HF-shaped data, ignoring the module entirely. LIVE leaks (rendered): (1) `PatientRiskHeatmap.tsx` (no props; seeded 50-patient `Math.sin` PRNG `:26-49`; HF columns Risk/GDMT-Pillars/EF% `:340-347`; HF recs ICD/CRT/SGLT2 `:150-185`) on ALL 6 service-line `heatmap` tabs (HF/EP/SH/CAD/VHD/PV); (2) `CareTeamNetworkGraph.tsx` (no props; hardcoded HFrEF/HFpEF/HFmrEF cohorts `:47-49`) on ALL 6 service-line `network` tabs; (3) `SHBenchmarksPanel.tsx:9` HF "Quadruple Therapy Rate" GDMT benchmark + drill-down `StructuralExecutiveView.tsx:184-196` on the SH EXECUTIVE view; (4) `EPRealTimeHospitalAlerts` HF 4-pillar (ARNI/BB/MRA/SGLT2) framed as AF therapy ("NOT ON 4 PILLARS"; recs "Initiate ARNi / spironolactone / empagliflozin") on EP care-team `EPCareTeamView.tsx:356` - clinically FALSE; (5) `SHRealTimeHospitalAlerts` HF GDMT framed as valve therapy + incoherent "TAVR evaluation (QRS 142ms)" on SH care-team `StructuralCareTeamView.tsx:204` - clinically FALSE; (6) `EPPatientDetailPanel.tsx:543` verbatim HF "4-Pillar Status" block (ARNi/ACEi/ARB, Beta-Blocker, SGLT2i, MRA), not even relabeled, on EP patients. DEAD-but-latent clones (confirmed 0 render sites, carry FALSE claims): `EPCareGapAnalyzer` ("Spironolactone reduces mortality in AF/AFL" - FALSE), `SHCareGapAnalyzer` ("...in valve disease" - FALSE), `EPPatientWorklistEnhanced`, `EPWorklist`, `EPTeamCollaborationPanel`, `SHPatientWorklistEnhanced`, `SHTeamCollaborationPanel`; plus fabricated-PHI latent dashboards `PatientRiskStratification` ("Margaret Johnson MRN 12345678") + `PopulationOverviewDashboard` (both unrendered). Cleared (module-correct): 5 of 6 Executive views (props-driven shared cards), CAD/VHD/PV care-team views, `DeviceUnderutilizationPanel` (HF-scoped), `DEMO_PATIENT_ROSTER` (intentional cross-module roster). SEVERITY ESCALATOR: the mechanical HF->module text-swap left affirmatively-FALSE clinical assertions (SGLT2i/ARNI/spironolactone mortality benefit in AF / valve disease) - not merely wrong-module, but FALSE clinical claims on a clinician/CMO-facing pilot surface; this is the clinical-content analogue of a wrong-drug miscode. (Clinical-content correctness, §20; **OPEN** 2026-06-17 - HIGH P1 proposed (clinical-content wrongness + wrong-cohort + FALSE claims x all-6-module breadth on the demo/pilot product surface; mitigated pre-DUA/synthetic - no real PHI - but the demo IS the pilot product); remediation tiered: L2-dead = DELETE the false-claim clones, L2-live = CORRECT-or-PULL the rendered leaks (PULL beats false on a pilot), L1 = parameterize `PatientRiskHeatmap`/`CareTeamNetworkGraph` by module + wire per-module data (interim: HIDE-or-clearly-label, never show fabricated HF data); operator-gated at every merge - clinical-content despite frontend `src/`; operator confirms severity per §18) **P1 EXECUTED 2026-06-17 (feat/audit-171-clinical-content-leak, off origin/main):** L2-dead DELETE done - removed 11 components, each re-verified 0 live render sites (SHPatientDetailPanel intra-set-dead: imported only by the also-deleted SHPatientWorklist + SHPatientWorklistEnhanced): EPCareGapAnalyzer, SHCareGapAnalyzer, EPPatientWorklistEnhanced, EPWorklist, EPTeamCollaborationPanel, SHPatientWorklist, SHPatientWorklistEnhanced, SHPatientDetailPanel, SHTeamCollaborationPanel, PatientRiskStratification, PopulationOverviewDashboard. No live render site touched; no dangling imports (no kept file referenced any deleted file); tsc clean + primitives harness 33/33 green. REMAINING: L2-live (EP/SH RealTimeHospitalAlerts, EPPatientDetailPanel 4-pillar block, SHBenchmarksPanel HF benchmark) + L1 (parameterize PatientRiskHeatmap / CareTeamNetworkGraph + per-module data) = P2/P3, pending operator review. **P2 EXECUTED 2026-06-17 (feat/audit-171-clinical-content-leak):** L2-live remediated by REMOVAL + EmptyState substitution (zero new clinical content authored). 6 dispositions: (1) EPRealTimeHospitalAlerts + (2) SHRealTimeHospitalAlerts panel bodies -> EmptyState (HF-GDMT-as-AF/valve false content removed); (3) PatientRiskHeatmap + (4) CareTeamNetworkGraph gutted to EmptyState, component shells RETAINED as the P4 parameterization seam (seeded-PRNG cohort + HFrEF/HFpEF/HFmrEF cohorts removed); (5) EPPatientDetailPanel HF "4-Pillar Status" block excised (rest of panel kept; now-dead pillar-helper fns left in place); (6) "Quadruple Therapy Rate" HF-GDMT benchmark removed from SHBenchmarksPanel + StructuralExecutiveView drill-down (TAVR/MitraClip kept; HF retains its own copy, legitimately). VERIFICATION: tsc clean; full Jest 46/46 in 4/4 suites (complete frontend set per react-scripts default roots=src + testMatch, NOT a filtered run); rendered-claim grep clean (Quadruple Therapy Rate now HF-only; ARNi/ACEi/ARB only in HF's own PatientDetailPanel; spironolactone/empagliflozin hits all legitimate CAD+DM/CKD trial citations + HF GDMT, not leaks). NEW FINDING (P1-class, surfaced BY the P2 grep, MISSED in the P1 enumeration): two DEAD/unrendered SH dashboards carry the same latent "Valve Therapy 4-Pillar" leak - `SHQualityMetricsDashboard` ("Valve Therapy 4-Pillar Optimization") + `SHValveTherapyAnalyticsDashboard` ("4-Pillar Achievement"/"4-Pillar Rate"), 0 render sites each; plus the EPPatientDetailPanel now-dead pillar-helper fns. Not rendered = no live false claim; recommend P1-style DELETION (operator-gated) as a P2-addendum. AUDIT-300 stays OPEN: P4 (parameterization + per-module real data + clinical authoring) outstanding. **P2-ADDENDUM EXECUTED 2026-06-17 (feat/audit-171-clinical-content-leak):** dead/latent leak class CLOSED. After a repo-wide class-completeness sweep (P1-protocol re-confirm = 0 external refs each: import + JSX + barrel `from`), git rm 6 dead clones: `SHQualityMetricsDashboard` ("Valve Therapy 4-Pillar Optimization"), `SHValveTherapyAnalyticsDashboard` ("4-Pillar Achievement/Rate"), `SHValveTherapyHeatmap` + `EPAnticoagulationHeatmap` (HF GDMT-pillar/EF/"Quadruple Therapy" heatmaps mislabeled valve/anticoag), `GDMTOptimizationTracker` + `TherapyGapDashboard` (dead HF GDMT trackers in components/therapyGap). Plus surgical cleanup in `EPPatientDetailPanel`: removed 7 now-dead HF-pillar helpers orphaned by the P2 4-Pillar excision (getAnticoagulationStatusColor/Icon, getContraindicationAlerts, checkARNi/BetaBlocker/SGLT2i/MRAContraindications); KEPT getAnticoagulationScoreColor (still called by the retained Overall-Score block). VERIFICATION: tsc clean; full Jest 46/46 (4/4 suites); rendered-claim grep = zero hits in EP/SH rendered code, deleted-clone content gone (the only residual checkARNi/4-Pillar refs are HF's OWN PatientDetailPanel - legitimate; HF is the GDMT home). SWEEP RESULT: bucket A (LIVE leaks) = NONE - P2 cleared all; the 4 per-module *ClinicalGapDetectionDashboards' GDMT refs are legitimate-in-context and RETAINED (EP device/CASTLE-AF eligibility; SH/VD COAPT-TEER prerequisite; CAD DM/CKD SGLT2i per EMPA-REG/DAPA-CKD). LESSONS: the class-completeness sweep found 4 dead clones P1 had MISSED (P1 deleted 11 by name; the dead heatmap clones + shared therapyGap trackers were not enumerated) - enumerate by signature, not by hand-list; and AUDIT-150's color-band fix had unknowingly edited one of these dead clones (`SHValveTherapyHeatmap`, renders nowhere) - verify render-status BEFORE fixing. AUDIT-300 stays OPEN: only P4 (parameterize the 2 EmptyState seams + per-module real data + clinical authoring) outstanding. **ID RECONCILED 2026-06-17 (pre-merge sync, merge commit 1f54f58):** originally filed as AUDIT-171, RENUMBERED to AUDIT-300 here. At sync, origin/main (4e0ae70) was found to carry its OWN AUDIT-171 - a DIFFERENT finding (SH-022 evaluator renamed / latent applyOverrides demotion, published via the SH close #406). The UI and clinical tracks independently assigned id 171 in parallel = collision: both took the next-free-id 171 off a register topping at AUDIT-170, on separate branches, before either merged. Resolution: main's published 171 (SH-022) left intact; renumbered first to 178. SECOND COLLISION at the next sync (origin/main 0ae6144, CAD close #408): main had RESERVED 178/179/180 for PV legacy-coverage findings, so 178 collided too. FINAL: renumbered to AUDIT-300 - deliberately clear of the clinical track's sequential range - and RESERVED for the UI/frontend track (see the AUDIT-300 reservation note). Branch name feat/audit-171-clinical-content-leak retained as historical. LESSON: two tracks sharing one register kept colliding on next-free-id (171, then 178) because each allocated sequentially off a stale max on its own unmerged branch. STANDING FIX: the UI/frontend track owns a high reserved range (AUDIT-300+); the clinical track keeps the low sequential range; each honors the other's reservations. Reactive renumbering loses the race; a reserved fixed slot does not.
- **AUDIT-301** - Pre-existing color-token misapplication (carmine on non-critical financial/benchmark states + CAD nav red): three pre-existing findings where carmine `#9B2438` (`--sem-critical`, the reserved clinical-alarm red) paints non-critical states on customer-facing surfaces - sibling to AUDIT-153 (red semantic overload). (1) MEDIUM - `SharedProjectedVsRealized.tsx:59-61` revenue "Gap" card carmine ink+tint -> amber `--sem-data-negative-ink #854F0B`; one shared-component fix covers HF `ProjectedVsRealizedChart.tsx:22` + PV `PeripheralExecutiveView.tsx:274`. (2) MEDIUM DECISION-GATED - `SharedBenchmarksPanel.tsx` binary `positive ? #2D6147 : #9B2438` (lines 29/56/63/70); below-benchmark cards carmine -> amber, preserving the favorable-negative green path (`lowerIsBetter:true`, the amputation-rate card); deferred pending a cross-module decision (recommended: amber-only). (3) LOW - `coronaryIntervention/config/careTeamConfig.tsx:715` `primaryColor: 'medical-red'` nav selected-state -> `'porsche'` (navy); render path already supports it. All pre-existing (git blame: findings 1+2 = `ba883884` 2026-03-31, finding 3 = `32d4672` 2026-03-16), all reproduce on origin/main `5be698c`; root class = alarm-red on non-critical states (UI_CANON §4.2). (UI/frontend-track color-token correctness; **OPEN** 2026-06-23 - MEDIUM (P2); disposition: findings 1+3 fixed this tranche (feat/audit-301-color-token-fixes), finding 2 OPEN pending decision; verdict CONFIRMED; operator confirms severity per §18) See detail ### AUDIT-301.
- **AUDIT-302** - Phase 0C design/data convergence: clone-and-diverge across all 18 view surfaces (3 tiers x 6 modules) + a parallel dead-config fork. Exec tier = 6 clones of one ~13-panel shared skeleton (Group-B duplicate BaseExecutiveView render CAD:408/VHD:360/PV:360; dead shell imports BaseExecutiveView+KPICard x3 + FinancialROIWaterfall HF; local-wrapper vs shared-direct financial split). Tier 2 Service-Line = genuine divergence + config-vs-view fork (dead/bypassed configs carrying richer wired panels; worst CAD 829-line, PV bypass). Tier 3 Care-Team = config-driven spread (CAD/PV) is the structural canonical; 4 bespoke views converge. Cross-cutting: near-total mock (only ServiceLineKPIBanner API-wired), cross-tier duplicate `*ClinicalGapDetectionDashboard` render, dead controls (AUDIT-149 class), dynamic-Tailwind purge-unsafe classes (CAD:258/575, PV:259). Data exemplar = HF; Recharts warning = no unbounded ResponsiveContainer found (defer to runtime). (UI/frontend-track Phase 0C convergence; **OPEN** 2026-06-26 - MEDIUM (P2); Layer 2 structural de-dup+unify then Layer 3 data-wire; absorbs the freemium data-accuracy finding; claims-honesty carved to AUDIT-303; verdict CONFIRMED; operator confirms severity per §18) See detail ### AUDIT-302.
- **AUDIT-149** - Dead interactive controls in EP care-team + Coronary worklist: fully-styled action buttons that look actionable but have NO `onClick` handler (or are wired to empty-body no-op functions), so a click does nothing. §1 re-verified against running code: EPAlertDashboard "Emergency EP Consult" / "Schedule EP" / "Generate Orders" (`EPAlertDashboard.tsx:336/340/344`, no onClick); EPFollowUpQueue buttons `:179/183` (labels are "Contact Now" / "Reschedule", NOT the 0C-2a-cited "Complete"/"Defer" - label corrected, defect stands); EPReferralTracker `:219/224`; EPReferralTrackerEnhanced `:756/761/766/976`; EPTeamCollaborationPanel `:743/746/777/780`; EPCareGapAnalyzer `:830/833`; EPActionQueue `:169`; EPPatientTimeline `:269`; EP "Open Chart" (`EPPriorityWorklist.tsx:230`, `EPPatientWorklist.tsx:233`, no onClick while the sibling "Review Patient" IS wired); CoronaryWorklist empty-body bulk handlers (`bulkScheduleCathLab` / `bulkOrderCardiacRehab` `:278-284` -> buttons `:553/559`) + Edit icon `:673`; PeripheralExecutiveView `onExport={() => {}}` (`:378`). EXCLUDED (re-verification false-positive): the EPRiskStratification CHA2DS2-VASc / HAS-BLED checkboxes (`:277/315`) have `onChange={() => {}}` but the wrapping `<label>` carries the REAL handler (`:272` `updateCHA2DS2VASc` / `:310` `updateHASBLED`) - the risk factor IS captured; the 0C-2a "records nothing" clinical-presentation claim does NOT hold and is NOT filed. Violates UI_CANON §7.2 functional-integrity rule. (Phase 0C-3 UI-canon findings, functional-defect tier; **OPEN** 2026-06-11 - MEDIUM P2 proposed (operator leaned HIGH on the checkbox clinical-presentation dimension, which re-verification DISPROVED; no clinical/PHI/data dimension remains - workflow-integrity UX defect, mitigated pre-DUA/synthetic); remediation v2.0 = wire each control or render it non-interactive; operator confirms severity per §18)
- **AUDIT-150** - Broken heatmap color-band map (clinical-adherence color mis-encoding): `getHeatColor` in `GDMTHeatmap.tsx:27-33` AND `SHValveTherapyHeatmap.tsx:27-33` (byte-identical) returns the SAME `bg-chrome-50` for adherence-score bands `>=80`, `>=70`, AND `>=65` (only `75-79` -> `bg-titanium-300`; `<65` -> `bg-red-500`), while the on-screen "Performance Key" legend (`GDMTHeatmap.tsx:67-72`) claims distinct bands - so the progress-bar FILL COLOR for a 65%-adherent site is identical to an 80%-adherent site and contradicts its own legend. MITIGATION (§1-verified, sharpens the 0C-2a framing): the numeric "Overall Score X%" is shown prominently (`:50`) and the bar WIDTH is proportional to score (`:59`), so the precise adherence is NOT hidden - only the redundant fill-color encoding is wrong/legend-inconsistent (65% and 80% differ in number and bar length, match only in color). Violates UI_CANON §6 (distinct-color-bands rule). (Phase 0C-3 UI-canon findings, functional-defect tier; **OPEN** 2026-06-11 - MEDIUM P2 proposed (operator leaned MEDIUM-HIGH; lowered because the numeric + bar-width carry the true value and the color is a redundant decoration, mitigated pre-DUA/synthetic); remediation v2.0 = distinct color per legend band; operator confirms severity per §18)
- **AUDIT-151** - View-body clone architecture (UI_CANON §1.4): the 6 clinical modules carry 18 bespoke per-module view files (6 x Executive/ServiceLine/CareTeam under `src/ui/<module>/views/`; 22 platform-wide incl. research's 3 + revenueCycle's 1) instead of the canonical ONE parameterized shared view per tab. §1 re-verified 2026-06-12: the 18 clinical files exist; EP and SH ExecutiveView are structural clones (identical 894-line length, identical section order at identical line ranges per 0C-1b; only module data/prefixes differ), as are VD=PV; `BaseExecutiveView` is IMPORTED by all 6 but RENDERED by only CAD/VD/PV (HF/EP/SH carry a dead import - the clone drift already realized); `BaseCareTeamView` is consumed by the 6 careTeamConfigs as a TYPE import only (`CareTeamViewConfig`), zero module renders. Remediation = the §1.4 consolidation mandate: collapse to 3 shared parameterized views driven by per-module config, clones DELETED. **This is the largest single v2.0 UI work item.** (Phase 0C-3 UI-canon findings, structural/component tier; **OPEN** 2026-06-12 - MEDIUM P2 proposed (architecture debt: no user-facing harm, but divergence is already realized and every cross-module fix costs 6x; mitigated pre-DUA/synthetic); remediation v2.0; operator confirms severity per §18)
- **AUDIT-152** - Component-system bypass (UI_CANON §3 + §3.5 full-migration mandate): the shared primitives exist but are bypassed at ~98-100% scale. Counts re-run at filing 2026-06-12: 845 `<button>` with only 19 `btn-*` class uses (~826 hand-rolled); 914 `rounded-full` pills with only 9 `<Badge>` uses (~905 hand-rolled); 7 `*ExecutiveKPICard.tsx` copies (re-verified file list incl. the SH double: SHExecutiveKPICard + StructuralExecutiveKPICard) vs 1 shared KPICard; **`BaseTable` has ZERO importers** (re-verification strengthened the 0C-1 "rarely used": literally nothing imports it) vs 65 raw `<table>` instances across 46 files. Remediation = §3.5 full migration + legacy DELETION (partial migration explicitly rejected - a third source of truth is worse than two). (Phase 0C-3 UI-canon findings, structural/component tier; **OPEN** 2026-06-12 - MEDIUM P2 proposed (the consistency-tier ROOT CAUSE: every polish defect downstream of it; no direct user harm; mitigated pre-DUA/synthetic); remediation v2.0; operator confirms severity per §18)
- **AUDIT-153** - Red semantic overload (UI_CANON §4.2 clinical-safety-legibility): carmine `#9B2438` carries 8 distinct meanings, 3 sharing the exact hex - §1 re-verified 2026-06-12: active-nav gradient (`index.css:281`), danger-button gradient (`:671`), critical status glow (`:706`) all carry `#9B2438` verbatim. Full 8-meaning table per UI_CANON §4.2 (critical/safety-alert, primary CTA, negative-number, active-nav, Structural module identity, allergy flag, error border, critical-row accent). The alarm color must mean alarm: a real clinical-critical signal loses its pre-attentive distinctness when the same red also paints CTAs and selected tabs. Remediation = the §4.2 token split (carmine RESERVED for clinical-critical; 7 other meanings reassigned to `--sem-cta`/`--sem-data-negative`/`--sem-selection`/etc.). (Phase 0C-3 UI-canon findings, structural/component tier; **OPEN** 2026-06-12 - MEDIUM P2 proposed (clinical-legibility dimension present but pre-DUA/synthetic; NOTE: the dimension STRENGTHENS at DUA when real alarms render alongside red CTAs - revisit severity then); remediation v2.0; operator confirms severity per §18)
- **AUDIT-155** - Cross-module structural + label drift bundle (UI_CANON §1.1-1.3 + §2.1-2.3): one root cause - no canonical skeleton existed until UI_CANON - many sites, all §1 re-verified 2026-06-12: Service Line group headers differ per module (HF "Clinical Analytics" `ServiceLineView.tsx:50` vs SH "Procedure Pathways" `StructuralServiceLineView.tsx:37`); gap-detection labels inconsistent across 6 ("Gap Detection (71-Gap)" CAD `serviceLineConfig.tsx:731`, "(20-Gap)" EP `:693`, "(25-Gap)" HF `:389`, "(12-Gap)" PV `:684`, "(8-Gap)" SH `:690`, "Clinical Gap Detection" no-count VD `:337`); SH STS duplicate (both `calculators` AND `sts-calculator` present, `serviceLineConfig.tsx:623/:629`); VD workflow relabeled "Valve Optimization" (`careTeamConfig.tsx:822` region); hospital-alerts in 3 of 6 (HF/EP/SH); KPI-Summary absent EP/SH; Revenue-by-Facility + Opportunities-Pipeline in 3 of 6 (HF/EP/SH); CAD-only Financial ROI Waterfall; network-triple label overlap (§2.3); worklist = 6 distinct files with EP alone carrying 4 variants (EPWorklist + EPPatientWorklist + EPPatientWorklistEnhanced + EPPriorityWorklist, re-verified on disk). Remediation = conform to UI_CANON §1-2 (the canonical skeletons + nav decisions). (Phase 0C-3 UI-canon findings, structural/component tier; **OPEN** 2026-06-12 - MEDIUM P2 proposed per the classify-up default (operator leaned LOW-MEDIUM; each site is individually cosmetic - the LOW argument - but the bundle spans all 6 modules and both nav tiers and includes user-facing label inconsistency; downgrade later with evidence per convention); remediation v2.0; operator confirms severity per §18)
- **AUDIT-156** - New HIGH esbuild advisories in the backend tree (via the `tsx` devDependency): esbuild 0.17.0-0.28.0 carried 2 HIGH CVEs - GHSA-gv7w-rqvm-qjhr (missing binary-integrity verification -> RCE via NPM_CONFIG_REGISTRY) + GHSA-g7r4-m6w7-qqqr (arbitrary file read via the esbuild dev server on Windows; our dev env IS Windows) - pulled transitively by `tsx` (`backend/package.json` devDependency `^4.1.0`, lockfile-pinned to tsx 4.21.0 -> esbuild 0.27.4). Surfaced 2026-06-12 by the Security Audit `npm audit --audit-level=high` gate on PR #384 (a docs-only PR; the gate audits backend deps regardless of PR content - working as designed). Sibling to AUDIT-007 (the uuid/node-cron MODERATE chain, still tracked; moderates do not trip the high gate). SEVERITY DRIVER (operator instruction): **tsx is a devDependency** - production runs compiled `node dist/server.js` with tsx/esbuild ABSENT from the Docker image, so no production / PHI / patient-data exposure; realized surface is the dev + CI build toolchain only, and the Windows dev-server file-read CVE additionally requires esbuild `serve` mode which this project never invokes. (Security / dependency hygiene; **RESOLVED 2026-06-12** - this PR: `tsx ^4.1.0 -> ^4.22.4` (esbuild 0.27.4 -> 0.28.1, the patched line - advisory caps at 0.28.0 inclusive, 0.29.0 does not exist); `npm audit --audit-level=high` exits 0 (2 HIGH cleared, 6 moderate remain under AUDIT-007); backend suite 1334 pass + 1 skip baseline held; tsx MIT, actively maintained, no major bump; MEDIUM P2 given devDependency scope, down from the HIGH CVE rating; operator confirms severity per §18)
- **AUDIT-157** - Aesthetic-system scatter: elevation + radius + glass (UI_CANON §5.1-5.3). Counts re-run at filing 2026-06-12: ELEVATION - 12 distinct `shadow-*` utilities in use (537 usages: shadow-lg 134, chrome-card 90, md 72, sm 59, chrome-elevated 45, 2xl 34, chrome-card-hover 31, xl 30, glass 30, metal-2/3, white) + 11 inline `boxShadow`, vs the canonical 4 levels (flat/raised/floating/modal). RADIUS - 10 distinct `rounded-*` with 6 in heavy use (lg 2451, xl 1009, full 914, 2xl 327, md 48, sm) coexisting as card radii, vs the canonical 3 (control/card/modal). GLASS - 607 `glass-panel`/`metal-card` vs 1578 plain `bg-white` = ~28% glass adoption, vs the canonical 100%-of-cards. One root cause (no enforced aesthetic system existed pre-UI_CANON). Remediation = the §5.1-5.3 consolidation. (Phase 0C-3 UI-canon findings, aesthetic/polish tier; **OPEN** 2026-06-12 - MEDIUM P2 proposed (the web-3.0/premium-depth gap core; spans the entire app's card surfaces; no functional harm; mitigated pre-DUA); remediation v2.0; operator confirms severity per §18)
- **AUDIT-158** - Motion-orchestration absence (UI_CANON §5.4). Re-verified 2026-06-12: **0** `framer-motion`/`AnimatePresence` imports (no route-transition, no card-entrance orchestration); 7 distinct `duration-*` (150/200/300/500/700/1000 + base) + 2 ad-hoc easings vs the canonical 3 durations + 1 easing pair; `CountUp` at only 4 sites (not platform-wide on KPI values). DISTINCT from AUDIT-157 because the remediation CLASS differs - a new declarative-motion dependency + a new motion layer (vs consolidating existing values); the dependency addition goes through the standard library review per the engineering bar (license / maintenance / breaking-change check). Remediation = the §5.4 motion system. (Phase 0C-3 UI-canon findings, aesthetic/polish tier; **OPEN** 2026-06-12 - MEDIUM P2 proposed (motion is the largest perceived-premium lever; no functional harm; mitigated pre-DUA); remediation v2.0; operator confirms severity per §18)
- **AUDIT-159** - Chart-correctness set (UI_CANON §6). Re-verified 2026-06-12: NO shared `ChartTheme` exists (grep: zero) - every chart re-specifies axis/tooltip/grid inline with mixed brand-threshold vs hardcoded fills + inconsistent gridlines. Instances: `CareGapFunnels` = hardcoded CSS-width divs `WIDTHS=['w-full','w-[85%]','w-[70%]','w-[55%]']` (not data-driven, not a tapering funnel; `CareGapFunnels.tsx:13-15`); `SharedROIWaterfall` (`:25`) + `FinancialROIWaterfall` (`:112-127`) labeled "waterfall" but rendered as left-aligned bars with no cumulative step; `DeviceFunnel` (`:15`) 3 independent bars no taper; `RevenuePipelineCard` YAxis has NO `domain={[0,..]}` (auto-scale magnitude distortion; `ForwardLookingCards.tsx:60-72`); Research Auto-Fill target range = overlapping dashed `ReferenceLine` pairs no legend (`ResearchExecutiveView.tsx:208-213`); `SharedProjectedVsRealized` absolutely-overlaid bars (darker obscures lighter; `:66-87`); HF `GeographicHeatMap` CSS-grid-not-a-map beside the real Leaflet `ZipHeatMap`. CROSS-REFERENCE AUDIT-150 (the heatmap color-band collapse, already filed; NOT re-filed here - sibling §6 instance). Root cause = no `ChartTheme`. Remediation = §6 (one ChartTheme; correct-chart-type; zero-baseline; required axis labels). (Phase 0C-3 UI-canon findings, aesthetic/polish tier; **OPEN** 2026-06-12 - MEDIUM P2 proposed (mislabeled chart types + auto-scale distortion are mild data-mis-presentation; mitigated pre-DUA); remediation v2.0; operator confirms severity per §18)
- **AUDIT-160** - Interaction-state coverage gaps (UI_CANON §7 state standard). Re-run 2026-06-12: NO shared spinner component (`LoadingSpinner`/`<Spinner` grep: 0); `animate-spin` in 19 files / 25 uses at inconsistent sizes (App `w-16`/border-4 vs ModuleLayout `w-10`/border-2); 48 `isLoading` branches but named surfaces with none (KPICard shows `--`, the heatmaps, SharedROIWaterfall); empty-state fragmentation - 12 `ChartEmptyState` + 11 other `EmptyState` + ~10 inline no-data strings at inconsistent sizing (py-16 vs h-48); 1411 ad-hoc `hover:` utilities with no shared treatment (incl. 66 off-token `hover:bg-gray`, cross-ref AUDIT-154). Remediation = the §7 one-spinner / one-empty / one-hover standard. (Phase 0C-3 UI-canon findings, aesthetic/polish tier; **OPEN** 2026-06-12 - MEDIUM P2 proposed per classify-up (operator leaned LOW-MEDIUM; LOW counter-arg: consistency-tier, no functional break - the 48 loading branches mostly cover async; MEDIUM because the gap spans the whole app's interaction states; downgrade-with-evidence open); remediation v2.0; operator confirms severity per §18)
- **AUDIT-161** - Flow inversions + dead navigation paths (UI_CANON §7 flow rules). Re-verified 2026-06-12: the Export utility renders FIRST, above the headline gap metric, in the Executive views - HF `<ExportButton>` `ExecutiveView.tsx:293` vs `<GapIntelligenceCard>` `:360`; CAD `<ExportButton>` `CoronaryExecutiveView.tsx:185` vs `<GapIntelligenceCard>` `:232` (2 modules re-verified, pattern is all 6 per 0C-2a); no data-driven deep-links between the 3 audience tabs (an Exec gap count does not link to the Service-Line detail or the Care-Team action - role-based nav only); orphan routes reachable but absent from the home launcher `MODULES` grid (`App.tsx:520` renders `MODULES.filter(m => m.id !== 'research')`; `/data` + `/patients` are routes `:761/766` with no tile; the revenueCycle module has a view but no `MODULES` entry; `/admin/*`). Remediation = §7 headline-first order + cross-tab deep-links + orphan-route disposition. (Phase 0C-3 UI-canon findings, aesthetic/polish tier; **OPEN** 2026-06-12 - MEDIUM P2 proposed per classify-up (operator leaned LOW-MEDIUM; the Export-above-headline inversion is on every Executive view a CMO sees = customer-facing information-hierarchy defect -> MEDIUM; LOW counter-arg: trivial per-view fix, and the inversion is auto-fixed once AUDIT-151 collapses the 6 Exec views to one canonical-order view); remediation v2.0; operator confirms severity per §18. 2026-07-08: the HF Export-inversion dimension is REMEDIATED at the HF Exec IA restructure - Export folded into the tier header, KPI summary first; non-HF modules + deep-links + orphan routes remain. 2026-07-09: the EP Export-inversion dimension is likewise REMEDIATED at the EP Exec convergence (branch `feat/ep-exec-convergence`, in PR) - Export folded into the tier header. 4 Executive views retain the inversion (SH / CAD / VHD / PV; each renders `<ExportButton>` before `<GapIntelligenceCard>`, grep-verified 2026-07-09: CAD `CoronaryExecutiveView.tsx:185`<`:232`, PV `PeripheralExecutiveView.tsx:176`<`:185`, SH `StructuralExecutiveView.tsx:471`<`:480`, VHD `ValvularExecutiveView.tsx:178`<`:187`); deep-links + orphan routes remain. Finding stays OPEN. 2026-07-10: the SH Export-inversion dimension is likewise REMEDIATED at the SH Exec convergence (branch `feat/sh-exec-convergence`) - Export folded into the tier header. 3 Executive views retain the inversion (CAD / VHD / PV; each renders `<ExportButton>` before `<GapIntelligenceCard>`, grep-verified 2026-07-10: CAD `CoronaryExecutiveView.tsx:185`<`:232`, VHD `ValvularExecutiveView.tsx:178`<`:187`, PV `PeripheralExecutiveView.tsx:176`<`:185`); deep-links + orphan routes remain. Finding stays OPEN.)
...
- **AUDIT-173** - CAD-REHAB over-detector: cardiac-rehab gaps GAP-CAD-029 (post-CABG) + GAP-CAD-046 (post-MI) fired on `hasCAD` alone. (CAD legacy-coverage audit, **RESOLVED 2026-06-18** - CAD chunk 0 split CAD-REHAB into gap-cad-rehab-cabg (Z95.1-gated) + gap-cad-rehab-mi (I21/I22/I25.2-gated); regression + no-over-narrowing tests in cadChunk0.test.ts. Both re-cited DET_OK. The secondary rehab-engagement guard was DROPPED per the 2026-06-18 proxy investigation - no reliable pre-DUA rehab-participation signal (ICD-10-CM has no cardiac-rehab-participation code; CPT 93797/93798 inert in all synthetic/seed data; Synthea codes procedures as SNOMED, not CPT). Path-B: a CPT 93797/93798 + SNOMED procedure guard is added post-DUA; the residual false-positive (re-recommending to an already-enrolled patient) is an accepted low-harm limitation.) See register AUDIT-173.
- **AUDIT-174** - CAD-061 DAPT-de-escalation miscite (cited Gap-50 detects P2Y12-ABSENCE, the opposite of de-escalation) + Gap-50 over-detected on stable CAD. (CAD legacy-coverage audit, **RESOLVED 2026-06-18** - CAD-061 reclassified SPEC_ONLY (disjoint-target miscite); Gap-50 tightened to the DAPT-indicated window (I21/I22 or Z95.5); guard tests confirm stable-CAD no-fire + post-ACS/post-PCI fire.) See register AUDIT-174.
- **AUDIT-176** - CAD-015 ticagrelor-blanket: post-ACS DAPT gap false-fired on patients correctly on prasugrel. (CAD legacy-coverage audit, **RESOLVED 2026-06-18** - added `!onPrasugrel` (613391) to the gate; recommendation reframed as the potent-P2Y12 choice; tests confirm prasugrel/ticagrelor no-fire, clopidogrel/none still fire.) See register AUDIT-176.
- **AUDIT-177** - CAD existence-proxy / narrative-vs-logic over-credits (CAD-007 + CAD-068 + CAD-018). (CAD legacy-coverage audit, **RESOLVED 2026-06-18** - all three reclassified DET_OK -> PARTIAL (honest underclaim: no lipid-panel-date / borderline-lesion / DAPT-start-date is threaded, so none can reach the spec target / DET_OK). 3-surface flipped.) See register AUDIT-177.
- **AUDIT-182** - I25.110 is NOT left main: the legacy CAD-IVUS rule gated `hasLeftMain = I25.110` (= ASHD + unstable angina per NLM, not left main), over-firing left-main IVUS for the entire unstable-angina cohort; chronic left-main has no ICD-10 code. (CAD chunk 1, **RESOLVED 2026-06-18** - CAD-IVUS firing + the wrong gate removed; CAD-069/070 -> SPEC_ONLY; CAD-071 holds SPEC_ONLY/Path-B; regression test confirms the over-fire closed.) See register AUDIT-182.
- **AUDIT-178** - PV over-credit cluster (5 rules): PV-6/GAP-PV-015 (HbA1c existence-proxy), PV-12/GAP-PV-033 (no resistant-HTN gate), PV-VARICOSE/GAP-PV-071 (bare I83 incl asymptomatic I83.9), PV-GRAFT-SURVEILLANCE/GAP-PV-098 (existence-proxy, no interval). (PV legacy-coverage audit, **RESOLVED 2026-06-18** - PV chunk 0 tightened all four to genuine gates (HbA1c >= 7.0%; >= 3 antihypertensive classes; symptomatic I83.0/.1/.2/.8; graft duplex > 12 mo OR never); regression + no-over-credit tests in pvChunk0.test.ts; all hold DET_OK now genuinely gated.) See register AUDIT-178.
- **AUDIT-179** - PV-003 ABI miscite + PV-076 PE code-mismatch: GAP-PV-003 ("Abnormal ABI <=0.90 without coded PAD") was fuzzy-matched to the antiplatelet rule with NO rule reading an ABI value threshold (the module's missing foundation); PV-ANTICOAG-VTE/GAP-PV-076 (spec post-PE) fired only on I82 (DVT), missing the PE population (I26) entirely. (PV legacy-coverage audit, **RESOLVED 2026-06-18** - PV chunk 0 built gap-pv-003-abnormal-abi (abi_left/right <= 0.90 + !hasPAD; >1.40 routed to PV-004) and re-cited GAP-PV-003 -> DET_OK; added I26 to the VTE rule; pvChunk0.test.ts guards both.) See register AUDIT-179.
- **AUDIT-180** - PV-BYPASS-EVAL over-match: one generic bypass-eval rule was cited for 3 distinct CLTI/iliac decision gaps (GAP-PV-017/018/024). (PV legacy-coverage audit, **RESOLVED 2026-06-18** - PV chunk 0 kept PV-017 (BEST-CLI heart-team, closest match); GAP-PV-018 (endo-vs-surgical) + GAP-PV-024 (TASC II C/D iliac) reclassified PARTIAL -> SPEC_ONLY (no anatomic/conduit/lesion-class signal threaded); applyOverrides PV entries.) See register AUDIT-180.
...
- **AUDIT-078** - Production Aurora backup config not in IaC; restore procedure untested (Phase 3, **RESOLVED 2026-06-07** - end-to-end restore test executed: EXACT row-count parity + 14,417/0 PHI-decrypt on a restored production snapshot (`beta1-phase1-signoff-20260607`), RTO ~20 min < 30 min target, clean teardown + leak verification; script defects fixed; see `docs/audit/BETA1_PHASE1_SIGNOFF.md`)
- **AUDIT-080** - RELOCATED to the MEDIUM (P2) section 2026-06-13 (supersede-not-delete tombstone): this LOW (P3) index placement was the stale outlier; the authoritative detail entry (line ~1414) + BUILD_STATE both read MEDIUM P2 production-readiness gate. See the MEDIUM (P2) section for the live entry. DRIFT-47 class.
...
- **AUDIT-120** - EP `EP-LAAC` (GAP-EP-011) uses `dxCodes.some(c => c.startsWith('Z88'))` as an OAC-contraindication gate (`gapRuleEngine.ts:4300-4301`), but ICD-10 `Z88.*` is "Allergy status" to ANY drug class (Z88.0 penicillin, Z88.1 antibiotics, Z88.4 anesthetic, Z88.5 narcotic, ...), not anticoagulant-specific - so the LAAC device-eligible gap FALSE-FIRES on any AF + age>=65 patient with any documented drug allergy (e.g. penicillin), an over-broad/over-detection precision defect. The sibling contraindication codes (D68.3 anticoagulant hemorrhagic disorder, K92.2 GI hemorrhage, I60/I61/I62 intracranial hemorrhage) are correctly OAC-relevant; only Z88 over-broadens. (Phase 0B EP clinical-code verification, **RESOLVED** 2026-06-16 - v3.0 EP chunk 1 dropped bare Z88, gate is bleeding-history only + GU major-bleed R31.0/N02; regression test added; LOW P3, was false-positive over-referral; full entry below)
- **AUDIT-121** - SH `SH-BICUSPID` (GAP-SH-008 + GAP-SH-009) gates on `dxCodes.some(c => c.startsWith('Q23.1'))` for "bicuspid aortic valve", but ICD-10 `Q23.1` is "Congenital insufficiency of aortic valve" (NLM-verified 2026-06-08) - bicuspid AV is `Q23.81` (`Q23.0` = "Congenital stenosis of aortic valve", also not bicuspid; the rule comment, the evidence triggerCriteria, AND the spec detectionLogic all miscode it). So the bicuspid-surveillance gap NEVER fires for a real bicuspid patient (Q23.81 never matches) and FALSE-FIRES on congenital aortic-insufficiency patients - a wrong-concept / cross-class miscode (sister to AUDIT-068). (Phase 0B SH clinical-code verification, **OPEN** 2026-06-08 - MEDIUM P2, missed-detection on the entire target population + false-positive, T3 surveillance, mitigated synthetic/pre-DUA; remediation = `Q23.1` -> `Q23.81`; operator confirms severity per §18)
- **AUDIT-122** - SH `SH-13` (GAP-SH-013, post-TAVR paravalvular-leak gap, `gapRuleEngine.ts:6462`) matches `dxCodes.some(c => c.startsWith('I35.1') || c.startsWith('I34.0'))` as "new regurgitation", but the spec gap is PVL-specific for an AORTIC prosthesis ("Post-TAVR paravalvular leak moderate+", structuredDataElements "TAVR, PVL grade"): `I35.1` (nonrheumatic aortic insufficiency) is the correct target, while `I34.0` (nonrheumatic MITRAL insufficiency) fires on post-prosthetic patients with unrelated mitral regurg and NO paravalvular leak - the first §16.6 over-broad / wrong-target case (sister to AUDIT-120; "clinically related" is not a pass). (Phase 0B SH clinical-code verification, **OPEN** 2026-06-08 - LOW P3, false-positive over-referral on a non-target population, T2 surveillance, mitigated synthetic/pre-DUA; remediation = drop `I34.0`, keep `I35.1`; operator confirms severity per §18) Classification effect: GAP-SH-013 DET_OK -> PARTIAL_DETECTION.
- **AUDIT-123** - Systematic ICD-10 valve-type concept confusion across the prosthetic-valve rule cluster (§16.6 propagation sweep: 81 `Z95.2/Z95.3/Z95.4` references in `gapRuleEngine.ts`). Per NLM: `Z95.2` = "Presence of prosthetic heart valve" (mechanical/general), `Z95.3` = "Presence of XENOGENIC heart valve" (= bioprosthetic/tissue), `Z95.4` = "Presence of other heart-valve replacement". The codebase has `Z95.2` and `Z95.3` INVERTED: `Z95.3` is treated as "mechanical" (`:11228`/`:11246`, `noMechanicalValveBio = !Z95.3`) and `Z95.2` is labeled "bioprosthetic" (`:5267`, `:5774`, `:10604` SH-VALVE-IN-VALVE). Effects: (a) bioprosthetic/TAVR-targeted rules (post-TAVR, ViV, PVL) gate on `Z95.2` (mechanical) - miss real bioprosthetic (`Z95.3`) + false-fire on mechanical; (b) the VD-1 mechanical-valve warfarin rule (`:4939`) flags ALL prosthetic valves incl. bioprosthetic (`Z95.3`) for lifelong warfarin, but only mechanical require it (bioprosthetic = 3-6mo then DAPT) - a clinical over-treatment / bleeding-risk sub-angle. Cross-module (SH + VHD). The SAFETY vector (the VD-1 warfarin over-treatment) is split out to **AUDIT-124**; AUDIT-123 is the detection-quality root cause. (Phase 0B SH clinical-code verification, **RESOLVED** 2026-06-17 - the bucket-D bioprosthetic inversions are corrected per the operator safety-first per-rule fork: broad bio rules now `Z95.2||Z95.3||Z95.4`; the drug-bearing aspirin gap + ViV are `Z95.3`-ONLY with the inverted `!Z95.3` logic removed; post-TAVR is `Z95.2||Z95.3`; the 3 stale "mechanical" trigger strings corrected. Guarded by `audit123Bioprosthetic.test.ts`. Path-B residual: generic-`Z95.2`-miscoded bioprosthetic, no mechanical-specific ICD-10 code.) Classification impact: GAP-SH-061 (ViV) + GAP-SH-011 (post-TAVR surveil) -> **APPLIED 2026-06-08 (SH synthesis) PARTIAL_DETECTION** (operator-decided 2026-06-08; §16.6 concept-match fail; the data-coupling caveat + 81-ref cross-module remediation are deferred, the classification is NOT - underclaim per the AUDIT-118 precedent).
- **AUDIT-124** (SAFETY) - the VD-1 mechanical-valve warfarin rule (`gapRuleEngine.ts:4939`, `hasMechanicalValve = Z95.2||Z95.3||Z95.4`) recommends lifelong anticoagulation (warfarin, RxNorm 11289) for ALL prosthetic valves incl. bioprosthetic (`Z95.3` = xenogenic, NLM-verified) and other-replacement (`Z95.4`), but per 2020 ACC/AHA VHD lifelong VKA is Class 1 for MECHANICAL valves ONLY - bioprosthetic take 3-6 months of anticoagulation then antiplatelet. So the rule drives over-anticoagulation / bleeding-risk over-treatment of bioprosthetic-valve patients (the opposite-harm mirror of a missed-anticoagulation gap). Root cause = the AUDIT-123 Z95.2/Z95.3 concept-inversion. (Phase 0B SH clinical-code verification, **RESOLVED** 2026-06-17 - the over-anticoagulation vector was code-closed by the AUDIT-164 shared-variable fix (`hasMechanicalValve = Z95.2||Z95.4`, excluding bioprosthetic `Z95.3`), which VD-1/VD-3/VD-6 all reference; the AUDIT-123 sweep added the NEGATIVE guard in `audit123Bioprosthetic.test.ts` (isolated `Z95.3` does NOT fire warfarin/DOAC/INR). DUA-escalation no longer applies. Path-B residual: generic-`Z95.2`-miscoded bioprosthetic treated as mechanical, safety-first.) VD-1 full reclassification stays with the VHD re-audit.
- **AUDIT-125** - §16.6(iii) severity-encoding class: SH DET_OK rules that target a SEVERE / moderate+ gap but gate only on `dx + age/symptom` with NO echo-severity threshold (EROA / regurgitant volume / valve area / mean gradient / RV grade) over-detect on sub-threshold (mild) lesions of the correct valve. Confirmed SH instances: **GAP-SH-064** (SH-11 `:6404`, `I34.0 + age>80`; gap targets severe MR for TMVR) + **GAP-SH-022** (SH-4 `:5337`, `I36.1 + right-heart symptoms`; gap targets severe/torrential TR) - both DET_OK -> PARTIAL. Predicate (per §16.6(iii)): severity IS echo-encoded in `labValues` and the rule IGNORES it, so this is over-detection, NOT an inherent ICD limitation. Class is cross-module (every dx-proxy DET_OK targeting a severity-gated gap across all 6 modules). (Phase 0B SH clinical-code verification, **OPEN** 2026-06-08 - LOW-MEDIUM, false-positive over-referral on sub-threshold lesions, mitigated synthetic/pre-DUA; remediation = add the lesion-appropriate echo-severity gate, data-coupled per AUDIT-118; operator confirms severity per §18) Cross-module retroactive re-application is a v2.0 Module-Parity pass (completed modules NOT re-opened here).
- **AUDIT-126** - SH `SH-15` (ASA-candidacy rule, `gapRuleEngine.ts:6519`) is mapped to GAP-SH-104 whose spec target is the OPPOSITE scenario: GAP-SH-104 = "Septal alcohol ablation: POST-procedure conduction surveillance" (DL "ASA procedure without post-procedure telemetry + ECG f/u"), but SH-15 detects PRE-procedure CANDIDACY (`I42.1 obstructive HCM + obstruction symptoms` -> "consider ASA or myectomy evaluation"). SH-15 never references ASA procedure history - it fires on symptomatic-obstructive-HCM patients who have NOT had ASA, while the gap targets patients who HAVE had ASA and need conduction monitoring - a §16.6(ii) wrong-target mismap (SH-15 correctly serves the sibling GAP-SH-105 ASA-vs-myectomy decision; the post-procedure-surveillance gap has no real detection). (Phase 0B SH clinical-code verification, **OPEN** 2026-06-08 - LOW P3, missed-detection of the post-procedure target + false-fire on pre-procedure HCM, T2 surveillance, mitigated synthetic/pre-DUA; remediation = a post-ASA procedure-history gate for GAP-SH-104; operator confirms severity per §18) GAP-SH-104 DET_OK -> **SPEC_ONLY** (operator-decided 2026-06-08: SH-15 is FULLY DISJOINT from the post-ASA target - zero true-positive overlap - so PARTIAL would overclaim; per the §16.6(ii) overlap rule, disjoint -> SPEC_ONLY).
- **AUDIT-127** - SH `SH-9` (GAP-SH-026 PFO closure, `gapRuleEngine.ts:5497`) fires on `I63.9 (cerebral infarction, unspecified) && age<60 && startsWith('Q21.1')` with NO coded stroke-etiology exclusion - the rule's OWN evidence object lists "Known alternative stroke etiology" as an exclusion (`:5522`) but the match logic never checks it (no `!hasAF (I48)` / `!hasCarotid (I65)`). The spec gap targets CRYPTOGENIC stroke, so an I63.9 + PFO + AF patient (cardioembolic, a competing cause) over-fires -> §16.6(ii) over-detection (evidence-vs-logic gap). (Phase 0B SH clinical-code verification, **RESOLVED** 2026-06-17 - the v3.0 SH close retired the legacy un-gated SH-9 and replaced it with SH-026 (PFO Q21.12 + cryptogenic I63.x + age<60 + EXCLUDES AF I48.x); shChunk5.test.ts guards the AF-exclusion. GAP-SH-026 -> DET_OK.) GAP-SH-026 was DET_OK -> PARTIAL 2026-06-08, now DET_OK 2026-06-17.
- **AUDIT-128** - SH `SH-ASD` (GAP-SH-027 ASD significant-shunt intervention, `gapRuleEngine.ts:10637`) gates significance on `I50.81` (right heart FAILURE), but the spec gap's significance signals are RV SIZE + PASP (structuredDataElements "ASD dx, RV size, PASP, closure"; DL "ASD with RV dilation OR PASP elevation"). RV failure is a LATE consequence: a significant ASD with RV dilation or PASP elevation but NOT yet RV failure is MISSED -> §16.5 under-detection (gate set too late on the disease course). (Phase 0B SH clinical-code verification, **RESOLVED** 2026-06-17 - the v3.0 SH close retired the legacy late-RV-failure-proxy SH-ASD and replaced it with SH-027 (significance gated on threaded PASP>=40 + EXCLUDES Eisenmenger I27.83). GAP-SH-027 held PARTIAL: RV size + Qp:Qs remain echo-morphology not threaded.) GAP-SH-027 DET_OK -> PARTIAL 2026-06-08, PARTIAL 2026-06-17 (legacy over-detection closed; residual Path-B).
- **AUDIT-129** - SH `SH-7` (endocarditis PROPHYLAXIS rule, `gapRuleEngine.ts:5434`) is mapped to GAP-SH-028 whose target is the disjoint scenario: GAP-SH-028 = "Infective endocarditis: Duke criteria WORKUP" (DL "Suspected IE without TEE + blood cultures x3"), but SH-7 detects PROPHYLAXIS candidates (`prosthetic valve Z95.2/3/4 + high-risk procedure Z01.2/Z96` -> "review endocarditis prophylaxis needs"). SH-7 never checks suspected-IE - it shares ZERO true positives with the Duke-workup gap (prevention vs diagnosis) -> §16.6(ii) fully-disjoint wrong-target. (Phase 0B SH clinical-code verification, **OPEN** 2026-06-08 - LOW P3, the suspected-IE workup gap has no real detection + the prophylaxis rule fires on a disjoint population, T2, mitigated synthetic/pre-DUA; remediation = a suspected-IE detection rule (I33 + TEE/blood-culture-absence) for GAP-SH-028, keep SH-7 for a prophylaxis gap; operator confirms severity per §18) GAP-SH-028 PARTIAL -> SPEC_ONLY (disjoint per the §16.6(ii) overlap rule).
- **AUDIT-141** - Unsourced financial-model multipliers rendered as dollars: two cited sites, one class - `godView.ts:236` (`REVENUE_PER_GAP_ESTIMATE = 2500`, multiplies into displayed dollars at `:256` / `:268` / `:343`) + `platformTotals.ts:36` (`totalOpportunity * 0.3` -> "quarterlyActionable"). Both are magic constants with no citation, config source, or dated provenance, surfaced as executive dollar figures. (Service-line aggregation Axis 3 / §20; **OPEN** 2026-06-11 - LOW P3, modeling-transparency (multiplier unsourced; figures carry estimate framing; no incorrect-real-source-data), mitigated synthetic/pre-DUA; remediation v2.0 = source or derive every financial multiplier; operator confirms severity per §18)
- **AUDIT-142** - Module-membership predicate divergence: `gaps.ts:58` counts per-module patients by FLAG-ONLY (`[field]: true`), but `godView.ts:29-37` / `modules.ts` declare the canonical predicate FLAG-OR-OPEN-GAP and a code comment EXPLICITLY claims they are mirrored ("Mirrored in modules.ts so GodView counts and per-module dashboard counts agree"). The same labeled "patients" cohort is a different population per surface (a patient with an open HF gap but no `heartFailurePatient` flag counts in godView/modules but not in gaps.ts), incl. the anomaly that a `gaps.ts/summary/all` module row can report `openGaps>0` with `patients=0`. `gaps.ts/summary/all` is currently an ORPHAN (zero `src/` consumers) but is a latent re-creation site for AUDIT-140 (a consumer summing `modules[].patients` re-creates the double-count). (Service-line aggregation Axis 3 / §20; **OPEN** 2026-06-11 - LOW P3, cohort-definition inconsistency on a currently-unconsumed endpoint, mitigated synthetic/pre-DUA; remediation v2.0 = adopt the canonical flag-OR-open-gap predicate; operator confirms severity per §18)
...
- **AUDIT-154** - Off-token neutrals (UI_CANON §4.3): standard Tailwind gray hexes bypass the titanium neutral scale. Counts re-run at filing 2026-06-12: 185 off-token gray uses (`#6B7280`=105, `#64748b`=28, `#374151`=15, `#1e293b`=13, `#E5E7EB`=11, `#111827`=8, `#9CA3AF`=4, `#F9FAFB`=1) + 66 `hover:bg-gray` utilities. Remediation = migrate to the titanium scale per the §4.3 mapping (`#6B7280`->titanium-500 etc.). (Phase 0C-3 UI-canon findings, structural/component tier; **OPEN** 2026-06-12 - LOW P3 proposed (pure token hygiene, no user-visible meaning change, mitigated pre-DUA/synthetic); remediation v2.0; operator confirms severity per §18) **SCOPE CORRECTION 2026-06-17 (UI Tranche A+B, feat/ui-tranche-a-b):** the 2026-06-12 filing UNDERCOUNTED. `gray` is NOT aliased to titanium in `tailwind.config.js` (no `gray:`/`slate:`/`neutral:` key - the config EXTENDS the palette, leaving Tailwind default `gray` live), so the true §4.3 surface is ~2458 = ~2277 `*-gray-N` utility classes (bg/text/border/hover/ring/from/to/etc.) + ~181 off-token gray hex literals - roughly 10x the 251 (185 hex + 66 `hover:bg-gray`) measured at filing. The hex-only / hover-only subset cannot be migrated coherently in isolation (changing `hover:bg-gray-N` while base `bg-gray-N` stays gray is incoherent). DISPOSITION (operator ruling 2026-06-17): NOT migrated in Tranche A+B; deferred to a dedicated whole-app neutral-tone tranche, folded into the Tranche-C view-consolidation (where call-sites are rewritten anyway, so the `gray`->titanium reclassification rides along). One in-flight exception corrected here: `index.css .status-glow-warning` off-token `#6B7280` -> gold, done as part of the AUDIT-153 warning-glow fix.
...
- **AUDIT-175** - CAD-NICORANDIL (`29987`) + CAD-TRIMETAZIDINE (`47832`): both RxCUIs NOT FOUND in RxNav (§16 fail); non-US / non-ACC-AHA-guideline drugs. (CAD legacy-coverage audit, **RESOLVED 2026-06-18** - both evaluator blocks + registry entries removed cleanly in CAD chunk 0 (dead code, unverifiable codes, no covered-spec-gap mapping); cadChunk0.test.ts confirms neither gap fires.) See register AUDIT-175.
- **AUDIT-181** - ApoB slug-thread: apolipoprotein B (LOINC 1884-6, NLM-verified) threaded to labValues['apob'] on both FHIR + CSV paths (AUDIT-070 lineage), converting CAD-009 from Path-B to DET_OK. (CAD chunk 1, **RESOLVED 2026-06-18** - both-paths threading + CAD-009 built; guarded by cadChunk1Batch.test.ts.) See register AUDIT-181.
- **AUDIT-183** - PV chunk-1 close (6th + FINAL module): 7 buildable gaps built (PV-004/034/038/040/041/058/062; 4 DET_OK + 3 PARTIAL); 3 of the banked-audit "~10 buildable" over-assumed threading and were NOT built (PV-021 wifi_score not wired to labValues, PV-037 ESR/CRP unthreaded, PV-060 no post-carotid-revasc ICD) - same over-assessment class as AUDIT-181. (PV chunk 1, **7 built RESOLVED 2026-06-18** + 3 Path-B **OPEN-DEFERRED** blocked-pile with unblock paths; validateCanonical 6/6; ALL-6-MODULES platform 315/603 = 52.2%.) [AUDIT-183-close figure 2026-06-18; current platform total 311/603 (51.6%) after AUDIT-191/194-B1/195, see PHASE_0B_CROSS_MODULE_SYNTHESIS.md.] See register AUDIT-183.
- **AUDIT-184** - HIGH (P1): hollow DET_OK - 22 DET_OK rules gated on lab/vital slugs threaded in NEITHER path (always undefined). 14 SILENT (0% sensitivity: HF-003/032/065/078/079/080/081, EP-004/005/030/033, CAD-023/031, PV-015) + 8 LIVE OVER-FIRE shipping false-positives in prod :315 (CAD-008/010/047/059, EP-043/044/048, PV-098). (T1 batch, **RESOLVED 2026-06-19** - threaded 12 slugs both paths (NLM-verified LOINCs) + CAD-008 slug-name fix; 8 over-fires verified CLOSED; hollowDetokThreading.test.ts 45 + hollowDetokRepair.test.ts 19. Honest DET_OK ~179 -> real 201. PV-098 self-catch = DRIFT-50.) See register AUDIT-184.
- **AUDIT-185** - LOW (P3): T0 net-new batch (6 built: PV-042 Behcet, PV-081 CTEPH-riociguat, PV-084 PAH ERA+PDE5i, PV-085 PAH-sotatercept, EP-010 rivaroxaban-food, EP-049 class-IC-CAST-safety) + the honest-count reckoning: the re-audit's "~78 free-DET_OK T0 sweep" was inflated by built-vs-buildable conflation + the hollow-check; the real net-new was 6 after 4 disqualifications (PV-086/SH-079/EP-084/EP-085) + the CTEPH I27.24 code self-catch. (T0 batch, **RESOLVED 2026-06-19** - 6 DET_OK, validateCanonical 6/6, real 201 -> 207. There is no large free T0 win; real levers are the T3/T1/T2 infra batches.) See register AUDIT-185.
- **AUDIT-184-CAD-EXT** - HIGH (P1): CAD hollow over-fire SURVIVING CLUSTER (AUDIT-184 extension) - PRE-EXISTING defect surfaced by the Synthea multi-file proof dry-run (NOT introduced by the ingestion work). CAD was 92% of all projected gaps (9,379 of 10,145; ~17 CAD gaps/CAD-patient), with 5 rules firing for 100% of CAD patients. Root: ~16 CORONARY_INTERVENTION rules gate on `hasCAD` (I25.*, correctly crosswalked from SNOMED 414545008) plus a negated secondary signal NEVER threaded by any ingestion path (PHQ scores; Z-codes Z23/Z50.0/Z71.3/Z70/Z73.6/Z13.3/Z66/Z82.4; CAD-FFR `stress_test` slug-name mismatch vs threaded `stress_test_months`) -> always-fire = zero discriminating signal. (Gap-engine remediation, **RESOLVED 2026-06-29** - 16 rules RETIRED -> SPEC_ONLY (left-main precedent): 10 hollow Z-code/PHQ + CAD-FFR slug-mismatch + CAD-BNP/CAD-CRP/CAD-LIVER operator-confirmed-SPEC_ONLY (Class 2a/2b optional / 2018-guideline-deprecated routine LFT) + CAD-STRESS de-dup-with-CAD-ECHO + women-specific primary-prevention-misfile; CAD-STATIN dose-unknown branch gated out (ARNI data-present precedent) keeping the genuine not-on-statin gap; 8 monitoring rules KEPT (staleness window = real overdue). NOT over-corrected: post-MI REHAB (dx-gated, AUDIT-173-accepted) + CAD-CALCIUM-SCORE + CAD-CCTA (real single-file-threaded slugs cac_score/ccta; over-fire is a multi-file data-threading limitation, not a defect) kept firing. Sample projection CAD 9,379 -> 4,113 (-56%); all 6 modules still fire. gaps.push 394 -> 378; audit184CadExtSuppress.test.ts 22 + updated count-guard/hollowDetokRepair/audit101. CANONICAL RECONCILIATION DONE (CAD-scoped): marker comments reformatted so extractCode no longer mis-parses them (evalOrphans 2 -> 0); applyOverrides --module CAD marked the 9 in-crosswalk suppressed rows SPEC_ONLY (GAP-CAD-010/044/045/048/060/068/079/080/089), the 8 registry-only rows are accepted registry-orphans; validateCanonical 6/6 VALID. The all-module code.json/cite-freshness residual is split out as AUDIT-191 (pre-existing drift, NOT this change). Local commits 9afcfea + 565fbfd; operator-merge gate.) See register AUDIT-184-CAD-EXT.
- **AUDIT-191** - MEDIUM (P2): pre-existing canonical cite drift - the committed `docs/audit/canonical/*.code.json` + crosswalk `ruleBodyCite.evaluatorBodyLineRange` are a self-consistent OLD SNAPSHOT that has drifted from the live `gapRuleEngine.ts`. A fresh `extractCode --all` + `refreshCites --all --dry-run` reports **250 stale evaluatorBodyLineRange cites across all 6 modules (HF 58, EP 26, SH 50, CAD 50, VHD 27, PV 39)**, accumulated from prior engine edits that landed without a canonical refresh. NO runtime correctness impact: validateCanonical passes 6/6 on the committed self-consistent snapshot (crosswalk cites match the committed code.json). It is canonical-freshness maintenance debt + the `auditCanonical` CI extractCode-diff staleness gate will flag it. PROOF it is pre-existing not AUDIT-184-CAD-EXT: HF 58 + EP 26 = 84 cites are for rules BEFORE the line-11620 CAD cut, which our change physically cannot shift. (Canonical maintenance, **RESOLVED 2026-06-30** - complete derived-artifact pipeline run (extractCode --all -> refreshCites --all -> reconcile --all -> renderAddendum --all -> renderSynthesis -> validateCanonical 6/6 VALID); the 250 predicted evaluatorBodyLineRange cites refreshed EXACTLY (HF 58, EP 26, SH 50, CAD 50, VHD 27, PV 39; specLine=0 regLine=0, evalBody-only), every changed crosswalk line a line-range integer with ZERO classification/gapType/count change. NOTE (self-catch, CI Gate 4): the first commit ran refreshCites/render but SKIPPED reconcile --all, so the committed *.reconciliation.json kept pre-refresh evaluatorCommentLine values; auditCanonical Gate 4 (derived-artifacts-divergent) correctly REJECTED it. Fixed by running the full pipeline incl reconcile --all (the §9.2 "no partial pipeline runs" discipline); non-CAD reconciliation.json = evaluatorCommentLine line-shift only, CAD reconciliation.json carries the same retired-rule catch-up (gapsPushCount 82 -> 66 + 17 registry-orphans). HONEST SCOPE NOTE: the refresh was 250 line-range cites PLUS one non-line-range delta - `extractCode` regen of `CAD.code.json` ALSO brought CAD into agreement with the already-merged-and-deployed (`:332`) AUDIT-184-CAD-EXT engine (`gapsPushCount` 82 -> 66 + 16 retired-rule entries dropped: CAD-FFR/STRESS-TEST/DEPRESSION/BNP-CAD/CRP/LIVER-STATIN/WOMEN-SPECIFIC + the hollow Z-code/PHQ discriminators). That CAD code.json content catch-up is the all-module-code.json residual #430 explicitly deferred here, NOT new drift; a faithful all-module extract MUST surface it (excluding it would leave CAD.code.json permanently stale). EP/HF/PV/SH/VHD code.json were pure line-number refreshes; code.meta.json changed only `generatedAt`. Operator-approved the full refresh including the CAD catch-up. NO runtime correctness impact - validateCanonical 6/6 VALID; the CI extractCode-diff staleness gate is now satisfied.) See register AUDIT-191.
- **AUDIT-192** - MEDIUM (P2): ingestion-write throughput + tenant-guard audit-noise - the bulk write path (patientWriter) did findUnique + per-Condition/per-Medication/per-Patient `upsert({where:{id}})` PER ROW, so the 25,571-patient proof `--execute` ran ~3h and emitted ~250K `TENANT_GUARD_VIOLATION` ERROR audit events (the audit-mode Layer-3 tenant guard fires on every non-create op whose `where` lacks a literal `hospitalId` - and `where:{id}` / `where:{hospitalId_mrn}` do). Root cost = BOTH ~300K serial Aurora round-trips (the structural dominant) AND the per-row winston audit-emit (file+console transport, NOT a DB write - no `audit_logs` bloat). (Ingestion ops, **RESOLVED 2026-06-29** - patientWriter rewritten to a BATCHED path: net-new rows via `createMany` (the op the tenant guard EXEMPTS by construction - schema-typed `data` carries `hospitalId`, so every row is tenant-scoped; PRESERVES the Layer-3 invariant, does NOT bypass it - no `__tenantGuardBypass`); existing-patient lookup via `findMany({where:{hospitalId, mrn:{in}}})` + lastAssessment update via batched `$transaction` of `updateMany({where:{id, hospitalId}})`; condition/medication re-affirm via bounded `updateMany` on re-ingest only. Round-trips ~302K -> ~5/batch (260 for the full 25,571 write); zero guard violations. Re-ingest UPDATES existing rows, not silent skip. Deterministic patient id `${hospitalId}::${mrn}` lets child createMany reference the PK without a per-row create round-trip. Tests: audit192BatchedWrite.test.ts (round-trips bounded not O(N); no per-row upsert/findUnique; every query carries hospitalId in where or createMany-data; re-ingest updates-not-skips) + medicationPersistence rewritten for the batched shape. ALSO fixed: the proof script's winston `[proof]` logs were invisible in prod (file transport) - switched to `console.log` (stdout -> CloudWatch). SCOPE NOTE: the writer still persists everything ACTIVE and does NOT parse medications.csv STOP / conditions.csv status, so true discontinuation/resolution capture (parse STOP + deactivate-absent-rows diff) is a SEPARATE follow-up filed as AUDIT-193; the batched re-affirm is the per-field-update hook. Coarser error granularity: a malformed row now fails its 500-row batch, not just itself. WHAT THIS DELIVERS NOW (honest scoping): FRESH-tenant ingest - the proof write, new demo tenants, MVP-wiring ingests - is fast + clean + correct. The AUDIT-193 status-change gap affects ONLY RE-ingest into an already-populated tenant (month-2+ of the recurring Epic extract), which is NOT the current use case.) See register AUDIT-192. Cross-ref AUDIT-193 (re-ingest status-change capture).
- **AUDIT-193** - MEDIUM (P2): re-ingest status-change capture - the AUDIT-192 batched write is production-ready for FRESH ingest, but the recurring monthly Epic full-extract model is a RE-ingest model, and the writer does NOT yet capture true clinical status CHANGE on month-2+ extracts. The batched re-affirm pass (the AUDIT-192 hook) refreshes `lastAssessment` and re-affirms existing rows ACTIVE, but it does NOT: (a) parse `medications.csv STOP` to mark a discontinued drug inactive, (b) parse condition resolution/abatement to resolve a no-longer-active dx, or (c) diff the extract against the stored set to deactivate rows ABSENT from the new month's extract. Net effect: a recurring re-ingest computes gaps against a stale "everything ever seen is still ACTIVE" picture. This is a clinical-decision-support CORRECTNESS issue, NOT cosmetic: e.g. a statin discontinued in month 2 still reads ACTIVE -> suppresses a real not-on-statin gap (false negative); or a resolved condition keeps firing its monitoring gaps (false positive). (Ingestion correctness, **RESOLVED 2026-07-01** - surfaced 2026-06-30 during the AUDIT-192 review; REQUIRED before a real client's SECOND monthly extract. Implemented with the STAGE-1-identified risk-split (STOP-parse safe in both modes; deactivate-diff guarded). (1) PARSER: read medications.csv STOP + conditions.csv STOP (MedicationRecord.stopDate optional; new ConditionRecord {icd10Code, stopDate}); absent = ongoing. (2) EXTRACT MODE (explicit, not assumed): new `ExtractMode = full | delta` param on writePatients, default full. STOP-parse runs in BOTH modes (explicit end date is unambiguous); the deactivate-diff runs ONLY in full-snapshot mode (in delta, absence = unchanged, never deactivate). (3) patientWriter re-ingest (all batched, AUDIT-192 win preserved): rows with a STOP -> DISCONTINUED+endDate (meds) / RESOLVED+abatementDate (conditions) via one uniform updateMany (active set) + one $transaction of updateMany (per-row-dated ended set), replacing the blanket ACTIVE re-affirm; then the GUARDED deactivate-diff = one hospitalId-scoped findMany of existing ACTIVE rows for PRESENT patients -> in-memory diff vs the new extract -> one updateMany deactivating the absent set (conditions -> INACTIVE, meds -> DISCONTINUED). (4) COMPLETENESS GUARD (fail-loud, two layers, both on the deactivate-diff): patient-count band - abort with a structured IngestCompletenessError (surfaced in WriteResult.completenessErrors, never silent) if the new extract's distinct-patient count is below 90% of the tenant's stored count (truncated-extract guard); per-patient scoping - deactivate ONLY for patients present in the extract (an entirely-absent patient's rows are never touched). (5) RUNNER COMPANION FIX (mandatory + standing-latent-bug fix): the runner previously included ALL conditions regardless of clinicalStatus (`conditions: true`), so a RESOLVED/INACTIVE condition still fired gaps for EVERY patient (not just re-ingest) - wrong independent of AUDIT-193. Fixed to `conditions: { where: { clinicalStatus: { notIn: [RESOLVED, INACTIVE] } } }`. notIn (NOT ==ACTIVE) deliberately - verify caught that ingestSynthea stores the raw uppercased FHIR code so genuinely-active RECURRENCE/RELAPSE would be wrongly dropped by ==ACTIVE; excluding only the two deactivation values keeps them firing. Medications already filtered status ACTIVE (the asymmetry the STAGE 1 inventory found - why the condition filter was the missing half). Round-trips: fresh ingest UNCHANGED (~5/batch, isReingest false -> no status pass, no diff; proof/new-tenant path zero added cost bar one count/extract); re-ingest ~8 -> ~13/batch worst case (BOUNDED per batch, constant, NOT O(N) - the AUDIT-192 ~1000x win preserved, +1.6x constant factor). Tests: audit193ReingestStatusChange.test.ts (11: STOP-parse both entities, deactivate-diff full-mode, guard aborts <90% with STOP-parse still applied, per-patient scoping, delta skips diff, runner notIn filter keeps ACTIVE/RECURRENCE/RELAPSE) + multiFileParser STOP-read + audit192/medicationPersistence mocks updated. Full suite green; tsc clean; DRIFT-44 clean. No gapRuleEngine change -> canonical unaffected. Lineage AUDIT-192; runner companion also closes the standing RESOLVED-condition-fires latent bug.) See register AUDIT-193. Lineage AUDIT-192.
- **AUDIT-194** - cross-module hollow over-fire sweep (the AUDIT-184-CAD-EXT hollow-read defect is a CODING PATTERN, not CAD-specific; swept HF/EP/SH/VHD/PV the same way CAD was found). Method: static threading analysis (a rule whose DISCRIMINATING gate negates a signal NO ingestion path threads is a tautology -> fires ~100% of its dx-eligible cohort; the fire rate is the symptom, the unthreaded gate is the provable cause). Every flagged gate quote-verified in gapRuleEngine.ts. Threading reference: the synthea/FHIR population reaches the engine with dx (SNOMED->ICD10 clinical codes), meds (RxNorm, dose null), age, gender, and labValues limited to the ~29 ECHO_LOINC_TO_SLUG observation slugs; ALWAYS EMPTY = race, procedureCodes ([]), med dose, all Z-codes, and every slug outside that map (bnp, nt_probnp, ferritin, tsat, sodium, potassium, egfr, qtc_interval, qrs_duration, abi_right/left, la_diameter, sts_score, pasp, nyha_class, echo_months, vegetation_size). Same AUDIT-070 threading root (FHIR/observation path threads far fewer slugs than the CSV LAB_FIELDS allowlist). TWO DISTINCT FINDINGS:
  - **PART A - OVER-FIRE (HIGH (P1), actionable now, same class as AUDIT-184-CAD-EXT):** HF and VHD carry the same hollow over-fire cluster CAD did. HF cluster = 6 live rules firing ~100%: HF-74 (bnp+nt_probnp both undefined, :5365), HF-90 (same double-negation, E85-gated, :9568), HF-91 (!Z99.8 uncrosswalked Z-code) [negated-unthreaded] + HF-20 cardiac-rehab, HF-37-FU discharge-followup, HF-38 influenza [bare `hasHF && !contra`, no discriminator, fire on every HF patient]. HF-38 is the IDENTICAL influenza/immunization-Z rule already retired to SPEC_ONLY on the CAD side (CAD-INFLUENZA) yet still LIVE in HF - proving AUDIT-184 fixed the INSTANCE, not the CLASS. (HF-024/025 additionally carry a non-functional !ICD Z95.810 guard - borderline.) VHD cluster = 8 live rules: VD-ECHO-INTERVAL (echo_months undefined, :16096, broadest - every valve dx), VD-PREOP-ASSESSMENT (sts_score undefined, :16157), VD-FUNCTIONAL-STATUS (nyha_class undefined, :16127), VD-PULMONARY-HTN (pasp undefined, :16187) [negated-unthreaded] + VD-7/16/18/10 [dx-only / symptom-negation over-broad, no severity gate]. MINOR (note, do NOT cluster, lower severity): EP = 5 benign SCREENING_DUE/DOCUMENTATION prompts (EP-SYNCOPE, EP-LEFT-ATRIAL-SIZE :14000, EP-ANTICOAG-REASSESS :14030 negated-unthreaded + EP-CARDIOVERSION-TIMING, EP-EXERCISE-EP bare-dx) - not actionable therapy gaps; PV = 1 rule (PV-2 ABI screening, !hasABI :7254). SH = CLEAN of over-fire (verified: every SH rule touching an unthreaded signal gates on it POSITIVELY -> silent-under-fire, not over-fire). Remediation: same SPEC_ONLY / slug-threading discipline as AUDIT-184-CAD-EXT, with per-rule sparse-real-vs-hollow clinical judgment (do NOT over-correct - suppressing a legitimate overdue-monitoring gap is as wrong as the over-fire, the discipline that preserved post-MI REHAB + CAC/CCTA in the CAD pass).
  - **PART B - UNDER-FIRE (the MIRROR defect, SEPARATE remediation, NOT fixed by suppression):** ~50 rules across EP/SH/VHD gate POSITIVELY on unthreaded signals (device Z95.x status, procedureCodes ([]), pasp/sts_score/tapse/fac/qtc/qrs/echo metrics) and therefore fire 0% - the entire mechanical/bioprosthetic-valve family (~20 VHD rules incl the Tier-S mechanical-valve anticoagulation/INR content) and the EP device family (~16 rules) are ENTIRELY DARK. Same AUDIT-070 threading root. This is under-detection + a coverage-ceiling driver; it requires SIGNAL THREADING (add the LOINC/Z-code/procedure ingestion paths), NOT rule suppression. Scoped as SEPARATE future work that threads into the coverage phase; NOT remediated with Part A.
  - **PART A REMEDIATION (RESOLVED 2026-06-30, HF+VHD):** 14 flagged rules dispositioned via the AUDIT-184-CAD-EXT RETIRE pattern (rule body replaced by a marker comment extractCode does not mis-parse; no runtime gaps.push; registry entry orphaned). 7 RETIRED to SPEC_ONLY (permanent): HF-38 influenza (IDENTICAL to CAD-INFLUENZA already retired under AUDIT-184-CAD-EXT - the concrete §20 pattern-class-sweep precedent), HF-91 sleep-apnea (!Z99.8 broken+uncrosswalked proxy), HF-37-FU discharge-followup (bare hasHF, non-functional 7-day check), VD-PREOP-ASSESSMENT (sts_score not threadable), VD-FUNCTIONAL-STATUS (nyha_class not threadable), VD-7 exercise-restriction-AS>65 (over-broad, no counseling signal), VD-16 mixed-valve (bare I35.2, no discriminator). 4 SPEC_ONLY-PENDING-THREADING (interim suppress, restore-on-threading, NOT permanent): HF-74 + HF-90 (bnp/nt_probnp -> Part B AUDIT-194-B1), VD-PULMONARY-HTN (pasp -> Part B AUDIT-194-B2), VD-ECHO-INTERVAL (echo_months -> Part B AUDIT-194-B3). 3 KEPT (do NOT suppress - would over-correct, post-MI-REHAB discipline): HF-20 cardiac-rehab (analog of preserved CAD post-MI REHAB, genuine under-utilized care gap, Class 2a), VD-18 exercise-test-asymptomatic-severe-valve (negation is on THREADED symptom R-codes, clinically correct Class 2a), VD-10 pregnancy-risk (real threaded gender/age/dx, narrow high-value cohort). VD-ECHO-INTERVAL fix-vs-suppress decision: SUPPRESSED (not fixed) - echo_months is derivable from ingested echo obs dates BUT not trivially (the engine receives labValues as slug->number only, no dates; deriving needs runner + projection + rule-logic + section-16 changes = a multi-file Part B feature), so per the operator rule (trivial->fix, else->suppress) it routes to pending-threading. EP (5 benign doc prompts) + PV (PV-2 ABI) NOT remediated here (minor, lower severity, separate). Engine gaps.push net -11 (HF -5, VHD -6); tsc clean; DRIFT-44 clean; canonical reconciled (full section-9.2 pipeline incl reconcile per the AUDIT-191 Gate-4 lesson), validateCanonical 6/6 VALID. Tests: audit194 suppress/keep coverage + full suite green.
  - **PART B WORK ITEMS (signal threading, AUDIT-070 lineage):** **AUDIT-194-B1 = PARTIALLY DELIVERED 2026-07-01 (Threading Tranche 1).** NT-proBNP (LOINC 33762-6, +71425-3) threaded into ECHO_LOINC_TO_SLUG - source-confirmed present in the Synthea observations.csv feed. **BNP (33747-0/30934-4) is ABSENT from Synthea (verify-don't-assume: 0 occurrences across 334K sampled rows) -> NOT threaded, stays real-EHR-only**; the bnp-threshold rules (HF-18/21/77/85/92) remain honestly dark on Synthea (real-EHR or a separate NT-proBNP-threshold rule-logic change, NOT a threading task - do not map NT-proBNP onto the bnp slug, different reference ranges). **HF-74 + HF-90 RESTORED** from SPEC_ONLY-pending-threading to live (gaps.push 367 -> 369; GAP-HF-051/052/053/055/056/134 reverted SPEC_ONLY -> PARTIAL_DETECTION, re-bound to gap-hf-90-amyloid-biomarker; validated NOT hollow - fire only for patients lacking a natriuretic peptide, not ~100%). Tranche 1 also threaded 8 more cheap serum labs (egfr 33914-3, ferritin 2276-4, potassium 6298-4/2823-3, ldl 18262-6, sodium 2947-0/2951-2, tsat 2502-3, bmi 39156-5, triglycerides 2571-8) + a creatinine completeness add (38483-4 blood variant) - all source-confirmed; these un-darken the HF renal/iron/K/hyponatremia + CAD lipid VALUE-threshold rules at RUNTIME (measurable only via a population re-ingest, in-VPC). AUDIT-194-B2 = thread echo PASP LOINC (restores VD-PULMONARY-HTN) - Tranche 2, PENDING. AUDIT-194-B3 = derive echo_months from most-recent IMAGING_TYPES observedDateTime in the runner (restores VD-ECHO-INTERVAL) - Tranche 2, PENDING. Plus the broader Part B under-fire tranche (~50 rules on Z95.x/procedureCodes/pasp/sts_score/qtc) - Tranche 3, PENDING.
    - **AUDIT-194-B3 = DELIVERED 2026-07-03 (Threading Tranche 2, echo cluster).** The Tranche-2 source-check (verify-don't-assume) FLIPPED the premise: Synthea emits ONLY LVEF among echo signals (S3 observations.csv 1.05M-row sample: LVEF 10230-1 present x229; the entire echo-morphometric cluster - pasp/lvesd/tapse/fac/valve_severity/mitral_regurg_grade/la_diameter/la_volume_index/aortic_root/vegetation_size - returns 0 rows; even the 13 already-mapped echo-valve LOINCs return 0). So the "medium-cost echo cluster mapping" is NOT a mapping add; it is REAL-EHR-ONLY. **B3 (echo_months) DELIVERED:** echo_months DERIVED in the runner (shared `deriveEchoMonths`: most-recent echo PROCEDURE date [SNOMED 40701008 Echocardiography x1037 / 433236007 TTE x283 / 105376000 TEE - Stage-1b procedures.csv source-confirmed present] union the lvef observation date, on the UNFILTERED set before the 365-day ECHO_CUTOFF). VD-ECHO-INTERVAL RESTORED with the HOLLOW-SAFE gate `echo_months !== undefined && echo_months >= 12` (fire only on a documented-but-stale echo, NEVER on absence - reverses the AUDIT-194 ~100% hollow). gaps.push 368->369; classified PARTIAL (flat 12mo approximates the severity-dependent interval; severity un-threaded). HF-72/HF-73 now discriminate (stop firing when a recent echo exists). Coverage TOTAL unchanged (311/603, 51.6%) - gap-vd-echo-interval is a generic surveillance rule not spec-crosswalked (reconcile conf 0.444 < threshold), so the win is RUNTIME (real overdue-echo detection), not a crosswalk-coverage increment; no force-bind (would be the section-16.6(ii) miscite defect). **B2 (pasp) = REAL-EHR-ONLY-DEFERRED:** pasp is Synthea-absent -> VD-PULMONARY-HTN re-tagged real-EHR-only (NOT pending-threading; threading cannot help pre-DUA); if restored on real-EHR the non-hollow gate is pasp !== undefined && pasp > 35. The echo-morphometric cluster + coronary_cta_months (no CT coronary angiography in Synthea; only invasive angiography 33367005) + graft_duplex_months (no graft duplex) all real-EHR-only-deferred. stress_test_months source-present (76746007 x64) but its consumer rules need clinical-semantics review -> filed AUDIT-197. Synthea-generation investment to emit echo morphometrics filed as strategic finding AUDIT-198. validateCanonical 6/6; full suite green; tests audit194B3EchoInterval + echoRecency.
  (Cross-module clinical-accuracy, **Part A RESOLVED 2026-06-30 (HF+VHD; EP/PV minor deferred) / Part B OPEN-DEFERRED**. Lineage AUDIT-184 -> AUDIT-184-CAD-EXT -> AUDIT-194; establishes CLAUDE.md §20 pattern-class-sweep discipline; AUDIT-070 threading lineage for Part B.) See register AUDIT-194.
- **AUDIT-195** - CAD lipid-intensification redundancy consolidation (CONSOLIDATION, not suppression). The Threading-Tranche-1 full-population proof (demo-synthea-threaded, 25,571 patients) surfaced an EXACT 2.00x redundancy in the CAD lipid gates: `gap-cad-ezetimibe` ("Consider ezetimibe add-on for LDL not at goal on statin") and `gap-cad-pcsk9` ("Consider PCSK9 inhibitor for LDL not at goal on maximally tolerated statin") gate on the IDENTICAL population (`hasCAD && !hospice && on-statin(83367/301542/36567/42463) && ldl>70`), differing only in the already-on-add-on negation (`!onEzetimibe` vs `!onPCSK9`). In Synthea no patient is on either specialty add-on, so both fired for the same cohort: measured ezetimibe 2,180 = pcsk9 2,180 = DISTINCT patients 2,180, total 4,360 = 2.00x. Root: two parallel gaps for ONE clinical decision point (LDL not at goal on statin -> intensify), presented as separate rows. (Clinical-content / gap-redundancy, **RESOLVED 2026-07-03** - the two firing blocks are CONSOLIDATED into ONE stepwise `gap-cad-lipid-intensification` gate `on-statin && ldl>70 && (!onEzetimibe || !onPCSK9)` that fires ONCE per patient and carries both non-statin add-on options in guideline order (Step 1 ezetimibe COR 2a LOE B-R IMPROVE-IT; Step 2 PCSK9i if LDL remains >70 on statin+ezetimibe, COR 2a LOE A FOURIER/ODYSSEY). NOT suppression: the LDL-not-at-goal gap is a real Class-I secondary-prevention item and still fires for the real subset; only the double-count is removed. Edge cases preserved: on-neither collapses 2->1, on-ezetimibe-only still fires once (surfaces PCSK9i step), on-both fires 0 (maximal). `gap-cad-ezetimibe` repurposed -> `gap-cad-lipid-intensification` (GAP-CAD-003 rebind, DET_OK anchor); `gap-cad-pcsk9` firing block RETIRED -> SPEC_ONLY (GAP-CAD-004/005; registry metadata retained as accepted regOrphan, AUDIT-184/194 retirement discipline). gaps.push 369->368; CAD evaluator/push 66->65; registry stays 83. Threaded-tenant effect: CAD drops EXACTLY 2,180 (55,936 -> 53,756), removing exactly the double-count, no more. GOTCHA CAUGHT + documented (AUDIT_METHODOLOGY §5.4): first retirement comment started with a line-leading PREFIX-NNN token (`CAD-PCSK9:`, then `AUDIT-195:`) which extractCode re-detected as a live block and swallowed the adjacent colchicine gaps.push; fixed to the plain-word-first AUDIT-182 left-main style, colchicine-fires-clean test added. Distinct-trigger CAD lipid rules (high-intensity-statin, lipid-panel-followup, icosapent, FH LDL>=190, Lp(a)) NOT touched. Full §9.2 pipeline incl reconcile; validateCanonical 6/6; full suite green (2202 pass); tsc clean; DRIFT-44 clean. Tests: audit195LipidConsolidation.test.ts (fires-once + both-options + 3 edge cases + colchicine-intact + distinct-rules-unchanged) + AUDIT-102 behavioral test adapted to consolidated semantics. Lineage AUDIT-194-B1 (the proof that surfaced it).) See register AUDIT-195. Carries AUDIT-196.
- **AUDIT-196** - Ezetimibe COR over-statement (section-16 clinical-accuracy correction, SEPARATE from the AUDIT-195 redundancy fix). The former `gap-cad-ezetimibe` rule labeled itself `classOfRecommendation: 'Class 1', levelOfEvidence: 'A'`. Per the 2018 AHA/ACC Guideline on the Management of Blood Cholesterol, ezetimibe add-on for very-high-risk ASCVD on maximally tolerated statin with LDL >=70 mg/dL is **COR 2a, LOE B-R (IMPROVE-IT)** - NOT Class 1. Only the statin itself is Class 1; the non-statin add-on step is a 2a "reasonable" recommendation. A COR over-statement mis-signals guideline strength to the reviewing clinician. (Phase 0B clinical-code / section-16 correctness, **RESOLVED 2026-07-03** - folded into the consolidated `gap-cad-lipid-intensification` rule which now carries `classOfRecommendation: 'Class 2a'` with stepwise LOE (B-R ezetimibe step; A PCSK9i step); registry `classOfRecommendation` corrected '1'->'2a', `levelOfEvidence` 'A'->'B-R'. Verified against the 2018 AHA/ACC Blood Cholesterol Guideline COR/LOE per section-16 authoritative-source discipline. Filed as its own clinical-accuracy line, not buried in the consolidation note.) See register AUDIT-196. Delivered with AUDIT-195.
- **AUDIT-197** - CAD-STRESS-MODERATE presence-proxy defect (section-16 clinical-accuracy, surfaced by the AUDIT-194-B3 Tranche-2 procedures source-check). The CAD-STRESS-MODERATE rule uses `labValues['stress_test_months'] !== undefined` as a proxy for "moderate ischemia" - i.e. it treats the mere PRESENCE of a prior stress test as evidence of ischemia and fires on it. That is clinically unsound: having had a stress test is not the same as having an abnormal/ischemic result (most stress tests are normal). The Tranche-2 procedures source-check confirmed the base signal IS present in Synthea (cardiovascular stress testing SNOMED 76746007, 64 rows in an 845K sample), so deriving stress_test_months would EXPOSE this by making the rule fire for every patient with any prior stress test. Companion note: CAD-STRESS-INTERMEDIATE gates on `stress_test_months === undefined` (fire-on-absence by clinical intent - "intermediate-risk patient with no stress test"), a different and defensible pattern. (Phase 0B clinical-code / section-16, **OPEN** - filed NOT fixed; needs a clinical-semantics review of the two stress-test consumer rules before stress_test_months is threaded. This is why stress_test_months was DEFERRED from Tranche 2 despite being source-present. Not force-threaded.) **RESOLVED 2026-07-08 (retired to SPEC_ONLY):** STAGE-1 identified the rule precisely as **CAD-ISCHEMIA-GUIDED** (registry `gap-cad-ischemia-guided`, crosswalk **GAP-CAD-031**, the code-name for the filed "CAD-STRESS-MODERATE"): `const hasModerateIschemia = labValues['stress_test_months'] !== undefined; // proxy for stress test done` - a variable named for a FINDING bound to a bare test-existence check (self-admitted by the comment + "Prior stress test documented" triggerCriteria). It never fired (stress_test_months unthreaded -> always undefined) and could not discriminate ischemia, so the **DET_OK crosswalk classification was an over-claim**. Moderate ischemia is a RESULT finding (abnormal stress-test result / reversible perfusion defect / ischemic-burden %), Synthea-ABSENT / unthreaded everywhere = real-EHR-only (no honest gate to re-key on). RETIRED: firing block removed (gaps.push 369->368; CAD evaluator/push 65->64), **GAP-CAD-031 DET_OK -> SPEC_ONLY** via applyOverrides, registryId dropped; any-coverage 311->310 (honest downward correction - the rule never truly detected the gap). Re-tagged real-EHR-only with the RESTORE PATH recorded: re-key on a threaded abnormal-stress-RESULT / ischemic-burden signal, NEVER stress_test_months (threading the recency slug would resurrect the false-positive - the reason Tranche 2 deferred it); 2021 ACC/AHA/SCAI (ISCHEMIA trial) COR 2a context preserved. **section-20 SINGLE-INSTANCE confirmed:** the disease-named-but-presence-checked misuse is unique to hasModerateIschemia; the presence-check pattern is legitimate everywhere else (data-availability / value-threshold), and the adjacent CAD-NUCLEAR-STRESS fire-on-absence (`stress_test_months === undefined` -> "no stress test documented") is a distinct valid care-gap, LEFT UNTOUCHED (proven still-firing). section-5.4 swallow guard held (adjacent CAD-MINOCA gaps.push intact - proven). validateCanonical 6/6; full suite green (117 suites/2255); tests audit197IschemiaGuided + hollowDetokRepair CAD-031 block updated to assert the retirement. See register AUDIT-197. Lineage AUDIT-194-B3.
- **AUDIT-198** - Synthea-generation strategic item: emitting echo morphometrics + PASP is a pre-DUA test-data investment decision, NOT a threading task (STRATEGIC, do NOT build). The AUDIT-194-B3 source-check proved the entire echo-morphometric cluster (pasp, lvesd, tapse, fac, valve_severity, mitral_regurg_grade, la_diameter, la_volume_index, aortic_root, vegetation_size) is ABSENT from standard Synthea - so the ~34 echo-morphology gaps + the mechanical/bioprosthetic-valve rule families stay dark on Synthea regardless of mapping work. Two forward paths for OPERATOR DECISION: (a) build a CUSTOM Synthea module that emits these echo observations (pre-DUA synthetic-data investment, unblocks pilot demos + the SH/VHD valve cluster now), vs (b) WAIT for real-EHR data post-DUA (no synthetic investment, but the valve cluster stays dark until the DUA lands). Filed as a PATH_TO_ROBUST forward-path note (Section 1.4 threading node). (Strategic / test-data, **OPEN - operator decision**, do not build.) See register AUDIT-198. Lineage AUDIT-194-B3 / AUDIT-070.
- **AUDIT-199** - HIGH (P1): PV-1 PAD-statin un-propagated section-20 sibling over-fire (the AUDIT-184-CAD-EXT dose-unknown suppression fixed the CAD INSTANCE but never propagated to the byte-for-byte-identical PV-1 rule - the HF-38 / CAD-INFLUENZA "fix-the-instance-not-the-class" redux, exactly the section-20 pattern-class-sweep failure mode). Surfaced by a STAGE-1 inventory whose STATED PREMISE was STALE (DRIFT-45 catch): the task described CAD-STATIN firing 7,616/7,616 = 100% as a live presence-as-proxy tautology, but CAD-STATIN was ALREADY twice-remediated - AUDIT-101 (#347, 2026-06-04) made it dose-aware via the shared `highIntensityStatinStatus` helper (agent identity + mg threshold, name-fallback so atorva/rosuva survive IN-vs-SCD coding), and AUDIT-184-CAD-EXT (2026-06-29, deployed :332) gated the dose-unknown branch OUT of the CAD gate (gapRuleEngine.ts:6829). So the 7,616/7,616 is a PRE-:332 snapshot; **CAD-STATIN needs no change** (STAGE-1 refuted the premise from running code). The DECISIVE fix-vs-SPEC_ONLY check found PV-1 (gap-pv-1-pad-statin, gapRuleEngine.ts:7225) shares the same helper but its gate (7227) excluded ONLY `on_high_intensity`, NOT `agent_dose_unknown` - so on a dose-less feed (Synthea; csvParser.ts:472 stores medicationName but hardcodes doseValue null, syntheaProofRun.ts:316 likewise) EVERY statin-present PAD patient resolves to agent_dose_unknown and the qualified branch fired for ~100% of PAD patients with zero discriminating signal (the identical hollow over-fire AUDIT-184 killed for CAD). **RESOLVED 2026-07-08 (option 2a - propagate the suppression):** PV-1 gate now excludes BOTH on_high_intensity AND agent_dose_unknown, mirroring CAD-STATIN:6829 exactly (fire-rate before/after: dose-unknown atorva/rosuva PAD patient was a QUALIFIED fire -> now NO fire; genuine not_high_intensity - no atorva/rosuva agent, e.g. no-statin or simvastatin/pravastatin-only PAD patient - STILL fires the real Class 1 gap, PRESERVED not over-corrected). **GAP-PV-008 DET_OK -> PARTIAL_DETECTION** via applyOverrides (the DET_OK was an over-claim - fired dose-unknown on ~100% with no discriminating signal - mirroring the GAP-CAD-031 correction; PARTIAL not SPEC_ONLY under underclaim-governs because PV-1 STILL detects the genuine no-agent gap, so it detects a real subset while dose-blind to the moderate-dose-should-intensify cohort; registryId retained; GAP-PV-009 was already PARTIAL). Coverage: any-coverage UNCHANGED 310/603 (DET_OK -> PARTIAL both count as coverage; unlike the AUDIT-197 full retirement which dropped 311->310, PV-1 stays live). gaps.push UNCHANGED (368; block retained, gate condition only - HF 86 / EP 72 / SH 55 / CAD 64 / VHD 46 / PV 45 all invariant). section-5.4 swallow guard held (suppression comment plain-word-first "Suppress the dose-unknown branch..."; verified neighbors keep their gaps.push - upstream VD mechanical-valve-warfarin + downstream PV-2 ABI both proven still-firing). PV-2 (ABI, AUDIT-194-dispositioned) untouched; the fail-SAFE dose-dependent under-detect rules (ARNI :5277, rivaroxaban/apixaban :5699/:5735/:5770, digoxin :14599, BB-target :14539 - all gate `doseValue != null && < target`, dose-less -> never fire -> safe direction) untouched. Guidelines: 2024 ACC/AHA/Multisociety PAD (Gornik 2024, COR 1 LOE A) referencing the 2018 AHA/ACC Blood Cholesterol high-intensity tier (atorvastatin 40-80mg / rosuvastatin 20-40mg); section-16 preserved (no new codes; AUDIT-101 RxNav set 83367/301542/36567/42463 unchanged). validateCanonical 6/6; full suite green (118 suites / 2267); tests audit199PadStatinSuppress (new: suppression + genuine-gap-preserved + CAD/PAD symmetry + neighbors-intact + hospice) + audit101StatinIntensity PAD dose-unknown assertion updated (QUALIFIED -> SUPPRESSED) + extractCode.test VD-PANNUS anchor 15732->15739 (+7 line-shift). (Cross-module clinical-accuracy / section-20 pattern-class sibling sweep, **RESOLVED 2026-07-08**. Lineage AUDIT-184-CAD-EXT -> AUDIT-194 -> AUDIT-199; DRIFT-45 stale-premise catch on CAD-STATIN.) See register AUDIT-199.
- **AUDIT-199-B** - Dose-parse threading, SCOPED ACTIVATION (statins + DOACs). Synthea's medications.csv DESCRIPTION carries the mg strength in text ("Atorvastatin 80 MG Oral Tablet") but csvParser.ts:472 dropped it (doseValue hardcoded null; syntheaProofRun.ts:316 likewise), so the dose-aware rules degraded to agent_dose_unknown and under-detected the on-moderate-dose cohort. **RESOLVED 2026-07-09 (SCOPED ACTIVATION):** two-point build - (1) PARSE: extended MedicationRecord (csvParser.ts:23) with doseValue/doseUnit + `parseDoseFromDescription` (single "N MG"; "N MCG" normalized /1000; COMPOUND "A MG / B MG" -> first token ONLY IF the leading agent is a single-agent statin, else null so the sacubitril/valsartan ARNI compound and Caduet/Vytorin combos hand NO value to any rule); parse wired at csvParser.ts:472 + mirrored in syntheaProofRun.ts:316 (proof honesty). (2) PERSIST: patientWriter.ts:244-250 now writes doseValue/doseUnit (schema Medication.doseValue/doseUnit already exist - no migration). ACTIVATED (clean parse, no gate change - highIntensityStatinStatus already discriminates once dose present): CAD-STATIN + PV-1 now fire for the moderate-dose atorva 10/20/30 + rosuva 5/10 cohort (the AUDIT-101 dose-blind false-negative CLOSED - the COVERAGE GAIN); statins are once-daily so tablet-mg == daily-dose (clean). HOLLOW GUARD proven: a mixed statin cohort fires a real SUBSET (3/5 in the golden test - the genuinely-not-high-intensity patients), NOT ~100%. DOACs ACTIVATED (apixaban verified UNAMBIGUOUS - each medications.csv row is a single-strength product "apixaban 5 MG" / "2.5 MG", not a compound; EP-003 riva + EP-004/005 apix fire only on their threaded eGFR/age/creatinine subsets). **GAP-PV-008 PARTIAL_DETECTION -> DET_OK** via applyOverrides (dose-blind false-negative closed on the CSV audit path; residuals documented, not re-opening PARTIAL). any-coverage UNCHANGED 310/603 (DET_OK 203->204, PARTIAL 107->106 - the gain is RUNTIME, not a crosswalk-count increment, like AUDIT-194-B3). gaps.push UNCHANGED 368 (no rule added; statin/DOAC activation is data-driven, BB/dig/ARNI guarded by a flag). validateCanonical 6/6; full suite green (119 suites/2285). Tests audit199bDoseParse (new: parse + statin coverage-gain + hollow-guard + DOAC subsets + kept-suppressed) + medicationPersistence dose round-trip + 6 pre-existing dose-fire tests flipped to kept-suppressed + extractCode VD-PANNUS 15739->15760 + multiFileParser medRecord shape. **FOUR follow-ons filed (kept dose-unknown-suppressed via AUDIT_199B_DOSE_RULE_SUPPRESSED, each removed individually when built):** AUDIT-199-B-BB, AUDIT-199-B-DIG, AUDIT-199-B-ARNI, AUDIT-199-B-REDOX (below). (Threading / AUDIT-070 lineage, **RESOLVED 2026-07-09 - statin+DOAC slice**.) See register AUDIT-199-B. Lineage AUDIT-199 / AUDIT-101 / AUDIT-070.
- **AUDIT-199-B-BB** - HF-BB-TARGET (gapRuleEngine.ts:14547) kept dose-unknown-suppressed after the AUDIT-199-B dose parse: the target is TOTAL-DAILY (carvedilol 50, metoprolol succinate 200, bisoprolol 10 mg/day) but the DESCRIPTION mg is PER-TABLET and BID/frequency is not threaded, so a carvedilol 25mg BID patient (=50/day, at target) would mis-read as `25 < 50` -> FALSE over-fire. Unblock: thread the frequency element (medications.csv has no explicit frequency column; a per-agent daily-dose derivation or a Redox dosageInstruction.timing signal) then remove the flag guard on `hfBBBelowTarget`. (Clinical-accuracy / dose-frequency, **OPEN - TRACKED**, 2022 AHA/ACC/HFSA COR 1 LOE A.) See register AUDIT-199-B-BB. Lineage AUDIT-199-B.
- **AUDIT-199-B-DIG** - HF-DIGOXIN (gapRuleEngine.ts:14606) kept dose-unknown-suppressed after the AUDIT-199-B dose parse: the rule threshold is 0.125 mg/day but digoxin is frequently coded in MCG ("Digoxin 125 MCG") and is a tablet-vs-daily case. parseDoseFromDescription normalizes mcg->mg, but the rule-level threshold semantics (0.125 mg/day vs a 0.25 mg tablet taken every other day) need re-confirmation before activation to avoid firing on every elderly-CKD digoxin patient. Unblock: confirm the mg/day normalization end-to-end + tablet-vs-daily, then remove the flag guard on `hfHighDoseDigoxin`. (Safety / dose-unit, **OPEN - TRACKED**, 2019 AGS Beers + 2022 AHA/ACC/HFSA COR 3 Harm.) See register AUDIT-199-B-DIG. Lineage AUDIT-199-B.
- **AUDIT-199-B-ARNI** - HF-30 ARNI underdosing (gapRuleEngine.ts:5277) kept dose-unknown-suppressed after the AUDIT-199-B dose parse: sacubitril/valsartan is a COMPOUND string ("sacubitril 97 MG / valsartan 103 MG"); parseDoseFromDescription returns null for it (correct - hands no ambiguous component to the rule), and the rule compares to the sacubitril 97 mg component target, so activation needs a compound-aware parse that deliberately extracts the sacubitril component (not valsartan / not total). Unblock: a compound-component parse keyed on the sacubitril token, then remove the flag guard on `hfARNIUnderdosed`. (Clinical-accuracy / compound-drug, **OPEN - TRACKED**, 2022 AHA/ACC/HFSA COR 1 LOE B-R.) See register AUDIT-199-B-ARNI. Lineage AUDIT-199-B.
- **AUDIT-199-B-REDOX** - real-EHR (Redox) dose coverage: the AUDIT-199-B parse fixes the CSV multi-file path, but the Redox webhook path (redox/fhirResourceHandlers.ts:175, handleMedicationRequest) sets only a `dose` STRING (value + unit), NOT the numeric `doseValue` column the rules read - so real-EHR-ingested patients stay dose-blind (statin/DOAC rules degrade to agent_dose_unknown) until this is threaded. NOTE: ingestSynthea.ts:318 (bulk Synthea-FHIR script) already sets numeric doseValue from doseQuantity; only the Redox real-EHR path is the gap. Unblock: set numeric doseValue/doseUnit in handleMedicationRequest from dosageInstruction.doseAndRate.doseQuantity (mirrors ingestSynthea). (Threading / AUDIT-070 lineage, **OPEN - TRACKED**, real-EHR dose coverage.) See register AUDIT-199-B-REDOX. Lineage AUDIT-199-B / AUDIT-070.
- **AUDIT-025-SLICE2** - Drift-on-populated-schema migration validation (Slice 2 of the AUDIT-025 hardening; TRACKED, do NOT build yet). The AUDIT-025 Phase-a `migration-validation` job (`ci.yml`, real `prisma migrate deploy` vs an ephemeral `postgres:15` on every PR, version-matched to Aurora 15.15) applies from an EMPTY DB, so it catches CLEAN-APPLY failures (the `CONCURRENTLY`-in-transaction PG-25001 class that caused the AUDIT-111 staging outage) but MISSES DATA-DEPENDENT failures - a migration that fails only against a populated schema (adding NOT NULL to a table with existing rows; a UNIQUE constraint violated by existing data). Fix shape: restore a PHI-SAFE schema-only (or synthetic-data) prod-like snapshot into the ephemeral PG, then `migrate deploy` ONLY the new migrations, failing the PR on any error. PHI guardrail: schema-only or synthetic - NEVER real prod PHI in CI. (CI / migration-safety, **OPEN - TRACKED**.) See register AUDIT-025-SLICE2. Lineage AUDIT-025 / AUDIT-024 / AUDIT-111.
- **AUDIT-107-STAGING-GATE** - Required/blocking staging gate posture (the B2-literal rung of the AUDIT-107 ladder), now UNBLOCKED. Previously RE-SCOPED as staging-dependent and blocked by AUDIT-111 (staging undeployable). Staging is now deployable (AUDIT-111 P3009 unblocked 2026-07-10; the "Build & Deploy to Staging" workflow is GREEN for the first time since 2026-05-03), so the required-check / staging-as-gate posture is FEASIBLE. DETECTION is already delivered (the `deploy-staging.yml` `if: failure()` alert issue, AUDIT-025-hardening PR); the BLOCKING posture is the remaining OPERATOR DECISION - minimally, add "Migration Validation" to the `main` ruleset (id 15265562) `required_status_checks` now (the outage-class enforce), with a staging-runtime required signal later. Distinct from AUDIT-025-SLICE2 (that is the check's COVERAGE) - this is the ENFORCEMENT posture. (CI / deploy-gate posture, **OPEN - operator decision**.) See register AUDIT-107-STAGING-GATE. Lineage AUDIT-107 rung-2 / AUDIT-111 step (2).
- **AUDIT-148 Slice 2** - trials-backend completion, READ half + registry model. DRIFT-45 correction: Slice 1 already shipped TWO endpoints (GET /trials getTrials AND GET /trials/:trialId/eligible-patients), not one - four ops remained, not five. **DELIVERED 2026-07-10:** the RegistryCase Prisma model (tenant-scoped hospitalId + patient FK onDelete Restrict; RegistryCaseStatus{DRAFT SUBMITTED APPROVED REJECTED} @default DRAFT; fields Json; approvedBy/approvedAt; @@unique[patientId,registryType,hospitalId]; @@map registry_cases) + migration 20260710000000 (plain in-transaction DDL, AUDIT-024-safe) + registryCase added to the wipePreReseed CHILD_TABLES (patient-FK, wiped before patient like TrialMatch) + GET /api/registry/:registryType/cases (read-only, hospitalId ALWAYS from JWT never params, tenant-scoped, mounted at /api/registry - the three registry ops 404'd before). Tests audit148Slice2Registry (route tenant-isolation + read-only guards + schema-shape + migration + wipe-order) green. **SLICE 3 SCOPE (deferred, do NOT build):** the three MUTATIONS - referPatientToTrial (POST /trials/:trialId/refer), updateRegistryCase (PATCH /registry/cases/:caseId), approveRegistryCase (POST /registry/cases/:caseId/approve) - are PHI-adjacent clinical decisions needing writeAuditLog (who/what/when), the DRAFT->SUBMITTED->APPROVED transition/authz rules, and the FDA-CDS clinician-decides framing. Frontend contract extension (consume the extended matchStatus+criteriaResults+indeterminateSignals shape) is a separate UI-track dependency. (8th-module backend, **Slice 2 RESOLVED 2026-07-10 / Slice 3 OPEN-DEFERRED**.) See register AUDIT-148.
- **AUDIT-200** - curated-seed dx calibration for Synthea coarse coding (the demo-enabling fix; **ID NOTE: this is the finding the AUDIT-148 Slice-1 session mis-referenced as "AUDIT-149" - AUDIT-149 is ALREADY the Phase-0C-3 "Dead interactive controls" UI finding, so the seed-calibration is filed here under the next-free id AUDIT-200; operator to confirm the id**). The SNOMED->ICD-10 crosswalk emits ONLY unspecified codes (414545008 -> I25.9; 84114007/88805009 -> I50.9) and the matcher prefix-matches, so the specific seed codes I50.2/I25.10 matched 0 Synthea patients -> 0 ELIGIBLE (the Slice-1 population smoke finding). **RESOLVED 2026-07-10 (option a):** broadened trial 1 dx I50.2->'I50' + trial 2 I25.10->'I25'; the real-EHR-correct specific codes are retained in the seed's section-16 note (swap back post-DUA). Clinically sound: HFrEF is HF + LVEF<=40, so the threaded LVEF<=40 gate does the discrimination (dx redundant); CAD is I25 + LDL>=70 + on-statin. Trials 3-4 (I27.0/E85.82) UNCHANGED (INDETERMINATE demos, pasp/bnp UNEVALUABLE regardless of dx). HOLLOW-GUARD PROVEN AT LOGIC LEVEL (audit200SeedCalibration.test.ts: ELIGIBLE requires dx AND lvef-value-present AND lvef<=40 AND not-on-SGLT2i - a 4-way conjunction, so a mixed I50.9 cohort resolves 2/5 ELIGIBLE, structurally NOT ~100% of the dx cohort). PROD-POPULATION MEASUREMENT PENDING: the exact ELIGIBLE/INELIGIBLE/INDETERMINATE split against the demo tenant was BLOCKED by the auto-mode classifier (it correctly denied a run-task against tailrd-production-cluster); measure via the endpoint's logged counts post-deploy, or on an operator-authorized prod read-only run-task. (Demo-data calibration / AUDIT-148 lineage, **RESOLVED 2026-07-10 - measurement pending**.) See register AUDIT-200. **[CORRECTION 2026-07-12 - measurement DELIVERED on the correct tenant, supersedes the pending status]:** the hollow-gate was re-measured via a read-only prod run-task on the correct tenant, **demo-synthea-threaded** (the post-threading population the trials serve), with the hospitalId passed as a LITERAL and echoed in the output. (An earlier probe mis-selected demo-synthea-proof - the pre-threading, LDL-absent baseline - by a largest-count heuristic; both tenants hold exactly 25,571 patients so count could not distinguish them; logged as DRIFT-51.) VALIDATED SPLIT: Trial 1 HFrEF-GDMT ELIGIBLE 68 / INELIGIBLE 664 / INDETERMINATE 24,839 (lvef) - E+I = 732 = lvef-evaluable; Trial 2 CAD-lipid ELIGIBLE 257 / INELIGIBLE 5,171 / INDETERMINATE 20,143 (ldl) - E+I = 5,428 = ldl-evaluable; Trials 3-4 PH/ATTR all-INDETERMINATE (pasp / bnp named, Synthea-absent by design); false-ELIGIBLE = 0 across all four. HOLLOW-GATE PASSES: no trial's ELIGIBLE approaches its dx-cohort (68 = 0.3% of 1,334 I50; 257 = 3.4% of 7,616 I25); Trial 2 (the calibration target) now DISCRIMINATES with real ELIGIBLE where on proof (ldl-absent) it was 0. The prior 484/1,834/23,253 was demo-synthea-proof and is SUPERSEDED. AUDIT-202 (LDL-dark / re-ingest regression) is REFUTED and NOT filed - a wrong-tenant artifact: demo-synthea-threaded has LDL abundantly (18,680 raw / 5,428 evaluable), and AUDIT-194-B1 threading + AUDIT-195 lipid consolidation are intact, no regression. Original "measurement pending" text retained above per §18.
- **AUDIT-201** - matcher INDETERMINATE-precedence over a definite INELIGIBLE (**ID NOTE: filed here, NOT under the operator-requested "AUDIT-150" - AUDIT-150 is ALREADY the Phase-0C-3 "Broken heatmap color-band map" UI finding; operator to confirm the id**). evaluateTrialMatch (trialMatchService.ts:147) returns INDETERMINATE whenever ANY criterion is UNEVALUABLE, even when a THREADED INCLUSION criterion has already definitively FAILED. So the PH/ATTR trials (pasp/bnp always UNEVALUABLE) return INDETERMINATE for EVERY active patient including those who plainly lack the dx - honest but noisy, and it makes the INDETERMINATE coordinator-worklist useless (it is the whole tenant, not "patients one test away from eligible"). FIX SHAPE (do NOT build here - matcher-logic change with its own test surface): a FAILED threaded INCLUSION criterion short-circuits to INELIGIBLE even when a later criterion is unevaluable ("already excluded, stop"); failing a criterion is a definite negative (unlike asserting eligibility on unknown data), so the never-assert-on-unknown guarantee is PRESERVED. (Matcher-logic / AUDIT-148 lineage, **OPEN - do NOT build in Slice 2**.) See register AUDIT-201. **[RESOLVED 2026-07-12 - two coupled fixes, section-20 close-the-class]:** the precedence swap makes a FAILED verdict LOAD-BEARING (a violation now short-circuits to INELIGIBLE), so every FAILED path had to be proven safe FIRST - and the STAGE-1 inventory found the safety predicate "FAILED always rests on present, threaded data" was FALSE for the procedure type. We do not ship a fix whose safety argument has a known counterexample, so both ship together. **FIX 1 (precedence, trialMatchService.ts):** `anyViolation ? 'INELIGIBLE' : anyUnevaluable ? 'INDETERMINATE' : 'ELIGIBLE'` - a definite violation (inclusion FAILED / exclusion MET) short-circuits to INELIGIBLE even when another criterion is UNEVALUABLE ("already excluded, stop"); a FAILED is a definite NEGATIVE on data we HAVE, unlike asserting a POSITIVE on data we lack. ELIGIBLE UNCHANGED - it still requires `anyUnevaluable === false`, so an unevaluable EXCLUSION resolves INDETERMINATE, NEVER ELIGIBLE (never-assert-on-unknown preserved). **FIX 2 (procedure UNEVALUABLE guard, evaluateCriterion):** the procedure type marked FAILED on an empty `procedureCodes` with NO presence guard; an empty list is AMBIGUOUS ("genuinely no procedure" vs "procedures not threaded for this patient/tenant" - the context cannot distinguish, both yield []), so it is now UNEVALUABLE (err toward unknown, never a false definite negative), mirroring the lab two-stage pattern (threading-check -> presence-check -> only then MET/FAILED). A NON-EMPTY list keeps a real no-match as FAILED (same threading-completeness caveat as dx/med). No current curated trial uses a procedure criterion - the risk was latent; closed proactively. Full suite green (122 suites / 2317); tests audit201Precedence (6 cases a-f) + NO existing test broke (every prior INDETERMINATE assertion has dx MET / no violation = the stays-INDETERMINATE case, behavior-preserving). Post-deploy measurement is on the LITERAL demo-synthea-threaded (DRIFT-51 discipline): ELIGIBLE must stay EXACTLY 68 / 257 (the fix cannot create a single new ELIGIBLE) and false-ELIGIBLE 0; INDETERMINATE collapses to the true one-test-away worklist (dx-fails move to INELIGIBLE). See register AUDIT-201.
- **AUDIT-203** - CrossReferral clinical-decision WRITE is absent from the HIPAA audit trail. The module-to-module referral CREATE path (`referrals.ts` POST /api/referrals -> `CrossReferralService.createManualReferral`) records the referral with `logger.info` ONLY (`referrals.ts:516-524`), NOT `writeAuditLog` - so a PHI-touching clinical decision about a NAMED patient (who referred whom, from/to which module, when) never reaches the append-only + DB audit log (`auditLog` table + the 6-year file transport). The status-transition path has the same gap (PUT /api/referrals/:id/status, `referrals.ts:312-333`, logger.info only). Grep-verified 2026-07-12: ZERO `writeAuditLog` / `auditLog` references in `referrals.ts` OR `crossReferralService.ts`. THREE surfaces to fix: (1) POST /api/referrals create -> `writeAuditLog(req, 'CROSS_REFERRAL_CREATED', 'CrossReferral', newReferral.id, <id/count-only desc>, null, { patientId, fromModule, toModule, priority })`; (2) PUT /api/referrals/:id/status -> action `'CROSS_REFERRAL_STATUS_CHANGED'` previousValues `{ status: oldStatus }` newValues `{ status: newStatus }`; (3) OR place the audit at the service layer (`crossReferralService.ts` createManualReferral / updateReferralStatus) if that is the better single-point integration. Descriptions id/count-only, NEVER patient PHI (per the AUDIT-018/075 sanitize precedent). OPEN QUESTION (do NOT resolve here): whether historical referrals are recoverable from the existing Winston file logs for a backfill of the `auditLog` table, or whether the durable trail simply starts at the fix. NOTE this is the exact precedent AUDIT-148 Slice 3 deliberately IMPROVES ON - the new TrialReferral write audits every mutation from day one; CrossReferral (the older sibling) must be brought up to that bar. (HIPAA audit-completeness / referrals; **OPEN** 2026-07-12 - HIGH (P1) proposed, a PHI-touching clinical-decision write missing from the audit trail, mitigated synthetic/pre-DUA; remediation = add `writeAuditLog` to the 2 referral mutation paths + a `CROSS_REFERRAL_*` action pair; operator confirms severity per §18.) See register AUDIT-203. Lineage AUDIT-148 Slice 3 (the improved-on precedent) / AUDIT-013 (audit-durability class).
- **AUDIT-148 Slice 3** - trials-module FIRST clinical-decision WRITE (the 3 mutations, delivered as 5 endpoints). **DELIVERED 2026-07-12 (branch feat/audit-148-slice3; tests green; pending operator merge):** (a) `TrialReferral` event-model - a SEPARATE durable event (not a field on the re-evaluated TrialMatch): { patientId / hospitalId / trialId FKs onDelete Restrict, referredBy (acting clinician, JWT userId), referredAt, status `TrialReferralStatus`{PENDING CONTACTED ENROLLED DECLINED}@default PENDING, notes?, matchStatusAtReferral `TrialMatchStatus`, @@unique[patientId,trialId,hospitalId], @@index[hospitalId,trialId,status], @@map trial_referrals } mirroring the CrossReferral manual-create shape; (b) `RegistryCase` maker-checker fields createdBy / updatedBy (person-level approver!=createdBy NOT enforced this slice but the fields EXIST for a later no-migration turn-on); migration 20260712000000 (plain in-transaction DDL, AUDIT-024-safe, no CONCURRENTLY) + `trialReferral` added to wipePreReseed CHILD_TABLES (patient FK, wiped before patient like trialMatch/registryCase). 5 endpoints, hospitalId ALWAYS from the verified JWT (cross-tenant trial/patient/case -> 404 not 403, no existence leak), EVERY mutation writeAuditLog'd: POST /trials/:trialId/refer (`TRIAL_REFERRAL_CREATED`; NOT gated on matchStatus - ELIGIBLE / INELIGIBLE / INDETERMINATE ALL accepted per the AUDIT-201 one-test-away rationale; records matchStatusAtReferral for the trail ONLY; P2002 unique-violation -> 409); PATCH /registry/cases/:caseId (`REGISTRY_CASE_UPDATED`, DRAFT-only else 409, audit carries a sha256 fields-DIGEST NEVER the raw PHI payload); POST .../submit (`REGISTRY_CASE_SUBMITTED`, DRAFT->SUBMITTED else 409); POST .../approve (`REGISTRY_CASE_APPROVED` HIPAA-grade throw-on-audit-DB-fail -> 500, SUBMITTED->APPROVED else 409, stamps approvedBy/approvedAt); POST .../reject (`REGISTRY_CASE_REJECTED` HIPAA-grade, SUBMITTED->REJECTED else 409, REQUIRED reason recorded in the audit newValues). ROLE-level maker-checker: refer/update/submit = [SUPER_ADMIN, HOSPITAL_ADMIN, PHYSICIAN, NURSE_MANAGER]; approve/reject = [SUPER_ADMIN, HOSPITAL_ADMIN, QUALITY_DIRECTOR] (a maker cannot self-approve). FDA-CDS structural framing: the platform RECORDS a human clinical decision (audit copy: "clinician referred" / "human sign-off"), it does NOT enroll or auto-approve. Tests trialsRegistrySlice3 (21 cases: audit-write per mutation; approve/reject HIPAA 500-on-audit-DB-fail; role gates PHYSICIAN-cannot-approve / QUALITY_DIRECTOR-can; status-guard 409s; tenant 404; refer accepts INDETERMINATE + ELIGIBLE + INELIGIBLE (NOT gated); matchStatusAtReferral recorded; reject-needs-reason 400; fields-digest-not-raw-PHI) GREEN 21/21. UI-TRACK DEPENDENCY (do NOT build): `src/services/api.ts` lacks `submitRegistryCase` + `rejectRegistryCase` client contracts (referPatientToTrial / updateRegistryCase / approveRegistryCase already declared); the 2 new transitions need client methods before any view can wire them. The 5 prisma stale-client tsc errors (trialReferral + registryCase.createdBy/updatedBy + clinicalTrial) are the RULE-6 WSL class (native `npx prisma generate` here cleared them; Docker/CI regenerates). AUDIT-203 filed separately as the CrossReferral audit-trail gap this slice improves on. See register AUDIT-148 Slice 3. Lineage AUDIT-148 / AUDIT-148-SLICE2 / AUDIT-201 / AUDIT-203.
...
- **Worklist addition (v3.0 EP buildout, 2026-06-16) - PRIORITY data element `weight`:** body weight (LOINC 29463-7, verify at build) is collected as a vital but is NOT threaded into the engine `labValues` namespace (only `bmi` is present). Threading `weight` (and confirming `height`) unblocks two precision items in the EP AF-dosing chunk: (a) true Cockcroft-Gault **CrCl** in place of the eGFR proxy used by **GAP-EP-003** (rivaroxaban renal dose) and **GAP-EP-009** (edoxaban CrCl>95) - the eGFR-vs-CrCl divergence is largest at weight extremes; and (b) the **weight<=60 kg** apixaban dose-reduction criterion currently dropped (Path-B) from **GAP-EP-004** / **GAP-EP-005**, which today under-detect weight-driven dose-reduction pairs. Named element: `weight` (kg). Gaps improved: EP-003, EP-004, EP-005, EP-009. This is a docs worklist note (no gap-code change here); the dosing gaps already ship with documented Path-B limits.
- **Cross-references:**
...
- **Severity:** MEDIUM (P2)
- **Status:** **RESOLVED 2026-06-07** (Phase C tooling shipped 2026-05-08; the remediation-step-3 end-to-end restore test executed 2026-06-07 - see the Resolution note at the end of this entry)
...
- **Resolution note (RESOLVED 2026-06-07) - remediation step 3 (end-to-end restore test) executed:** restored a real production snapshot into a scratch cluster, verified read-only, torn down clean. Zero `update-service` calls, zero production-DB writes. (1) **T0 baseline** on the current production image (ECS task `f9369758`; evidence `docs/audit/sweeps/step6-t0-baseline-counts.js`). (2) **Snapshot** `beta1-phase1-signoff-20260607` (engine 15.15, KMS `ec93e66e...`, HIPAA-6yr tag; creation 257s; retained). (3) **Restore** -> `tailrd-signoff-restore-20260607` (engine inherited 15.15; same subnet group / SG / KMS; deletion-protection OFF; ServerlessV2 Min 0.5) + db.serverless writer. (4) **RTO (D4):** restore call -> first verified connection = **~1,199s (~20 min), under the 30-min target** (cluster-available sub-leg ~812s). (5) **Verification** (ECS RunTask, throwaway task-def `tailrd-backend:276` with `DATABASE_URL` env-overridden to scratch, `PHI_ENCRYPTION_KEY`/taskRole/region unchanged; task `ec192573`; evidence `docs/audit/sweeps/step7-restore-verify.js`): **D3(a/b) row-count parity EXACT** for all immutable tables (patients 6,147 / conditions 225,439 / medications 220,552 / recommendations 8 / users 1) and audit_logs exact (7,316=7,316); **D3(d) full canonical-decrypt** = `audit_logs.description` 7,316/7,316 + `newValues` 7,101/7,101 = **14,417 envelopes / 0 failures** (PHI provably recoverable from a restored backup). (6) **Teardown + leak verification:** writer + cluster deleted; `describe-db-clusters` -> `DBClusterNotFoundFault`, `describe-db-instances` -> `DBInstanceNotFound`, `describe-db-cluster-automated-backups` -> `DBClusterAutomatedBackupNotFoundFault`; throwaway task-def `:276` deregistered (latest reverts to `:275`); scratch DATABASE_URL secret force-deleted. Cost well under $1. **Script defects fixed in this PR** (`infrastructure/scripts/audit-078/03-execute-restore-test.sh`): removed the stale `--engine-version 15.14` pin (inherit from snapshot); D3(b) queries the real `patients` table + removed the `|| printf '0'` silent-success mask; parameterized the snapshot identifier. Full chain in `docs/audit/BETA1_PHASE1_SIGNOFF.md`. Closes the HIPAA §164.308(a)(7)(ii)(B) DR-testing gap.
...
- **Status note (HF-side remediation, lineage):** 2026-07-07 - the HF Executive tier (listed UNAFFECTED above for the silent-mock class) received the full data-reconciliation + honesty pass at PR #442 squash `4e6987e`: patient total / GDMT-optimized / device candidates / open-gaps count wired to the real HFDashboardData contract; all revenue figures reconciled to ONE demo model (`hfDemoFinancials.ts`, sums derived in code); the false "Live tracking" pill removed from the shared GapResponseRateCard (all 6 exec views benefit); DemoDataBadge (KCCQ pattern) applied to every mock-fed card; honest offline/loading/empty states throughout. Follow-on batch 2 (same lineage, this PR): dead controls removed (facility no-op click affordance, bubble-only "View Pipeline Details" button given an explicit handler, dead "View Details in Service Line" modal button dropped) + white-card restyle (white surfaces, titanium borders, solid primary accents; shared components via opt-in `cleanSurface` prop so the 5 non-HF views render unchanged). The 5-view non-HF core of this finding remains OPEN; per-module sweeps will reuse the DemoDataBadge + `cleanSurface` + honest-empty-state patterns established here. VHD Exec "6 auto-detected gaps" and EP Exec "20 gaps" mock/type-count instances are logged for those per-module passes.
- **Status note (EP-side remediation, lineage):** 2026-07-09 - the EP Executive tier (one of the 5 non-HF views listed above) received the full convergence pass on branch `feat/ep-exec-convergence` (in PR): the dead `dashboardError` is now consumed via the new `EPExecutiveSummary`, which renders honest loading ("Loading live data") / error ("Live data unavailable") / empty states instead of silently falling back to a fabricated number; live patients / open gaps / device candidates read the real EP dashboard contract (`EPDashboardData`); the GapIntelligenceCard headline reads the live `totalOpenGaps` across the real `totalPatients` (retiring the EP "20 gaps" default and the 1,730 category-sum-as-population mislabel logged above), with the donut/top-gaps composition demo-badged; every mock-fed financial surface (waterfall, DRG/CMI, P-v-R, benchmarks, facility, pipeline-detail) + the fabricated facility-detail modal carry a DemoDataBadge; revenue reconciled to one derived `epDemoFinancials` model. **The EP-side dead-`dashboardError` + no-indicator defect is thereby CLOSED**; the remaining non-HF core of AUDIT-099 is now 4 views (SH / CAD / VHD / PV), which stay OPEN. Reuses the DemoDataBadge + honest-empty-state patterns from the HF exemplar; adds the opt-in `totalPatients` / `compositionDemo` GapIntelligenceCard props (default-off, so the other 4 views render byte-identical).
- **Status note (SH-side remediation, lineage):** 2026-07-10 - the SH (Structural Heart) Executive tier (one of the 4 remaining non-HF views) received the full convergence pass on branch `feat/sh-exec-convergence`: the dead `dashboardError` (destructured but never consumed - the same defect as pre-fix EP) is now consumed via the new `SHExecutiveSummary`, which renders honest loading ("Loading live data") / error ("Live data unavailable") / empty states instead of silently falling back to a fabricated number (the `?? 8` gap default is retired); live patients / open gaps / device candidates read the real SH dashboard contract (`SHDashboardData`, from `GET /structural-heart/dashboard`); the GapIntelligenceCard headline reads the live `totalOpenGaps` across the real `totalPatients` (killing the 830 category-sum-as-population mislabel), with the donut/top-gaps composition demo-badged; every mock-fed financial surface (waterfall, DRG/CMI, P-v-R, benchmarks, facility, pipeline-detail) + the fabricated facility-detail and DRG-detail modals carry a DemoDataBadge; revenue reconciled to one derived `shDemoFinancials` model (fixes the 10.3-vs-10.9M total split and the 5.0-vs-6.2M at-risk break). **The SH-side dead-`dashboardError` + no-indicator defect is thereby CLOSED**; the remaining non-HF core of AUDIT-099 is now 3 views (CAD / VHD / PV), which stay OPEN.
...
- **Severity:** HIGH (P1)
- **Status:** RESOLVED 2026-06-07 (option-(B) decision MADE + implemented as a staged ladder; see the dated Resolution note at the end of this entry). Severity unchanged at HIGH per §18; the operator owns any re-grade.
...
- **Option-(B) deploy-gating enumeration (the final AUDIT-107 slice; operator decision):** the post-deploy smoke is currently a non-blocking `workflow_run` decoration - nothing consumes its conclusion (the original AUDIT-107 detection-gap). Four postures, least-to-most coupled:
  - **(B1) Loud-fail-only (alert, do not gate):** keep the smoke non-blocking but make its failure impossible to miss - fix AUDIT-114 (`permissions: issues: write`) so the alert-only `gh issue create` actually files, and/or add a Slack/email/PagerDuty notify. Trade-off: zero deploy-flow risk and no false-positive blast radius, but a real regression still ships - it is detected fast, not prevented. Lowest effort (the AUDIT-114 one-liner). **Update 2026-06-07:** B1's blocker (AUDIT-114) is RESOLVED via PRs #361 + #362 with the alert path verified end-to-end (synthetic run 27107094279 -> issue #363); option-(B) rung 1 is COMPLETE (verified-functional alert path), so rung 2+ (required check / block-next-deploy / auto-rollback) remain the open operator decision. 
...
- **Resolution note (RESOLVED 2026-06-07) - option-(B) decision MADE + implemented as a staged ladder:**
  - **Rung 1 (B1, loud-fail-only):** the smoke-failure alert path is verified-functional via **AUDIT-114** (PRs #361 + #362; synthetic run 27107094279 -> issue #363). A failed post-deploy smoke now files a loud issue.
  - **Rung 2 (deploy-time hold = the B3 "block-next-deploy" posture, fail-open):** implemented in `deploy.yml` (**PR #365 / merge `b23a8d4`**) as a gate first-step (after checkout, before build) that refuses image promotion while the most recent DECISIVE post-deploy smoke JOB on main is red. **Critical correctness property:** it reads the smoke **JOB** conclusion, never the workflow conclusion - a held deploy fires a chained smoke whose job SKIPS (on its `conclusion=='success'` guard) and whose run concludes non-failure; reading the workflow conclusion would let a red deploy manufacture a false green and silently clear its own hold. Fail-open on no-signal / API error (loud `::warning::` + summary); deduped loud alert issue on hold; audited `workflow_dispatch` override (`override_smoke_hold` + `override_reason`, empty-reason rejected, logged to summary + hold-issue comment); `gate_only_dryrun` for image-promotion-free verification.
  - **Live evaluation (real merge deploy):** deploy **27108166282** (head `b23a8d4`) - gate step `success` (PROCEED on the green board) -> build/deploy/frontend all green -> chained smoke **27108278106** (workflow_run) green.
  - **Synthetic verification (all deploy dispatches `gate_only_dryrun=true`; production never promoted):** leg 1 red smoke **27108316015** (job=failure) + AUDIT-114 issue **#366**; leg 2 deploy **27108328676** -> **HOLD** (gate `failure`, build skipped) + hold issue **#367** (body links held deploy + red smoke); **leg 3 (false-green immunity, the critical leg):** the chained smoke **27108333461** off the held deploy concluded JOB=**skipped** (workflow=skipped), and a fresh deploy **27108369016** STILL **HELD** (gate `failure`) - the skip did NOT clear the hold, proving the JOB-conclusion read defeats the false green; leg 4 override deploy **27108386269** -> gate `success` (override), build skipped, override audited as a comment on issue #367 (actor + reason "rung-2 verification"); leg 5 green smoke **27108402573** (job=success) -> deploy **27108413018** gate `success` (PROCEED); leg 6 cleanup: issues #366 + #367 closed as verification artifacts, board ended green (0 open issues).
  - **B2-literal (pre-merge required status check) - RE-SCOPED, not done here:** a *merge-time* required check gating the NEXT change is dependent on the staging cluster being deployable (it would gate on a staging/pre-merge signal). That is blocked by **AUDIT-111** (staging undeployable, Prisma P3009) + **AUDIT-025** (no pre-merge migration validation gate) + **AUDIT-024** (CONCURRENTLY-in-transaction). Cross-linked at AUDIT-111. The rung-2 deploy-time hold delivers the *deploy-time* protection B2 was reaching for, without the staging dependency.
  - **B4 (auto-rollback on red smoke) - DEFERRED post-DUA:** gated on migration-compat. Per CLAUDE.md RULE 2 the deploy CMD runs `prisma migrate deploy` before server start, so rolling back to an older image against a newer schema can crashloop; auto-rollback needs a migration-compatibility precondition before it is safe. Not pursued at the current pre-DUA stage.
  - **Conclusion:** the option-(B) decision (the sole remaining AUDIT-107 slice) is now MADE and the chosen rungs (1 + 2) are implemented + verified. AUDIT-107 -> RESOLVED. The remaining ladder rungs (B2-literal, B4) are explicitly scoped/deferred with their preconditions named, not left as an open undecided fork.
...
- **Progress note (2026-06-10):** parseExistingAddendum integration test decoupled from the VHD-005 clinical-verdict pin on PR #374 (asserts parser provenance, not classification); the suite-wide sweep of remaining hardcoded clinical-value pins (applyOverrides.test.ts EP-079 x3 / CAD-016, the stale :249 comment) remains tracked under this finding.
- **Progress note (2026-06-11):** remediation step 2 COMPLETE - the suite is clean of real-source-coupled clinical-verdict pins. applyOverrides.test.ts dict-anchored: the OVERRIDES const is now exported (CLI main() is require.main-gated, so the import is side-effect-free) and the EP-079 write-path pins (default-mode + candidate-mode tests) assert classification + auditNote against OVERRIDES.EP directly instead of hardcoded literals; the DET_OK-preservation and PARTIAL-with-cite pins (--all test) select their exemplar DYNAMICALLY from the exported dict by declared class, with a loud console.warn skip if a class empties (mirroring the empty-module-coverage pattern) so a hollow pass cannot hide the coverage gap; the stale VHD-005 comment rewritten verdict-free (presence-only assertion; verdict owned by the audit layer). Verified: auditCanonical suite 140/140, full backend jest 1334 pass + 1 skip (baseline), neither dynamic-skip warning fired. Together with PR #374 this closes step 2. Steps 1 (content-anchor the two absolute-line assertions: extractCode VD-PANNUS commentLine, extractSpec VHD-005 specLine) and 3 (pre-push hook covering the required gate set) remain OPEN; status unchanged.
...
- **Blast radius (staging-based verification claims since 2026-05-03):** with staging undeployable for ~1 month, any claim of staging-RUNTIME verification in that window would be unsound. A targeted read-only review found NONE load-bearing: AUDIT-011 Phase d soak = production (above); AUDIT-016 key rotation, AUDIT-022 PHI backfill, and AUDIT-108 plaintext backfill were executed/verified against PRODUCTION per their register notes; AUDIT-025 Phase b (the proposed "Staging Migration Validation" pre-merge CI against `tailrd-staging-aurora`) was never implemented (no such workflow exists - only the post-merge `deploy-staging.yml`), so nothing actually gated on staging; AUDIT-025 Phase a (CI migration validation) is also unimplemented and CI uses `prisma db push --force-reset`, which never exercises the migration runner, so CI never saw the failed-migration state either. COMPLETENESS CAVEAT: this is a targeted scan plus a reasoned review of the major landed work, not an exhaustive audit of every "verified"/"confirmed" claim since 2026-05-03; residual unknown is bounded but non-zero. **[CORRECTION 2026-07-10 (§18 dated-correction-supersedes; AUDIT-025 hardening review):** the clause above "AUDIT-025 Phase a (CI migration validation) is also unimplemented and CI uses `prisma db push --force-reset`" is STALE and conflated two DISTINCT CI jobs. AUDIT-025 Phase a IS implemented: the `migration-validation` job in `.github/workflows/ci.yml` (added by PR #223, commit `3f4ef57`, 2026-05-03) runs a REAL `prisma migrate deploy` from clean against an ephemeral `postgres:15` service container (version-matched to Aurora 15.15) on every `pull_request` to main, then `prisma migrate status`. The `prisma db push --force-reset` cited is the SEPARATE `test` (Jest) job's fixture reset, which indeed does not exercise the migration runner - the two jobs were conflated. So the remaining AUDIT-025 gap is NOT "no check" but "the check is ADVISORY, not REQUIRED": the `main` ruleset (id 15265562) `required_status_checks` omit "Migration Validation", so a red gate does not currently block merge (operator ruleset-add pending). Phase b (drift-on-populated-schema validation, the "Staging Migration Validation" against a prod-like snapshot) remains genuinely unimplemented and is now tracked as AUDIT-025 Slice 2. Original text retained above per §18. **UPDATE 2026-07-10 (ruleset change COMPLETED):** the advisory-not-required gap is now CLOSED - the `main` ruleset (id 15265562) `required_status_checks` were expanded from 5 to 8; **"Migration Validation" + "Audit Canonical Gates" + "Evidence Object Consistency" are now merge-blocking** (live-GET verified: 8 required checks; all 4 rules - deletion / non_fast_forward / pull_request / required_status_checks - intact). The outage-class (a migration that fails CI's ephemeral-DB `migrate deploy`) can no longer merge. Remaining AUDIT-025 scope is now only Phase b / AUDIT-025-SLICE2 (drift-on-populated-schema coverage).]
...
- **Cross-link note (2026-06-07, AUDIT-107 rung-2 dependency):** AUDIT-107 RESOLVED its option-(B) decision via a staged ladder (rung 1 alert path + rung 2 deploy-time smoke-hold, PR #365). The AUDIT-107 ladder's **B2-literal rung (a pre-merge required status check)** was explicitly RE-SCOPED as **dependent on this finding**: a merge-time required check gating the NEXT change needs a deployable staging / pre-merge migration signal, which is blocked by this AUDIT-111 staging outage + AUDIT-025 (no pre-merge migration validation) + AUDIT-024. So B2-literal is gated on AUDIT-111 (immediate unblock) + AUDIT-025 Phase a/b. The AUDIT-107 rung-2 deploy-time hold delivers the deploy-time protection without this staging dependency; the merge-time tightening waits on AUDIT-111/025/024.
...
### AUDIT-117 - EP dabigatran SAFETY rule uses a single-formulation SCD code (150mg capsule), not the ingredient: false-negative on a Class-3-Harm contraindication

- **Phase:** 0B EP clinical-code verification (surfaced 2026-06-08, EP audit Batch 1 §16 RxNorm verification).
- **Severity:** HIGH (P1). Missed-detection on a SAFETY rule (Class 3 Harm), mitigated synthetic/pre-DUA (no real patients yet). Operator confirms per §18.
- **Status:** OPEN (recorded by the audit; remediation is a separate PR - this audit does not fix code).
- **Defect:** `RXNORM_DOACS.DABIGATRAN = '1037045'` (`backend/src/terminology/cardiovascularValuesets.ts:295`) resolves via RxNav `properties.json` (2026-06-08) to **"dabigatran etexilate 150 MG Oral Capsule"** (tty=SCD) - a single Semantic Clinical Drug, NOT the ingredient. The other three DOACs in the same object are ingredient-level (apixaban 1364430 / rivaroxaban 1114195 / edoxaban 1599538, all IN). The correct dabigatran ingredient is **`1037042`** ("dabigatran etexilate", tty=IN, RxNav-verified).
- **Impact 1 (GAP-EP-006 SAFETY, `gapRuleEngine.ts:4187`):** `if (medCodes.includes('1037045') && labValues['egfr'] < 30)` fires only for the exact 150mg capsule. **Dabigatran 75mg is the dose used in moderate renal impairment**, so a patient on dabigatran 75mg with eGFR<30 - the precise contraindication the rule exists to catch (2023 ACC/AHA/ACCP/HRS AF, Class 3 Harm; FDA Pradaxa PI) - is **MISSED (false-negative on a SAFETY rule)**.
- **Impact 2 (GAP-EP-001 EP-OAC, `gapRuleEngine.ts:4140-4141`):** `OAC_CODES` includes `'1037045'` with exact membership; a patient on dabigatran 75/110mg reads as **not anticoagulated** -> false-positive "OAC not prescribed."
- **Compounded by AUDIT-118:** even the ingredient fix (`1037042`) does not fully close detection, because the matcher does no ingredient->descendant expansion (see AUDIT-118), so an ingredient code still misses SCD/product-coded patient meds. The full fix is the ingredient code AND ingredient-normalized matching.
- **Classification effect:** GAP-EP-001 + GAP-EP-006 proposed DET_OK -> PARTIAL_DETECTION (operator review); see `docs/audit/EP_BATCH1_SAFETY_ANTICOAG_AAD.md`.
- **Remediation candidate (NOT done here):** `DABIGATRAN -> '1037042'` (verified ingredient) + ingredient-normalized matching per AUDIT-118; add positive/negative tests (dabigatran 75mg + eGFR<30 must fire).
- **Cross-references:** `cardiovascularValuesets.ts:295`; `gapRuleEngine.ts:4140,4187`; AUDIT-118 (the matching-architecture parent); AUDIT-042/056/057 (Cat A wrong-drug class - this is wrong-GRANULARITY, a sub-class); AUDIT-031/032 (EP SAFETY sisters).

---

### AUDIT-118 - Medication matching is exact-RxCUI membership with no RxNorm ingredient expansion: ingredient value sets silently under-detect product-coded patient meds

- **Phase:** 0B EP clinical-code verification (surfaced 2026-06-08, EP audit Batch 1 §1 rule-body trace; operator-prioritized classification-correctness precondition).
- **Severity:** HIGH (P1) candidate. Silent under-detection of medication presence across rules; mitigated synthetic/pre-DUA. Operator confirms severity + scope per §18.
- **Status:** OPEN (recorded; remediation separate).
- **Mechanism (read, not inferred):** both gap runners build `medCodes` as the raw stored code: `medCodes = patient.medications.map(m => m.rxNormCode).filter(Boolean)` (`backend/src/ingestion/runGapDetectionForPatient.ts:53`; `backend/src/ingestion/gapDetectionRunner.ts:100`). Every rule then matches by exact string membership (`medCodes.some(c => SET.includes(c))` / `medCodes.includes('<rxcui>')`). There is **no RxNorm ingredient->descendant (IN->SCD/SBD) expansion** anywhere in the path.
- **Granularity mismatch:** the value sets are predominantly ingredient-level (tty=IN), but ingested medication codes are product-level: `backend/src/scripts/ingestSynthea.ts:309` stores the FHIR `coding.code` verbatim (`rxNormCode: coding.system?.includes('rxnorm') ? coding.code : null`), and Synthea (the primary demo/pilot data source) emits SCD/clinical-drug RxCUIs. So an ingredient-coded value set matches a patient ONLY if that patient's medication happens to be stored at the ingredient level - otherwise the medication is silently invisible to detection.
- **Self-corroborating evidence in-codebase:** the authors already worked around this case-by-case - `RXNORM_RATE_CONTROL`/digoxin enumerates "ingredient + 3 formulations" (`gapRuleEngine.ts:4256-4261`), and AUDIT-101 added a statin SCD/SBD->ingredient resolver (`gapRuleEngine.ts:89-94`) precisely because `rxNormCode` "may be a dose-level SCD/SBD rather than the ingredient." These workarounds exist because the default path does not normalize.
- **Blast radius:** module-wide and cross-module - every medication-presence rule (DOACs, AADs incl. amiodarone 703 / dronedarone 233698 / dofetilide 49247, statins, BBs, MRAs, SGLT2i, etc.) under-detects product-coded meds. The realized rate depends on the coding-granularity mix of the ingested cohort; for SCD-coded cohorts (Synthea) it can approach total non-detection of the affected medications.
- **Highest-severity instances surfaced by the EP audit (remediation-priority anchors):** (1) **GAP-EP-079** (WPW + AF on an AV-nodal blocker, **CRITICAL**) - a fatal-VF false-negative when the AVN blocker is SCD-coded; its `AVN_BLOCKER_CODES_EP079` set is MIXED (digoxin arm descendant-enumerated, but the 8 BB + 2 non-DHP-CCB arms ingredient-only), so the BB/CCB exposure under-detects. (2) **GAP-EP-017** (HFrEF on a non-DHP CCB, **Class 3 Harm**) - diltiazem/verapamil ingredient-only, the SAFETY contraindication under-detects. (3) **GAP-EP-006** (dabigatran in severe renal impairment, **Class 3 Harm**; via AUDIT-117) - doubly broken by also being a single SCD. **A single ingredient-normalize-at-match remediation (roll each patient med RxCUI up to its ingredient before matching) closes all three at once** - so AUDIT-118 is the high-leverage safety fix, strictly preferable to piecemeal per-rule descendant enumeration (which would have to be repeated for every value set and re-verified each time codes change).
- **Classification effect:** reinforces the GAP-EP-001/006 PARTIAL flips and extends to **GAP-EP-007** (DOAC + mechanical valve, cross-module VD-6) -> proposed DET_OK -> PARTIAL. The AAD ingredient-matched DET_OK rules (GAP-EP-043/044/046/048) carry the same vulnerability; whether to cascade-flip them on this cross-cutting finding is an operator decision (recorded, not auto-applied).
- **Remediation candidate (NOT done here):** ingredient-normalize at match time (roll each patient med RxCUI up to its ingredient via an RxNorm IN map / RxNav relatedByType) and match on ingredient sets; OR expand each ingredient value set to its descendant SCD/SBD set at build time. Add tests proving an SCD-coded med matches its ingredient value set.
- **Cross-references:** `runGapDetectionForPatient.ts:53`; `gapDetectionRunner.ts:100`; `ingestSynthea.ts:309`; `gapRuleEngine.ts:89-94,4140,4187,4256-4261`; AUDIT-117 (the dabigatran instance, doubly broken); AUDIT-101 (the statin precedent that proves the pattern); AUDIT-049 (formulation-vs-ingredient Cat D precedent).

---

### AUDIT-119 - EP registry/evaluator/gaps.push counts do not reconcile (48/47/49); the registry-without-evaluator (the AUDIT-035 orphan) silently never fires, and automated reconciliation is blocked by the missing join key

- **Phase:** 0B EP clinical-code verification (surfaced 2026-06-08, EP audit Batch 1; AUDIT-106 class).
- **Severity:** MEDIUM (P2). Operator confirms per §18.
- **Status:** OPEN (recorded; remediation = the AUDIT-106 join-key work).
- **Finding:** per `docs/audit/canonical/EP.code.json`, EP has registry=48, evaluatorBlocks=47, gapsPushCount=49. The three counts do not reconcile: registry (48) > evaluator (47) implies **>=1 registry entry with no evaluator block** - registered (appears in metadata/UI provenance) but whose detection logic never executes, so it **silently never fires** (a real detection gap); gaps.push (49) > registry (48) implies **>=1 `gaps.push` with no registry id** (unguarded provenance, the AUDIT-106 B-axis).
- **Fold-in enumeration (the registry-without-evaluator IS identifiable, via cross-reference not automation):** the 48-vs-47 registry-without-evaluator is **`gap-ep-anticoag-interruption`** (registry `gapRuleEngine.ts:2023`). It has NO EP evaluator block - the only periprocedural/bridging-anticoag block in the engine is `CAD-HEPARIN-BRIDGE` (`gapRuleEngine.ts:8685`), which pushes a `CORONARY_INTERVENTION` gap, not an EP one. So `gap-ep-anticoag-interruption` is registered but never fires - which is exactly the **already-filed AUDIT-035 EP registry-only orphan** (MEDIUM). The count mismatch and AUDIT-035 are the same defect viewed two ways.
- **Automated enumeration FAILED (the AUDIT-106 manifestation):** a best-effort fuzzy match of registry `id`s against evaluator `name`s produced 2 candidates (`gap-ep-rate-control-afib`, `gap-ep-direct-cardioversion`), BOTH false (they have evaluators `EP-RC` `:5098` and `EP-CARDIOVERSION-TIMING` `:9893`), and did NOT surface the true orphan. Registry uses `id` (`gap-ep-...`) while evaluators use unrelated `name`s (`EP-RC`/`Gap-39`/`EP-OAC`) - **no clean join key** - so the orphan was only found by following the AUDIT-035 lead, not by reconciliation. The gaps.push(49)>registry(48) unguarded-provenance instance is similarly not automation-enumerable.
- **Remediation candidate (NOT done here):** establish the registry<->evaluator<->gaps.push join key (AUDIT-106 remediation - a stable gap id threaded from registry through the evaluator comment and `gaps.push`), then a count-reconciliation CI check would catch a registry-only orphan (AUDIT-035 class) and an unguarded `gaps.push` automatically instead of by manual audit.
- **Cross-references:** `docs/audit/canonical/EP.code.json` (the counts); `gapRuleEngine.ts:2023` (the orphan registry entry), `:8685` (the CAD block that is NOT its evaluator), `:5098`/`:9893` (the fuzzy-match false positives); **AUDIT-035** (the same registry-only orphan, already OPEN MEDIUM); AUDIT-106 (the parent join-key gate-coverage gap).

---

### AUDIT-120 - EP LAAC contraindication gate uses Z88 ("Allergy status", any drug) for OAC-contraindication: over-broad, false-fires on any drug allergy

- **Phase:** 0B EP clinical-code verification (surfaced 2026-06-08, EP audit Batch 5 §16 ICD-10 verification).
- **Severity:** LOW (P3). False-positive over-referral (LAAC device-eligible gap), no missed-gap / safety / PHI harm; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** RESOLVED 2026-06-16 (v3.0 EP buildout chunk 1, branch `feat/ep-chunk1-af-anticoag`). The bare `Z88` (allergy-status to ANY drug) was removed from the OAC-contraindication set; the gate is now bleeding/hemorrhage-history only (D68.3, K92.2, I60/I61/I62) plus the operator-ruling GU major-bleed additions (R31.0 gross hematuria, N02 recurrent/persistent hematuria; microscopic hematuria R31.1/R31.2 and epistaxis R04.0 deliberately excluded - ICD-10-CM verified via icd10data/AAPC per §16). Regression test added: an AF + age>=65 patient with a drug allergy (Z88.0) and no bleed must NOT fire LAAC (`backend/tests/ingestion/epChunk1.test.ts`). The DET_OK flip for GAP-EP-011 is justified at the EP canonical close.
- **Defect:** `EP-LAAC` (GAP-EP-011, `gapRuleEngine.ts:4299-4302`) fires the LAAC device-eligible gap on `hasAF && age >= 65 && hasOACContraindication`, where `hasOACContraindication = dxCodes.some(c => c.startsWith('Z88') || c.startsWith('D68.3') || c.startsWith('K92.2') || c.startsWith('I60') || c.startsWith('I61') || c.startsWith('I62'))`. ICD-10-CM `Z88.*` is "Allergy status" to ANY drug/substance class (verified NLM 2026-06-08: Z88.0 penicillin, Z88.1 other antibiotics, Z88.4 anesthetic, Z88.5 narcotic, Z88.6 analgesic, Z88.8 other drugs, ...) - it is NOT anticoagulant-specific. So `startsWith('Z88')` over-matches: an AF patient age>=65 with a penicillin allergy (Z88.0) and no actual OAC contraindication is flagged LAAC-eligible.
- **Impact:** FALSE-POSITIVE over-detection / over-referral. The other contraindication codes are correct and OAC-relevant (D68.3 = hemorrhagic disorder due to circulating anticoagulants; K92.2 = GI hemorrhage; I60/I61/I62 = nontraumatic intracranial hemorrhage), so the rule still catches true contraindications - it just also fires on unrelated drug allergies. Direction: precision defect (false-positive), opposite to the AUDIT-118 under-detection class.
- **Classification effect:** GAP-EP-011 proposed DET_OK -> PARTIAL_DETECTION (operator review) - the rule over-fires on a population outside the spec gap. This is a distinct driver from AUDIT-117/118/§16.5 (which are under-detection); AUDIT-120 is over-detection. See `docs/audit/EP_BATCH5_LAAC_SYNCOPE.md`.
- **Remediation candidate (NOT done here):** drop `Z88` from the OAC-contraindication set (the bleeding/hemorrhage codes carry the real signal), OR narrow to an anticoagulant-specific encoding (e.g. a personal-history-of-anticoagulant-adverse-effect / bleeding-on-anticoagulant code such as D68.32 / Z79.01-history, or T45.515-class adverse effect), with a negative test (penicillin-allergy-only patient must NOT fire LAAC).
- **Cross-references:** `gapRuleEngine.ts:4299-4302`; NLM ICD-10-CM Z88 (allergy-status category); AUDIT-068 (cross-class-match Cat A precedent - a code mapped to an unrelated concept); the EP audit Batch-5 findings doc.

**EP module-close provenance (2026-06-08):** the EP module re-audit completed 89/89 (§16 external code verification + §1 direct rule-body read + the §16.5 medication-match modifier; the extraction subagent was DROPPED after confabulating in 2 of 3 batches - the authoritative gap list was pulled from `EP.spec.json` + `EP.crosswalk.json` and every DET_OK match expression read directly from the running code). Canonical `docs/audit/canonical/EP.crosswalk.json` was regenerated via the §9.2 full-pipeline-regen (extractCode -> extractSpec -> reconcile -> refreshCites -> applyOverrides -> renderAddendum -> renderSynthesis -> validateCanonical): the 13 operator-approved DET_OK -> PARTIAL_DETECTION flips are now APPLIED (were proposed), moving the distribution **21 / 26 / 42 -> 8 DET_OK / 39 PARTIAL / 42 SPEC_ONLY** (any-coverage 47/89 = 52.8% UNCHANGED; trustworthy-detection 23.6% -> 9.0%). 12 of 13 flips are AUDIT-118 / §16.5 (medication-presence under-detection); 1 is AUDIT-120 (Z88 over-detection). Findings filed by this audit: **AUDIT-117** (HIGH P1, dabigatran SCD `1037045` not ingredient `1037042`), **AUDIT-118** (HIGH P1, exact-RxCUI match with no ingredient->descendant expansion, all-module blast radius), **AUDIT-119** (MEDIUM P2, registry/evaluator/gaps.push 48/47/49 = the AUDIT-035 orphan), **AUDIT-120** (LOW P3, Z88 over-broad OAC-contraindication gate). The rendered `docs/audit/PHASE_0B_EP_AUDIT_ADDENDUM.md` dated-supersedes the 2026-05-04 version (prior retained in git history); §6.3 / §11.5 / §12 hand-authored, §1 / §7 / §13 / §14 generated. validateCanonical: all 6 modules VALID. Wall-clock in `docs/audit/canonical/audit_runs.jsonl` (runs `EP-2026-06-08-batch1` through `batch5`; ~72 min AI-assisted for the full 89-gap classification + ~40 external code verifications).

### AUDIT-121 - SH bicuspid-AV surveillance rule gates on Q23.1 (congenital aortic insufficiency), not the bicuspid code Q23.81: wrong-concept miscode that misses the entire target population

- **Phase:** 0B SH clinical-code verification (surfaced 2026-06-08, SH audit Batch 1 §16 ICD-10 verification).
- **Severity:** MEDIUM (P2). Wrong-concept code: complete false-negative for the target bicuspid-AV population (Q23.81 never matches) plus false-positive on congenital aortic-insufficiency (Q23.1) patients. T3 surveillance gap; no safety / PHI dimension; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (recorded; remediation separate).
- **Defect:** `SH-BICUSPID` (GAP-SH-008 DET_OK + GAP-SH-009 PARTIAL, `gapRuleEngine.ts:10543`) fires on `dxCodes.some(c => c.startsWith('Q23.1'))`, and the rule comment (`:10542`) + evidence triggerCriteria (`:10560`) both label it "Bicuspid aortic valve (Q23.1)". Per NLM Clinical Tables (ICD-10-CM, verified 2026-06-08): `Q23.1` = "Congenital insufficiency of aortic valve"; `Q23.0` = "Congenital stenosis of aortic valve"; **bicuspid aortic valve = `Q23.81`**. The spec detectionLogic independently miscodes it as "Q23.0 BAV". None of the three references the correct code.
- **Blast radius (SH Batch-2 STEP-0 completeness scan, 2026-06-08):** the same Q23.1-as-bicuspid miscode recurs at TWO more code sites beyond SH-BICUSPID, both VHD-module evaluator blocks UNMAPPED to any crosswalk gap (AUDIT-106 orphan class): `VD-BICUSPID-AORTOPATHY` (`gapRuleEngine.ts:10899`) reuses the SAME `hasBicuspid` const defined at `:10543`, and `VD-15` Aortic Root Dilation Monitoring (`:6677-6678`) has its own `dxCodes.some(c => c.startsWith('Q23.1') || c.startsWith('Q87.4'))`. So the wrong-concept defect spans 3 rule sites across 2 modules. Remediation is 2 edits: the shared `hasBicuspid` const at `:10543` (fixes SH-BICUSPID AND VD-BICUSPID-AORTOPATHY at once) plus the VD-15 match at `:6678`. The 2 VD rules' own classification is deferred to the VHD re-audit; the code defect is filed here.
- **Impact:** FALSE-NEGATIVE for every actual bicuspid patient (coded Q23.81, never matched) - the surveillance gap never fires for the population it exists to protect - PLUS FALSE-POSITIVE on congenital aortic-insufficiency patients (Q23.1). A cross-class mismatch (miss-target AND wrong-target), distinct from AUDIT-120's same-target over-broadening.
- **Classification effect:** GAP-SH-008 APPLIED 2026-06-08 (SH synthesis) DET_OK -> PARTIAL_DETECTION (operator review) - the evaluator exists but detects the wrong concept entirely. GAP-SH-009 (BAV aortopathy, same evaluator) already PARTIAL; also affected. Driver is §16 code-correctness, NOT §16.5. See `docs/audit/SH_BATCH1_AS_BAV.md`.
- **Remediation candidate (NOT done here):** replace `Q23.1` with `Q23.81` (bicuspid aortic valve) in SH-BICUSPID; if a broader BAV-spectrum match is intended, enumerate `Q23.81` plus the relevant AS/AI codes explicitly, with a positive test (a Q23.81 patient must fire) and a negative test (a Q23.1-only patient must NOT fire as bicuspid).
- **Cross-references:** `gapRuleEngine.ts:10542-10543,10560` (SH-BICUSPID), `:10899-10920` (VD-BICUSPID-AORTOPATHY), `:6677-6693` (VD-15); NLM ICD-10-CM Q23.81 (bicuspid) vs Q23.1 (congenital AI); AUDIT-068 (cross-class-match Cat A precedent); AUDIT-106 (the 2 VD sites are unmapped-evaluator orphans); the SH audit Batch-1 + Batch-2 findings docs.

---

### AUDIT-122 - SH post-TAVR PVL gap (GAP-SH-013) over-detects via I34.0 mitral regurgitation on an aortic-paravalvular-leak-specific target: first §16.6 over-broad case

- **Phase:** 0B SH clinical-code verification (surfaced 2026-06-08, SH Batch-2 / adjudicated under the new §16.6 over-detection axis).
- **Severity:** LOW (P3). False-positive over-referral on a non-target population; T2 surveillance; no safety / PHI dimension; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (recorded; remediation separate).
- **Defect:** `SH-13` (GAP-SH-013, `gapRuleEngine.ts:6462-6486`) matches `hasProsthetic13 (Z95.2) && (I35.1 || I34.0)` as "new regurgitation". The spec gap is PVL-specific for an aortic prosthesis (name "Post-TAVR paravalvular leak moderate+"; structuredDataElements "TAVR, PVL grade"). `I35.1` ("Nonrheumatic aortic valve insufficiency", NLM-verified) is the correct aortic-regurg target of an aortic-prosthesis paravalvular leak. `I34.0` ("Nonrheumatic mitral valve insufficiency", NLM-verified) is a DIFFERENT valve - it fires on post-prosthetic patients with unrelated mitral regurgitation and no paravalvular leak.
- **Impact:** FALSE-POSITIVE - a post-TAVR patient with incidental mitral regurg (I34.0) and no PVL receives the paravalvular-leak follow-up recommendation. The §16.6 test: a non-target patient (mitral regurg, no PVL) fires; "clinically related" (both are cardiac valve findings) is NOT a pass. This is over-detection, distinct from AUDIT-121's wrong-concept miss.
- **Classification effect:** GAP-SH-013 APPLIED 2026-06-08 (SH synthesis) DET_OK -> PARTIAL_DETECTION (operator review). The Batch-2 findings doc's prior "minor / clinically-related, holds DET_OK" framing is CORRECTED here by the §16.6 spec-based test. See `docs/audit/SH_BATCH2_TAVR_AORTIC.md`.
- **Remediation candidate (NOT done here):** drop `I34.0` from SH-13 (keep `I35.1` for aortic-prosthesis PVL); add a negative test (a post-TAVR + isolated-mitral-regurg patient must NOT fire) and a positive test (aortic PVL via I35.1 must fire). Also entangled with AUDIT-123 (the Z95.2 valve-type code).
- **Cross-references:** `gapRuleEngine.ts:6461-6486`; NLM ICD-10-CM I35.1 (aortic) vs I34.0 (mitral); AUDIT-120 (the over-detection precedent); AUDIT_METHODOLOGY.md §16.6; the SH audit Batch-2 findings doc.

---

### AUDIT-123 - Systematic ICD-10 valve-type concept confusion (Z95.2 prosthetic/mechanical vs Z95.3 xenogenic/bioprosthetic INVERTED) across the prosthetic-valve rule cluster

- **Phase:** 0B SH clinical-code verification (surfaced 2026-06-08 via the §16.6 propagation sweep on the Batch-2 Z95.2 rules).
- **Severity:** MEDIUM (P2) proposed. Systematic concept-match defect spanning SH + VHD valve + anticoagulation rules (81 `Z95.x` references); carries a safety-adjacent warfarin over-treatment sub-angle. Mitigated synthetic/pre-DUA. Operator confirms severity + scope per §18.
- **Status:** RESOLVED 2026-06-17 (v3.0 SH buildout, branch `feat/sh-chunk1-as-severity`; operator clinical sign-off on the safety-first per-rule fork).
- **Resolution (2026-06-17):** the bucket-D bioprosthetic-rule inversions are corrected per the operator fork ruling. (1) Broad bio surveillance/degeneration/anticoag-3mo/PVL/obstruction rules now use `Z95.2 || Z95.3 || Z95.4` (added the wrongly-excluded `Z95.3`; safety-first - missing a bio patient is the harm, a surveillance nudge on a mechanical valve is harmless). (2) The drug-bearing aspirin gap (`VD-ANTIPLATELET-BIOPROSTHETIC`) is now `Z95.3`-ONLY and its inverted `noMechanicalValveBio = !Z95.3` logic + "Not a mechanical valve (Z95.3)" comment are removed (Z95.3 IS bioprosthetic; never recommend antiplatelet-only to a possibly-mechanical generic-`Z95.2` patient). (3) ViV (GAP-SH-061) is now `Z95.3`-ONLY (never false-fires on a mechanical valve). (4) Post-TAVR (GAP-SH-011) is now `Z95.2 || Z95.3` (TAVR tissue valves may be coded either way). (5) The 3 stale "Mechanical valve (Z95.2/Z95.3/Z95.4)" triggerCriteria strings (`:6603`, VD-3, VD-6) are corrected to "(Z95.2 or Z95.4)" to match the AUDIT-164 predicate. Guarded by `backend/tests/ingestion/audit123Bioprosthetic.test.ts` (positive: isolated `Z95.3` fires bio surveillance/degeneration/aspirin/post-TAVR/ViV; the aspirin + ViV inversion regressions). **Documented Path-B residual (known-limit, not a defect):** ICD-10-CM has no mechanical-specific code, so a bioprosthetic miscoded as the generic `Z95.2` is treated as mechanical by the safety-first warfarin/DOAC convention, and is not caught by the `Z95.3`-only aspirin/ViV gaps. Bucket-B "any prosthetic" rules (endocarditis/dental prophylaxis, AVR-already-replaced exclusion, valve-surgery-planning) correctly keep `Z95.2||Z95.3||Z95.4`.
- **Defect:** per NLM Clinical Tables (ICD-10-CM, verified 2026-06-08, re-verified 2026-06-17): `Z95.2` = "Presence of prosthetic heart valve" (mechanical / general), `Z95.3` = "Presence of XENOGENIC heart valve" (= bioprosthetic / tissue, what TAVR and surgical bioprostheses are), `Z95.4` = "Presence of other heart-valve replacement". The codebase inverts `Z95.2` and `Z95.3`: it treats `Z95.3` as "mechanical" (`:11228` `noMechanicalValveBio = !dxCodes.some(c => c.startsWith('Z95.3'))`, comment `:11246` "Not a mechanical valve (Z95.3)") and labels `Z95.2` "bioprosthetic" (`:5267`, `:5774`, `:6611`, `:10604` SH-VALVE-IN-VALVE, `:10804`, `:11227`). Other sites call `Z95.2/Z95.3/Z95.4` collectively "mechanical" (`:4957`, `:5551`, `:5642`).
- **Impact (detection-quality; the SAFETY vector is AUDIT-124):** bioprosthetic/TAVR-targeted rules (post-TAVR surveillance, ViV, PVL) gate on `Z95.2` (mechanical) - they MISS real bioprosthetic patients (coded `Z95.3`) and FALSE-FIRE on mechanical-valve patients (for whom ViV is inapplicable). The clinical over-anticoagulation consequence in the VD-1 warfarin rule (`:4939`) is split into its own SAFETY-tagged finding **AUDIT-124** (this entry stays the root-cause concept-inversion). Blast radius cross-module (SH + VHD), 81 references.
- **Classification effect:** GAP-SH-061 (ViV TAVR, SH-VALVE-IN-VALVE) and GAP-SH-011 (post-TAVR surveillance echo, SH-6) are §16.6-affected DET_OK whose detection rests on the inverted `Z95.2` semantics - **APPLIED 2026-06-08 (SH synthesis) DET_OK -> PARTIAL_DETECTION (operator-decided 2026-06-08).** Holding DET_OK would overclaim: per the underclaim principle + the AUDIT-118 precedent, a data-coupled rule defect flips to PARTIAL regardless of whether the ingested coding (Z95.2 vs Z95.3) currently masks it. The data-coupling caveat + the 81-ref cross-module remediation are operator-gated and deferred; the classification is not. GAP-SH-013 flips via AUDIT-122. VHD-rule reclassification deferred to the VHD re-audit.
- **Remediation candidate (NOT done here):** correct the `Z95.2`/`Z95.3` semantics codebase-wide - each valve rule must state its intended valve type (mechanical vs bioprosthetic/xenogenic vs any) and use the matching code(s), with tests (a mechanical-only patient must not fire a bioprosthetic-only rule and vice-versa; the VD-1 warfarin rule must not fire on an isolated bioprosthetic valve). Repo-wide, not per-gap.
- **Cross-references:** `gapRuleEngine.ts` 81 `Z95.x` refs (key: `:4939`/`:4957` VD-1 warfarin, `:5404` SH-6, `:6462` SH-13, `:10604` SH-VALVE-IN-VALVE, `:11227-11246`); NLM ICD-10-CM Z95.2/Z95.3/Z95.4; AUDIT-118 (the data-coupling precedent); AUDIT-121 (sister concept-match); AUDIT-124 (the SAFETY vector split out); AUDIT_METHODOLOGY.md §16.6 (propagation-sweep exemplar #2); the SH audit Batch-2 findings doc.
- **VHD-side propagation sites (added 2026-06-10, VHD clinical-gap audit; no new id minted):** the Z95.3-as-mechanical / Z95.2-as-bioprosthetic inversion also lands on VHD rules VD-2 (`gap-vd-2-bioprosthetic-echo`), VD-11 (`gap-vd-11-bioprosthetic-degeneration`), VD-13, and VD-ANTIPLATELET-BIOPROSTHETIC (`gapRuleEngine.ts:11228` / `:11246`, `noMechanicalValveBio = !Z95.3`). These are additional instances of the same root inversion within the VHD cluster, NOT new findings - folded here per the §16.6 propagation-sweep model.
- **VD-6 DOAC-contraindication SAFETY consequence (added 2026-06-10; the former AUDIT-132 vector, folded into AUDIT-123 per operator instruction - NOT minted as a new id):** VD-6 (`gap-vd-6-doac-mechanical-valve`, the DOAC + mechanical-valve Class-3-Harm rule) inherits the Z95.2/Z95.3 inversion, so its mechanical-valve predicate keys off the wrong valve-type code; combined with the AUDIT-117 exact-RxCUI DOAC match (no ingredient->descendant expansion) this yields a false-positive / false-negative contraindication-detection axis on a CRITICAL SAFETY rule. The detection-quality root cause is this AUDIT-123 inversion; the safety surface is shared with AUDIT-124 (VD-1 warfarin over-treatment). GAP-VHD-005 is held PARTIAL on this basis (AUDIT-123 + AUDIT-117). Severity of AUDIT-123 is UNCHANGED by this fold (still MEDIUM P2; the DUA-escalation safety angle is carried by AUDIT-124).

---

### AUDIT-124 (SAFETY) - VD-1 warfarin rule recommends lifelong anticoagulation for bioprosthetic valves: over-anticoagulation / bleeding-risk over-treatment

- **Phase:** 0B SH clinical-code verification (surfaced 2026-06-08; SAFETY vector split from AUDIT-123 per operator instruction).
- **Severity:** MEDIUM (P2) under current pre-DUA synthetic mitigation. **ESCALATE-TO-HIGH (P1) AT DUA:** once real valve-coded patient data is ingested, this becomes a direct patient-harm vector (chronic anticoagulation recommended to bioprosthetic-valve patients who do not need it = avoidable major-bleeding risk). Operator confirms per §18.
- **Status:** RESOLVED 2026-06-17 (the over-anticoagulation bleeding-risk vector is code-closed). The DUA-escalation no longer applies.
- **Resolution (2026-06-17):** the safety vector was transitively closed by the AUDIT-164 fix (2026-06-16), which narrowed the SHARED `hasMechanicalValve` variable to `Z95.2 || Z95.4` (dropping the bioprosthetic `Z95.3`). VD-1 (warfarin-needed), VD-3 (INR-monitoring), and VD-6 (DOAC-contraindication) all reference that one variable, so an isolated bioprosthetic (`Z95.3`) patient no longer fires any of them - no lifelong-warfarin recommendation, no DOAC-contraindication false-alert. The AUDIT-123 sweep (2026-06-17) added the explicit NEGATIVE guard: `backend/tests/ingestion/audit123Bioprosthetic.test.ts` asserts an isolated `Z95.3` patient does NOT fire warfarin-needed / DOAC-contraindication / INR-monitoring, plus a control that a mechanical `Z95.2` patient still fires (under-anticoagulating a mechanical valve is catastrophic, so the safety-first `Z95.2`-counts-as-mechanical convention is retained). **Path-B residual:** a bioprosthetic miscoded as the generic `Z95.2` is still treated as mechanical (ICD-10-CM has no mechanical-specific code) - a documented known-limit, accepted as safety-first.
- **Defect:** VD-1 (`gapRuleEngine.ts:4939`) computes `hasMechanicalValve = dxCodes.some(c => c.startsWith('Z95.2') || c.startsWith('Z95.3') || c.startsWith('Z95.4'))` and, if the patient is not on warfarin (RxNorm 11289), pushes a `MEDICATION_MISSING` "warfarin not active with mechanical valve" gap. But `Z95.3` = "Presence of xenogenic heart valve" (bioprosthetic, NLM-verified) and `Z95.4` = "Presence of other heart-valve replacement" - neither is mechanical. Per 2020 ACC/AHA VHD, lifelong VKA is Class 1 for MECHANICAL valves only; bioprosthetic valves receive 3-6 months of anticoagulation (or antiplatelet) then antiplatelet alone.
- **Impact:** the rule fires "lifelong warfarin missing" on bioprosthetic (`Z95.3`) and other-replacement (`Z95.4`) valve patients, recommending chronic anticoagulation they should not be on - an over-treatment / major-bleeding direction. SAFETY-tagged because the recommended ACTION carries its own risk (the opposite-harm mirror of a missed-anticoagulation gap).
- **Classification effect:** VD-1 is a VHD-module rule; its own classification is the VHD re-audit's scope. Recorded here as a cross-module SAFETY finding surfaced by the SH §16.6 propagation sweep. No SH-gap classification change from AUDIT-124 (SH-061/011 flip under AUDIT-123).
- **Remediation candidate (NOT done here):** narrow VD-1's mechanical-valve predicate to the mechanical code(s) only (correct `Z95.2` mechanical vs `Z95.3` xenogenic per AUDIT-123) so the lifelong-VKA recommendation fires only for mechanical valves; add a negative test (an isolated bioprosthetic / `Z95.3` patient must NOT trigger a lifelong-warfarin gap) and a positive test (a mechanical-valve patient off warfarin must fire).
- **Cross-references:** `gapRuleEngine.ts:4935-4960` (VD-1); NLM ICD-10-CM Z95.2 (mechanical) vs Z95.3 (xenogenic/bioprosthetic); RxNorm 11289 (warfarin); 2020 ACC/AHA VHD anticoagulation recommendations; **AUDIT-123** (root-cause concept-inversion); AUDIT_METHODOLOGY.md §16.6.

---

### AUDIT-125 - Severity-encoding over-detection (§16.6(iii)): dx + age/symptom DET_OK rules fire on sub-threshold lesions where the gap targets severe/moderate+

- **Phase:** 0B SH clinical-code verification (surfaced 2026-06-08 at SH Batch-3 close; codified as §16.6(iii)).
- **Severity:** LOW-MEDIUM proposed. False-positive over-referral on sub-threshold (mild) lesions of the correct valve/condition; no safety / PHI dimension; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (recorded; remediation separate).
- **Defect:** a rule targeting a severity-gated spec gap (target "severe" / "moderate+") that gates only on a diagnosis code + an age or symptom proxy, with NO echo-severity threshold in `labValues` (EROA / regurgitant volume / valve area / mean gradient / RV grade), fires on ANY-severity lesion including mild. Per §16.6(iii) the predicate is that severity IS echo-encoded in `labValues` (the rule could read it) and the rule IGNORES it - over-detection, not inherent ICD-10 severity-blindness.
- **Confirmed SH instances (DET_OK -> PARTIAL):**
  - **GAP-SH-064** (SH-11, `gapRuleEngine.ts:6404`): `hasMR10 (I34.0) && age > 80`. Spec gap "Transcatheter MVR (Tendyne/Intrepid) candidate for non-TEER anatomy", DL "Severe MR ineligible for TEER". Targets SEVERE MR; zero severity gate (dx + age) -> fires on mild MR + age>80.
  - **GAP-SH-022** (SH-4, `gapRuleEngine.ts:5337`): `hasTricuspidRegurg (I36.1) && (R60||R16||R18)`. Spec gap "Severe TR: transcatheter evaluation gap", DL "Severe or torrential TR + NYHA II+ on diuretics". Targets SEVERE/torrential TR; zero TR-grade gate (dx + symptom) -> fires on mild TR + edema of another cause.
- **Also §16.6(iii)-affected but ALREADY PARTIAL (no flip; recorded for completeness):** SH-1/SH-2 AS rules (`I35.0` + an LVEF flag; LVEF is an LV-FUNCTION axis, NOT an AS-severity gate, which is valve area / mean gradient - so they fail (iii) too, but are already PARTIAL via §3.2.1 broad consolidation); SH-3 (`LVEF<60 || symptom`, MR), SH-12 (symptom-only, TR). These do not change classification.
- **Impact:** FALSE-POSITIVE over-referral - a mild-lesion patient receives a severe-lesion intervention-evaluation recommendation. Direction: over-detection (the §16.6 family); milder per-instance than a wrong-target miss but HIGH-VOLUME (the class is large).
- **Classification effect:** GAP-SH-064 + GAP-SH-022 APPLIED 2026-06-08 (SH synthesis) DET_OK -> PARTIAL_DETECTION (operator review). Cross-module: every dx-proxy DET_OK targeting a severity-gated gap across HF / CAD / PV / VHD / EP is in this class; per the §16.5 precedent the retroactive re-application is a v2.0 Module-Parity pass - completed modules are NOT re-opened here.
- **Remediation candidate (NOT done here):** add the lesion-appropriate echo-severity gate to each affected rule (MR: EROA / regurg volume; TR: RV / TR grade; AS: valve area / mean gradient), with a negative test (a sub-threshold lesion does NOT fire) and a positive test (a target-severity lesion DOES). Data-coupled (the severity labValues must be ingested), the AUDIT-118 pattern.
- **Cross-references:** `gapRuleEngine.ts:6404` (SH-11), `:5337` (SH-4), `:4905`/`:5240` (SH-1/SH-2 AS); AUDIT_METHODOLOGY.md §16.6(iii); AUDIT-118 (data-coupling precedent); AUDIT-122 (sister §16.6 over-detection); the SH audit Batch-1/2/3 findings docs.

---

### AUDIT-126 - SH post-ASA conduction-surveillance gap (GAP-SH-104) mapped to the ASA-candidacy rule (SH-15): §16.6(ii) wrong-target

- **Phase:** 0B SH clinical-code verification (surfaced 2026-06-08, SH Batch 4 §1 read).
- **Severity:** LOW (P3). Missed-detection of the post-procedure target + false-fire on pre-procedure HCM candidacy; T2 surveillance; no safety / PHI dimension; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (recorded; remediation separate).
- **Defect:** GAP-SH-104 spec = "Septal alcohol ablation: post-procedure conduction surveillance" (DL "ASA procedure without post-procedure telemetry + ECG f/u") - target is a patient who HAS UNDERGONE alcohol septal ablation and needs conduction (heart-block) monitoring. The mapped rule SH-15 (`gapRuleEngine.ts:6519`) gates on `hasHCM15 (I42.1 obstructive HCM) && hasObstructionProxy (R06||R55||R00)` and recommends "alcohol septal ablation or myectomy evaluation" - PRE-procedure CANDIDACY. SH-15 never references ASA procedure history; it fires on symptomatic obstructive-HCM patients who have NOT had ASA.
- **Impact:** the post-ASA conduction-surveillance gap is NOT detected (false-negative on its target - post-ASA patients are not identified for telemetry), while SH-15 false-fires on pre-procedure HCM candidacy patients (who are the target of the sibling gap GAP-SH-105, ASA-vs-myectomy decision, which SH-15 correctly serves). A §16.6(ii) wrong-target mismap from §3.2.1 broad consolidation.
- **Classification effect:** GAP-SH-104 APPLIED 2026-06-08 (SH synthesis) DET_OK -> **SPEC_ONLY** (operator-decided 2026-06-08). SH-15 (ASA candidacy) is FULLY DISJOINT from the post-ASA-surveillance target - it shares zero true positives with the gap - so PARTIAL would overclaim coverage. Per the §16.6(ii) overlap rule (codified this block: partial-overlap -> PARTIAL per AUDIT-122; disjoint -> SPEC_ONLY), GAP-SH-104 -> SPEC_ONLY. The spurious mapping remains the §16.6(ii) defect to remediate. GAP-SH-104 drops OUT of SH any-coverage. See `docs/audit/SH_BATCH4_PFO_ACHD_HCM.md`.
- **Remediation candidate (NOT done here):** add a post-ASA procedure-history gate to a dedicated GAP-SH-104 rule, OR reclassify GAP-SH-104 SPEC_ONLY (no current rule detects post-procedure surveillance). Keep SH-15 for GAP-SH-105 (candidacy/decision, which it correctly serves).
- **Cross-references:** `gapRuleEngine.ts:6516-6543` (SH-15); NLM ICD-10-CM I42.1 (obstructive HCM, verified); AUDIT_METHODOLOGY.md §16.6(ii) (incl. the overlap rule); AUDIT-122 (sister wrong-target, PARTIAL); AUDIT-106 (broad-consolidation mapping class); the SH audit Batch-4 findings doc.

---

### AUDIT-127 - SH PFO-closure rule (SH-9 / GAP-SH-026) has no coded stroke-etiology exclusion: over-detects on non-cryptogenic strokes

- **Phase:** 0B SH clinical-code verification (surfaced 2026-06-08, SH Batch-5 STEP-0 SH-026 verification).
- **Severity:** LOW (P3). False-positive over-referral on strokes with a competing/known etiology; T2; no safety / PHI dimension; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** RESOLVED 2026-06-17 (v3.0 SH module close).
- **Resolution (2026-06-17):** the properly-scoped SH-026 (v3.0 SH chunk 5) replaces the legacy rule - it gates PFO `Q21.12` (specific, not broad Q21.1) + cryptogenic stroke `I63.x` + age<60 + EXCLUDES AF `I48.x` (the cardioembolic competing source this finding named). The legacy SH-9 (`gap-sh-9-pfo-closure`) was RETIRED at this close (supersede-not-delete, registry retained for lineage, reconciled to SPEC_ONLY), so the un-gated over-detector no longer fires. GAP-SH-026 -> DET_OK. Guarded by shChunk5.test.ts (PFO + stroke + AF -> does NOT fire; PFO + stroke + no-AF -> fires). Same retirement precedent as SH-12 -> SH-022 (AUDIT-167).
- **Defect:** SH-9 (`gapRuleEngine.ts:5497-5525`) fires on `hasCryptogenicStroke (I63.9) && age < 60 && hasPFO (startsWith('Q21.1'))`. The spec gap GAP-SH-026 targets CRYPTOGENIC stroke. The rule proxies cryptogenic with `I63.9` ("Cerebral infarction, unspecified", NLM-verified) but applies NO coded etiology exclusion - the rule's OWN evidence object lists "Known alternative stroke etiology" as an exclusion (`:5522`), yet the match logic never checks `!hasAF (I48)` / `!hasCarotid (I65)` / other named cause. An evidence-vs-logic gap.
- **Impact:** FALSE-POSITIVE - a patient coded I63.9 + PFO + age<60 who ALSO has AF (I48, a likely cardioembolic source) fires for PFO closure, though closure is not indicated when a competing high-probability cause is present. Over-detection on a non-cryptogenic population.
- **Classification effect:** GAP-SH-026 APPLIED 2026-06-08 (SH synthesis) DET_OK -> PARTIAL_DETECTION (operator review). The rule PARTIALLY OVERLAPS the target (true cryptogenic I63.9 + PFO patients ARE caught), so PARTIAL not SPEC_ONLY per the §16.6(ii) overlap rule. See `docs/audit/SH_BATCH4_PFO_ACHD_HCM.md`.
- **Remediation candidate (NOT done here):** add the coded etiology exclusion the evidence already names (exclude I48 AF / I65 carotid / other documented cause), with a negative test (I63.9 + PFO + AF must NOT fire) and a positive test (I63.9 + PFO + no competing cause fires).
- **Cross-references:** `gapRuleEngine.ts:5497-5522`; NLM ICD-10-CM I63.9 (cerebral infarction unspecified); AUDIT_METHODOLOGY.md §16.6(ii); AUDIT-122 (sister over-detection -> PARTIAL); the SH audit Batch-4 findings doc.

---

### AUDIT-128 - SH ASD significant-shunt rule (SH-ASD / GAP-SH-027) gates on RV failure (I50.81), a late proxy that misses pre-failure significant shunts: §16.5 under-detection

- **Phase:** 0B SH clinical-code verification (surfaced 2026-06-08, SH Batch-5 STEP-0 SH-027 verification).
- **Severity:** LOW (P3). Missed-detection of earlier-stage significant shunts; T2; no safety / PHI dimension; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** RESOLVED 2026-06-17 (v3.0 SH module close).
- **Resolution (2026-06-17):** the properly-scoped SH-027 (v3.0 SH chunk 5) replaces the legacy rule - it gates ASD significance on the threaded PASP (`labValues['pasp'] >= 40`, the echo significance signal this finding named) + EXCLUDES Eisenmenger `I27.83` (closure-contraindicated), rather than the late RV-failure `I50.81` proxy. The legacy SH-ASD (`gap-sh-asd-closure`) was RETIRED at this close (supersede-not-delete, registry retained, reconciled to SPEC_ONLY). GAP-SH-027 held PARTIAL: RV size + Qp:Qs (2 of the 3 significance signals) remain echo-morphology not threaded - only PASP is. Guarded by shChunk5.test.ts.
- **Defect:** SH-ASD (`gapRuleEngine.ts:10637-10665`) fires on `hasASD (startsWith('Q21.1')) && (startsWith('I50.810') || startsWith('I50.81'))`. The spec gap GAP-SH-027 "ASD significant shunt" defines significance by RV SIZE + PASP (structuredDataElements "ASD dx, RV size, PASP, closure"; DL "ASD with RV dilation OR PASP elevation"). The rule gates on `I50.81` = right heart FAILURE, a LATE hemodynamic consequence. A significant ASD (Qp:Qs >= 1.5) with RV DILATION or PASP elevation but NOT yet RV FAILURE carries no I50.81 code and is MISSED.
- **Impact:** UNDER-detection (false-negative) - the gap exists to catch significant ASD shunts BEFORE RV failure (when closure prevents progression); gating on RV failure misses exactly the pre-failure window the intervention targets. (The under-detection mirror of the §16.6(iii) over-detection class: there the gate was absent, here it is real but set at the wrong/late point on the disease course.)
- **Classification effect:** GAP-SH-027 APPLIED 2026-06-08 (SH synthesis) DET_OK -> PARTIAL_DETECTION (operator review). The rule PARTIALLY OVERLAPS (RV-failure ASD patients ARE caught), so PARTIAL not SPEC_ONLY. See `docs/audit/SH_BATCH4_PFO_ACHD_HCM.md`.
- **Remediation candidate (NOT done here):** gate significance on RV size / Qp:Qs / PASP echo signals in `labValues` (the gap's actual significance axis) rather than the late RV-failure dx, with a negative test (mild ASD no RV dilation must NOT fire) and a positive test (significant ASD with RV dilation pre-failure fires). Data-coupled (echo signals must be ingested), the AUDIT-118 pattern.
- **Cross-references:** `gapRuleEngine.ts:10637-10665`; NLM ICD-10-CM I50.81 (right heart failure) vs the gap's RV-size/PASP significance axis; AUDIT-118 (data-coupling precedent); AUDIT-125 (sister severity-encoding); AUDIT_METHODOLOGY.md §16.5; the SH audit Batch-4 findings doc.

---

### AUDIT-129 - SH IE Duke-workup gap (GAP-SH-028) mapped to the endocarditis-PROPHYLAXIS rule (SH-7): §16.6(ii) fully-disjoint wrong-target

- **Phase:** 0B SH clinical-code verification (surfaced 2026-06-08, SH Batch 5).
- **Severity:** LOW (P3). The suspected-IE workup gap has no real detection + the prophylaxis rule fires on a disjoint population; T2; no safety / PHI dimension; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (recorded; remediation separate).
- **Defect:** GAP-SH-028 = "Infective endocarditis: Duke criteria workup" (DL "Suspected IE without TEE + blood cultures x3") - target is a patient with SUSPECTED active IE needing diagnostic workup. The mapped rule SH-7 (`gapRuleEngine.ts:5434-5463`) gates on `hasProstheticValve (Z95.2/3/4) && hasHighRiskProcedure (Z01.2 dental / Z96 implant)` and recommends "review endocarditis PROPHYLAXIS needs" - antibiotic PREVENTION before a procedure. SH-7 never references suspected IE, blood cultures, TEE, or Duke criteria. The auto-mapper linked the two by the shared "endocarditis" keyword.
- **Impact:** the IE Duke-workup gap is NOT detected (false-negative on suspected-IE patients), while SH-7 fires on prosthetic-valve prophylaxis candidates (a disjoint population). Zero true-positive overlap.
- **Classification effect:** GAP-SH-028 APPLIED 2026-06-08 (SH synthesis) PARTIAL_DETECTION -> **SPEC_ONLY** (operator-decided 2026-06-08). Per the §16.6(ii) overlap rule, a fully-disjoint mapping is SPEC_ONLY (PARTIAL would overclaim). Mirrors AUDIT-126. See `docs/audit/SH_BATCH5_PHTN_PE_IE_MASSES.md`.
- **Remediation candidate (NOT done here):** author a suspected-IE detection rule for GAP-SH-028 (e.g. I33 acute/subacute endocarditis dx + absence of TEE / blood-culture orders), and keep SH-7 for a dedicated endocarditis-prophylaxis gap (not currently enumerated in the SH spec).
- **Cross-references:** `gapRuleEngine.ts:5431-5463` (SH-7); 2021 AHA IE Prevention (prophylaxis) vs 2023 Duke-ISCVID criteria (workup); AUDIT_METHODOLOGY.md §16.6(ii) overlap rule; AUDIT-126 (sister disjoint -> SPEC_ONLY); AUDIT-106 (broad-consolidation mapping class); the SH audit Batch-5 findings doc.

---

### AUDIT-130 - Single Netlify site serves both frozen-demo and production-frontend roles; CI deploy-frontend collides with the intentional demo lock

- **Phase:** Deploy infrastructure / frontend (surfaced 2026-06-10, post-merge CI review of PR #372 merge `48b7aeb`).
- **Severity:** MEDIUM (P2). Deploy-pipeline role-collision; no clinical / PHI / safety dimension. Operator confirms per §18.
- **Status:** OPEN (interim gate applied; full remediation deferred until a real-API frontend has a consumer, post-DUA).
- **Defect:** the repo references exactly ONE Netlify site: `tailrddemo.netlify.app` (targeted by CI via `secrets.NETLIFY_SITE_ID`; sole Netlify origin in the backend CORS allowlist `backend/src/server.ts:99` and the staging CFN CORS list `infrastructure/cloudformation/tailrd-staging.yml:439`; archived 2026-04-16 docs declare it the production frontend URL). That site was deliberately locked on Netlify as the frozen demo as of 2026-06-08. The CI `deploy-frontend` job (`.github/workflows/deploy.yml`) still attempts a `--prod` deploy to it on every main merge, so the job now fails on the lock: `Error: Deployments are "locked" for production context of this project.` (first collision: 2026-06-10 merge `48b7aeb`, run 27262523498; the job had succeeded on the 2026-06-08 merge `55c8952`, pre-lock). One site cannot simultaneously be a frozen demo and a continuously-deployed production frontend.
- **Impact:** every main merge produces a red `Build & Deploy to ECS` run conclusion even when the backend deploy succeeds (alarm-fatigue / silent-red-adjacent class), and an accidental unlock-and-deploy would overwrite the deliberately frozen demo.
- **Interim mitigation (this PR):** `deploy-frontend` is gated behind the repo variable `DEPLOY_FRONTEND_ENABLED` (absent/false = job skipped; the existing `run_build` gate is preserved). Re-enable by setting `DEPLOY_FRONTEND_ENABLED=true` AFTER re-pointing `NETLIFY_SITE_ID` at a dedicated production site.
- **Remediation candidate (NOT done here):** dedicated production-frontend site (app.tailrd-heart.com per the section 9 deployment plan), then: re-point `NETLIFY_SITE_ID`; add the new origin to the backend CORS allowlist (`backend/src/server.ts:99`) and the staging CFN CORS list (`infrastructure/cloudformation/tailrd-staging.yml:439`); CSRF-on-refresh fix as a precondition for a real-API frontend session surviving reload. Deferred until a real-API frontend has a consumer (post-DUA).
- **Cross-references:** `.github/workflows/deploy.yml` (deploy-frontend job + gate comment); `backend/src/server.ts:99`; `infrastructure/cloudformation/tailrd-staging.yml:439`; `netlify.toml` (no site ID; lock state lives Netlify-side); CLAUDE.md section 9 (frontend deploy + app.tailrd-heart.com DNS "not yet done"); AUDIT-099 (the silent-mock frontend surface that makes a real-API frontend premature); run 27262523498 (the collision evidence).

---

### AUDIT-131 - Staging ECS deploy fails health verification identically on the 4 most recent main merges; staging environment unreachable independent of code changes

- **Phase:** Operational maturity / staging environment (surfaced 2026-06-10, post-merge CI review of PR #372 merge `48b7aeb`).
- **Severity:** MEDIUM (P2). Silent-red post-merge gate class (AUDIT-107 / AUDIT-111 / AUDIT-112 sisters); no clinical / PHI dimension. Operator confirms per §18.
- **Status:** OPEN (needs infra investigation; not remediated here).
- **Defect:** the `Build & Deploy to Staging` workflow fails its health-verification step with an identical signature on the 4 most recent main merges (2026-06-08 `0cfa579d`, `72d58fef`, `55c89528`; 2026-06-10 `48b7aeb5`): 20/20 polls return `Staging health: unavailable`, then `ERROR: Staging deploy did not stabilize in 5 minutes`. The failing merges include docs-only and test-only changes, so the failure is independent of code content - the staging environment itself is not coming healthy.
- **Impact:** staging provides zero deploy-verification value while appearing as just another red post-merge run; any change that DOES break staging would be indistinguishable from the standing outage (the AUDIT-107 silent-red lesson).
- **Likely-related (NOT verified):** AUDIT-111 documented staging undeployable on a Prisma P3009 failed `_prisma_migrations` row (staging-only, failed 2026-05-03, OPEN). This outage may be the same condition still standing, but the root cause has NOT been re-verified - the P3009 row, staging task logs, and ALB target health all need direct inspection.
- **Remediation candidate (NOT done here):** infra investigation (staging ECS task logs, `_prisma_migrations` P3009 row per AUDIT-111, ALB target-group health), fix the root cause, then make the staging gate loud per the AUDIT-107 precedent rather than silently red.
- **Cross-references:** runs 27262523279 / 27117852094-era staging failures (4-run signature); AUDIT-111 (staging P3009, the probable root cause); AUDIT-107 (silent-red gate precedent); AUDIT-025 (schema migration validation gate); `infrastructure/cloudformation/tailrd-staging.yml`; CLAUDE.md section 9 (staging stack `tailrd-staging`).

---

### AUDIT-133 - T1 sub-therapeutic-INR detection is absent engine-wide: VD-3 fires only on INR-ABSENT, no INR-to-target comparison exists; the 2026-05-04 baseline over-claimed PARTIAL on disjoint triggers (VHD-001)

- **Phase:** 0B VHD clinical-code verification (surfaced 2026-06-10, VHD re-audit).
- **Severity:** MEDIUM (P2). Missed-detection of the T1 sub-therapeutic-INR target; no engine rule computes INR-vs-target, so the gap has zero true detection; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** **RESOLVED 2026-06-17** (v3.0 VHD chunk 3). The purpose-built `gap-vhd-001-subtherapeutic-inr` evaluator (mechanical valve Z95.2/Z95.4 + warfarin RxNorm 11289 + `labValues['inr'] < 2.0`) now performs the INR-to-target comparison, reading the AUDIT-170 INR slug-fix (LOINC 34714-6 -> `inr`) that threaded the value to the engine. Guarded by vhdChunk3.test.ts (fires at INR 1.5, gates at 2.5, null-Path-B when INR not threaded). GAP-VHD-001 SPEC_ONLY -> DET_OK.
- **Defect:** GAP-VHD-001 (mechanical valve with INR below the valve-specific target) requires an INR-value-to-target comparison. The prior 2026-05-04 cite was VD-3 (`gap-vd-3-inr-monitoring`), which fires on INR-data-ABSENT (no INR on file); the separate mechanical-valve anticoagulation rule fires on warfarin-ABSENT (no warfarin on file). A mechanical-valve patient ON warfarin WITH an INR below target fires NEITHER rule - the two triggers are disjoint from the spec target. No engine rule anywhere performs an INR-to-target-range comparison.
- **Impact:** UNDER-detection (false-negative) on the entire VHD-001 population (sub-therapeutic INR on a mechanical valve = an active thrombosis / stroke risk window). The baseline PARTIAL classification over-claimed coverage on disjoint triggers.
- **Classification effect:** GAP-VHD-001 PARTIAL_DETECTION -> **SPEC_ONLY** (operator-decided 2026-06-10). Per §16.6(ii) a disjoint-target mapping is SPEC_ONLY (PARTIAL would overclaim). registryId dropped.
- **Remediation candidate (NOT done here, v2.0):** author an INR-to-target rule (mechanical valve + latest INR below the valve-specific target range), data-coupled on ingested INR labValues per the AUDIT-118 pattern, with a negative test (in-range INR does NOT fire) and a positive test (sub-therapeutic INR fires).
- **Cross-references:** `gapRuleEngine.ts` VD-3 (`gap-vd-3-inr-monitoring`); AUDIT_METHODOLOGY.md §16.6(ii) overlap rule; AUDIT-118 (data-coupling precedent); AUDIT-126 / AUDIT-129 (sister disjoint-target -> SPEC_ONLY); the VHD audit findings.

---

### AUDIT-134 - Echo existence-proxy (`labValues['lvef'] / ['inr'] === undefined`) substituted for a severity / interval gate: DET_OK over-credit (VD-2 / VHD-010 + VD-5 / VHD-103 the first two sites)

- **Phase:** 0B VHD clinical-code verification (surfaced 2026-06-10, VHD re-audit).
- **Severity:** MEDIUM (P2). Over-credit of detection fidelity (DET_OK on an existence-proxy that does not encode the gap's severity / interval target); mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** **RESOLVED 2026-06-17**. The over-credit itself was corrected at the 2026-06-10 re-audit (both sites DET_OK -> SPEC_ONLY, so no site is over-credited). v3.0 VHD chunk 1 then built the purpose-built `gap-vhd-103-severe-ar-surgical` evaluator with a genuine severity gate (`severeAR` helper: valve_severity>=5 OR aortic_vena_contracta>=0.6, + LVEF<=55), replacing the VD-5 existence-proxy -> GAP-VHD-103 SPEC_ONLY -> PARTIAL (LVEF surgical arm; the LVESD>=50 arm is Path-B, un-threaded). The second site VD-2 (`gap-vd-2-bioprosthetic-echo`) / GAP-VHD-010 was NOT in the VHD buildable scope and holds SPEC_ONLY (VD-2 remains an existence-proxy; a future bioprosthetic surveillance-interval gate is the remediation there).
- **Defect:** rules gate on the mere ABSENCE / PRESENCE of an echo value (`labValues['lvef'] === undefined`, `labValues['inr'] === undefined`) as a proxy for "echo overdue" or "value abnormal", where the spec gap actually wants a severity threshold or a surveillance INTERVAL (e.g. moderate+ regurgitation, or an echo older than the guideline interval). An existence check fires (or fails to fire) independent of the severity / interval the gap targets, so the rule cannot discriminate the spec population - yet the 2026-05-04 baseline credited it DET_OK.
- **Confirmed first two instances (DET_OK -> SPEC_ONLY):**
  - **VD-2** (`gap-vd-2-bioprosthetic-echo`) cited for **GAP-VHD-010**: existence-of-LVEF proxy, not the bioprosthetic surveillance-interval / severity target.
  - **VD-5** (`gap-vd-5-aortic-regurgitation`) cited for **GAP-VHD-103**: existence proxy, not the AR-severity target.
- **Impact:** the gap is not genuinely detected (underclaim governs); the existence-proxy is incidental coverage, not the spec target. The class is likely broader than these two sites (first two confirmed; a full engine sweep is v2.0 work).
- **Classification effect:** GAP-VHD-010 + GAP-VHD-103 DET_OK -> **SPEC_ONLY** (operator-decided 2026-06-10; both were failed 2026-05-04 baseline DET_OK overrides). registryId dropped.
- **Remediation candidate (NOT done here, v2.0):** replace the existence-proxy with the lesion-appropriate severity gate (echo grade) or the surveillance-interval comparison (last-echo-date vs the guideline interval), data-coupled per AUDIT-118; negative + positive tests per site.
- **Cross-references:** `gapRuleEngine.ts` VD-2 (`gap-vd-2-bioprosthetic-echo`) + VD-5 (`gap-vd-5-aortic-regurgitation`); AUDIT-125 (sister severity-encoding over-detection); AUDIT-128 (sister late / wrong-proxy gate); AUDIT_METHODOLOGY.md §16.5 / §16.6(iii); the VHD audit findings.

---

### AUDIT-135 - Medication-gap rule omits the therapy-ABSENT check, so it over-fires on already-treated patients (VD-14 / VHD-063; siblings VD-13 + VD-ANTIPLATELET DO check)

- **Phase:** 0B VHD clinical-code verification (surfaced 2026-06-10, VHD re-audit).
- **Severity:** MEDIUM (P2). False-positive over-firing on patients already on the therapy; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** **RESOLVED 2026-06-17** (v3.0 VHD chunk 3). The therapy-absent guard was added to VD-14: `onProphylaxisAbx_VD14 = medCodes.includes('723') || medCodes.includes('2582')` (amoxicillin / clindamycin) is now in the if-condition, mirroring the VD-13 / VD-ANTIPLATELET sibling guards. VD-14 no longer over-fires on already-prophylaxed patients. Guarded by vhdChunk3.test.ts (still fires with no abx; gates when on amoxicillin).
- **Defect:** a MEDICATION-gap rule should fire only when the recommended therapy is ABSENT (the gap is "missing therapy"). VD-14 (`gap-vd-14-dental-prophylaxis`) omitted the therapy-absent guard and fired on the qualifying diagnosis alone, so it over-fired on patients who already have the prophylaxis / therapy documented. The sibling rules DO guard: VD-13 checks therapy-absent at `gapRuleEngine.ts:6620`, and VD-ANTIPLATELET-BIOPROSTHETIC checks at `:11231` - VD-14 is the outlier.
- **Impact:** FALSE-POSITIVE over-referral on already-treated patients (a "missing prophylaxis" recommendation surfaced for a patient who is not missing it). Over-detection (the §16.6 family).
- **Classification effect:** GAP-VHD-063 DET_OK -> **PARTIAL_DETECTION** (operator-decided 2026-06-10; the 2026-05-04 baseline DET_OK over-credited fidelity). VD-14 partially covers the target but over-fires for lack of the therapy-absent gate, so PARTIAL per §3.2.1 / §16.6.
- **Remediation candidate (NOT done here, v2.0):** add the therapy-absent guard to VD-14 mirroring VD-13 (`:6620`) / VD-ANTIPLATELET-BIOPROSTHETIC (`:11231`), with a negative test (an already-prophylaxed patient does NOT fire) and a positive test (eligible + no prophylaxis fires).
- **Cross-references:** `gapRuleEngine.ts` VD-14 (`gap-vd-14-dental-prophylaxis`), VD-13 (`:6620`), VD-ANTIPLATELET-BIOPROSTHETIC (`:11231`); AUDIT_METHODOLOGY.md §16.6; AUDIT-127 (sister evidence-vs-logic missing-guard); the VHD audit findings.

---

### AUDIT-136 - Broad-rule consolidation onto gaps the rule's OWN exclusion criteria make it structurally unable to fire for: VD-10 carries EXCLUSION_PREGNANCY yet was credited for pregnant-patient gaps VHD-099/100/101 (VHD-099 Tier-S / ESCALATE-AT-DUA)

- **Phase:** 0B VHD clinical-code verification (surfaced 2026-06-10, VHD re-audit).
- **Severity:** MEDIUM (P2). **Carries a SAFETY sub-note:** GAP-VHD-099 (mechanical-valve-in-pregnancy anticoagulation, 1st-trimester warfarin->LMWH transition) is a structurally-inferred Tier-S SAFETY item with ZERO coverage; **ESCALATE-AT-DUA** (real pregnant + mechanical-valve patient data turns the uncovered warfarin-embryopathy / maternal-valve-thrombosis window into a direct patient-harm vector). Mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (recorded; remediation deferred to v2.0).
- **Defect:** the §3.2.1 broad-rule consolidation mapped GAP-VHD-099 / 100 / 101 (all PREGNANT mechanical-valve anticoagulation protocols) onto VD-10 (`gap-vd-10-pregnancy-risk`). But VD-10 carries `EXCLUSION_PREGNANCY` (`gapRuleEngine.ts:5748`) - it EXCLUDES pregnant patients by its own criteria, so it is structurally UNABLE to fire for any pregnant-patient gap. Crediting it for VHD-099/100/101 maps a gap onto a rule whose own exclusion criteria guarantee a zero true-positive intersection.
- **Impact:** UNDER-detection (false-negative) with zero detection on the three pregnant mechanical-valve anticoagulation gaps. VHD-099 specifically (1st-trimester warfarin >5mg/day teratogenicity vs the LMWH dose-transition protocol) is the highest-stakes: warfarin embryopathy and maternal valve-thrombosis are both in play and nothing detects the gap.
- **Classification effect:** GAP-VHD-099 + GAP-VHD-100 + GAP-VHD-101 PARTIAL_DETECTION -> **SPEC_ONLY** (operator-decided 2026-06-10). registryId dropped. GAP-VHD-099 additionally carries an `inferredSafetyTag: STRUCTURAL_SAFETY` (the spec carries no SAFETY tag; operator-decided Tier-S ESCALATE-AT-DUA). GAP-VHD-098 (broad pregnancy + valve-risk) HOLDS PARTIAL on VD-10 (it is not pregnancy-excluded in the protocol-specific way the three siblings are).
- **Remediation candidate (NOT done here, v2.0):** author dedicated pregnant-mechanical-valve anticoagulation rules (1st-trimester warfarin-dose / LMWH-transition for VHD-099; anti-Xa monitoring for VHD-100; near-term LMWH-restart + delivery-plan for VHD-101) that do NOT inherit VD-10's pregnancy exclusion, with positive tests per protocol. VHD-099 is queued as a Tier-S structural-safety item for DUA escalation.
- **Cross-references:** `gapRuleEngine.ts` VD-10 (`gap-vd-10-pregnancy-risk`, `EXCLUSION_PREGNANCY` `:5748`); AUDIT_METHODOLOGY.md §3.2.1 (broad consolidation) + §6.3 (Tier-S triage) + §16.6(ii); AUDIT-124 (sister VHD SAFETY-tagged finding); the VHD audit findings.

---

### AUDIT-137 - META: the 2026-05-04 VHD baseline override pass was systematically unsound (4/4 DET_OK overrides failed re-verify; the "clean reconciliation, no EXTRA rules" claim was false - 16 of 32 rules EXTRA); flags the pre-§16.6 baseline layer as an untrustworthy v2.0 starting point

- **Phase:** 0B VHD clinical-code verification (surfaced 2026-06-10, VHD re-audit; meta-finding on audit-process trustworthiness).
- **Severity:** MEDIUM (P2). Process / methodology trustworthiness finding, not a runtime clinical defect; its consequence is that the 2026-05-04 baseline cannot be trusted as a v2.0 starting point without re-verification. Operator confirms per §18.
- **Status:** OPEN (recorded; informs v2.0 sequencing).
- **Defect:** the 2026-05-04 VHD baseline override pass (pre-§16.6 methodology) was systematically unsound on re-verify: (a) **4 of 4** baseline DET_OK overrides failed re-verification - VHD-010 (VD-2), VHD-080 (VD-8), VHD-103 (VD-5) flipped DET_OK -> SPEC_ONLY and VHD-063 (VD-14) flipped DET_OK -> PARTIAL once §16.5 / §16.6 + clinical-code verification were applied (underclaim governs); (b) the baseline's "reconciliation is clean, no EXTRA rules" claim was false - **16 of 32** VHD rules are EXTRA (rules without a clean 1:1 spec-gap match), so the consolidation the baseline rested on was under-stated.
- **Impact:** the entire pre-§16.6 baseline classification layer for VHD is untrustworthy as a starting point. The 2026-06-10 re-audit (0/11/94, supersedes the 2026-05-04 5/16/84) is the trusted layer; the baseline is retained only in git history / BUILD_STATE / register for provenance. Generalizes a caution for any module still resting on a pre-§16.6 baseline (cf. the EP and SH re-audits, which found the same DET_OK deflation).
- **Classification effect:** no single-gap flip is owned by this meta-finding (the per-gap flips are AUDIT-133/134/135/136); it records WHY the whole VHD baseline override layer was re-done rather than incrementally patched.
- **Remediation candidate (NOT done here, v2.0):** treat the 2026-06-10 canonical crosswalk as the VHD source of truth; do not re-import 2026-05-04 baseline classifications into v2.0 without §16.6 re-verification; apply the same re-verify discipline to any module whose last audit predates §16.6.
- **Cross-references:** `docs/audit/canonical/VHD.crosswalk.json` (2026-06-10 re-audit); AUDIT_METHODOLOGY.md §16 / §16.5 / §16.6 (the verification standard the baseline predates); AUDIT-133 / AUDIT-134 / AUDIT-135 / AUDIT-136 (the per-gap flips); the EP / SH re-audit DET_OK-deflation precedent; the VHD audit findings.

---

### AUDIT-138 - Post-deploy smoke "Wait for ECS stabilization" window (2 min) is shorter than a real single-task rollout-to-healthy; smoke false-fails healthy deploys (issue #375)

- **Phase:** Operational maturity / production verification (surfaced 2026-06-10, post-deploy smoke of the 2ae35c5 merge; filed 2026-06-11).
- **Severity:** MEDIUM (P2). CI-gate false-positive / alarm-fatigue class; no clinical / PHI dimension. Operator confirms per §18.
- **Status:** RESOLVED 2026-06-11 (this PR).
- **Defect:** `.github/workflows/smoke-test.yml` "Wait for ECS stabilization" allowed max 24 polls x 5s = 2 minutes, but the single-task production service's real rollout-to-healthy exceeds 2 minutes. deploy.yml's own stabilization gate allows 20 polls x 15s = 5 minutes and passes on the same rollouts. The smoke gate therefore false-fails real deploys on a pure timing race: on the 2ae35c5 deploy the new ECS task came up healthy (HTTP 200, ~357s uptime when probed) but the 2-min smoke window expired before ALB stabilization, and the AUDIT-114 alert path auto-filed issue #375; a manual re-run of the same run went fully green.
- **Impact:** recurring false-positive smoke failures on healthy deploys: noise issues auto-filed through the very alert path AUDIT-114 made loud (alarm fatigue), and under the AUDIT-107 rung-2 deploy-time smoke-hold a false-red smoke holds the NEXT deploy until a manual re-run or audited override.
- **Fix (this PR):** the smoke "Wait for ECS stabilization" window is widened to match deploy.yml's proven stabilization gate exactly: 20 polls x 15s = 5 minutes (was 24 x 5s = 2 min), with a dated comment above the step. Issue #375 closed with transient-race wording (distinct root cause from the #363/#366 synthetic verification artifacts).
- **Cross-references:** `.github/workflows/smoke-test.yml` (Wait for ECS stabilization step); `.github/workflows/deploy.yml` (the proven 5-min stabilization gate this mirrors); issue #375 (the false-positive artifact); AUDIT-114 (smoke alert path / smoke-gate class); AUDIT-107 (rung-2 smoke-hold consumes smoke conclusions, so false reds have deploy-side cost); AUDIT-112 (smoke workflow defect-class history).

---

### AUDIT-139 - SH GAP-SH-008 bicuspid-AV surveillance spec carries the wrong ICD-10 (Q23.0 = congenital aortic stenosis) instead of the bicuspid code Q23.81; spec-side counterpart to the AUDIT-121 code-side miscode

- **Phase:** 0B SH clinical-code verification / spec source-of-truth (surfaced 2026-06-08 during SH audit Batch 1 §16 ICD verification, noted inline in AUDIT-121; filed + fixed 2026-06-11).
- **Severity:** MEDIUM (P2). Spec-text / source-of-truth correctness defect; the spec is the canonical clinical definition the extractSpec pipeline reads. No runtime detection change (the engine miscode is the separate AUDIT-121); no safety / PHI dimension; T3 surveillance gap; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** RESOLVED 2026-06-11 (this PR).
- **Defect:** `docs/clinical/CLINICAL_KNOWLEDGE_BASE_v4.0.md:464` (the GAP-SH-008 row, BAV/Aortopathy subcategory) detectionLogic cell read "Bicuspid AV surveillance gap. Q23.0 BAV without annual echo". Per NLM Clinical Tables (ICD-10-CM, verified 2026-06-08 in the AUDIT-121 line): `Q23.0` = "Congenital stenosis of aortic valve" (NOT bicuspid); bicuspid aortic valve = `Q23.81`, a discrete code in the FY2025 (effective 2024-10-01) Q23.8x congenital aortic/mitral series. The spec therefore mis-named the target population's code.
- **Fix (this PR):** single same-line text correction `Q23.0 -> Q23.81` in KB:464, then the full §9.2 canonical regen (extractSpec -> extractCode -> reconcile -> refreshCites -> applyOverrides -> renderAddendum -> renderSynthesis -> validateCanonical, all `--all`). The only regenerated-artifact delta is `docs/audit/canonical/SH.spec.json:296` detectionLogic "Q23.0 BAV..." -> "Q23.81 BAV..."; specLine unchanged (464, no range shift, Gate 3 staleness clean). The SH addendum was UNCHANGED because it renders the crosswalk override note, not the spec detectionLogic - so the Q23.0 mis-code had exactly one generated propagation (SH.spec.json), not two.
- **Classification impact: NONE (pure spec-text fix).** The spec detectionLogic is descriptive text; no detection rule, crosswalk mapping, or classifier keys off it. Post-regen `SH.crosswalk.json` is byte-identical (verified `git diff` empty); SH distribution unchanged at 0 DET_OK / 30 PARTIAL / 58 SPEC_ONLY; GAP-SH-008 stays PARTIAL_DETECTION with its AUDIT-121 manual override note intact and unmodified. The spec being correct does NOT change that the engine is still wrong.
- **Relationship to AUDIT-121 (DISTINCT; AUDIT-139 does NOT resolve it):** AUDIT-121 is the code-side miscode - the `SH-BICUSPID` evaluator gates on `dxCodes.some(c => c.startsWith('Q23.1'))` (congenital aortic insufficiency) and spans 3 rule sites across SH + VHD; it is OPEN. GAP-SH-008 stays PARTIAL_DETECTION until the AUDIT-121 engine fix (Q23.1 -> Q23.81) lands. AUDIT-139 corrects only the spec source-of-truth; the two share a target gap but are separately scoped (spec text vs engine logic) and must not be merged.
- **Blast radius (tracked, NOT fixed here):** (a) `backend/src/terminology/icd10.ts:183` defines `Q23.1` with a misleading parenthetical "(bicuspid aortic)" - that conflation belongs to the AUDIT-121 code-side cleanup, not this spec PR. (b) ~15 frontend demo strings labeling "BAV (Q23.0)" across `VDClinicalGapDetectionDashboard.tsx`, `SHClinicalGapDetectionDashboard.tsx`, `PVClinicalGapDetectionDashboard.tsx` are hardcoded mock-display labels - a separate cosmetic mis-label cluster, not the canonical spec, out of scope here.
- **Cross-references:** `docs/clinical/CLINICAL_KNOWLEDGE_BASE_v4.0.md:464` (fix site); `docs/audit/canonical/SH.spec.json:296` (regenerated artifact); AUDIT-121 (the DISTINCT code-side counterpart, OPEN); `docs/audit/SH_BATCH1_AS_BAV.md` (the Batch-1 finding that first noted the spec mis-code inline); NLM ICD-10-CM Q23.81 (bicuspid) vs Q23.0 (congenital aortic stenosis) vs Q23.1 (congenital aortic insufficiency); CLAUDE.md §19.4 (KB is PARSED-CANONICAL-DOC, catch-#89 line-range parse exemplar - full regen mandatory).

---

### Service-line aggregation + provenance finding set (AUDIT-140..146) - §20 classification-pass summary (2026-06-11)

The first application of `AUDIT_METHODOLOGY.md` §20 (aggregation + provenance verification standard). PHASE 0 inventory + a six-surface classification pass produced a 6x4 verdict matrix (6 surfaces x 4 axes): **13 DEFECT / 3 LATENT / 5 CORRECT / 3 N/A**. The DEFECT/LATENT cells are grouped by ROOT CAUSE (per §20.6 one-finding-per-root-cause) into the seven findings below. Surfaces: `src/data/platformTotals.ts`, `backend/src/routes/platform.ts`, `backend/src/routes/godView.ts`, `backend/src/routes/gaps.ts` (/summary/all), `backend/src/services/vbcService.ts`, `src/components/free-tier/FreeTierDashboard.tsx` + `data.ts` + `sections/*`. All seven are **OPEN**, remediation deferred to **v2.0**, mitigated synthetic/pre-DUA except where a marker is noted (AUDIT-145 MUST-FIX-BEFORE-ANY-VBC-CLAIM + ESCALATE-AT-DUA). Severities are register-owned per §18 (operator confirms). The v2.0 author should treat AUDIT-140..146 as ONE work block. This pass is read-only classification: NO source code changed and NO surface was edited; the §20 worked-example platformTotals.ts cells are anchored in the methodology, the other surfaces' cells are filed here.

### AUDIT-140 - Cross-module patient double-count (summed-not-unioned), platform-wide: three implementations of one root cause; godView renders ">100% actively managed" to SUPER_ADMIN

- **Phase:** Service-line aggregation classification pass, Axis 2 (cross-module patient-set dedup) / §20 (surfaced 2026-06-11).
- **Severity:** MEDIUM (P2). Incorrect headline aggregation on a customer/admin-facing surface; no clinical / PHI / production-exposure dimension; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (remediation v2.0).
- **Defect (3 sites, 1 root):** a cross-module patient count is produced by SUMMING per-module counts rather than UNIONING patient-id sets. (a) `src/data/platformTotals.ts:30` - `totalPatients = hf.patients + ... + pv.patients` where each module total is itself a sum of per-gap cohort sizes (`:17`); (b) `backend/src/routes/platform.ts:53` - `totalPatients += gaps.length` per module (mixed-semantics: each module's `groupBy patientId` is deduped WITHIN module, then summed ACROSS, so each per-module figure is individually correct and the total is not); (c) `backend/src/routes/godView.ts:292` - `managed = Object.values(byModule).reduce((sum,c) => sum+c, 0)` over `buildModulePatientWhere` counts, then `coverage = managed/totalPatients` (`:296`).
- **Mechanical-test trace (§20.3):** construct a patient P with an open HF gap AND an open CAD gap. In each site P contributes to both the HF and CAD per-module counts, so P is counted twice in the cross-module total. In godView, since `totalPatients` (`:287`) is a deduped distinct count, `coverage` exceeds 1.0 whenever average module membership > 1 and renders as ">100% actively managed" at `src/ui/admin/GodView/GodView.tsx:253` - the most visible instance.
- **Reference fix pattern (in-repo):** `backend/src/services/vbcService.ts:57` uses `distinct: ['patientId']` across a cross-module gap-type set - the correct union, 30 lines from one of the defects. Remediation v2.0 = union patient-id sets (or a single cross-module `groupBy patientId`) wherever a patient count is claimed.
- **Cross-references:** platformTotals.ts:30 / platform.ts:53 / godView.ts:292 / GodView.tsx:253; vbcService.ts:57 (fix pattern); `AUDIT_METHODOLOGY.md` §20.3 + §20.7 (the platformTotals Axis 2 cell is the worked example); AUDIT-143 (the platform.ts mock/real divergence is a sibling cell on the same backend surface).

### AUDIT-141 - Unsourced financial-model multipliers rendered as executive dollar figures

- **Phase:** Service-line aggregation classification pass, Axis 3 (denominator/cohort - magic-multiplier signature) / §20 (surfaced 2026-06-11).
- **Severity:** LOW (P3). Modeling-transparency defect: the multiplier is unsourced, but it is applied to real counts and the figures carry "estimate" framing (no incorrect-real-source-data). Mitigated synthetic/pre-DUA. Classify-up considered; landed LOW because it is a sourcing/transparency gap on demo financials, not a wrong-computation. Operator confirms per §18.
- **Status:** OPEN (remediation v2.0).
- **Defect (one class, two sites):** (a) `backend/src/routes/godView.ts:236` - `const REVENUE_PER_GAP_ESTIMATE = 2500;` (only annotation: the adjective "Conservative"), multiplied into displayed dollars at `:256` (`revenueOpportunity`), `:268` (`calculateTotalRevenue`), `:343-344` (`totalOpportunity` / `capturedRevenue`); (b) `src/data/platformTotals.ts:36` - `quarterlyActionable: Math.round(totalOpportunity * 0.3)`, the `0.3` an unsourced fraction surfaced as "actionable this quarter". Neither carries a citation, config source, or dated provenance.
- **Remediation (v2.0):** source or derive every financial multiplier (per-gap-type revenue from a sourced fee schedule; the quarterly fraction from a stated basis), or mark each as an explicit assumption with provenance.
- **Cross-references:** godView.ts:236/256/268/343; platformTotals.ts:36; `AUDIT_METHODOLOGY.md` §20.4 (magic-multiplier signature) + §20.7 (the platformTotals Axis 3 cell).

### AUDIT-142 - Module-membership predicate divergence: gaps.ts flag-only vs the declared-canonical flag-OR-open-gap

- **Phase:** Service-line aggregation classification pass, Axis 3 (cohort-definition) / §20 (surfaced 2026-06-11).
- **Severity:** LOW (P3). Cohort-definition inconsistency on a currently-unconsumed endpoint; latent re-creation site for AUDIT-140. Mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (remediation v2.0).
- **Defect:** `backend/src/routes/gaps.ts:57-59` counts per-module patients by FLAG-ONLY (`where: { hospitalId, isActive: true, [field]: true }`), but `backend/src/routes/godView.ts:29-37` declares the canonical predicate FLAG-OR-OPEN-GAP (`buildModulePatientWhere`) and its comment (`:25-28`) EXPLICITLY claims it is "Mirrored in backend/src/routes/modules.ts so GodView counts and per-module dashboard counts agree." gaps.ts breaks that agreement: a patient with an open HF gap but no `heartFailurePatient` flag is in the godView/modules HF cohort and NOT the gaps.ts one, so the same labeled "patients" cohort is a different population per surface - including the visible anomaly that a `/summary/all` module row can report `openGaps > 0` with `patients = 0`.
- **Consumer status:** `GET /gaps/summary/all` is currently an ORPHAN - a repo-wide grep of `src/` finds zero consumers (only the route's own definition + the §20 methodology doc). It is a LATENT re-creation site for AUDIT-140: a future consumer summing `modules[].patients` would re-create the cross-module double-count.
- **Remediation (v2.0):** adopt the canonical flag-OR-open-gap predicate in gaps.ts (import/share `buildModulePatientWhere`), restoring the cross-surface agreement the comment already promises.
- **Cross-references:** gaps.ts:57-59; godView.ts:25-37 (canonical predicate + mirror claim); modules.ts (the mirror target); AUDIT-140 (the double-count this endpoint can re-create); `AUDIT_METHODOLOGY.md` §20.4.

### AUDIT-143 - platform.ts <-> platformTotals mock/real parity break + cohort divergence + undefined-on-real-path cast

- **Phase:** Service-line aggregation classification pass, Axis 4 (mock/real parity) / §20 (surfaced 2026-06-11).
- **Severity:** MEDIUM (P2). The flip from demo to live silently redefines the headline `totalPatients` and leaves `totalOpportunity` undefined on the real path; expressed the moment the real path is wired. Mitigated synthetic/pre-DUA (real path not yet connected from FreeTierDashboard, which imports the static const). Operator confirms per §18.
- **Status:** OPEN (remediation v2.0).
- **Defect (one surface pair, three dimensions):** under the single label `totalPatients`, (a) PARITY: real `backend/src/routes/platform.ts:39-55` computes sum-over-modules of (count of distinct patientIds with any gap in that module) while mock `src/data/platformTotals.ts` computes sum-over-gaps of per-gap cohort-size estimates - different quantities; (b) SCOPE: real is hospital-scoped (`hospitalId` filter), mock is platform-wide; (c) SOURCE/COHORT: `platform.ts:42,45` omit the `resolvedAt: null` filter, so the real path counts RESOLVED gaps and their patients, unlike the open-only siblings `gaps.ts:37` and `godView.ts:254`. Separately, `platformTotals.ts:48` casts the real payload `as typeof PLATFORM_TOTALS`, but the real `/platform/totals` response has no `totalOpportunity` / `quarterlyActionable` and its `modules` entries are `{patients, gaps}` not `{patients, opportunity, gaps}`, so any consumer reading `.totalOpportunity` off a real load gets `undefined`; the totals-less mock-signal shape (`platform.ts:21-24`) is cast the same way.
- **Remediation (v2.0):** define the one canonical quantity for `totalPatients` (recommend distinct-patient union, hospital-scoped, open-gaps-only) and make both paths compute it; fix the `as`-cast to a validated shape or align the real payload to the type.
- **Cross-references:** platform.ts:21-24/39-55/42/45; platformTotals.ts:30/36/48; `AUDIT_METHODOLOGY.md` §20.5 + §20.7 (the platformTotals Axis 4 cell, which this finding's backend side confirms and extends); AUDIT-140 (the dedup root these `totalPatients` defects share); AUDIT-141 (the `totalOpportunity` magic multiplier whose field is the one undefined on the real path).

### AUDIT-144 - Backend freshness-labeling failures: hardcoded 'real-time' + manual-trigger staleness under a YTD-current label

- **Phase:** Service-line aggregation classification pass, Axis 1 (provenance/honesty - freshness) / §20 (surfaced 2026-06-11).
- **Severity:** LOW (P3). Freshness-labeling honesty; no wrong computation. Mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (remediation v2.0).
- **Defect (one class, two sites):** (a) `backend/src/routes/godView.ts:118` emits `dataFreshness: 'real-time'` as a hardcoded string - the flip test fails (nothing can ever change it; the inline comment `// or timestamp of last data update` admits the unwired intent). It is truthful today only because the helpers happen to query per-request; it is wired to nothing. (b) `backend/src/services/vbcService.ts` recomputes quality-measure rows ONLY on a manual `POST /analytics/vbc/recalculate` (`backend/src/routes/analytics.ts:667-678`, fire-and-forget `setImmediate`), with no cron / ingestion hook, yet stamps `reportingPeriod: 'YYYY-YTD'` (`vbcService.ts:68`) - a label asserting current-YTD currency over a snapshot that may be arbitrarily stale, and the read endpoint (`analytics.ts:656-660`) exposes no computed-at marker (`periodEnd` is overwritten to calc-time on update, conflating period with freshness).
- **Remediation (v2.0):** wire `dataFreshness` to an actual compute timestamp; surface a `computedAt` on the VBC read path and/or recompute on a schedule so the YTD label is honest.
- **Cross-references:** godView.ts:118; vbcService.ts:68 + analytics.ts:656-678; `AUDIT_METHODOLOGY.md` §20.2 (latent-mislabel signature); AUDIT-145 (the deeper vbcService defect on the same service); the FreeTierDashboard provenance-blind badge files separately at AUDIT-146 (operator-decided).

### AUDIT-145 - vbcService CMS/HEDIS quality-measure service is structurally dead-on-arrival (no rows, or 100%-compliance); customer-facing VBC correctness

- **Phase:** Service-line aggregation classification pass, Axis 1 + Axis 3 / §20 (surfaced 2026-06-11). HIGHEST-SEVERITY finding of the pass.
- **Severity:** HIGH (P1). Customer-facing value-based-care correctness on the platform's quality-measure surface. **MARKERS: MUST-FIX-BEFORE-ANY-VBC-CLAIM + ESCALATE-AT-DUA** (same class as GAP-VHD-099 - the day a real health system connects and views quality measures, the surface is immediately, visibly broken in the flattering direction). Mitigated synthetic/pre-DUA only because no real tenant has connected. Operator confirms per §18.
- **Status:** OPEN (remediation v2.0).
- **Defect (numerator unsatisfiable):** the numerator query (`backend/src/services/vbcService.ts:50-58`) filters `therapyGap.gapType` against `measure.tailrdGapIds` (`:16-40`, kebab-case rule-ids e.g. `'cad-statin-high-intensity'`). But `therapyGap.gapType` is the Prisma enum `TherapyGapType` (14 UPPER_SNAKE category values, `backend/prisma/schema.prisma:1502-1517`), which is what gap detection writes (`gapDetectionRunner.ts:129`, `gapType: gap.type`). The rule-ids are (a) not members of the enum and (b) not persisted anywhere on `TherapyGap` (the model has no rule-id column, `schema.prisma:1275-1307`); the `as any` cast at `:53` suppresses exactly this type error. Behavior (DB not executable in-audit, both branches stated): Postgres rejects the invalid enum labels -> the per-measure `try/catch` (`:102-107`) swallows -> NO `qualityMeasure` rows are ever written -> the read endpoint returns empty; OR, if the filter were tolerated, it matches zero rows -> `nonCompliant = 0` -> every measure reports 100% compliance. Both wrong.
- **Compounding Axis-3 defects (same finding):** (a) DENOMINATOR MISMATCH: `totalEligible` = ALL active patients in the hospital (`:60-62`), not the measure-eligible cohort, and the numerator is the COMPLEMENT (`compliant = totalEligible - nonCompliant`, `:65`), so every never-eligible patient counts as "compliant" and the rate INFLATES toward 100% even if the query worked (worked example: 1,000 actives / 100 ASCVD / 40 with an open statin gap -> true rate 60/100 = 60%, computed (1000-40)/1000 = 96%; error grows with hospital size). (b) NO EXCLUSIONS: none of the CMS/HEDIS required denominator exclusions (hospice/palliative, statin contraindications + pregnancy, ESRD/frailty, age bounds) are implemented. (c) INVERSE-MEASURE DIRECTION: CMS122 is an inverse measure (eCQI/CMS-verified: rate counts poor control, higher = worse), but the code stores `rate = compliant/total` higher-is-better under that code.
- **Architectural root:** the gap-dedup key `patientId + gapType + module` (`gapDetectionRunner.ts:43`) collapses ~263 distinct rules into 14 `gapType` categories - no rule-level identity is persisted - so a measure-specific (rule-specific) numerator is UNEXPRESSIBLE without a schema change (a persisted rule-id column or a measure-mapping table). This is why the service cannot be fixed by a query tweak alone.
- **Measure-ID / name audit (authoritative web verification 2026-06-11; per the operator's do-not-assert-from-memory instruction):**
  - `ACO-13` labeled "Statin Therapy for CVD Prevention" - **MISASSIGNED, CONFIRMED.** CMS/eCQI: ACO-13 is "Falls: Screening for Future Fall Risk" (CARE-2 / NQF 0101 / CMS139). The MSSP statin measure is a different ID.
  - `HEDIS-AOB` labeled "Anticoagulation Monitoring for AF" - **UNVERIFIED.** An authoritative-source search (NCQA) returned no HEDIS measure with the abbreviation "AOB"; could not confirm it exists. Flagged for clinical/regulatory review; NOT asserted as fake.
  - `HEDIS-SPC` "Statin Use in Persons with Cardiovascular Disease" - **PAIRING CORRECT.** NCQA SPC = "Statin Therapy for Patients With Cardiovascular Disease" (abbreviation + concept match; minor wording difference only).
  - `HEDIS-CBP` "Controlling High Blood Pressure" - **PAIRING CORRECT.** NCQA CBP = "Controlling High Blood Pressure".
  - `CMS122` "Diabetes: HbA1c Poor Control" - **PAIRING CORRECT** (eCQI CMS122 = "Diabetes: Hemoglobin A1c (HbA1c) Poor Control (>9%)"), and confirmed INVERSE (supports defect (c) above).
- **Remediation (v2.0):** persist rule-level identity on `TherapyGap` (or a measure-mapping table) so a measure numerator is expressible; compute each measure against its measure-eligible denominator with the spec's exclusions; handle inverse measures by direction; correct the ACO-13 mapping and resolve the HEDIS-AOB identity before any VBC claim. Add tests pinning a known patient set to a known rate per measure.
- **Cross-references:** vbcService.ts:16-40/50-66/102-107; schema.prisma:1502-1517/1275-1307; gapDetectionRunner.ts:43/129; analytics.ts:656-678 (read + recalc endpoints); CMS/eCQI (ACO-13 Falls, CMS122 inverse) + NCQA (SPC / CBP / no-AOB) verified 2026-06-11; AUDIT-099 (sibling fabricated-executive-surface class); AUDIT-144 (the freshness-labeling defect on the same service); GAP-VHD-099 (the ESCALATE-AT-DUA marker precedent); `AUDIT_METHODOLOGY.md` §16 (external clinical-code verification discipline applied to the measure-ID audit) + §20.4.

### AUDIT-146 - Fabricated static-table executive surface + provenance-blind badge on the /service-line FreeTierDashboard (DISTINCT from AUDIT-099)

- **Phase:** Service-line aggregation classification pass, Axis 1 + Axis 3 + Axis 4 / §20 (surfaced 2026-06-11).
- **Severity:** MEDIUM (P2). Same fabricated-executive-KPI CLASS as AUDIT-099 (HIGH P1), but this surface carries the Demo/CMS-Estimate disclosure AUDIT-099 lacks, lowering realized risk; the LATENT mislabel-on-connect keeps it above LOW. Mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (remediation v2.0).
- **Relationship to AUDIT-099 (DISTINCT; cross-reference, NOT a re-scope):** AUDIT-099's scope is the per-module `*ExecutiveView.tsx` files - fabricated KPI cells with NO Demo-Data indicator (undisclosed fabrication). THIS finding is the standalone `/service-line` FreeTierDashboard - a different location with a DIFFERENT disclosure posture. The two are the same class, separately scoped; AUDIT-146 does not re-open or absorb AUDIT-099.
- **Defect (two parts, one surface):** (1) FABRICATED LITERALS - `src/components/free-tier/data.ts` (entire file, lines 1-938) is hardcoded constants with no derivation from any clinical data: every rate, dollar, margin, LOS, volume, and benchmark is a literal presented to a CMO as quality / financial / operational metrics (representative: `CMS_KPIS` "CV Service Revenue" `stateAValue: 142000000` `data.ts:46`; `QUALITY_BENCHMARKS` "30-Day Mortality" block `:82-89`; `MODULE_DETAIL_DATA.hf` "LVAD Implantation margin 50.2" `:610`). Of the 19 rendered surfaces, 17 are literal and 2 are mock-derived (the gap-intelligence banner + PremiumUnlock, both reading `PLATFORM_TOTALS`). (2) PROVENANCE-BLIND BADGE - `<Badge variant={hasUploadedFiles ? 'verified' : 'estimate'} />` recurs across 8 sections (KPIStrip:158, QualityBenchmark:160, FinancialBenchmarking:112, BenchmarkPositioning:21, PopulationImpact:105, CareGapFunnels:66, DRGProcedureLOS:90, OpportunityRadar:50), and `hasUploadedFiles` is hardcoded `false` (`FreeTierDashboard.tsx:44`), so the badge ALWAYS renders "CMS Estimate" and the "Verified" state is UNREACHABLE. The input is an upload flag, not data provenance (provenance-blind), and on a future connect it would badge hardcoded constants "Verified" (LATENT mislabel-on-connect).
- **Axis-4 internal inconsistency (same finding):** two different HF patient counts render on one page under the same concept - the computed banner `PLATFORM_TOTALS.modules.hf.patients` (`FreeTierDashboard.tsx:91`) vs the hardcoded `MODULE_TILES` `patients: 4250` (`data.ts:136`, rendered via CommandGrid `:109`).
- **Honesty nuance (load-bearing, MUST be stated):** this surface DOES disclose - the banner reads "Demo data - Representative 12-hospital cardiovascular program" (`FreeTierDashboard.tsx:79`) and the badge says "CMS Estimate". The defect is therefore PROVENANCE-BLIND-INDICATOR + UNREACHABLE-HONEST-STATE (+ latent mislabel-on-connect), NOT the undisclosed-fabrication of AUDIT-099. The disclosure is correct today by accident of the hardcoded constant, not by wiring.
- **Extension (2026-06-11, registry/trial classification pass - free-tier trial/registry tiles as a named sub-surface; severity UNCHANGED):** two FreeTierDashboard sections - `src/components/free-tier/sections/ClinicalTrialEnrollment.tsx` and `src/components/free-tier/sections/RegistryEligibility.tsx` (rendered by `FreeTierDashboard.tsx:169/172`) - are part of THIS surface and are WORSE-disclosed than the rest of it. They carry NO `<Badge>` and NO "Demo data" hedge at all (unlike the `data.ts` sections, which at least badge "CMS Estimate"), and `ClinicalTrialEnrollment`'s subtitle "Active trials matching your patient population" (`ClinicalTrialEnrollment.tsx:94`) AFFIRMATIVELY asserts the count reflects the user's real patients. So within AUDIT-146 these two tiles sit CLOSER to the AUDIT-099 undisclosed-fabrication signature than to AUDIT-146's disclosed-but-blind badge. Fabricated literals: `estimatedMatch` 94/67/31/52 (`ClinicalTrialEnrollment.tsx:32-59`); registry `eligible`/`submitted`/`rate` (`RegistryEligibility.tsx:15-19`), where `rate` is a STORED literal redundant with `submitted/eligible` (latent drift if either is edited without updating it; `gap = eligible - submitted` IS computed at `:33`). The "Est." prefix (`ClinicalTrialEnrollment.tsx:125`) is the only hedge and is insufficient against the "matching your patient population" assertion. This extension does NOT change AUDIT-146's MEDIUM (P2) severity; it flags the no-disclosure tiles as the WORST-disclosure sub-case of the surface, to be fixed FIRST in the v2.0 badge-wiring remediation. The "requires Premium" framing ON these same two tiles is a DISTINCT commercial-claim defect filed separately at AUDIT-148.
- **Cross-reference to the cross-module double-count:** the banner's `PLATFORM_TOTALS.totalPatients` (`FreeTierDashboard.tsx:70/97`) is the AUDIT-140 / §20.7 platformTotals cell - pointer, not re-filed here; this surface does no additional patient summing of its own.
- **Remediation (v2.0):** wire the badge to actual data provenance (real vs mock vs literal), make the honest state reachable, and either back the static tables with real aggregation or badge each unbacked cell explicitly; resolve the dual-source HF count to one source.
- **Cross-references:** FreeTierDashboard.tsx:44/70/79/91/97/109; data.ts:46/82-89/136/610 (+ whole file); the 8 badge sites above; Badge.tsx:12-25 ("estimate" -> "CMS Estimate", "verified" -> "Verified"); AUDIT-099 (the sibling fabricated-executive-KPI class, per-module ExecutiveViews; HIGH P1); AUDIT-098 (RESOLVED; `ServiceLineKPIBanner.tsx` is the honest-badge reference implementation); AUDIT-140 (the banner's totalPatients double-count); `AUDIT_METHODOLOGY.md` §20.1 scope-boundary note + §20.2 (provenance-blind / unreachable-honest-state / latent-mislabel signatures).

---

### Registry / clinical-trial / research finding set (AUDIT-147..148 + AUDIT-146 extension) - §20 provenance audit summary (2026-06-11)

Second application of `AUDIT_METHODOLOGY.md` §20, to the registry/clinical-trial/research surface. PHASE 0 established the decisive fact: there is **NO trial-matching logic and NO trial/registry schema** - no `ClinicalTrial` / `TrialMatch` / `TrialPatientMatch` / `RegistrySubmission` / `Enrollment` model anywhere in `backend/prisma/schema.prisma`, no `/api` trial/registry route, no eligibility-evaluation code. The surface is demo-scaffold (hardcoded "N patients match" counts) PLUS one honestly-built external feed (the `/research` module's live ClinicalTrials.gov listing). So §20 applied DIRECTLY as a provenance audit (no new methodology, no matching-fidelity dimension to author - there is nothing to evaluate criteria against). A three-surface classification pass (R1 `/research` view, R2 free-tier tiles, R3 advertised capability) yielded 8 DEFECT cells + 1 CORRECT (R1's honest live feed). Filed as: **AUDIT-147** (research-module fabricated trial/registry data, MEDIUM P2), **AUDIT-148** (advertised-but-absent capability, HIGH P1 proposed, MUST-ADDRESS-BEFORE-ADVERTISING-RESEARCH-CAPABILITY), and an **extension of AUDIT-146** (the free-tier trial/registry tiles as a no-Badge worse-disclosure sub-surface). Read-only classification: NO source code changed. v2.0 author should read AUDIT-146-ext / 147 / 148 with the AUDIT-140..146 service-line block as the platform's provenance-honesty work.

### AUDIT-147 - Research-module fabricated trial/registry data on the /research ResearchServiceLineView (DISTINCT surface from AUDIT-146; contains the in-repo honest-external-feed counter-example)

- **Phase:** Registry/trial provenance classification pass, §20 Axis 1 + Axis 3 + Axis 4 (surfaced 2026-06-11). §20 applied directly; no new methodology (PHASE 0 established there is no trial-matching logic and no trial/registry schema, so this is a provenance audit, not a matching-fidelity audit).
- **Severity:** MEDIUM (P2). Fabricated display-data on the `/research` module, same class as AUDIT-146 (MEDIUM); no clinical / PHI / production-exposure dimension; mitigated synthetic/pre-DUA. Operator confirms per §18.
- **Status:** OPEN (remediation v2.0).
- **Defect (fabricated trial/registry data, 3 parts):** (1) `curatedTrials` is 14 inline object literals (`src/ui/research/views/ResearchServiceLineView.tsx:281-295`), each with a hardcoded `eligiblePatients` count (127, 198, 312, 43, 94, ...) rendered beside static `inclusion`/`exclusion` criteria string-arrays that are displayed but evaluated against NO patient data (no matching logic exists; no trial/match schema - PHASE 0). (2) Tell-tale pseudo-computed literal: T01 `eligiblePatients: PLATFORM_TOTALS.modules.hf.patients > 0 ? 127 : 0` (`:282`) - gated on the mock HF total being nonzero, but the value 127 is invented (the ternary launders a literal as if derived). (3) The registry tab renders `registryCasesMap` / `caseFlags` mock case records (e.g. CP-001) as pre-populated registry data, and the "eligibility" panel is field-confidence arithmetic over hardcoded `sections[].fields[].confidence` literals (`:569-574`) - not a patient query.
- **Honest counter-example (file IN this entry; v2.0 MUST preserve):** the same surface's LIVE ClinicalTrials.gov feed (`:519-560`) is correctly built - a client-side `fetch` to the public `clinicaltrials.gov/api/v2` API with: honest live-vs-curated labeling ("Live from ClinicalTrials.gov" header `:963`, `sponsorLabel('api') = 'ClinicalTrials.gov'` `:496`, sponsor badges on curated trials), a graceful error fallback that DISCLOSES the substitution ("Live search temporarily unavailable - showing curated trials" `:909`, plus a loading state `:904`), and NO patient data sent (the URL `:524-525` is a static condition query). This is the in-repo honest-external-data reference pattern - the CONTRAST to AUDIT-143's silent mock fallback (which returns mock on error with no indicator). The remediation must KEEP this pattern, not regress it.
- **Axis-4 duplicate-NCT risk:** curated and live trials render together; the live query (recruiting AS/HF/AF/CAD trials) can return an NCT that is also in `curatedTrials` - e.g. EARLY-TAVR `NCT03042104` is curated T14 (`:295`) - so the same trial can appear twice on one surface with divergent fields (live vs hardcoded). Internal-inconsistency risk; the fabricated `eligiblePatients` have no real twin to parity-check.
- **Relationship to AUDIT-146 (DISTINCT surface, same class):** AUDIT-146 is the `/service-line` FreeTierDashboard; AUDIT-147 is the `/research` ResearchServiceLineView - same fabricated-static-display class, different module/route, separately scoped. The free-tier trial/registry tiles (a third location) are filed as the AUDIT-146 extension, not here.
- **Remediation (v2.0):** back `eligiblePatients` with real eligibility evaluation (requires the trial/match schema that does not exist - see AUDIT-148) or label the curated counts as illustrative; remove the pseudo-computed ternary; preserve the live-feed honest pattern; de-duplicate curated-vs-live by NCT.
- **Cross-references:** ResearchServiceLineView.tsx:281-295/282/496/519-560/524-525/569-574/904/909/963; App.tsx:526/749 (the `/research` route + tile); AUDIT-146 (sibling fabricated-static surface, `/service-line`); AUDIT-143 (the silent-fallback this surface's live feed correctly avoids); AUDIT-148 (the advertised-but-absent capability that makes real `eligiblePatients` matching impossible today); `AUDIT_METHODOLOGY.md` §20.2 (provenance) + §20.4 (cohort) + §20.5 (consistency).

### AUDIT-148 - Advertised-but-absent research / trial-matching capability: "requires Premium" frames a non-existent capability as paywalled (commercial-claim honesty; HEADLINE of the registry/trial audit)

- **Phase:** Registry/trial provenance classification pass, Angle B (advertised-but-absent capability) - a named extension of §20 at the commercial-claim layer (surfaced 2026-06-11).
- **Severity:** HIGH (P1) - **FINALIZED 2026-06-11 (operator); fork resolved, not open.** Reasoning in the Severity-reasoning bullet below. **MARKER: MUST-ADDRESS-BEFORE-ADVERTISING-RESEARCH-CAPABILITY** (analogous to AUDIT-145's MUST-FIX-BEFORE-ANY-VBC-CLAIM).
- **Status:** OPEN (remediation v2.0).
- **Defect (advertised-but-absent):** the platform advertises a clinical-trial-matching + registry-submission capability for which NO backend service, NO schema model, and NO matching logic exist (PHASE 0 schema check: no `ClinicalTrial` / `TrialMatch` / `TrialPatientMatch` / `RegistrySubmission` / `Enrollment` model anywhere in `backend/prisma/schema.prisma`; no `/api` trial/registry route; no eligibility-evaluation code). The advertising surfaces: `featureFlags: { clinicalTrials: true, registryAssist: true }` (`backend/src/routes/admin.ts:1182`), tier gate `'clinical-trials'` (`backend/src/middleware/tierEnforcement.ts:61`), and the "Clinical Research Assist" dashboard tile (`src/App.tsx:526`).
- **Core defect - the "requires Premium" framing:** "Patient-level matching requires Premium - upgrade to identify eligible patients by name, eligibility criteria, and enrollment status" (`src/components/free-tier/sections/ClinicalTrialEnrollment.tsx:143`) and "Patient-level matching and submission workflow: Premium" (`src/components/free-tier/sections/RegistryEligibility.tsx:90`) frame an ABSENT capability as PAYWALLED - implying it exists at a higher tier, when no matching or submission engine exists at ANY tier. This is NOT an honest not-built disclosure; an honest not-built disclosure would say "coming soon" / "in development", not "upgrade to unlock". A prospect shown this in a sales demo, or a customer who pays for Premium expecting patient-level trial matching, receives a capability that does not exist.
- **Partial mitigation:** the `/research` tile carries a "Beta" badge (`src/App.tsx:537`), a hedge on the research module - but it does NOT hedge the free-tier "requires Premium" framing on the two tiles above, which is where the paywall implication is asserted.
- **Severity reasoning (HIGH P1 proposed; MEDIUM counter-argument stated):** HIGH because (a) it is an active MISREPRESENTATION ("requires Premium" asserts existence-behind-paywall, not a passive omission) on the program's stated highest-value research feature; (b) the harm is realized at first commercial contact - a demo or a paid upgrade - mirroring the AUDIT-145 "immediately visibly broken on view" logic transposed to the commercial-claim layer; (c) the advertised audience is health-system executives evaluating purchase (BSW, HCA, CommonSpirit, Mount Sinai). MEDIUM counter-argument (considered and REJECTED, recorded to document the reasoning): it is UI-framing only with NO clinical / PHI / data-integrity / safety dimension, the "Beta" badge partially hedges, and it is mitigated pre-DUA/synthetic (no real customer has yet paid for the absent capability). **Severity FINALIZED HIGH (P1) 2026-06-11 (operator): active commercial misrepresentation at point-of-sale ("requires Premium" frames an absent capability as paywalled) outweighs the UI-only / Beta-hedged / pre-DUA mitigation; the MUST-ADDRESS-BEFORE-ADVERTISING-RESEARCH-CAPABILITY marker is the operative control.** The fork is resolved, not open.
- **Relationship to siblings (DISTINCT defect):** AUDIT-145 is advertised-but-dead at the DATA-CORRECTNESS layer (a VBC service that runs but produces wrong/empty results); AUDIT-148 is advertised-but-absent at the CAPABILITY-CLAIM layer (a feature that does not exist at all, framed as paywalled). AUDIT-146/147 are the fabricated-DATA findings (the numbers shown are invented); AUDIT-148 is the fabricated-CAPABILITY finding (the feature itself is claimed but absent). Same honesty family, distinct layer.
- **Remediation (v2.0):** either BUILD the capability (requires the trial/match/registry-submission schema + a matching engine + ClinicalTrials.gov-backed eligibility - a large feature) OR change the framing to an honest not-built disclosure ("in development" / remove the "requires Premium" upgrade CTA + the feature flags + the tier gate until the capability exists). Do NOT advertise patient-level matching or registry submission until one of these lands.
- **Cross-references:** admin.ts:1182 (feature flags); tierEnforcement.ts:61 (tier gate); App.tsx:526/537/749 (tile + Beta badge + route); ClinicalTrialEnrollment.tsx:143 + RegistryEligibility.tsx:90 (the "requires Premium" framing); `backend/prisma/schema.prisma` (the absent trial/registry models); AUDIT-145 (advertised-but-dead sibling, data layer; HIGH P1 + marker); AUDIT-146 + AUDIT-147 (the fabricated-data findings on the same surfaces); `AUDIT_METHODOLOGY.md` §20 (the provenance standard this Angle-B finding extends to the commercial-claim layer).

---

### UI_CANON findings stream - BATCH 1 (functional-defect tier), 2026-06-11

Opens the Phase 0C-3 findings stream against `docs/design/UI_CANON.md` (authored 0C-2b, PR #382 / `16bf87c`). This batch is the FUNCTIONAL-defect tier (controls that do not work or mis-present data); the consistency tier (component sprawl, label/structure drift) and the aesthetic tier (elevation/radius/glass/motion) follow in BATCH 2-3. Each finding is §1 re-verified against running code before filing (the 0C-2a resolution-pass facts are predicates to confirm, not trust), cites the UI_CANON section it violates, is OPEN with remediation v2.0, and is mitigated pre-DUA/synthetic. **Re-verification corrections this batch:** (a) AUDIT-149 drops the EPRiskStratification risk-score-checkbox sub-claim as a FALSE POSITIVE - the `onChange={() => {}}` is a benign React controlled-input pattern and the wrapping `<label>` carries the real mutation handler, so the risk factor IS captured (the 0C-2a "records nothing" / clinical-presentation claim does not hold). (b) The planned AUDIT-151 (stub navigation tabs) is **WITHDRAWN - not filed** - §1 re-verification of `renderTabContent()` found ALL five candidate "stub" tabs have a real render case: CAD `on-off-pump` (`CoronaryServiceLineView.tsx:327`), CAD `analytics` (`:416`), CAD `outcomes` (`:523`), VD `network` (`ValvularServiceLineView.tsx:82`), PV `network` (`PeripheralServiceLineView.tsx:298`). The 0C-2a "no case handler / falls to default" claim was wrong (the switch was not fully read). No stub-tab defect exists; id 151 is UNCONSUMED and remains the next free id for BATCH 2.

### AUDIT-149 - Dead interactive controls: action buttons that look actionable but have no handler (EP care-team + Coronary worklist + a no-op export)

- **Phase:** Phase 0C-3 UI-canon findings, functional-defect tier (surfaced 0C-2a resolution pass, §1 re-verified + filed 2026-06-11).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator).** Finalization reasoning: the HIGH basis (the EPRiskStratification checkbox clinical-input "records nothing" claim) was DISPROVEN by §1 file-time re-verification (see Defect / EXCLUDED); the remaining ~15 dead buttons are workflow-integrity UX defects with no clinical / PHI / data-integrity dimension - actionable-looking controls that silently no-op, customer/clinician-facing but mitigated pre-DUA/synthetic (no real care team uses the surface yet).
- **Status:** OPEN (remediation v2.0).
- **Defect (dead controls, §1 re-verified):** each cited `<button>` is fully styled (CTA gradients, hover states, icons) but has NO `onClick` handler in its opening tag, or is wired to an empty-body function - so a click does nothing. Confirmed sites:
  - EPAlertDashboard "Emergency EP Consult" / "Schedule EP" / "Generate Orders" (`src/ui/electrophysiology/components/care-team/EPAlertDashboard.tsx:336/340/344`) - 3 buttons, no onClick.
  - EPFollowUpQueue `:179/183` - 2 buttons, no onClick. **Label correction:** the buttons read "Contact Now" / "Reschedule", NOT the 0C-2a-cited "Complete" / "Defer"; the dead-control defect stands, the labels were mis-cited.
  - EPReferralTracker `:219/224`; EPReferralTrackerEnhanced `:756/761/766/976`; EPTeamCollaborationPanel `:743/746/777/780`; EPCareGapAnalyzer `:830/833`; EPActionQueue `:169`; EPPatientTimeline `:269` - grep-verified: no `onClick` in the 3 lines preceding each cited `<button className=...>`.
  - EP "Open Chart" (`EPPriorityWorklist.tsx:230`, `EPPatientWorklist.tsx:233`) - no onClick, while the SIBLING "Review Patient" button IS wired (`onClick={() => setSelectedPatient(...)}`), confirming the dead one is an oversight not a design.
  - CoronaryWorklist bulk handlers wired to empty bodies: `const bulkScheduleCathLab = () => { {} }` / `const bulkOrderCardiacRehab = () => { {} }` (`:278-284`), bound to the "Schedule Cath Lab" / "Order Cardiac Rehab" buttons (`:553/559`) that act on the selected-patient set; plus the row Edit icon (`:673`, no onClick).
  - PeripheralExecutiveView `onExport={() => {}}` (`:378`) - the export affordance on a detail modal is a no-op.
- **EXCLUDED (re-verification false-positive, recorded so it is not re-flagged):** the EPRiskStratification CHA2DS2-VASc and HAS-BLED checkboxes (`src/ui/electrophysiology/components/executive/EPRiskStratification.tsx:277/315`) have `onChange={() => {}}`, but the wrapping `<label>` carries the REAL mutation handler (`:272` `onClick={() => updateCHA2DS2VASc(...)}`; `:310` `onClick={() => updateHASBLED(...)}`). The `onChange={() => {}}` is the standard controlled-checkbox pattern (React requires an onChange to avoid the read-only warning; the state mutation lives on the label click). The risk factor IS captured; the 0C-2a "look interactive but record nothing" claim does NOT hold and is NOT filed.
- **UI_CANON violation:** §7.2 functional-integrity rule - "every interactive control MUST have a real handler or NOT appear interactive ... a no-op handler on an actionable-looking control is drift." Each confirmed button is the mechanical case the rule names.
- **Remediation (v2.0):** wire each control to its real handler, OR render it as static (non-button) markup so it does not advertise an action; for the bulk handlers, implement the action or remove the buttons. Concentration: EP care-team components carry the bulk of the dead controls.
- **Cross-references:** the file:line list above; `UI_CANON.md` §7.2 (functional-integrity); 0C-2a Stream-2 dead-interaction inventory (the predicate, re-verified here).

### AUDIT-150 - Broken heatmap color-band map: the adherence fill-color collapses three legend bands into one

- **Phase:** Phase 0C-3 UI-canon findings, functional-defect tier (surfaced 0C-2a Stream-3, §1 re-verified + filed 2026-06-11).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator).** Finalization reasoning: the numeric score and the bar WIDTH carry the true value (re-verified `:50`/`:59`); the defect is a legend-contradicting REDUNDANT encoding, not value-hiding. The MEDIUM-HIGH lean (clinical-data mis-presentation) was lowered on that basis; no live clinical decision rides on it pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Defect (§1 re-verified verbatim, both files byte-identical):** `getHeatColor(score)` in `src/ui/heartFailure/components/GDMTHeatmap.tsx:27-33` and `src/ui/structuralHeart/components/SHValveTherapyHeatmap.tsx:27-33` is: `if (score >= 80) return "bg-chrome-50"; if (score >= 75) return "bg-titanium-300"; if (score >= 70) return "bg-chrome-50"; if (score >= 65) return "bg-chrome-50"; return "bg-red-500";`. So bands `>=80`, `>=70`, and `>=65` ALL return `bg-chrome-50` (only `75-79` gets the distinct `bg-titanium-300`). The on-screen "Performance Key" legend (`GDMTHeatmap.tsx:67-72`) claims distinct bands (`>=80%` etc.), so the rendered progress-bar fill color contradicts the legend and is identical for a 65%-adherent and an 80%-adherent site on a GDMT / valve-therapy adherence heatmap.
- **Mitigation (sharpens the 0C-2a "65% renders identical to 80%" framing):** the numeric "Overall Score X%" is shown prominently above the bar (`GDMTHeatmap.tsx:50`, text-2xl) and the bar WIDTH is `style={{ width: ${score}% }}` proportional to score (`:59`), so a 65% and an 80% site differ in their displayed number AND bar length - they match ONLY in fill color. The defect is a redundant-encoding / legend-inconsistency defect, not a value-hiding one.
- **UI_CANON violation:** §6 - "a heatmap's color bands are DISTINCT per band; a color function that returns one color for multiple bands is non-conformant." This is the exact mechanical case §6 names.
- **Remediation (v2.0):** assign a distinct color per legend band (the band thresholds already exist; only the return values collapse); fix both files (identical defect). Route the heatmap through the §6 `ChartTheme` so the color encoding is centralized.
- **Cross-references:** GDMTHeatmap.tsx:27-33/50/59/67-72; SHValveTherapyHeatmap.tsx:27-33; `UI_CANON.md` §6 (distinct-color-bands); 0C-2a Stream-3 chart-correctness pass (the predicate, re-verified here).

### UI_CANON findings stream - BATCH 2 (structural + component tier), 2026-06-12

Second batch of the Phase 0C-3 stream against `docs/design/UI_CANON.md`. Files the structural/component (consistency) tier: AUDIT-151..155, grouped by canon section per the root-cause-grouping precedent (AUDIT-140..146). Every per-file claim was §1 re-verified at filing and every headline grep RE-RUN so the register carries current-at-filing numbers (the Batch-1 standing rule, after 3 of 3 examined 0C-2a claims proved false). **Re-verification refinements this batch (import-vs-render class):** (a) `BaseExecutiveView` is imported by all 6 ExecutiveViews but RENDERED only by CAD/VD/PV - HF/EP/SH carry a dead import (the 0C-1b "used by CAD/VD/PV only" claim holds at the render level); (b) `BaseCareTeamView` is consumed by all 6 careTeamConfigs but ONLY as a type import (`CareTeamViewConfig`) - zero module renders (the "used by none" claim holds at the component level); (c) `BaseTable` re-verification STRENGTHENED the claim: zero importers anywhere (0C-1 said "rarely used"). The aesthetic tier (elevation/radius/glass/motion vs §5) follows in BATCH 3. All five OPEN, remediation v2.0, mitigated pre-DUA/synthetic; severities proposed, operator finalizes per §18.

### AUDIT-151 - View-body clone architecture: 18 bespoke per-module view files vs the canonical one-parameterized-view-per-tab (UI_CANON §1.4)

- **Phase:** Phase 0C-3 UI-canon findings, structural/component tier (filed 2026-06-12).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator).** Architecture debt: no user-facing harm, but the divergence cost is already realized (pairwise clone drift, dead imports) and every cross-module UI fix costs 6x. Mitigated pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Defect (§1 re-verified):** the 6 clinical modules carry 18 bespoke view bodies under `src/ui/<module>/views/` (22 platform-wide incl. research 3 + revenueCycle 1) where UI_CANON §1.4 mandates ONE parameterized shared view per audience tab. Clone evidence: EP `EPExecutiveView.tsx` and SH `StructuralExecutiveView.tsx` are structural clones - identical 894-line length, identical section order at identical line ranges (0C-1b: both render the same 13 sections at `:479-778`); VD=PV likewise. Drift evidence: `BaseExecutiveView` imported by all 6 but rendered only by CAD/VD/PV (`<BaseExecutiveView` grep: CAD/VD/PV = 1 each, HF/EP/SH = 0 with a live import statement - dead imports); `BaseCareTeamView` consumed type-only by the 6 configs, zero renders.
- **Remediation (v2.0):** the §1.4 consolidation mandate - collapse to 3 shared parameterized views (`ExecutiveView` / `ServiceLineView` / `CareTeamView`) driven entirely by per-module config; clone files DELETED. **The largest single v2.0 UI work item.**
- **Cross-references:** `UI_CANON.md` §1.4; the 18 file paths under `src/ui/*/views/`; `src/components/shared/BaseExecutiveView` + `BaseCareTeamView` (the partial scaffolds); AUDIT-152 (the component-level sibling); 0C-1/0C-1b structural inventory (the predicate).

### AUDIT-152 - Component-system bypass: shared primitives exist but ~98-100% of instances are hand-rolled (UI_CANON §3 + §3.5)

- **Phase:** Phase 0C-3 UI-canon findings, structural/component tier (filed 2026-06-12).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator).** The consistency-tier ROOT CAUSE - every downstream polish/consistency defect traces to primitives being bypassed; no direct user harm; mitigated pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Defect (counts re-run at filing, 2026-06-12):** buttons - 845 `<button>` instances, only 19 use a `btn-*` class (~826 hand-rolled; no shared React Button exists); pills - 914 `rounded-full` instances, only 9 `<Badge>` uses (~905 hand-rolled; `Badge.tsx` covers only provenance variants); KPI cards - 7 `*ExecutiveKPICard.tsx` near-identical copies (CAD/EP/HF/PAD/Valvular + the SH DOUBLE: `SHExecutiveKPICard.tsx` AND `StructuralExecutiveKPICard.tsx`, file list re-verified) vs 1 shared `KPICard.tsx`; tables - `BaseTable.tsx` has **ZERO importers** (re-verified: nothing imports or renders it) vs 65 raw `<table>` instances across 46 files.
- **Remediation (v2.0):** §3 canonical Button/Badge/KPICard/Chart + the §3.5 FULL-migration mandate: all hand-rolled instances migrate AND the legacy implementations are deleted so they cannot be re-copied. Partial migration explicitly rejected (a third source of truth is worse than two).
- **Cross-references:** `UI_CANON.md` §3/§3.5; `src/design-system/Badge.tsx`; `src/components/shared/KPICard.tsx`; `src/components/visualizations/BaseTable.tsx` (zero importers); the 7 `*ExecutiveKPICard.tsx` paths; `index.css:619-684` (`btn-*` classes); AUDIT-151 (view-level sibling).
- **Note (SH convergence, 2026-07-10):** the AUDIT-304 SH Exec convergence (branch `feat/sh-exec-convergence`) built a fresh `SHExecutiveSummary` (mirroring the HF/EP exemplar) rather than reusing the SH KPI-card copies - re-confirming that both `src/ui/structuralHeart/components/executive/SHExecutiveKPICard.tsx` AND `.../StructuralExecutiveKPICard.tsx` are **unimported dead code** (grep: zero renders on `StructuralExecutiveView.tsx`). Flagged for deletion in the AUDIT-152 v2.0 migration; NOT deleted in the SH convergence batch (out of scope - would be a separate cleanup PR).

### AUDIT-153 - Red semantic overload: carmine #9B2438 carries 8 meanings, 3 on the exact hex (UI_CANON §4.2 clinical-safety-legibility)

- **Phase:** Phase 0C-3 UI-canon findings, structural/component tier (filed 2026-06-12).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator, with the revisit-at-DUA note as filed).** Clinical-legibility dimension (alarm-color dilution) present but mitigated pre-DUA/synthetic. NOTE: the dimension STRENGTHENS at DUA - when real clinical-critical alarms render alongside red CTAs and red active-tabs, the alarm loses pre-attentive distinctness; revisit severity at DUA.
- **Status:** OPEN (remediation v2.0).
- **Defect (§1 re-verified 2026-06-12):** the same `#9B2438` paints three semantically unrelated states verbatim - the active-nav indicator gradient (`src/index.css:281`), the danger-button gradient (`:671`), and the critical status glow (`:706`) - and the arterial-red family carries 8 distinct meanings total (per the UI_CANON §4.2 table: clinical-critical/safety-alert, primary CTA, negative-number/decline, active-nav/selection, Structural Heart module identity, allergy/contraindication flag, error-state border, critical patient-row accent). The alarm color must mean alarm.
- **Remediation (v2.0):** the §4.2 token split - carmine/arterial RESERVED for clinical-critical/safety only; the 7 other meanings reassigned to distinct semantic tokens (`--sem-cta`, `--sem-data-negative`, `--sem-selection`, `--mod-structural`, `--sem-warning`, `--sem-error`).
- **Cross-references:** `index.css:281/671/706` (re-verified); `UI_CANON.md` §4.2 (the token table); 0C-1b F3 red-overload measurement (the predicate); AUDIT-150 (chart color-encoding sibling, §6).

### AUDIT-154 - Off-token neutrals: 185 standard-Tailwind-gray hexes + 66 hover:bg-gray bypass the titanium scale (UI_CANON §4.3)

- **Phase:** Phase 0C-3 UI-canon findings, structural/component tier (filed 2026-06-12).
- **Severity:** **FINALIZED LOW (P3) 2026-06-12 (operator).** Pure token hygiene; no user-visible meaning change; mitigated pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Defect (counts re-run at filing, 2026-06-12):** 185 off-token gray hex uses in `src/` - `#6B7280`=105, `#64748b`=28, `#374151`=15, `#1e293b`=13, `#E5E7EB`=11, `#111827`=8, `#9CA3AF`=4, `#F9FAFB`=1 - plus 66 `hover:bg-gray` utilities, all bypassing the titanium neutral scale.
- **Remediation (v2.0):** migrate per the §4.3 mapping (`#6B7280`->titanium-500, `#E5E7EB`->titanium-200, `#374151`->titanium-700, `#64748b`->titanium-500, `#1e293b`->titanium-800, `#111827`->titanium-900); `hover:bg-gray-*` -> `hover:bg-titanium-*`.
- **Cross-references:** `UI_CANON.md` §4.3; 0C-1b F2 hardcoded-hex measurement (the predicate, re-run here).

### AUDIT-155 - Cross-module structural + label drift bundle: the pre-canon deviations now enumerable against UI_CANON §1-2

- **Phase:** Phase 0C-3 UI-canon findings, structural/component tier (filed 2026-06-12).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator, adopting the classify-up reasoning over the original LOW-MEDIUM lean).** Finalization: classify-up default applied - the bundle spans all 6 modules + both nav tiers + user-facing labels; the downgrade-with-evidence path stays open per the standard rule; remediation largely subsumed by the AUDIT-151 consolidation. The LOW argument (each site individually cosmetic) is recorded; per the convention (between two tiers take the higher; downgrade later with evidence), MEDIUM. Mitigated pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Defect (one root cause - no canonical skeleton existed until UI_CANON - all sites §1 re-verified 2026-06-12):**
  - Service Line GROUP HEADERS differ per module: HF "Clinical Analytics" (`ServiceLineView.tsx:50`) vs SH "Procedure Pathways" (`StructuralServiceLineView.tsx:37`); plus CAD "Risk Assessment" / VD "Quality & Analytics" / EP "Clinical Tools" / HF "Device & Advanced Therapy" (0C-1b).
  - GAP-DETECTION labels/ids inconsistent across all 6: "Gap Detection (25-Gap)" HF `serviceLineConfig.tsx:389`, "(20-Gap)" EP `:693`, "(8-Gap)" SH `:690`, "(71-Gap)" CAD `:731`, "(12-Gap)" PV `:684`, "Clinical Gap Detection" (no count) VD `:337` - violates §2.1.
  - SH STS DUPLICATE: both `calculators` (`serviceLineConfig.tsx:623`, contains STS) and standalone `sts-calculator` (`:629`) - violates §2.2.
  - VD workflow relabeled "Valve Optimization" (`careTeamConfig.tsx:822` region; id `workflow`) - violates §1.3.
  - PARTIAL-PRESENCE sections: `hospital-alerts` in 3 of 6 (HF/EP/SH); KPI Summary Cards absent EP/SH; Revenue-by-Facility + Revenue-Opportunities-Pipeline in HF/EP/SH only; CAD-only Financial ROI Waterfall - violate the §1.1/§1.3 closed-allowance rule.
  - NETWORK-TRIPLE label overlap (§2.3): SH + CAD each render the module-network / CrossReferralEngine / CareTeamNetworkGraph triple with overlapping labels.
  - WORKLIST sprawl: 6 distinct per-module worklist files, EP alone carrying 4 variants (`EPWorklist` + `EPPatientWorklist` + `EPPatientWorklistEnhanced` + `EPPriorityWorklist`, re-verified on disk).
- **Remediation (v2.0):** conform to UI_CANON §1.1-1.3 (canonical skeletons, closed allowances, promote-or-remove dispositions) + §2.1-2.3 (label standardization, STS de-dup, network-triple distinct labels). Largely subsumed by the AUDIT-151 consolidation (a single parameterized view cannot drift per-module); the config-level label fixes are independent quick passes.
- **Cross-references:** the file:line list above; `UI_CANON.md` §1.1-1.3/§2.1-2.3; AUDIT-151 (the architecture that makes this drift structural); 0C-1b/0C-2a cross-module tables (the predicates).
### AUDIT-156 - New HIGH esbuild advisories (via the tsx devDependency); cleared by a tsx bump in this PR

- **Phase:** Security / dependency hygiene (surfaced 2026-06-12 by the Security Audit gate on PR #384; remediated this PR).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator): tsx is a devDependency absent from the production image - the 2 HIGH CVEs have no production/PHI exposure; realized surface is dev+CI toolchain only. RESOLVED this PR.** Supporting detail: the 2 underlying CVEs are HIGH-rated upstream (one RCE-class), which was the HIGH lean; the dev-vs-runtime determination DROVE the finalization - **tsx is a `devDependency`** (`backend/package.json` devDependencies block), NOT a production/runtime dependency - the production image runs compiled `node dist/server.js` and contains neither tsx nor esbuild. The Windows dev-server file-read CVE (GHSA-g7r4-m6w7-qqqr) additionally requires running esbuild's `serve` dev-server, which this project never invokes (tsx uses esbuild as a transform, not its serve mode).
- **Status:** **RESOLVED 2026-06-12 (this PR).**
- **Defect (as surfaced):** `npm audit --audit-level=high` reported esbuild 0.17.0-0.28.0 with 2 HIGH advisories: GHSA-gv7w-rqvm-qjhr (missing binary-integrity verification in the Deno module -> RCE via NPM_CONFIG_REGISTRY) and GHSA-g7r4-m6w7-qqqr (arbitrary file read when running the esbuild dev server on Windows). The vulnerable esbuild (0.27.4) was pulled transitively by `tsx` (`backend/package.json` devDependency `^4.1.0`, lockfile-pinned to tsx 4.21.0 -> esbuild 0.27.4). The gate fired on PR #384, a docs-only register/BUILD_STATE PR - the Security Audit job audits the backend dependency tree on every PR regardless of what the PR touches, so the failure was environmental (a newly-published upstream advisory), NOT introduced by #384 (the gate working as designed).
- **Fix (this PR; backend package.json + lockfile ONLY):** `tsx ^4.1.0 -> ^4.22.4`. tsx 4.22.4 (latest; published 2026-05-31; MIT; actively maintained) pins `esbuild ~0.28.0`, which resolves to **esbuild 0.28.1** - the patched line. NOTE the version arithmetic: the advisory range is 0.17.0-0.28.0 INCLUSIVE, so 0.28.1 is the first fixed release; **0.29.0 does not exist** (latest esbuild is 0.28.1), so the operator's ">=0.29.0" target was an over-estimate - 0.28.1 clears it. No `npm audit fix --force` (the node-cron@4.2.1 breaking change is out of scope; the moderate chain stays under AUDIT-007). All-within-tsx-4.x = no major bump, semver-compatible, no breaking changes between the prior 4.21.0 and the new 4.22.4.
- **Verification (all green, this PR):** `npm install` clean; resolved tsx 4.22.4 / esbuild 0.28.1; `npm audit --audit-level=high` exits 0 (the 2 HIGH cleared); 6 moderate vulnerabilities remain (brace-expansion + qs-via-body-parser/express + uuid-via-node-cron) - they do NOT trip the high gate and stay tracked under AUDIT-007; full backend jest suite **62 suites / 1334 passed + 1 skipped (1335 total)** = the known baseline held (tsx is exercised by the jest/tsx toolchain, so the green suite is the functional confirmation that the bump did not regress the dev/test path).
- **Residual:** the 2 HIGH advisories are cleared; the residual 6 moderate are AUDIT-007 scope (uuid/node-cron requires the breaking node-cron@4.2.1 upgrade; brace-expansion/qs are transitive moderates without a non-force fix). No further AUDIT-156 action.
- **Cross-references:** `backend/package.json` (tsx bump) + `backend/package-lock.json`; GHSA-gv7w-rqvm-qjhr + GHSA-g7r4-m6w7-qqqr (the 2 HIGH esbuild advisories); AUDIT-007 (the sibling moderate uuid/node-cron chain, still OPEN); PR #384 (the docs PR whose gate surfaced this - it merges through the green gate after this lands); CLAUDE.md §15 RULE 1-3 (production runs compiled dist, not tsx - the devDependency-scope basis).

### UI_CANON findings stream - BATCH 3 (aesthetic + polish tier) + STREAM CLOSE, 2026-06-12

Final batch of the Phase 0C-3 stream against `docs/design/UI_CANON.md`. Files the aesthetic/polish tier: AUDIT-157..162 against UI_CANON §5-§7, grouped by canon section. Every per-file claim §1 re-verified at filing and every headline grep RE-RUN (current-at-filing numbers per the Batch-1 standing rule). Zero exclusions/false-positives this batch (the aesthetic claims are grep-measured; the render-order + YAxis-domain + launcher-omission spot-checks confirmed). **PHASE 0C-3 (UI_CANON findings stream) is now COMPLETE:** Batch 1 functional-defect (AUDIT-149/150), Batch 2 structural/component (AUDIT-151..155), Batch 3 aesthetic/polish (AUDIT-157..162), plus AUDIT-156 (security, RESOLVED) surfaced + cleared en route by the Security Audit gate. The remaining 0C phase is **Phase 0C-4** (rendered references + the iconography/logo workstream per UI_CANON §8), to be executed as rendered options against the §4 ratified palette + §5 aesthetic system. All Batch-3 findings OPEN, remediation v2.0, mitigated pre-DUA/synthetic; severities proposed, operator finalizes per §18.

### AUDIT-157 - Aesthetic-system scatter: elevation + radius + glass vs the canonical few (UI_CANON §5.1-5.3)

- **Phase:** Phase 0C-3 UI-canon findings, aesthetic/polish tier (filed 2026-06-12).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator).** The web-3.0 / premium-depth gap core - spans the entire app's card surfaces; no functional harm; mitigated pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Defect (counts re-run at filing, three dimensions, one root cause):** (1) ELEVATION - 12 distinct `shadow-*` utilities in actual use (537 usages; the 5 Tailwind defaults sm/md/lg/xl/2xl coexist with the custom chrome-card / chrome-card-hover / chrome-elevated / glass / metal-2 / metal-3 / white set) + 11 inline `boxShadow`, vs the canonical 4 levels (§5.1: flat/raised/floating/modal). (2) RADIUS - 10 distinct `rounded-*` with 6 in heavy use (rounded-lg 2451, xl 1009, full 914, 2xl 327, md 48, sm) all coexisting as card radii, vs the canonical 3 (§5.2: control/card/modal + full for circles). (3) GLASS - 607 `glass-panel`/`metal-card` vs 1578 plain `bg-white` = ~28% glass adoption, vs the §5.3 canonical 100%-of-cards. Root cause: no enforced aesthetic system existed until UI_CANON.
- **Remediation (v2.0):** the §5.1-5.3 consolidation - collapse shadows to the 4 tokens (remove raw sm/md/lg/xl/2xl + inline boxShadow), radii to the 3 tokens, and roll glass to 100% of card surfaces (`glass-panel` replaces plain `bg-white`).
- **Cross-references:** `UI_CANON.md` §5.1-5.3; `index.css` (the glass + shadow token definitions); 0C-1b G1/G5/F4 measurements (the predicates, re-run here); AUDIT-158 (the motion dimension of the same §5 aesthetic gap, filed separately).

### AUDIT-158 - Motion-orchestration absence: no route/entrance motion, scattered durations (UI_CANON §5.4)

- **Phase:** Phase 0C-3 UI-canon findings, aesthetic/polish tier (filed 2026-06-12).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator).** Motion is the largest single perceived-premium lever; no functional harm; mitigated pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Defect (re-verified):** **0** `framer-motion` / `AnimatePresence` imports across `src/` - there is no route-transition and no card-entrance orchestration (routes change and cards paint instantly). 7 distinct transition `duration-*` values (150/200/300/500/700/1000 + base) + 2 ad-hoc easing curves, vs the §5.4 canonical 3 durations (fast/base/slow) + 1 easing pair. `CountUp` number-animation is wired at only 4 sites (not applied platform-wide to KPI values per §5.4).
- **Distinct from AUDIT-157 (different remediation class):** §157 consolidates EXISTING values (shadows/radii/glass already in the codebase); §158 ADDS a capability - a new declarative-motion dependency (framer-motion or equivalent) + a new motion layer. The dependency addition goes through the standard library review per the engineering bar (license, maintenance, breaking-change check, bundle-size) before adoption.
- **Remediation (v2.0):** the §5.4 motion system - framer-motion (or equivalent) for route + card-entrance orchestration, count-up formalized onto every KPI value, durations/easing collapsed to the canonical set.
- **Cross-references:** `UI_CANON.md` §5.4; 0C-1b G2 motion measurement (the predicate, re-run here); AUDIT-157 (the static-aesthetic sibling under §5).

### AUDIT-159 - Chart-correctness set: mislabeled chart types + auto-scale distortion + no ChartTheme (UI_CANON §6)

- **Phase:** Phase 0C-3 UI-canon findings, aesthetic/polish tier (filed 2026-06-12).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator).** Mislabeled chart types and the auto-scale distortion are mild data-mis-presentation (a "waterfall"/"funnel" that does not show the cumulative structure it implies; a bar chart whose non-zero baseline distorts magnitude); mitigated pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Defect (re-verified per instance; NO shared `ChartTheme` exists - grep returns zero, so every chart re-specifies axis/tooltip/grid inline with mixed brand-threshold vs hardcoded fills + inconsistent gridlines):**
  - `CareGapFunnels` - hardcoded CSS-width divs `WIDTHS = ['w-full','w-[85%]','w-[70%]','w-[55%]']` (`CareGapFunnels.tsx:13-15`): the widths are hardcoded, not data-driven, and it is not a cumulatively-tapering funnel.
  - `SharedROIWaterfall` (`:25`) + `FinancialROIWaterfall` (`:112-127`) - labeled "waterfall" but rendered as left-aligned CSS-width bars with no cumulative running-total step.
  - `DeviceFunnel` (`:15`) - 3 independent stacked bars, no cumulative taper.
  - `RevenuePipelineCard` - YAxis has NO `domain={[0, ...]}` (`ForwardLookingCards.tsx:60-72`), so recharts auto-scales and a non-zero baseline distorts the bar-magnitude comparison.
  - Research Auto-Fill target range - overlapping dashed `ReferenceLine` pairs (`ResearchExecutiveView.tsx:208-213`, `targetLow`/`targetHigh` per registry), no legend.
  - `SharedProjectedVsRealized` - absolutely-overlaid projected/realized bars (`:66-87`), the darker obscures the lighter rather than grouping.
  - HF `GeographicHeatMap` - a CSS grid of gradient cards labeled "heatmap", NOT a map, beside the real Leaflet `ZipHeatMap`.
- **Cross-reference (NOT re-filed):** AUDIT-150 (the GDMT/SHValveTherapy heatmap color-band collapse) is a filed §6 sibling instance - cited here, not duplicated.
- **Remediation (v2.0):** §6 - one `ChartTheme` every chart consumes (axis labels+units, one gridline/tooltip, zero-baseline where magnitude is implied, correct-chart-type: real cumulative funnels/waterfalls, distinct heatmap bands).
- **Cross-references:** the file:line list above; `UI_CANON.md` §6; AUDIT-150 (sibling §6 color-band instance); 0C-2a Stream-3 chart-correctness pass (the predicate, re-verified here).

### AUDIT-160 - Interaction-state coverage gaps: no shared spinner/empty/hover standard (UI_CANON §7 state)

- **Phase:** Phase 0C-3 UI-canon findings, aesthetic/polish tier (filed 2026-06-12).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator): classify-up default applied.** LOW counter-argument recorded as considered: consistency-tier, no functional break - the 48 loading branches mostly cover async surfaces. MEDIUM because the gap spans the whole app's interaction states (spinner, empty, hover) with no shared standard; **downgrade-with-evidence path open.** Mitigated pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Defect (counts re-run at filing):** NO shared spinner component (`LoadingSpinner` / `<Spinner` grep: 0) - `animate-spin` appears in 19 files / 25 uses at inconsistent sizes (App `w-16` / border-4 vs ModuleLayout `w-10` / border-2); 48 `isLoading` branches exist but named surfaces have none (KPICard renders `--`, the heatmaps, SharedROIWaterfall); empty-state fragmentation - 12 `ChartEmptyState` + 11 other `EmptyState` + ~10 inline no-data strings at inconsistent sizing (py-16 vs the h-48 full-card case from AUDIT-150's sibling surface); 1411 ad-hoc `hover:` utilities with no shared treatment (incl. 66 off-token `hover:bg-gray`, cross-ref AUDIT-154).
- **Remediation (v2.0):** the §7 state standard - one shared Spinner/LoadingState at one size set, one EmptyState (consistent sizing), one hover treatment (defined ring/elevation), loading required on every async surface.
- **Cross-references:** `UI_CANON.md` §7 (state); AUDIT-154 (the off-token `hover:bg-gray` overlap); 0C-2a Stream-4 state-coverage pass (the predicate, re-run here).

### AUDIT-161 - Flow inversions + dead navigation paths: Export-first, no deep-links, orphan routes (UI_CANON §7 flow)

- **Phase:** Phase 0C-3 UI-canon findings, aesthetic/polish tier (filed 2026-06-12).
- **Severity:** **FINALIZED MEDIUM (P2) 2026-06-12 (operator): classify-up default applied.** The Export-above-headline inversion is on every Executive view a CMO sees - a customer-facing information-hierarchy defect - so MEDIUM. LOW counter-argument recorded as considered: the fix is trivial per view, and the inversion is auto-fixed once AUDIT-151 collapses the 6 Executive views to one canonical-order view (the auto-resolved-by-151 cross-ref stands). **Downgrade-with-evidence path open.** Mitigated pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Status note (HF Export-inversion dimension REMEDIATED):** 2026-07-08 - defect (1) is closed for HF at the HF Exec IA restructure (feat/hf-exec-ia-restructure): the standalone Export row is removed and the Export control renders as a right-aligned utility inside the tier header, with the KPI summary as the first section (headline-first per UI_CANON section 7). The 5 non-HF modules' Export inversion plus defect (2) deep-links and defect (3) orphan routes remain OPEN, so the overall status is unchanged; the per-module Exec passes inherit the HF header pattern.
- **Defect (re-verified):** (1) the Export utility renders FIRST, above the headline gap metric, in the Executive views - HF `<ExportButton>` (`ExecutiveView.tsx:293`) above `<GapIntelligenceCard>` (`:360`); CAD `<ExportButton>` (`CoronaryExecutiveView.tsx:185`) above `<GapIntelligenceCard>` (`:232`) - 2 modules re-verified, the pattern is all 6 per 0C-2a; violates the §7 headline-first rule. (2) No data-driven deep-links between the 3 audience tabs - an Executive gap/opportunity count does not link to the Service-Line detail or the Care-Team action for the same cohort; navigation is role-based only. (3) Orphan routes reachable but absent from the home launcher `MODULES` grid: `App.tsx:520` renders `MODULES.filter(m => m.id !== 'research')`; `/data` + `/patients` are routes (`:761/766`) with no launcher tile; the revenueCycle module has a view (`RCExecutiveView`) but no `MODULES` entry; `/admin/*`.
- **Remediation (v2.0):** §7 - headline-first render order (Export demoted, auto-resolved by the AUDIT-151 single-view consolidation); data-driven cross-tab deep-links; orphan-route disposition (launcher entry or intentional admin-only).
- **Cross-references:** `ExecutiveView.tsx:293/360`; `CoronaryExecutiveView.tsx:185/232`; `App.tsx:520/761/766`; `UI_CANON.md` §7 (flow); AUDIT-151 (the consolidation that auto-fixes the Export inversion); 0C-2a Stream-5 flow pass (the predicate, re-verified here).

### AUDIT-162 - Responsive/layout non-conforming residue (UI_CANON §7 responsive)

- **Phase:** Phase 0C-3 UI-canon findings, aesthetic/polish tier (filed 2026-06-12).
- **Severity:** **FINALIZED LOW (P3) 2026-06-12 (operator).** Desktop-first is acceptable per §7 (clinical desktop tool); the non-conforming residue is bounded and the app is already substantially responsive; mitigated pre-DUA/synthetic.
- **Status:** OPEN (remediation v2.0).
- **Defect (re-run at filing):** ~16 `grid-cols-2/3/4` with NO breakpoint (named per 0C-2a: ReferralPathwayCard, CommandGrid, CompetitorMarketShare, several HF detail modals, CareGapOrchestrationCard, PatientRiskStratification, QualityReportGenerator); 38 fixed `w-[Npx]` incl. **`w-[1600px]` and `w-[1800px]`** (hard desktop-width surfaces); 52 `overflow-x-auto` scroll tables; the `ModuleLayout` sticky 3-tab bar does not collapse/scroll on narrow widths; TopBar search is `hidden md:block` (removed, not collapsed).
- **HONEST MITIGATION (re-verified, per the operator instruction):** the platform IS substantially responsive - 752 breakpoint usages (sm 26 / md 322 / lg 349 / xl 46 / 2xl 9), 199 files use the `grid-cols-1 md:/lg:grid-cols-N` reflow pattern, 131 chart `ResponsiveContainer`. This finding is the specific non-conforming RESIDUE, NOT a responsive rebuild.
- **Remediation (v2.0):** the §7 responsive mandates - no breakpoint-less multi-column grids, no hard layout px-widths, a collapsible/scrollable ModuleLayout tab bar on narrow.
- **Cross-references:** `UI_CANON.md` §7 (responsive); 0C-2a Stream-6 responsive pass (the predicate, re-run here).

### AUDIT-301 - Pre-existing color-token misapplication: carmine (alarm-red) on non-critical financial/benchmark states + CAD nav red (UI_CANON §4.2)

- **Phase:** UI/frontend-track color-token correctness (filed 2026-06-23, feat/audit-301-color-token-fixes off origin/main `5be698c`). Sibling to AUDIT-153 (red semantic overload) - the same alarm-red-on-non-critical class, on three specific customer-facing surfaces the A+B Phase 0C styling pass did not reach.
- **Severity:** MEDIUM (P2) proposed, UI/frontend-track. Root class = the reserved clinical-alarm red (`--sem-critical #9B2438`) applied to non-critical financial/benchmark/nav states on demo/pilot surfaces; the alarm color must mean alarm. Mitigated pre-DUA/synthetic. Operator confirms severity per §18.
- **Status:** OPEN (findings 1+3 fixed this tranche; finding 2 OPEN pending a cross-module decision).
- **Verdict:** CONFIRMED. All three reproduce on origin/main `5be698c`; all pre-existing (git blame: findings 1+2 = `ba883884` 2026-03-31, finding 3 = `32d4672` 2026-03-16) - none introduced by the A+B tranche.
- **Finding 1 (MEDIUM):** `SharedProjectedVsRealized.tsx:59-61` - the revenue "Gap" card hardcodes carmine `#9B2438` on the label (`:60`) and value (`:61`) plus a carmine tint bg `#FDF2F3` / border `#F5C0C8` (`:59`); a revenue shortfall is a non-alarm negative, not a clinical alarm. One shared-component fix recolors both render sites: HF `ProjectedVsRealizedChart.tsx:22` and PV `PeripheralExecutiveView.tsx:274`. Fix = amber semantic tokens (`--sem-data-negative-ink #854F0B` + `--sem-data-negative-bg` + a new `--sem-data-negative-border`).
- **Finding 2 (MEDIUM, DECISION-GATED):** `SharedBenchmarksPanel.tsx` - `isPositiveTrend()` (`:34-37`) drives a binary `positive ? #2D6147 (green) : #9B2438 (carmine)` at `:29/:56/:63/:70`; below-benchmark cards (ABI / WIfI / Supervised Exercise) hit the carmine branch. The favorable-negative green path (`lowerIsBetter:true` -> the 30-Day Amputation Rate card, `PeripheralExecutiveView.tsx` padBenchmarks `:44-49`) MUST be preserved. Deferred pending a cross-module decision (recommended: amber for below-benchmark, reserve carmine for a genuine critical floor if one is introduced).
- **Finding 3 (LOW):** `coronaryIntervention/config/careTeamConfig.tsx:715` `primaryColor: 'medical-red'` - the CAD care-team nav selected-state renders red (`CoronaryCareTeamView.tsx` `getColorClasses` `:182`; `colors.*` used only at the nav active-tab `:247/:252/:253/:258`), unlike HF (`careTeamConfig.tsx:680` `'porsche'` navy) and EP (`'chrome-blue'`). Fix = flip to `'porsche'`; the render path already supports it (the `'porsche'` case exists in `getColorClasses`).
- **Disposition:** findings 1 + 3 fixed in this tranche (amber tokens + nav flip + 2 harness tests); finding 2 OPEN pending the cross-module amber-vs-critical decision.
- **Cross-references:** AUDIT-153 (red semantic overload, the parent class); AUDIT-150 (heatmap color-band, sibling color finding); `src/index.css` `--sem-data-negative-*` tokens (A+B layer); origin/main `5be698c`.

### AUDIT-302 - Phase 0C design/data convergence: clone-and-diverge across all 18 view surfaces (3 tiers x 6 modules) + parallel dead-config fork (UI_CANON §1.4)

- **Phase:** UI/frontend-track Phase 0C convergence audit (filed 2026-06-26, read-only sweep against origin/main `6f97010`). The structural successor to AUDIT-151/152 (view-clone + component-bypass) and the root the color tranches (AUDIT-300/301) could not reach. Method: classify each surface against a per-tier canonical; the 18 surfaces collapse to a small canonical set + per-surface deviation maps.
- **Severity:** MEDIUM (P2) proposed, UI/frontend-track. Structural + data-depth debt on the demo/pilot product surface; no clinical-rule or PHI-decrypt defect (the fabricated-PHI-on-screen sub-flag is mitigated pre-DUA/synthetic). Operator confirms severity per §18.
- **Status:** OPEN (Layer 2 structural de-dup + unify, then Layer 3 data-wire).
- **Verdict:** CONFIRMED. All findings reproduce on origin/main `6f97010` (ref-based reads).
- **Exec tier (Tier 1):** the 6 Executive views are clones of one bespoke ~13-panel skeleton built from the SAME shared components (the "rich" GapIntelligenceCard donut + ranked top-gaps + safety summary is shared, present on all 6, NOT EP-unique). Two structural divergence axes: (1) GROUP-B duplicate render - CAD/VHD/PV render BaseExecutiveView a second time beneath the bespoke page (`CoronaryExecutiveView.tsx:408`, `ValvularExecutiveView.tsx:360`, `PeripheralExecutiveView.tsx:360`), producing a duplicate KPI row + duplicate DRG/CMI block (`BaseExecutiveView.tsx:67-115` + `:126-191`); (2) dead shell imports - BaseExecutiveView + KPICard imported-never-rendered in all 3 Group-A views (HF/EP/SH lines 4/5), plus FinancialROIWaterfall dead in HF (`ExecutiveView.tsx:8`). Financial-panel routing split: Group-A {HF/EP/SH} route 3 financial panels through module-local wrappers (`*ROIWaterfall` / `*ProjectedVsRealizedChart` / `*BenchmarksPanel`); Group-B {CAD/VHD/PV} use the shared components directly. Canonical = the cleaned Group-A 13-panel skeleton, refactored to ONE shared parameterized Executive view (kill Group-B duplication, retire the 3x3 local wrappers, clear dead imports).
- **Tier 2 (Service Line):** GENUINE divergence (bespoke grouped-tab shells, 11-17 module-specific tabs each), sharing a scaffolding pattern (ServiceLineKPIBanner on top + grouped tab-nav + a `*ClinicalGapDetectionDashboard` gap tab + reused shared viz). Dominant defect = CONFIG-VS-VIEW FORK: each module's `serviceLineConfig.tsx` is typed against BaseServiceLineView (type-only; never rendered) and is DEAD or BYPASSED, while carrying a parallel - often richer, data-wired - panel set the view ignores (worst: CAD 829-line `serviceLineConfig.tsx` incl. the gap-data-wired CADInterventionPipeline, unrendered; PV view renders thinner inline stubs and bypasses the config's table panels + PVVascularPipeline). Canonical = reconcile to ONE source per module, keeping the config's richer wired panels; render the chosen source via the shared base.
- **Tier 3 (Care Team):** PARTIAL collapse - a clean config-driven spread pattern exists but only 2 of 6 use it: CAD (`CoronaryCareTeamView.tsx`) + PV (`PeripheralCareTeamView.tsx`) spread+extend careTeamConfig into a map-driven tabContent shell; HF/EP/SH/VHD are bespoke inline. Shared sub-pattern: a nested ClinicalToolsPanel calculator suite (fed DEMO_PATIENT_ROSTER) + the gap dashboard, on all 6. Canonical structure = the CAD/PV config-driven spread; the 4 bespoke views converge onto it.
- **Cross-cutting:** (a) near-total mock (AUDIT-099 class) across all 18 surfaces - the only reliably API-wired surface is ServiceLineKPIBanner (Service-Line tier); Care-Team is mostly hardcoded (EP/CAD/PV care-team have zero backend wiring). (b) cross-tier DUPLICATE render: each `*ClinicalGapDetectionDashboard` renders on BOTH the Service-Line gap tab AND the Care-Team clinical-gaps tab (all 6 modules). (c) dead controls (AUDIT-149 class) resurface: no-op buttons in `coronaryIntervention/config/careTeamConfig.tsx:467/471`, PV care-team Team/Documentation panels, HF quick-actions. (d) dynamic-Tailwind purge-unsafe class interpolation: `from-${colors.accent}-400/20` in `CoronaryCareTeamView.tsx:258` + `PeripheralCareTeamView.tsx:259`, `bg-${template.color}-50` in `coronaryIntervention/config/careTeamConfig.tsx:575` (silent style-drop risk; same interpolation family as the AUDIT-301 nav gradient). (e) lint-level dead imports per module (HF Users/ReferralTrackerEnhanced/Clock/Download; EP EPPatientDetailPanel + orphan outcomes-cohort tab; VHD Activity/Calendar; etc.).
- **Data exemplar (Layer 3):** HF - the only module with meaningful live-API wiring (real-API gap-intelligence + KCCQ on the Exec tier; live-dashboard Workflow/Safety/Documentation on the Care-Team tier). Layer 3 wires the shared parameterized views to real per-module data using HF as the reference.
- **Recharts width(-1)/height(-1):** NOT found on any audited surface. Zero Recharts in either director tier (all 6 modules); the only ResponsiveContainers are the bounded Exec-tier shared cards (GapIntelligenceCard in `w-[140px] h-[140px]`; ForwardLookingCards in `h-[180px]`; GapResponseRateCard/PredictiveMetricsBanner with explicit `height={200}`). No unbounded ResponsiveContainer exists; defer to a one-off runtime capture to pin the transient warning rather than further static search.
- **Disposition:** Layer 2 = structural de-dup + unify (collapse to shared parameterized views per tier; kill Group-B duplicate render; reconcile the Tier-2 config-vs-view fork; converge Tier-3 onto the config-driven pattern); Layer 3 = data-wire the shared panels (HF exemplar). ABSORBS the freemium data-accuracy finding (revenue 192M-vs-142M module-tile/headline mismatch + the three-way patient-count divergence 12,480 / 13,560 / 43,841) as the Layer-3 design surface; the data-source correction hands to the clinical track. Claims-honesty sub-flags carved OUT to AUDIT-303 (higher priority); synthetic-PHI-on-screen stays here as a pre-DUA design flag.
- **Cross-references:** AUDIT-151 (view-body clone), AUDIT-152 (component-system bypass), AUDIT-099 (silent-mock surface), AUDIT-149 (dead controls), AUDIT-300 (clone-and-diverge clinical-content class), AUDIT-301 (color-token sub-class); origin/main `6f97010`.

### AUDIT-303 - False capability / provenance labels on the demo/pilot surface (AUDIT-300-class claims-honesty): four affirmatively-misleading UI strings

- **Phase:** UI/frontend-track Phase 0C claims-honesty audit (filed 2026-06-26, ref-based origin/main `6f97010`). Carved out of the AUDIT-302 convergence sweep as a higher-priority, separable class: strings that ASSERT a capability/provenance the code does not have, on the clinician/CMO-facing pilot surface. Sibling to AUDIT-300 (false clinical claims) and AUDIT-146/147 (provenance-blind / fabricated-display). Scope STRICT to capability/provenance label honesty; synthetic-patient-data and cosmetic flags stay in AUDIT-302.
- **Severity:** HIGH (P1) proposed, UI/frontend-track - driven by the two HIGH items (a false "AI" claim that also misrepresents the §8 FDA-CDS deterministic posture, and a false "auto-calculated from EHR" provenance claim on static data). Mitigated pre-DUA/synthetic, but the demo IS the pilot product. Operator confirms severity per §18.
- **Status:** OPEN (all LABEL remediation COMPLETE 2026-06-27: the 4 originally-cited items + 67 class-wide false-capability/provenance label occurrences fixed across all 6 modules + free-tier [AI/ML 22, EHR-provenance 31, Real-time 14] on branch feat/audit-303-honesty-edits; item-1 KCCQ DATA-WIRE still deferred to Layer 3 - AUDIT-303 stays OPEN until that lands, but all LABEL remediation is complete; see Class-wide remediation below).
- **Remediation (2026-06-27, feat/audit-303-honesty-edits):** the 4 CITED items' labels fixed - item 2 `ValvularServiceLineView.tsx:46` 'AI-driven' -> 'Guideline-based clinical gap detection'; item 1 `KCCQOutcomesPanel.tsx:42` 'Auto-calculated from EHR data' -> 'Demo data &middot; EHR integration pending'; item 3 dropped 'Real-time' from `StructuralCareTeamView.tsx:90` + `EPCareTeamView.tsx:101`; item 4 `ValvularCareTeamView.tsx:105` 'Valve Worklist' -> 'Surgical Referral Network'. Item-1 DATA-WIRING (real KCCQ feed) deferred to Layer 3 (AUDIT-302). Label-only edits; tsc clean + primitives harness green.
- **Class-wide remediation (2026-06-27, feat/audit-303-honesty-edits expansion):** the grep-sweep confirmed the cited 4 were a SAMPLE; the full AI-driven / Auto-calculated-from-EHR / Real-time false-claim classes (67 additional label occurrences across all 6 modules + free-tier) are now ALL REMEDIATED, label-only. **AI/ML (22):** 'AI-driven' -> 'Guideline-based' on all 6 Gap-Detection tab descriptions (HF `ServiceLineView:64`, EP `EPServiceLineView:196`, SH `StructuralServiceLineView:53`, PV `PeripheralServiceLineView:268`, VHD `ValvularCareTeamView:102`, CAD via `serviceLineConfig`) + the 4 rendered `*ClinicalGapDetectionDashboard` headers (CAD `:6214`, PV `:3306`, SH `:2602`, VD `:3490` - HF/EP gap dashboards have no such header) + the 5 service-line config descriptions + VD `careTeamConfig:819`; EP 'AI-powered' x2 (`serviceLineConfig:647`, `EPAutomatedClinicalSupport:356`) -> 'Guideline-based'; free-tier `AIInsightCards` title 'AI-Detected Insights' -> 'Guideline-Based Insights', subtitle 'Machine learning analysis' -> 'Guideline-based analysis', summary 'AI-detected gaps' -> 'guideline-based gaps', and the `AIPoweredBadge` rendered text 'AI Powered' -> 'Guideline-Based' (component identifier kept; rendered-text label-only). **EHR-provenance (31):** the 'Automatically calculated from EHR-sourced data ... No manual entry required' tooltip on all 6 gap dashboards (2x each = 12), CAD `SAQOutcomesPanel:24` + HF `ExecutiveView:314` + CAD `CoronaryExecutiveView:203` badges (3), 'Auto-calculated from structured EHR data' (EP/HF, 2) + 'Auto-calculated from EHR flowsheet data' (CAD/EP/HF, 3), the 5 config EHR-tails, and the 5 freemium 'Patients automatically identified from EHR integration' banners + CAD `serviceLineConfig:727` pipeline description (1) -> demo-honest phrasing ('Demo data &middot; EHR integration pending' / 'structured demo data' / 'live EHR integration pending'). **Real-time (14):** HF `CareTeamView:140` 'Real-time heart failure hospital alerts' -> 'Heart failure hospital alerts'; the 2 care-network subtitles HF `HFCareNetworkVisualization:620` + SH `SHValveCareNetworkVisualization:620` (static network data - only a setInterval clock ticks, no fetch/stream; RECLASSIFIED from the earlier 'HF API-wired defensible' skip, which was wrong - the network DATA is static); 10 more verified-static surfaces had 'Real-time' dropped (CAD `PCINetworkVisualization:467`, EP `EPDeviceNetworkVisualization:700`, SH `TAVRAnalyticsDashboard:295` subtitle, PV `PADReportingSystem:285`/`:487`, RC `RCOperationsView:201`, HF `HFRiskAlerts:69`, EP `EPAlertDashboard:188`, CAD `CoronaryCareTeamView:151`, `DRGOptimizationAlert:60`); and `NotificationPanel:276` 'Active care gap orchestrations with real-time status tracking' -> '... with status tracking' (descriptive prose, no wiring - the last non-view-mode capability claim, fixed 2026-06-28). **DRIFT-44 folded in (PRE-EXISTING, not introduced here):** 6 em-dash (U+2014) in the 5 config EHR-tails + CAD `:727`, and 1 middle-dot (U+00B7) on the `AIInsightCards` subtitle line, all converted to ASCII on the touched lines. **INTENTIONAL SKIPS (verified non-false or out-of-label-scope):** EP `careTeamConfig:697` 'ML-driven predictive modeling' (future-feature roadmap toast - 'will be available in the next release', not a current capability claim); EP `EPClinicalGapDetectionDashboard:989` 'STOP-BANG score >=3 from EHR data' (clinical trigger-criteria EVIDENCE string, not a UI label - editing clinical evidence text exceeds label-only scope); the conditional 'Live Data'/'Demo Data' badges (honest AUDIT-146 pattern - left untouched). **Real-time GENUINELY-WIRED keeps** (verified live fetch -> 'real-time' accurate, kept): HF `RealTimeHospitalAlerts.tsx:34` (getHeartFailureDashboard on mount), HF GDMT `serviceLineConfig:319` + `ServiceLineView:54` (render GDMTAnalyticsDashboard -> Promise.all([getHeartFailureDashboard(), getHeartFailureWorklist()]) on mount), HF `careTeamConfig:635` (the HF care-team dashboard tab renders the wired RealTimeHospitalAlerts). **Real-time INTENTIONAL KEEPS** (only the 2 view-mode controls remain): CAD `PCINetworkVisualization:482` 'Real-Time Flow' + SH `TAVRAnalyticsDashboard:335` 'Real-time Dashboard' - these name a user-selectable view MODE, not a static-data capability claim, so they stay. (The 3rd former flag, `NotificationPanel:276` prose, was FIXED 2026-06-28 - see the Real-time(14) list.) Verification: tsc --noEmit EXIT 0, primitives harness 35/35, DRIFT-44 added-line scan CLEAN (48 added lines, zero non-ASCII). The §8 dimension was the priority: 'AI-driven' on a deterministic gap engine was a systemic FDA-CDS-posture misrepresentation. Item-1 KCCQ DATA-WIRE still deferred to Layer 3; AUDIT-303 stays OPEN until that lands, but all LABEL remediation is COMPLETE.
- **Real-time-class finalize + render-test (2026-06-28, same branch):** the broader Real-time-on-static set flagged in the expansion grep-sweep is now CLOSED - 13 surfaces remediated (the 2 care-network subtitles + 10 static labels + the `NotificationPanel:276` prose, in the Real-time(14) list above), each verified wired-vs-static before editing (no blanket replace). WIRED keeps (4) + VIEW-MODE keeps (2, user-selectable modes - the prose 3rd flag is now fixed) are enumerated in the Class-wide remediation skips. Standing verification artifact added: `src/design-system/__tests__/audit303.labels.test.tsx` (30 tests, react-dom/client + act - RTL is intentionally absent from this project) - it REPLACES the auth-blocked live visual check (every target surface is behind `<ProtectedRoute>` and this branch deliberately requires login, so an offline screenshot pass is impossible). It renders one self-contained component per class (AIInsightCards / SAQOutcomesPanel / HF+SH+CAD+EP network + alert components), asserts corrected strings present + false strings absent, adds config-value breadth across all 6 modules, and asserts-at-source the heavy view-shells. Verification: tsc --noEmit EXIT 0, full suite 78/78 (5 suites, 23 snapshots), DRIFT-44 added-lines CLEAN.
- **Verdict:** CONFIRMED. All four literals verified in running code on origin/main `6f97010`; the grep-sweep further confirms the claim classes are systemic (the cited 4 were a sample of 30+ occurrences).
- **Item 1 (HIGH) - false EHR-auto-calc provenance:** `KCCQOutcomesPanel.tsx:42` renders the literal "Auto-calculated from EHR data - No manual entry required" while the panel's data is a static const (domainData + hardcoded metric cards 47.3 / +14.2 / 312 / 48). TRUE STATE: no EHR integration; the numbers are hardcoded. Affirmatively claims a live data pipeline that does not exist. Disposition: HONESTY-EDIT (drop the "Auto-calculated from EHR" claim, or label "Demo data") now; WIRE-DATA later (Layer 3). Cross-ref CLAUDE.md §14 "never hardcode Live/real-time labels on static data".
- **Item 2 (HIGH) - false AI claim + §8 posture misrepresentation:** `ValvularServiceLineView.tsx:46` declares the Gap-Detection tab description "AI-driven clinical gap detection". TRUE STATE: gap detection is deterministic, rule-based (CLAUDE.md §8 mandates deterministic rule-based logic and EXPLICITLY prohibits ML/AI for gap detection, for FDA CDS-exemption transparency). "AI-driven" is both factually false AND misrepresents the regulatory posture the platform depends on. Highest-priority item. Disposition: HONESTY-EDIT (replace "AI-driven" with "guideline-based" / "deterministic rule-based clinical gap detection"). No data-wiring needed - the engine is correct; only the label lies.
- **Item 3 (LOW - downgraded) - overpromising tab descriptions (component already honest):** the tab DESCRIPTION strings still claim real-time alerts: `StructuralCareTeamView.tsx:90` "Real-time structural heart hospital alerts" and `EPCareTeamView.tsx:101` "Real-time EP hospital alerts" (+ a stale comment `EPCareTeamView.tsx:163`). TRUE STATE: the rendered components `SHRealTimeHospitalAlerts.tsx` and `EPRealTimeHospitalAlerts.tsx` were already gutted to an honest `<EmptyState title="Real-time alerts not yet available for this module">` by AUDIT-300 P2 - so the surface a user SEES is honest; only the nav-metadata description overpromises. DOWNGRADED from MEDIUM to LOW (label-only; not static-data-mislabeled). Disposition: HONESTY-EDIT the 2 tab descriptions (drop "Real-time" until wired). Note also the component name `*RealTimeHospitalAlerts` is now a misnomer (renders an EmptyState).
- **Item 4 (MEDIUM) - label/content mismatch (re-attributed to VHD, NOT PV):** `ValvularCareTeamView.tsx:105` declares a tab id 'worklist' label "Valve Worklist" / description "Comprehensive valve worklist", but the case at `ValvularCareTeamView.tsx:294` renders `<ValvularSurgicalNetworkVisualization />` - a referral-network graph, not a worklist. TRUE STATE: a worklist-labeled tab shows a network diagram. PV is NOT affected (`PeripheralCareTeamView.tsx:171` worklist -> the real PeripheralWorklist component) - the original draft mis-attributed this to PV. Disposition: either relabel the tab (it is a surgical-referral network view) or render the actual valve worklist.
- **Cross-references:** AUDIT-300 (false clinical claims, parent class), AUDIT-146/147 (provenance-blind / fabricated-display), CLAUDE.md §8 (deterministic-not-AI mandate) + §14 (no false Live/real-time labels); AUDIT-302 (the convergence sweep this was carved from); origin/main `6f97010`.

---

### AUDIT-304 - UI/UX craft, honesty, and information-architecture remediation across the platform surfaces (shell, module tiles, per-module tiers)

- **Phase:** Phase 0C UI/UX remediation arc - the consolidated frontend finding; parent umbrella for the shell + module-tile + per-module-tier work.
- **Severity:** MEDIUM (P2), UI/frontend-track. Operator confirms per §18.
- **Status:** OPEN (exemplar-first; HF Executive complete, EP Executive replicated (MERGED PR #456), SH Executive replicated (in PR), the other 3 modules pending replication).
- **Summary:** The consolidated UI/UX finding driving the frontend remediation arc. Covers (1) global shell - logo, login, toasts, breadcrumbs, notifications, profile honesty; (2) module-tile landing craft; (3) per-module tier remediation on three axes: data-honesty (wired-vs-mock reconciliation, demo badging, false-live-label removal), controls (dead-control removal/wiring), and information architecture (exec-narrative panel order, redundant-panel consolidation, tier-appropriate relocation).
- **Disposition:** exemplar-first, per-module replication. HF Executive is the completed exemplar; the other 5 modules (EP / SH / CAD / VHD / PV) replicate the exemplar per-tier.
- **HF exemplar (COMPLETE):** shell batch (PR #438, `c4dffc9`); HF Exec batch 1 - data reconciliation + honesty labels (PR #442, `4e6987e`); HF Exec batch 2 - dead-control fixes + white-card restyle (PR #448, `d4af97c`); HF Exec batch 3 - IA restructure (15 -> 8+1 panels in exec-narrative order, forward-trio consolidation, KCCQ dissolve into the KPI summary, tier-appropriate relocation, Export folded into the tier header) on branch `feat/hf-exec-ia-restructure`.
- **EP replication (IN PR, branch `feat/ep-exec-convergence`):** full single-PR convergence of the EP Executive tier to the HF exemplar (batches 1-3 folded). New `EPExecutiveSummary` wires the live EP dashboard contract (patients / open gaps / device candidates) with honest loading/error/empty states (closes the EP-side AUDIT-099 dead-`dashboardError`); `GapIntelligenceCard` gains opt-in `totalPatients` + `compositionDemo` props so its headline reads live `totalOpenGaps` across the real patient total (kills the 1,730 category-sum-as-population mislabel) while the donut/top-gaps stay demo-badged; new `epDemoFinancials` single-source model (all sums derived in code, fixes the $6.7M != $8.4M at-risk break; $8.9M annual); DemoDataBadge on the 6 mock-fed financial surfaces + the fabricated facility-detail modal; new `EPForwardOutlookPanel` consolidates the pipeline/at-risk/predictive trio; IA reordered to the HF 8-panel exec narrative (DRG/CMI promoted, Export folded into the tier header); Gap Response Rate + Trajectory Trends relocated to EP Service Line (gap-detection + heatmap tabs). Frontend only; the EP dashboard endpoint already emits the wired fields (no backend/contract change). 36-assertion mirror suite `epexec.convergence.test.tsx` (incl. 4 DRG-crash regression tests). **MERGED 2026-07-09 as PR #456 (squash `b66f3270`); tsc 0, Jest 218/218, ESLint 0; operator Chrome-verified.**
- **SH replication (IN PR, branch `feat/sh-exec-convergence`):** full single-PR convergence of the SH (Structural Heart) Executive tier to the HF/EP exemplar. New `SHExecutiveSummary` wires the live SH dashboard contract (`SHDashboardData`: patients / open gaps / device candidates) with honest loading/error/empty states (closes the SH-side AUDIT-099 dead-`dashboardError`; retires the `?? 8` default); `GapIntelligenceCard` reuses the opt-in `totalPatients` + `compositionDemo` props so its headline reads live `totalOpenGaps` across the real patient total (kills the 830 category-sum mislabel); new `shDemoFinancials` single-source model (all sums derived, fixes the 10.3-vs-10.9M total split + the 5.0-vs-6.2M at-risk break; $10.9M annual); DemoDataBadge on the 6 mock-fed financial surfaces + the facility-detail and DRG-detail modals; new `SHForwardOutlookPanel` consolidates the pipeline/at-risk/predictive trio; IA reordered to the 8-panel exec narrative (DRG/CMI promoted, Export folded into the tier header); Gap Response Rate + Trajectory Trends relocated to SH Service Line (gap-detection + risk-heatmap tabs). **DRG-crash fix (the cross-module watch-item):** the SH DRG drill-down had the identical latent bug as EP (`cases:number` passed where the modal expects `Case[]`, and the modal's `(cases || [])` guard was false safety - `45 || []` -> `45` -> `[...45]` throws) - hardened `SHDRGDetailModal` with an `Array.isArray` guard + honest empty state + badge, corrected the feed via `getSHDRGDetailData`, plus regression tests. Frontend only; the SH dashboard endpoint already emits the wired fields (no backend/contract change). 36-assertion mirror suite `shexec.convergence.test.tsx` (incl. 4 DRG-crash regression tests). Operator Chrome verify + separately-authorized push pending.
- **Cross-references:** AUDIT-099 (fabricated-KPI silent-mock; HF + EP + SH sides complete, the 3-view non-HF core (CAD / VHD / PV) OPEN); AUDIT-161 (export-inversion; HF + EP + SH dimensions remediated, 3 remain); AUDIT-152 (dead SH `SHExecutiveKPICard` / `StructuralExecutiveKPICard` copies - unimported, flagged for deletion, not deleted in this batch); AUDIT-303 (false-capability labels); sibling UI findings AUDIT-300 (false-clinical-claims parent class), AUDIT-301 (color-token misapplication), AUDIT-302 (design/data convergence).
- **Verdict:** CONFIRMED (18-surface audit + operator Chrome verification).

---

### Follow-on candidates (named, NOT implemented; logged per operator instruction 2026-06-06)

- **AUDIT-XXX-future-ep-facility-modal-empty-values** (LOW, UI/frontend-track; logged 2026-07-11 per §17.3 during the SH convergence): `EPFacilityDetailModal` carries the SAME facility-modal defect just fixed for SH (found in SH Chrome verify): it reads the top-level rate as `ablationRate` (`EPFacilityDetailModal.tsx:137`/`:243`) but `getElectrophysiologyFacilityData` emits `gdmtRate` (`EPExecutiveView.tsx:330`/`:344`) -> a bare "%" prop-name mismatch; and the first-branch `electrophysiologyData.facilities[]` providers lack `revenueImpact`, so `formatMoney(undefined)` renders a fabricated "$0" instead of an honest "-". On origin/main (EP merged as PR #456 `b66f3270`). Fix = port the SH remediation: rename the rate at the call site (real value renders) + normalize providers + honest-empty guards in the modal. NOT fixed in the SH batch (§17.3 scope discipline; EP is a separate merged tier).
...
- **AUDIT-XXX-future-rendersynthesis-emdash-drift44** (LOW): `renderSynthesis.ts:178` emits an em-dash (U+2014) in the section 3.1 header template `(${explicit.length} <sep> automatic Tier S)`, so every render of `docs/audit/PHASE_0B_CROSS_MODULE_SYNTHESIS.md` writes a DRIFT-44-violating non-ASCII char; because the count changes per audit, the whole line re-adds in any diff carrying the em-dash (e.g. count 0 -> 4 at the EP module close). Fix = replace the em-dash with an ASCII hyphen in the template + add a render-stability assertion. NOTE only, NOT fixed here. Surfaced by the EP module-close synthesis (AUDIT-117/118/119/120).
- **AUDIT-XXX-future-audit016-pr3-jest-flake** (LOW): `backend/tests/scripts/migrations/audit-016-pr3-v0v1-to-v2.test.ts:454` intermittently throws `TypeError: Cannot read properties of undefined (reading 'get')` at `await main(opts)` under parallel jest - a mock/map appears uninitialized under worker parallelism. Observed RED at PR #371 run 1 (commit `ad4b099`) and GREEN at run 2 (commit `a95dd01`) with NO change to the migration path between them (the only diff was `applyOverrides.test.ts`); merge-base is origin/main HEAD `72d58fe` where Jest is green. Confirmed flake, not a production defect. Candidate: audit the test mock setup (`queryRawUnsafe` / `routeSql` initialization) for a parallel-safe per-test reset. Surfaced during the EP module-close PR #371.
...
## KB-Completeness (Spec-Completeness) Findings

**The class (new, established by AUDIT-163).** Every prior clinical finding (the Phase-0B AUDIT-NNN set) is a **code-vs-KB** finding: it audits whether the gap-detection code correctly implements a gap that EXISTS in the clinical knowledge base (`docs/clinical/CLINICAL_KNOWLEDGE_BASE_v4.0.md`). The Phase-0B audits are structurally blind to a different failure: a clinically-real, guideline-grounded gap that is **absent from the KB itself** - the code cannot be wrong about a gap the KB never specified, so a code-vs-KB audit can never surface it. This is the **KB-completeness / spec-completeness** class: the KB audited against clinical reality (guidelines + the practising-clinician's domain map), not the code audited against the KB.

- **Representation (operator-confirm):** filed in the unified `AUDIT-NNN` id-space (so cross-references stay simple and there is one findings register per CLAUDE.md §6), tagged ``[KB-COMPLETENESS]`` in the severity index and hosted in this dedicated detail section so the class is visible. Recommended over a separate register (which would fork tooling, the §18 three-surface discipline, and id-space for no benefit). The CLASS is what is new; the id-space is shared.
- **Remediation shape:** unlike code-vs-KB findings (fixed by changing code), a KB-completeness finding is remediated by **ADDING the gap to the KB**, which then feeds the v3.0 clinical buildout as new spec - i.e. it grows the SPEC_ONLY denominator before it is built.

---

### AUDIT-163 - POAF (post-operative atrial fibrillation) prophylaxis absent from KB - cardiac-surgery population

- **Class:** KB-completeness / spec-completeness (missed-and-unknown; NOT catchable by the code-vs-KB Phase-0B audit).
- **Severity:** FINALIZED HIGH (P1) 2026-06-15 (operator): anchor of a systematic peri-operative blind spot (7/8 STS quality concerns absent), COR I established standard-of-care, high-incidence, commercial-pathway-relevant (LOS/readmission/DRG).
- **Status:** OPEN - 2026-06-15.
- **Discovered:** 2026-06-15 via operator (cardiac clinician) clinical-reality flag + a read-only KB verification pass (HEAD 8fb01fa).
- **Clinical substance:** post-operative atrial fibrillation is the most common complication after cardiac surgery - incidence ~30-50% post-CABG and post-valve. Peri-operative **beta-blocker prophylaxis is Class I** (continue/initiate a beta-blocker peri-operatively unless contraindicated) per the STS practice guideline and the 2023 ACC/AHA/ACCP/HRS AF guideline; **amiodarone prophylaxis is Class IIa** for patients at high risk of POAF. The unflagged population: a cardiac-surgery patient with **no peri-operative beta-blocker on the medication list and no documented contraindication** is in a guideline-directed prophylaxis gap that the platform does not surface.
- **Evidence of absence (read-only verification, 2026-06-15):** no POAF-prophylaxis gap exists in `CLINICAL_KNOWLEDGE_BASE_v4.0.md` or in any per-module spec.json / crosswalk. The three nearest gaps target adjacent-but-different concerns and none is prevention: **GAP-VHD-077** (post-valve AF *anticoagulation maintenance* - treats established post-op AF; PARTIAL_DETECTION), **GAP-EP-073** (concomitant *Maze* at cardiac surgery for *pre-existing* AF; SPEC_ONLY), **GAP-CAD-055** (post-CABG *sternal-wound surveillance* - not AF; SPEC_ONLY). The targeted "cardiac-surgery + beta-blocker/amiodarone to prevent AF" search returned empty.
- **Owning module (recommend):** **EP primary** (the gap is a rhythm-prophylaxis gap - AF prevention is EP's clinical domain, and the existing AF gaps are GAP-EP-NNN; the eventual buildout id is naturally GAP-EP-NNN), **cross-listed to SH (valve surgery) + CAD (CABG)** because the trigger population spans both surgical domains. (The broader surgical KB-completeness tranche - the other 7/8 spot-check concerns - will be predominantly SH/surgical-owned; POAF is the EP-flavoured anchor of that tranche.)
- **Proposed rule shape (for the v3.0 buildout, NOT built here):** recent cardiac-surgery procedure code (CABG / valve CPT or ICD-10-PCS) + ABSENCE of a peri-operative beta-blocker on the active medication list + NO documented beta-blocker contraindication -> POAF-prophylaxis gap. This fits the existing **absence-rule** detection pattern cleanly (the same shape as GAP-EP-013 / GAP-EP-024 / the GDMT-not-prescribed rules), so the buildout cost is low once the spec is added.
- **ROI pathway tag:** POAF drives **length-of-stay** (1-2 added days typical) -> the avoided-admission / readmission + DRG / observed-vs-expected-LOS BSW ROI pathways. A high-incidence, LOS-driving, COR-I-preventable complication is a strong service-line ROI story.
- **Severity rationale (proposed HIGH P1):** (1) high incidence (30-50%); (2) the prophylaxis is **COR I** (not a soft recommendation); (3) commercial-pathway-relevant (LOS / DRG ROI, cardiac-surgery quality); (4) **representative of a systematic blind spot** - the spot-check found 7 of 8 peri-operative quality concerns absent (prolonged ventilation, post-op AKI, re-operation for bleeding, post-op delirium, peri-op glucose, early extubation/mobilization, LOS-as-care-process all ABSENT; only sternal-wound surveillance partially present and SPEC_ONLY). A HIGH anchor calibrates the urgency of the surrounding audit. Operator finalizes per §18.
- **Remediation:** add the POAF-prophylaxis gap to `CLINICAL_KNOWLEDGE_BASE_v4.0.md` (EP module, cross-listed SH/CAD) with COR/LOE + codes + the absence-rule detection logic, then author the detection rule in the v3.0 **surgical-completeness buildout tranche**. Two steps: spec-add (this class's remediation) then code-build (the v3.0 buildout).
- **Forward reference:** **Finding #1 of the surgical peri-operative KB-completeness audit.** Steps 2-3 (author the KB-completeness methodology; run the full surgical peri-operative audit) follow. The spot-check (7/8 peri-op concerns absent) is the justification that POAF is representative, not a one-off, and that the full surgical-completeness audit is warranted and likely urgent (pre-commercial-claim for any cardiac-surgery quality surface).
- **Cross-references:** GAP-VHD-077 / GAP-EP-073 / GAP-CAD-055 (the adjacent non-prevention gaps); the v3.0 clinical buildout (`docs/PATH_TO_ROBUST_v3.0.md` Track A - surgical-completeness tranche added); the read-only POAF verification (2026-06-15, this branch's predecessor pass).

---

### AUDIT-164 - EP/VD shared `hasMechanicalValve` helper included Z95.3 (bioprosthetic): CRITICAL-severity false-positive in shipped code

- **Phase:** v3.0 EP buildout - legacy DET_OK fold-in clinical re-review (surfaced 2026-06-16).
- **Severity:** HIGH (P1). A CRITICAL-labeled SAFETY_ALERT (Class 3 Harm) fired on the WRONG valve subgroup. False-positive over-alert, not a missed gap; mitigated synthetic/pre-DUA, but it is a shipped-code clinical-correctness defect in a CRITICAL rule. Operator confirms per section 18.
- **Status:** RESOLVED 2026-06-16 (v3.0 EP buildout branch `feat/ep-chunk1-af-anticoag`).
- **Defect:** the shared helper `hasMechanicalValve` (`gapRuleEngine.ts`) was `dxCodes.some(c => c.startsWith('Z95.2') || c.startsWith('Z95.3') || c.startsWith('Z95.4'))`. ICD-10-CM **Z95.3 is "Presence of xenogenic heart valve" = a BIOPROSTHETIC / tissue valve** (verified per section 16), NOT a mechanical valve. Two consumers were wrong as a result: (a) the EP/VD DOAC-contraindication rule (`gap-vd-6`, reconciled to GAP-EP-007, RE-ALIGN Class 3 Harm) FALSELY flagged a bioprosthetic-valve patient on a DOAC as "DOAC contraindicated" - but DOACs are guideline-permitted in bioprosthetic AF (after ~3 months); and (b) the VD "mechanical valve needs lifelong warfarin" rule over-fired on bioprosthetic patients who do NOT need lifelong warfarin.
- **Impact:** FALSE-POSITIVE over-alert on the bioprosthetic-valve population in a CRITICAL/Class-3-Harm rule (urgent-review framing). Same detection-correct-but-wrong-for-subgroup class as AUDIT-120 (Z88) and the EP-001 mechanical-valve recommendation fix.
- **Fix:** `hasMechanicalValve = Z95.2 || Z95.4` (Z95.3 dropped). The root-fix on the shared helper corrects BOTH consumers simultaneously - bioprosthetic valves neither contraindicate a DOAC nor mandate lifelong warfarin, so the narrowing is correct in both places.
- **Regression test:** `backend/tests/ingestion/epLegacyFixes.test.ts` - bioprosthetic (Z95.3) + DOAC -> GATES; mechanical (Z95.2 / Z95.4) + DOAC -> FIRES. The AUDIT-118 cascade test (`MECH_VALVE_DX = 'Z95.2'`) remains green (11/11).
- **Classification effect:** NONE. GAP-EP-007 stays DET_OK (the fix corrects a false-positive within the rule; detection of the true mechanical-valve population is unchanged). Register entry + dated note per section 18 (no three-surface needed).
- **Cross-references:** `gapRuleEngine.ts hasMechanicalValve`; ICD-10-CM Z95.2/Z95.3/Z95.4 (NLM); AUDIT-120 + EP-001 (the sibling subgroup-correctness fixes); RE-ALIGN (DOAC-in-mechanical-valve harm).

---

### AUDIT-165 - patientWriter silently dropped csvSchema-defined, csvParser-validated valve-severity fields (echo data-loss)

- **Phase:** v3.0 echo-severity threading investigation + build (surfaced 2026-06-17).
- **Severity:** MEDIUM (P2). Silent data-loss of modeled+validated clinical data. No runtime harm pre-DUA (the values were never stored, so no wrong result), but it blocked the ENTIRE SH+VHD valve-severity gap surface (~64 gaps) - the values were parsed and validated, then discarded at the write step. Operator confirms per section 18.
- **Status:** RESOLVED 2026-06-17 (v3.0 echo-severity threading build, branch `feat/echo-severity-threading`).
- **Defect:** `csvSchema.ts` `SH_COLUMNS` defines `aortic_valve_vmax`, `aortic_valve_mean_gradient`, `aortic_valve_area`, `mitral_regurg_grade` (validated 0-4), `sts_score`; `VD_COLUMNS` defines `valve_severity`, `valve_lesion_type`, `last_echo_date`. `csvParser.ts` parses + validates them. But `patientWriter.ts` persisted observations only for a hardcoded 16-field `labFields` allow-list (`lvef/bnp/.../abi_left`) with NO valve field, so every validated valve value was silently dropped at `for (const field of labFields)`. The engine's `labValues` therefore never carried any valve-severity numeric (only `lvef/pasp/aortic_root/la_volume_index` echo keys existed), and SH/VHD severity rules fell back to ICD-10 dx codes (severity = the diagnosis, not the gradient).
- **Fix (this build):** extended the `labFields` allow-list with the 5 numeric valve fields (thread via `valueNumeric`); added an ordinal-encode block for the coded `valve_severity` string (operator sign-off: grade-PRESERVING 0-5 scale - none 0 / mild 1 / mild-moderate 2 / moderate 3 / moderate-severe 4 / severe 5 - so each gap sets its own threshold; design option A, zero engine-signature ripple; distinct from `mitral_regurg_grade`'s standard 0-4 MR scale); added the valve slugs to both gap runners' `IMAGING_TYPES` (365-day echo freshness, not 180-day lab). PATH 2 (FHIR): added `ECHO_LOINC_TO_SLUG` (section-16-verified NLM LOINCs) at the observationService transform.
...
### AUDIT-168 - Unverified structural CPTs in allGapValueSets.ts (suspect-provenance scaffolding, not section-16 product)

- **Phase:** v3.0 SH buildout chunk 5 procedure-detection architecture investigation (read-only scoping, 2026-06-17).
- **Severity:** LOW (P3) - LATENT, not live. Scoped blast radius is ZERO currently-active gaps (see below). Severity would escalate to MEDIUM the moment any firing path is wired to consume `ALL_GAP_VALUE_SETS`. Operator-confirmed per section 18.
- **Status:** RESOLVED 2026-06-17 (v3.0 SH buildout; the suspect CPTs were re-verified against manufacturer reimbursement guides + operator sign-off, and the one confirmed-wrong code was corrected).
- **Resolution (2026-06-17):** the suspect CPTs were verified against manufacturer reimbursement guides (the reachable public market-access sources) under the two-key bar (manufacturer-source + operator-confirm): TAVR `33361-33366` CONFIRMED (Edwards SAPIEN / Medtronic Evolut billing guides), mitral TEER `33418/33419` CONFIRMED (Abbott MitraClip guide), LAAC `33340` CONFIRMED ("Percutaneous transcatheter closure of the LAA with endocardial implant" + ICD-10 I48.x/Z00.6, Boston Scientific Watchman guide - matching the live `cardiovascularValuesets.LAAC_CPT`). ONE confirmed-WRONG: `GAP_068 "Tricuspid TEER Candidacy"` used `0544T/0545T`, which are **Transcatheter Valve Annulus Reconstruction (Cardioband)** - 0544T mitral, 0545T tricuspid - NOT edge-to-edge TEER. Per the Abbott TriClip coding guide, tricuspid TEER (TriClip) = `0569T/0570T`. CORRECTED in `allGapValueSets.ts` to `['0569T','0570T']` with an inline note. **Blast radius ZERO** (`ALL_GAP_VALUE_SETS` has no firing consumer - dormant scaffolding), so this was a LATENT wrong-code corrected before it could go live, NOT a live patient-safety defect. The KNOWN-WRONG echo LOINCs `18148-8/77912-1` in the same file are a separate open item (not in this finding's scope).
- **Defect:** `backend/src/terminology/valueSets/allGapValueSets.ts` declared structural-procedure CPTs with NO section-16 verification provenance: TAVR `33361-33366` (GAP_002/066/077/078), TEER/MitraClip `33418/33419` (GAP_067), tricuspid-TEER `0544T/0545T` (GAP_068 - the confirmed-wrong one), LAAC `33340` (GAP_002-region). The SAME file carries the KNOWN-WRONG echo LOINCs `18148-8` / `77912-1` (the codes the echo-severity investigation flagged as mislabeled), so the file is suspect-provenance scaffolding, NOT section-16 product. Per section 16 (codebase trust is INSUFFICIENT), these CPTs must not be trusted without an authoritative AMA/CMS verification.
- **Blast-radius scoping (read-only):** ZERO currently-active gaps gate on these suspect CPTs. The runtime gap-firing path is `evaluateGapRules` (gapRuleEngine.ts) driven by the two runners; it consumes dx-proxies + the threaded `procedureCodes` matched against the SEPARATE, section-16-VERIFIED CPT constants in `cardiovascularValuesets.ts` (`EP_ABLATION_CPT`, `CIED_IMPLANT_CPT`, `CIED_EXTRACTION_CPT`, `LAAC_CPT`). `ALL_GAP_VALUE_SETS` is consumed only by the barrel `terminology/index.ts` (re-export) and `terminology/types.ts` (typedef); its sole would-be consumer `redox/batchGapDetection.ts` is a STUB with its value-set consumption entirely commented-out pseudocode. No live gap's firing depends on a possibly-wrong CPT.
- **Distinction:** the EP CPT sets in `cardiovascularValuesets.ts` WERE section-16-verified 2026-06-16 against AMA CPT Assistant / AAPC Codify / CMS coverage articles (provenance comments inline) - they are section-16 product, NOT suspect. This finding is scoped strictly to the `allGapValueSets.ts` declarative scaffolding.
...
### AUDIT-171 - SH-022 evaluator renamed but registry id not migrated: latent applyOverrides demotion (V.5-RECOVERY catch from the SH close)

- **Phase:** v3.0 VHD module close (surfaced + fixed 2026-06-17, branch `feat/vhd-chunk1-ar-severity`). Canonical-grep-first / V.5-RECOVERY catch per AUDIT_METHODOLOGY.md §17.1.
- **Severity:** LOW (P3). Canonical-pipeline metadata defect (a cite that could not be rebuilt), not a runtime detection defect - the SH-022 evaluator fires correctly; only the crosswalk cite-pairing was broken. Mitigated synthetic/pre-DUA.
- **Status:** RESOLVED 2026-06-17 (v3.0 VHD close).
- **Defect:** the v3.0 SH close renamed the tricuspid-assessment evaluator comment SH-4 -> `// Gap SH-022:` (AUDIT-125 severity-gating tightening) but left the registry id `gap-sh-4-tricuspid-assessment`. The reconcile similarity-matcher could not pair the SH-022 evaluator block to the gap-sh-4 registry id (token mismatch), so `gap-sh-4-tricuspid-assessment` surfaced as a regOrphan and the GAP-SH-022 override (DET_OK, registryId `gap-sh-4-tricuspid-assessment`) could not rebuild its cite -> `applyOverrides` demoted it to SPEC_ONLY with an "Investigate" marker. The SH source was byte-identical to origin/main, so the demotion was pre-existing on main (the SH close left it latent); it surfaced here because the VHD close re-ran the full §9.2 pipeline and the applyOverrides demoted-count test (demoted=0) caught it.
- **Impact:** silent over-write of a genuine DET_OK cite to SPEC_ONLY in the SH crosswalk (a cite that should pair but did not). No runtime impact (the evaluator fires); the defect is in the canonical classification metadata.
- **Fix:** migrated the registry id `gap-sh-4-tricuspid-assessment` -> `gap-sh-022-tricuspid-assessment` on both source refs (`gapRuleEngine.ts` registry entry + `applyOverrides.ts` GAP-SH-022 override registryId) so the cite pairs and naming matches the evaluator + spec GAP-SH-022. Re-ran the full pipeline: SH demoted=0, the regOrphan resolved, validateCanonical 6/6. Separately, the chunk-3 SH-012 superseded marker (`// Gap SH-012:`) was de-tokenized to `// SUPERSEDED ...` so extractCode no longer parses the retired marker as an evaluator block (evalOrphan -> 0), mirroring the clean SH-9 / SH-ASD retirements.
- **Cross-references:** AUDIT-125 (the SH-022 severity-gating tightening that renamed the evaluator); AUDIT-167 / AUDIT-169 (SH-12 / SH-012 retirements); `applyOverrides.ts` GAP-SH-022 override; `gapRuleEngine.ts` `gap-sh-022-tricuspid-assessment`; AUDIT-110 (the line-pinned-assertion brittleness class); the V.5-RECOVERY discipline (§17.1 / §19.5).

---

### AUDIT-172 - Cross-module IE / AF-anticoagulation overlap reconciliation at the VHD close (VHD-083/057/059 vs VD-12 / EP-008 / SH-029)

- **Phase:** v3.0 VHD module close - overlap reconciliation (operator-directed, 2026-06-17, branch `feat/vhd-chunk1-ar-severity`).
- **Severity:** MEDIUM (P2). The rheumatic-AF arm carries a patient-safety driver: a generic "may use DOAC" recommendation co-firing with the warfarin mandate could be acted on as a DOAC, which is contraindicated in rheumatic MS (INVICTUS). Mitigated synthetic/pre-DUA.
- **Status:** RESOLVED 2026-06-17 (folded into PR #406 before merge).
- **Defect:** the v3.0 VHD buildout added granular gaps (VHD-083 rheumatic-AF warfarin mandate; VHD-057/059 IE surgical indications) that overlapped pre-existing broader rules:
  - **VHD-083 vs VD-12 (`gap-vd-12-af-valve-anticoag`) + EP-008 (`gap-ep-008-doac-mitral-stenosis`):** a rheumatic-MS + AF patient (I05.x + I48 + a co-coded I34-I37 valve dx) could trigger VHD-083 (warfarin) AND VD-12 ("may use DOAC", generic) AND EP-008 (switch DOAC->warfarin). VD-12's generic DOAC-permitting note is unsafe in rheumatic MS (DOAC contraindicated per INVICTUS / 2020 ACC/AHA VHD).
  - **VHD-057/059 vs SH-029 (`gap-sh-029-ie-early-surgery`):** SH-029 lumped IE + (HF | embolic | uncontrolled-infection) into one surgery gap; its HF arm (I33.0 + I50) was an exact duplicate of the new granular VHD-057.
- **Fix:**
  - **Reconciliation 2 (safety):** VHD-083 OWNS the rheumatic-AF anticoagulation recommendation. VD-12 now excludes rheumatic mitral disease (`hasRheumaticMitral_VD12 = dxCodes.some(c => c.startsWith('I05'))`); EP-008 narrowed its MS set to nonrheumatic only (`I34.2`, dropping rheumatic `I05.0`). Result: a rheumatic-MS+AF patient gets ONE coherent fire (VHD-083, warfarin). Guard tests confirm the single-fire AND that non-rheumatic AF+valve still fires VD-12 (no over-narrowing) and nonrheumatic MS+DOAC still fires EP-008.
  - **Reconciliation 1:** SH-029 read = general/native IE (I33.0), NOT structural-device-specific. Scope-narrowed: the HF arm is REMOVED (VHD-057 owns IE+HF); the embolic arm is RETAINED (broader than VHD-059, which gates on on-anticoagulation - removing it would lose the embolic-not-anticoagulated population) and the uncontrolled-infection arm (A41/R65.2) is RETAINED (unique to SH-029). A benign residual overlap with VHD-059 on the anticoagulated-embolic subset remains (consistent surgery-eval recommendation; non-harmful).
- **Classification effect:** none (evaluator conditions narrowed; no DET_OK/PARTIAL/SPEC_ONLY tier changed - confirmed by the full §9.2 pipeline: VHD 12/13/80, EP 42/16/31, SH 31/18/39 unchanged).
...
- **Status:** RESOLVED 2026-06-18 (CAD chunk 0). Split into gap-cad-rehab-cabg (Z95.1) + gap-cad-rehab-mi (I21/I22/I25.2); regression + no-over-narrowing tests in cadChunk0.test.ts. Both re-cited DET_OK. The CABG/MI procedure gate is the load-bearing over-detection fix. The secondary rehab-engagement guard was DROPPED per the proxy investigation (2026-06-18): ICD-10-CM has no cardiac-rehab-participation code (it is a service, not a diagnosis; Z48.812 surgical-aftercare / Z71.82 exercise-counseling are confounders), CPT 93797/93798 are absent from all synthetic/seed/test data (inert pre-DUA), and Synthea codes procedures as SNOMED CT, not CPT - so no proxy is data-backed today. Path-B: a guard reading BOTH CPT 93797/93798 AND the SNOMED cardiac-rehab procedure code is added post-DUA; until then the residual false-positive (re-recommending rehab to an already-enrolled patient) is an accepted low-harm limitation. This removes the §16.7 CPT-confirm question.
...
- **Status:** RESOLVED 2026-06-18 (CAD chunk 0). CAD-061 reclassified SPEC_ONLY (disjoint-target miscite per §16.6(ii); registryId dropped). Gap-50's gate changed `hasStentOrCAD = hasCAD || Z95.5` -> `hasDaptIndication = I21/I22 || Z95.5` so stable CAD no longer fires; guard tests in cadChunk0.test.ts confirm stable-CAD no-fire + post-ACS/post-PCI fire.
...
- **Status:** RESOLVED 2026-06-18 (CAD chunk 0). Both evaluator blocks AND their registry entries (gap-cad-nicorandil, gap-cad-trimetazidine) removed cleanly. The pre-existing clinicalCodeCorrections.test.ts reference to 47832 is unaffected (it tests the nitroglycerin guard, asserting 47832 is a distinct non-nitroglycerin code). cadChunk0.test.ts confirms neither gap fires. gaps.push 371 -> 370 (with the CAD-REHAB split net).
...
- **Status:** RESOLVED 2026-06-18 (CAD chunk 0). Added `onPrasugrelACS = medCodes.includes('613391')` and gated `!onTicagrelorACS && !onPrasugrelACS`; the evidence object already listed "On prasugrel" as an exclusion (evidence-vs-logic mismatch now reconciled). Recommendation reframed as the potent-P2Y12 choice (ticagrelor or prasugrel) over clopidogrel. Tests confirm prasugrel/ticagrelor no-fire; clopidogrel/none still fire.
...
- **Status:** RESOLVED 2026-06-18 (CAD chunk 0). All three reclassified DET_OK -> PARTIAL via applyOverrides (honest underclaim). Threaded-data check: the only "months-since" interval slugs threaded are coronary_cta_months / echo_months / graft_duplex_months / stress_test_months - there is NO lipid-panel-date (CAD-007), DAPT-start-date (CAD-018), or borderline-lesion-severity (CAD-068) threaded, so none can reach the spec interval/lesion target or DET_OK. Held PARTIAL; documented Path-B.
...
### AUDIT-178 - PV over-credit cluster: 4 PV rules classified DET_OK while firing on existence-proxies / missing gates (HbA1c, resistant-HTN, varicose subcodes, graft interval)

- **Phase:** PV legacy-coverage audit (read-only) -> PV chunk 0 buildout (2026-06-18, branch `feat/pv-chunk0-tightenings`).
- **Severity:** MEDIUM (P2). Over-detection in 4 live rules (false-positive gaps surfaced to clinicians); mitigated synthetic/pre-DUA. Same class as AUDIT-177 (CAD over-credits).
- **Status:** RESOLVED 2026-06-18 (PV chunk 0). All four tightened to genuine gates; regression + no-over-credit tests in pvChunk0.test.ts; applyOverrides PV entries pin DET_OK with the tightening provenance.
- **Defect/gap:** four PV rules were credited DET_OK but did not genuinely gate their spec population: (1) **PV-6 / GAP-PV-015** (HbA1c control in PAD+DM) fired on `labValues['hba1c'] === undefined` - an existence-proxy for "not measured", not the spec "A1c above target" gap (always-fire for any unmeasured patient, the AUDIT-134 class); (2) **PV-12 / GAP-PV-033** (renal-artery-stenosis screen in resistant HTN) fired on PAD + I10 + eGFR<60 with NO resistant-HTN signal despite "resistant HTN" in the recommendation text; (3) **PV-VARICOSE / GAP-PV-071** (spec CEAP 3+ symptomatic) fired on bare `I83` including asymptomatic `I83.9`; (4) **PV-GRAFT-SURVEILLANCE / GAP-PV-098** fired on `graft_duplex_months === undefined` with no interval comparison despite the value being threaded.
- **Fix:** (1) PV-6 -> `hba1c !== undefined && hba1c >= 7.0%` (standard ADA/ACC glycemic target; type MONITORING_OVERDUE -> MEDICATION_NOT_OPTIMIZED); (2) PV-12 -> require `>= 3` concurrent antihypertensive classes (RAAS + beta-blocker + CCB + diuretic), section-16-verified codes (nadolol 7226, diltiazem 3443, verapamil 11170, spironolactone 9997), the operational apparent-treatment-resistant proxy (BP-control status not threaded -> Path-B documented); (3) PV-VARICOSE -> complicated/symptomatic subcodes `I83.0/I83.1/I83.2/I83.8` (ulcer/inflammation/pain), excluding asymptomatic `I83.9`; (4) PV-GRAFT-SURVEILLANCE -> overdue when `graft_duplex_months > 12` (past the annual interval) OR never documented, so a recently-surveilled patient (<= 12 mo) now gates out. All four hold DET_OK, now genuinely gated. Guarded by pvChunk0.test.ts (each fix has a regression + a no-over-credit assertion).
- **Cross-references:** `gapRuleEngine.ts` PV-6 / PV-12 / PV-VARICOSE / PV-GRAFT-SURVEILLANCE; AUDIT-177 + AUDIT-134 (existence-proxy / narrative-vs-logic precedents); AUDIT-179/180 (PV chunk 0 siblings); 2024 ACC/AHA PAD Guideline; ADA Standards of Care; 2024 SVS/AVF Varicose Vein Guidelines.

---

### AUDIT-179 - PV-003 ABI-value miscite (no rule read ABI <=0.90 - the module's missing foundation) + PV-076 post-PE rule fired only on DVT, missing the PE population

- **Phase:** PV legacy-coverage audit (read-only) -> PV chunk 0 buildout (2026-06-18, branch `feat/pv-chunk0-tightenings`). Section-16 + foundational build.
- **Severity:** MEDIUM (P2). One coverage gap (the module's foundational abnormal-ABI detection was absent) + one code-mismatch under-detection (PE population missed). Mitigated synthetic/pre-DUA.
- **Status:** RESOLVED 2026-06-18 (PV chunk 0). PV-003 built + re-cited DET_OK; PV-076 PE code added; both guarded by pvChunk0.test.ts.
- **Defect/gap:** (a) **GAP-PV-003** ("Abnormal ABI <=0.90 without coded PAD") was fuzzy-name-matched in the crosswalk to `gap-pv-3-antiplatelet` (a different clinical concept) and classified PARTIAL - but **no rule anywhere read an ABI VALUE threshold**. ABI was threaded (`labValues['abi_left'/'abi_right']`) yet used only as a presence flag (PV-2 ABI-screening). The single most foundational PAD-detection rule (a resting ABI <=0.90 is diagnostic of PAD, Class 1) was missing. (b) **PV-ANTICOAG-VTE / GAP-PV-076** (spec "post-PE anticoagulation duration") fired only on `I82` (DVT), **missing the PE population (`I26`) entirely** - the gap did not detect its own spec target.
- **Fix:** (a) built `gap-pv-003-abnormal-abi` (type DOCUMENTATION_GAP): `(abi_left <= 0.90 || abi_right <= 0.90) && !hasPAD` -> undiagnosed PAD, COR 1 / LOE B-NR per the 2024 ACC/AHA/SVM/SCAI Lower Extremity PAD Guideline. Subgroup-aware: a falsely-elevated ABI `>1.40` (non-compressible/calcified) is routed to the separate PV-004 gap, NOT conflated here; TBI confirmation for borderline (0.91-0.99) is not threaded (Path-B). Re-cited GAP-PV-003 -> the new rule (applyOverrides), PARTIAL -> DET_OK. Added a registry entry `gap-pv-003-abnormal-abi`. (b) added `I26` to the VTE rule trigger (`hasVTE = I26 || I82`); I82 retained since VTE duration review applies to both. Guarded by pvChunk0.test.ts (PV-003: fires abi 0.70/0.90, gates abi 1.00, routes abi 1.50 to PV-004, gates coded-PAD, null-Path no-fire; PV-076: PE I26.99 fires, DVT I82.401 retained, no-OAC gates).
- **Cross-references:** `gapRuleEngine.ts` PV-003 (`gap-pv-003-abnormal-abi`) / PV-ANTICOAG-VTE; PV-2 (the presence-flag-only ABI rule); AUDIT-070 (ABI threading lineage); AUDIT-178/180 (PV chunk 0 siblings); 2024 ACC/AHA/SVM/SCAI Lower Extremity PAD Guideline; 2021 ASH / 2020 CHEST VTE Guidelines; NLM I26 (PE) / I25.110-class verification.

---

### AUDIT-180 - PV-BYPASS-EVAL over-match: one generic bypass-eval rule cited as the satisfying evaluator for 3 distinct CLTI/iliac decision gaps

- **Phase:** PV legacy-coverage audit (read-only) -> PV chunk 0 reconcile (2026-06-18, branch `feat/pv-chunk0-tightenings`).
- **Severity:** MEDIUM (P2). Over-credit (one rule claimed to satisfy 3 distinct spec gaps it does not encode); canonical-classification accuracy. Mitigated synthetic/pre-DUA.
- **Status:** RESOLVED 2026-06-18 (PV chunk 0). PV-017 kept; GAP-PV-018 + GAP-PV-024 reclassified PARTIAL -> SPEC_ONLY (applyOverrides PV entries; cite dropped). Reconcile-only change (no runtime edit), so verified at the canonical close, not in pvChunk0.test.ts.
- **Defect/gap:** the generic `gap-pv-bypass-eval` rule (fires on PAD + I70.21-25) was cited by the fuzzy reconcile as the satisfying evaluator for THREE distinct decision gaps: **GAP-PV-017** ("CLTI: BEST-CLI-eligible heart-team discussion"), **GAP-PV-018** ("CLTI: endovascular vs surgical decision, BEST-CLI"), and **GAP-PV-024** ("TASC II C/D iliac: endovascular experienced operator or surgical bypass"). The generic rule does not encode the endovascular-vs-surgical decision logic (GAP-PV-018) or the TASC II lesion-class / iliac-specific signal (GAP-PV-024) - both need anatomic/conduit/lesion-class data not threaded.
- **Fix:** kept the single closest match (**GAP-PV-017**, the BEST-CLI heart-team-discussion gap - the bypass-eval rule's actual surface) at its existing classification; reclassified **GAP-PV-018** and **GAP-PV-024** PARTIAL -> SPEC_ONLY in applyOverrides with the cite dropped (no genuine evaluator). One evaluator may satisfy at most one spec gap unless it genuinely encodes each (the §16.6 disjoint-target discipline, the CAD-061 precedent).
- **Cross-references:** `gapRuleEngine.ts` PV-BYPASS-EVAL (`gap-pv-bypass-eval`); GAP-PV-017/018/024; `applyOverrides.ts` PV section; AUDIT-174 / CAD-061 (the miscite-to-disjoint-target precedent); AUDIT-178/179 (PV chunk 0 siblings); BEST-CLI trial; 2024 ACC/AHA PAD Guideline.

> Note: AUDIT-300 is RESERVED for the UI/frontend-track clinical-content-leak finding. STANDING CONVENTION: UI/frontend-track findings allocate from a HIGH reserved range (AUDIT-300+), clear of the clinical track's sequential low range, to prevent id collisions - this finding collided twice on next-free-id (171, then 178) before reserving 300; the clinical track keeps the low sequential range and honors AUDIT-300+.
...
- **Status:** RESOLVED 2026-06-18 (CAD chunk 1 close). The CAD-IVUS firing + the wrong `hasLeftMain = I25.110` gate were REMOVED (de-tokenized retirement comment so extractCode does not parse a phantom; gap-cad-ivus registry retained as a regOrphan for lineage). CAD-069/070 (cited the retired rule) reclassified PARTIAL -> SPEC_ONLY; CAD-071 holds SPEC_ONLY/Path-B (no left-main ICD - I25.110 = unstable angina; only I21.01 acute-STEMI-of-left-main exists; chronic left-main is an angiographic finding, not threaded). Regression test in cadChunk1Batch.test.ts: an I25.110 patient does NOT fire a left-main IVUS recommendation. 3-surface flipped.
...
### AUDIT-183 - PV chunk-1 close (6th + FINAL module): 7 buildable gaps built; 3 of the banked-audit "~10 buildable" are threading/code-blocked (over-assessment correction)

- **Phase:** PV chunk 1 buildout + canonical close (2026-06-18, branch `feat/pv-chunk0-tightenings`). The 6th and FINAL clinical module - completes the all-6 canonical buildout.
- **Severity:** LOW (P3). The 7 built gaps are net-additive coverage; the 3 Path-B's are a scoping/threading correction with no runtime defect (the gaps were never built, so no over-credit shipped). The 3 are blocked-pile / OPEN-DEFERRED with unblock paths. Mitigated synthetic/pre-DUA.
- **Status:** 7 built RESOLVED 2026-06-18; 3 Path-B sub-items OPEN-DEFERRED (blocked-pile, feed the post-modules re-audit).
- **Built (7, section-16 NLM-verified, subgroup-aware, COR matches recommendation):** GAP-PV-004 non-compressible ABI >1.40 -> TBI (PARTIAL - TBI-absent not threaded; completes the PV-003/004 ABI pair, disjoint ranges); GAP-PV-034 FMD screening in young (<35) hypertensive women (PARTIAL - imaging-screening status proxied by !I77.3); GAP-PV-038 Takayasu (M31.4) immunosuppression gap (DET_OK); GAP-PV-040 GCA (M31.5/M31.6) urgent glucocorticoid gap (DET_OK; distinct from Takayasu); GAP-PV-041 Buerger (I73.1) + tobacco cessation-is-treatment (DET_OK); GAP-PV-058 symptomatic carotid (I65.2x + I63/G45) revasc evaluation (PARTIAL - SYMPTOMATIC arm only, >=70% severity not codable); GAP-PV-062 symptomatic intracranial stenosis (I67.2 + event) SAMMPRIS aggressive medical, NOT stenting (DET_OK).
- **The over-assessment correction (3 Path-B, same class as AUDIT-181 ApoB over-assumption):** the banked PV legacy audit listed ~10 buildable; §16/threading verification found 3 over-assumed threading and they were NOT built (avoiding the AUDIT-177 always-fire-proxy class): **GAP-PV-021** (WIfI staging) - `wifi_score` is a `type:string` csvSchema column never wired to `labValues` (`Record<string,number>`, populated from the patientWriter `labFields` allowlist), so `labValues['wifi_score']` is always undefined; **GAP-PV-037** (Takayasu serial ESR/CRP) - neither ESR nor CRP is threaded (no observationService FHIR LOINC->slug mapping, not in the CSV `labFields`); **GAP-PV-060** (post-CEA/CAS antiplatelet+statin) - no clean post-carotid-revascularization ICD-10 code (Z95.820 = peripheral angioplasty status, not carotid; CEA leaves no implant).
- **Unblock paths (blocked-pile, post-modules re-audit):** PV-021 + PV-037 = a lab/score-threading PR (AUDIT-070 class - map WIfI to a numeric/staged observation; map ESR + CRP LOINCs both FHIR + CSV paths); PV-060 = a procedure-signal floor (CPT 35301 CEA / 37215-37216 CAS) once procedure threading covers carotid revasc, OR a Z-code convention. Recorded in `DUA_DEFERRED_GAP_REGISTER.md` (Tranche 2/3 lab+score-threading + code-floor pointer).
- **PV final counts (of 105):** 21 DET_OK + 14 PARTIAL + 70 SPEC_ONLY = 35 active (33.3% any-coverage). **ALL-6-MODULES platform (of 603):** 201 DET_OK + 114 PARTIAL + 288 SPEC_ONLY = 315 covered = 52.2% any-coverage. validateCanonical 6/6 VALID. [AUDIT-183-close figures 2026-06-18; the split has since shifted to 205 DET_OK + 106 PARTIAL + 292 SPEC_ONLY = 311 covered = 51.6% (DET_OK +4, PARTIAL -8 across AUDIT-191/194-B1; total 315 -> 313 AUDIT-194-B1 -> 311 AUDIT-195 GAP-CAD-004/005 -> SPEC_ONLY). Current derived-from-crosswalk truth per PHASE_0B_CROSS_MODULE_SYNTHESIS.md.]
- **Cross-references:** `gapRuleEngine.ts` PV-004/034/038/040/041/058/062 (gap-pv-* evaluators); `applyOverrides.ts` PV section (18 overrides, demoted=0); `pvChunk1.test.ts` (29 assertions); AUDIT-181 (the ApoB threading over-assumption precedent); AUDIT-070 (FHIR-ingestion threading-gap parent class); AUDIT-177 (the always-fire-proxy class avoided); `DUA_DEFERRED_GAP_REGISTER.md`; NLM I77.3 / M31.4 / M31.5 / M31.6 / I73.1 / I65.2x / I67.2 / I63 / G45; RxNav tocilizumab 612865.

---

### AUDIT-184 - Hollow DET_OK: 22 DET_OK rules gated on lab/vital slugs threaded in NEITHER path (14 silent never-fire + 8 live over-fire); repaired by the T1 slug-thread

- **Phase:** Full-spec re-audit hollow-DET_OK integrity gate -> T1 slug-thread + hollow repair (2026-06-19, branch `feat/t1-slug-thread-and-hollow-repair`). Surfaced by the re-audit's threading-inventory sweep (PV-6/CAD-CRP flag generalized to an exhaustive cross-reference).
- **Severity:** HIGH (P1). The 8 over-fire rules shipped FALSE-POSITIVE gaps on real cohorts in production (task-def :315): a clinical-accuracy / patient-safety-class defect. The 14 silent rules were 0%-sensitivity (a DET_OK that detects nothing). Honest functional DET_OK was ~179, not the claimed 201.
- **Status:** RESOLVED 2026-06-19 (PART 1+2 of the T1 batch) - all 12 slugs threaded both paths; the 8 over-fires verified CLOSED + the silent rules now fire/gate. Guarded by hollowDetokThreading.test.ts (45) + hollowDetokRepair.test.ts (19).
- **Defect:** the engine reads 62 distinct `labValues` slugs; 33 were threaded in NEITHER the FHIR path (`observationService ECHO_LOINC_TO_SLUG`) NOR the CSV path (`patientWriter labFields` + `csvSchema`), so they are always `undefined` at the engine. 22 currently-DET_OK rules gate on one of these. TWO failure modes: (A) SILENT never-fire (14) - the gate REQUIRES the value (`labValues['x'] !== undefined && ... threshold`), so with x unthreaded the rule never fires: HF-003 (heart_rate+systolic_bp), HF-032/079 (hemoglobin), HF-065/078 (heart_rate), HF-080 (tsh), HF-081 / PV-015 (hba1c), EP-004/005 (creatinine), EP-030/033 (heart_rate), CAD-023 (crp>2), CAD-031 (stress_test_months). (B) OVER-FIRE existence-proxy (8) - the gate fires when `labValues['x'] === undefined` (proxy for "not measured"), so with x unthreaded it fires for EVERY patient in the cohort (the AUDIT-177 always-fire class, but LIVE): CAD-008 (a slug-name bug - read `lipoprotein_a`, threaded slug is `lpa`), CAD-010 (crp/hs_crp), CAD-047 (ccta), CAD-059 (cac_score), EP-043/044 (tsh/alt/ast), EP-048 (creatinine), PV-098 (graft_duplex_months).
- **Root cause:** rules were authored reading a slug without verifying it was threaded on either ingestion path (codebase-trust, not threading-verified - the AUDIT-070 class). The threading-inventory was never a per-rule authoring gate.
- **Fix (PART 1 thread):** threaded 12 slugs both paths, all LOINCs NLM-verified by exact-code lookup - heart_rate 8867-4, systolic_bp 8480-6, diastolic_bp 8462-4, hemoglobin 718-7, hba1c 4548-4, tsh 3016-3, creatinine 2160-0, crp 1988-5, hs_crp 30522-7, alt 1742-6, ast 1920-8, lpa 10835-7 (FHIR map + COMMON_COLUMNS + labFields); cac_score / stress_test_months / ccta / graft_duplex_months CSV-only (derived/temporal/presence, no verifiable LOINC, FHIR omitted per the LVESD no-guess rule). **(PART 2 repair):** CAD-008 slug-name canonicalized `lipoprotein_a` -> `lpa`; all other rules were logic-correct (threading alone repairs them - the silent rules now receive data and fire; the over-fire proxies now gate out a patient whose value is present). hollowDetokRepair.test.ts proves each of the 8 over-fires GATES when the value is present (the live-prod safety fix) and FIRES only for the genuine gap.
- **Honest-count correction:** 201 claimed DET_OK -> ~179 functional before this fix -> a real 201 functional after (the 22 now work as classified). The T1 NEW gaps (35, PART 3) are additive on top.
- **Self-catch (recurrence):** GAP-PV-098 was one of the 8 over-fires and was MY OWN PV chunk-0 work (AUDIT-178) - I added the `graft_duplex_months === undefined || > 12` interval without verifying `graft_duplex_months` was threaded, so it shipped an always-fire over-credit. Logged as DRIFT-50 (unthreaded-slug-read at authoring, recurrence of the AUDIT-070/177 class). This finding's hollow-DET_OK threading-inventory check is the prevention.
- **Cross-references:** `observationService.ts ECHO_LOINC_TO_SLUG`; `patientWriter.ts labFields/getUnit/getObservationName`; `csvSchema.ts COMMON_COLUMNS`; `gapRuleEngine.ts` the 22 rules; AUDIT-070 (FHIR-ingestion threading-gap parent) / AUDIT-170 (INR both-paths) / AUDIT-181 (ApoB both-paths) / AUDIT-177 (always-fire-proxy class); DRIFT-50; NLM LOINCs above.

---

### AUDIT-185 - T0 net-new batch (6 built) + the honest-count reckoning: the re-audit's "~78-DET_OK free T0 sweep" was inflated; the real net-new was 6

- **Phase:** T0 net-new buildout + close (2026-06-19, branch `feat/t0-authoring-sweep`). Follows the full-spec re-audit + the AUDIT-184 hollow repair.
- **Severity:** LOW (P3). Net-additive build (6 new DET_OK) + a scoping-accuracy finding for the v-next coverage baseline; no runtime defect. The CTEPH code self-catch was caught pre-merge (no wrong code shipped).
- **Status:** RESOLVED 2026-06-19. 6 built DET_OK; validateCanonical 6/6; real 201 -> 207 DET_OK.
- **Built (6, dx+med only, all codes NLM/RxNav exact-verified):** GAP-PV-042 (Behcet M35.2 + vascular thrombosis I82/I80 -> immunosuppression, EULAR 2018 COR 1); GAP-PV-081 (CTEPH I27.24 -> riociguat 1439816, CHEST-1 COR 1); GAP-PV-084 (Group-1 PAH I27.0/I27.21 -> ERA+PDE5i combination, AMBITION COR 1); GAP-PV-085 (PAH on background combination -> sotatercept 2678930 add-on, STELLAR COR 2a; mutually exclusive with PV-084); GAP-EP-010 (rivaroxaban 1114195 -> take-with-food counseling, FDA PI COR 1); GAP-EP-049 (SAFETY: flecainide 4441 / propafenone 8754 + structural HD I25/I21/I22/I42/I50 -> CAST contraindication, SAFETY_ALERT COR 3 Harm). Guarded by t0NetNew.test.ts (19).
- **The honest-count reckoning (the load-bearing finding):** the full-spec re-audit's optimistic "T0 = 121 gaps / ~78 free DET_OK" was DOUBLY inflated: (1) **built-vs-buildable conflation** - it counted gaps that gate on threaded data INCLUDING the ones already DET_OK (each module's DET_OK surface was already authored on the available threaded data), so the true net-new = SPEC_ONLY -> DET_OK, not the full threaded-gateable set; (2) **the hollow-check removes most SPEC_ONLY** - of the 288 SPEC_ONLY gaps the overwhelming majority need an unthreaded element (unsourced CPT, Synthea-extension value, temporal/serial, imaging morphology, genetics, telemetry, free-text - the DRIFT-50-forbidden classes). The genuine net-new collapsed to ~12-20, then to **6** after 4 hollow-check/section-16 disqualifications (PV-086 WHO-FC unthreaded; SH-079 no clean aortitis dx; EP-084 no SQTS ICD; EP-085 no early-repolarization ICD) + the **CTEPH code self-catch** (a mid-stream "correction" of the spec's correct I27.24 to a wrong I27.22 was reverted by exact-code re-verify). Implication for the v-next baseline: **there is no large free T0 win**; the real coverage levers all require an infra batch (T3 CPT-sourcing ~60 across modules, T1-broader threading ~25, T2 Synthea ~50 - reachable but real work, not a sweep). Honest trajectory: real 207 now, climbing through named infra batches.
- **Cross-references:** `gapRuleEngine.ts` gap-pv-042/081/084/085 + gap-ep-010/049; `applyOverrides.ts` PV/EP T0 entries; `rxnormIngredientMap.ts` (7 PAH/CTEPH INs); `t0NetNew.test.ts`; AUDIT-184 (the hollow-check that this batch applied per-authoring) / AUDIT-181 (the ApoB over-assumption precedent) / DRIFT-50 (the per-authoring threading gate); NLM I27.24 (CTEPH, exact-verified) / M35.2 / I27.0 / I42; RxNav ambrisentan 358274 / bosentan 75207 / macitentan 1442132 / sildenafil 136411 / tadalafil 358263 / riociguat 1439816 / sotatercept 2678930 / flecainide 4441 / propafenone 8754.

### AUDIT-186 - T1-broader LVESD batch (the first named infra batch per AUDIT-185): +4 net-new DET_OK + 2 sensitivity/specificity enhancements + 3 correctly-Path-B; the AR COR fork is the load-bearing clinical catch

- **Phase:** T1-broader lab-threading buildout + close (2026-06-22, branch `feat/t1-broader-lvesd`). The first of the AUDIT-185 named infra levers (T1-broader threading). Follows the T0 net-new close (AUDIT-185, real 207).
- **Severity:** LOW (P3). Net-additive build (4 new DET_OK) + 2 clinical-sensitivity enhancements of existing DET_OK rules + an AR COR-mismatch catch corrected at design time; no runtime defect shipped. Three candidate singles correctly NOT built (Path-B).
- **Status:** RESOLVED 2026-06-22. 4 built DET_OK; validateCanonical 6/6; real 207 -> 211 DET_OK. PR pending.
- **Threading (5 slugs):** `lvesd` / `tapse` / `fac` / `vegetation_size` CSV-only (echo/TEE-derived, NLM has no clean simple-measurement LOINC -> FHIR map omitted per the no-guess rule, cac_score precedent); `anti_xa` threaded BOTH-paths (LOINC 31159-7 "Heparin anti Xa", NLM exact-code-verified). All in csvSchema (COMMON: lvesd; SH: tapse/fac; VD: vegetation_size/anti_xa) + patientWriter labFields/getUnit/getObservationName; anti_xa also in observationService ECHO_LOINC_TO_SLUG.
- **The LVESD finding (the sixth built-vs-buildable catch):** threading LVESD itself yielded **0 direct net-new**. All three LVESD-gated spec gaps already fire on another threaded value, so LVESD adds sensitivity/specificity, NOT count: **SH-014** primary MR (already DET_OK on LVEF<=60) - added the LVESD>=40 mm LV-dilation arm (Class 1, 2020 ACC/AHA VHD), a sensitivity enhancement that catches the preserved-EF-but-dilated-LV patient the LVEF gate missed; **SH-018** COAPT secondary-MR TEER (already DET_OK on EROA>=0.30) - added the LVESD<=70 mm COAPT-eligibility exclusion, a specificity tightening; **GAP-VHD-103** AR (already DET_OK on LVEF<=55 as the Class-1 arm; was PARTIAL because the LVESD arm was unthreaded) - now PARTIAL -> DET_OK via a 2nd push for the LVESD>50 arm. SH-007 ("LVESD increase on serial echo") was excluded as temporal/Path-B.
- **The AR COR fork (the load-bearing clinical catch):** for asymptomatic severe AR the 2020 ACC/AHA VHD splits by EF - **LVEF<55% = Class 1** (Stage C2, the existing VHD-103 arm) but **preserved-EF + LVESD>50 mm = Class 2a** (also Stage C2). Folding LVESD>50 into the Class-1 VHD-103 push would have mis-tagged a Class-2a trigger as Class-1 (a section-16/section-17.5 COR-mismatch defect). The fix: the LVESD arm is a SECOND push under the same spec gap GAP-VHD-103 carrying its own Class-2a evidence (mutually exclusive with the Class-1 arm on LVEF >55 vs <=55: no double-fire). The canonical VHD crosswalk had independently flagged this exact arm ("the spec surgical threshold is LVESD>=50 OR LVEF<=55%; LVESD is not threaded, so the evaluator covers the LVEF arm only"). The +1 net-new is the GAP-VHD-103 PARTIAL -> DET_OK flip.
- **PART 2 - the 3 SPEC_ONLY singles built (each COR section-16-verified BEFORE build; spec.json carries detectionLogic but no COR):** **GAP-SH-024** (severe TR I36.1 + RV systolic dysfunction TAPSE<17 mm or FAC<35% -> heart-team intervention-timing eval; Class 2a / LOE C-LD; TAPSE<17 = ASE RV-dysfunction cut; partitions from SH-022 on !congestion); **GAP-VHD-060** (IE I33.0 + isolated mobile vegetation >10 mm + no embolic event -> early surgery; Class 2b - the standalone-vegetation arm, distinct from VHD-059's Class-2a recurrent-embolism arm, partitions on !embolic); **GAP-VHD-100** (mechanical valve Z95.2/Z95.4 + pregnancy + on LMWH enoxaparin 67108/dalteparin 67109/tinzaparin 69646 + anti-Xa outside 0.8-1.2 U/mL -> dose adjustment; Class 1 / LOE B-NR; intentional non-redundant overlap with VHD-099 strategy review). Each was a pre-existing data-blocked SPEC_ONLY override now lifted.
- **The 3 NOT built (Path-B, the SPEC_ONLY-verify gate working):** GAP-HF-153 tacrolimus ("level within past 3 months" = temporal/serial); GAP-HF-052 FLC + GAP-HF-134 SPEP ("never measured" / "workup not ordered" = existence-proxy `=== undefined`, the DRIFT-50 over-fire mode + under-threaded amyloid red-flags; building them would re-create the AUDIT-184 hollow defect).
- **Cross-references:** `gapRuleEngine.ts` gap-sh-024-tr-rv-dysfunction / gap-vhd-060-ie-large-vegetation / gap-vhd-100-mech-valve-pregnancy-antixa + the GAP-VHD-103 LVESD 2nd-arm push + the SH-014/SH-018 LVESD arms; `csvSchema.ts` + `patientWriter.ts` + `observationService.ts` (5-slug threading); `rxnormIngredientMap.ts` (3 LMWH INs); `applyOverrides.ts` GAP-SH-024/VHD-060/VHD-100/VHD-103 DET_OK; `t1BroaderLvesd.test.ts` (15) + `t1BroaderPart2.test.ts` (16); AUDIT-185 (the named-infra-batch trajectory this realizes) / AUDIT-184 (the hollow-defect class the 3 Path-B singles avoid) / DRIFT-50 (the per-authoring threading gate); NLM I33.0 / Z95.2 / Z95.4 / Z33.1 (exact-verified) + LOINC 31159-7 (anti-Xa, exact-verified); RxNav enoxaparin 67108 / dalteparin 67109 / tinzaparin 69646; 2020 ACC/AHA VHD (MR LVESD>=40=Class1, AR LVESD>50=Class2a, COAPT LVESD<=70, TR Class2a, IE veg>10mm Class2b, mech-valve-pregnancy anti-Xa 0.8-1.2 Class1).

### AUDIT-187 - Freemium service-line ROI: 2 double-count lever pairs in the Revenue Recovery Calculator + ROI fully decoupled from real gap data + 1 undisclosed backend hardcoded-revenue constant

- **Phase:** Freemium-MVP backend-slice review (2026-06-22, read-only; the UI chat owns the rendered surface). Verification of the service-line ROI per the operator's 3 questions: (a) double-count, (b) fabricated constant dressed as projection, (c) rollup respects DET_OK/PARTIAL/SPEC_ONLY.
- **Severity:** MEDIUM (P2). No clinical/PHI risk. A health-system CMO is the audience; an over-stated "Total Estimated Opportunity" that double-books dollars + a backend revenue figure with false data-provenance are credibility/trust risks for the freemium hook. Read-only finding; no code changed.
- **Status:** BACKEND PORTION RESOLVED 2026-06-22 (verdict b backend constant fixed + a 2nd identical instance found and fixed - see Backend resolution below). Frontend portions (verdict a lever double-count + claimsData duplicate dollars + cross-surface consistency) remain OPEN on the UI-track (alongside AUDIT-099 silent-mock/fabricated-KPI lineage + AUDIT-171 UI-chat).
- **Premise correction (the framing catch):** the operator's described model - "service-line freemium MVP reads stored gap data + computes 4 ROI pathways (DRG capture, specialty-pharmacy retention, avoided readmission, risk adjustment)" - does NOT exist as described. There is NO backend ROI engine. The actual freemium ROI is a FRONTEND what-if calculator (`src/components/free-tier/sections/RevenueRecoveryCalculator.tsx`) over 4 hardcoded levers - **readmission / gdmt / referral / ablation** (NOT DRG/pharmacy/readmission/risk-adjust) - decoupled from the real gap data entirely. Specialty-pharmacy-retention and risk-adjustment pathways are not built at all.
- **(a) DOUBLE-COUNT - YES, 2 overlapping lever pairs in `totalOpportunity = LEVERS.reduce(sum + calcOpportunity)` (`RevenueRecoveryCalculator.tsx:227`):** (1) **readmission ($340K/%) overlaps gdmt ($120K/%)** - the readmission lever's $340K explicitly bundles "penalty savings PLUS quality bonus uplift" while the gdmt lever's $120K IS "quality bonus payments", and gdmt's own explanation books "reduces downstream costs from preventable hospitalizations" (i.e. fewer readmissions) - the GDMT->readmission->penalty dollar is monetized by BOTH levers; (2) **referral ($420K/%) overlaps ablation ($310K/%)** - the ablation lever's explanation literally attributes its growth to "referral optimization" ("the $310K figure reflects a typical 2.5-case per quarter improvement FROM REFERRAL OPTIMIZATION"), so the recovered referrals the referral lever books as $420K are the SAME cases the ablation lever books as $310K. The headline summed total over-states by these overlaps. SECONDARY surface: the legacy `src/data/claimsData.ts` books the identical DRG-upcoding dollars in BOTH `revenueCycleData.drgOptimization.potentialUpcoding.revenueImpact` (2340, 4680) AND `cdiAlerts[].impactAnalysis.reimbursementImpact` (4680, 2340) - same $ on two surfaces (mock data, not currently summed, but a double-count trap if rolled up).
- **(b) FABRICATED CONSTANTS - mostly DISCLOSED on the frontend, ONE undisclosed on the backend:** every ROI dollar is a hardcoded literal (lever `dollarPerUnit` $340K/$120K/$420K/$310K; `free-tier/data.ts` TOP_OPPORTUNITIES "$2.4M/$1.8M/$3.2M/$1.1M" + PLATFORM_TOTALS; all of `claimsData.ts`). The frontend calculator is HONESTLY captioned ("Based on CMS national benchmarks for facilities your size - connect your EHR to verify", "Annualised estimate - connect EHR data to verify") with a grounded per-lever `explanation` - an acceptable disclosed-assumption pattern, NOT the hollow defect. The genuine hollow instance is in the BACKEND: `backend/src/routes/modules.ts:1768-1769` returns hardcoded `qualityMetrics: { ffrUtilization: '68.4%', benchmark: '75%', improvementOpportunity: '+$1.2M annual revenue with optimal FFR utilization' }` + `revenueImplication` from a `source: 'database'` endpoint WITHOUT the frontend's estimate/connect-EHR disclaimer - a fabricated figure with false data-provenance. Minor internal inconsistency: the ablation lever's `dollarPerUnit` $310K contradicts its own explanation's "~$124K per additional case per quarter".
- **(c) ROLLUP INTEGRITY - the ROI never reads gap data, so it cannot respect classification; the real gap-count rollup counts PARTIAL as fully-detected:** the calculator is pure slider x benchmark - it does NOT consume the backend gap counts, so DET_OK/PARTIAL/SPEC_ONLY is N/A by-construction (the "connect EHR to verify" path that would wire ROI to gaps is not built). Separately, the real backend dashboard rollup (`modules.ts` `totalOpenGaps = openGapsByType.reduce(...)`) counts stored `TherapyGap` rows: SPEC_ONLY gaps have no evaluator so are never stored (correctly absent, NOT over-counted - good), but DET_OK and PARTIAL rows are counted IDENTICALLY - a PARTIAL gap (crosswalk-flagged as under-claiming / not fully detecting its spec target) is counted as a fully-detected gap. The service-line analog of the hollow-DET_OK concern, in milder form (no SPEC_ONLY inflation, but PARTIAL-as-DET_OK undifferentiation).
- **Mapped ROI data flow:** stored `TherapyGap` -> `backend/src/routes/modules.ts` dashboard route -> real COUNTS by module (`source: 'database'`, DET_OK+PARTIAL undifferentiated, SPEC_ONLY absent) + scattered hardcoded revenue strings; SEPARATELY the freemium ROI (`RevenueRecoveryCalculator.tsx` LEVERS + `free-tier/data.ts` + `claimsData.ts`) = hardcoded benchmark constants x slider inputs, summed into `totalOpportunity`, NOT derived from the gap counts. The two surfaces never meet.
- **Recommended (deferred, not this read-only pass):** (1) de-overlap the levers - either gate gdmt's dollars to the non-readmission portion + ablation's to the non-referral portion, or render them as a non-summed menu rather than an additive `totalOpportunity`; (2) move the backend `modules.ts:1768-1769` hardcoded FFR/$1.2M figures behind the same "estimate / connect-EHR" disclosure the frontend uses, or compute them, or drop them; (3) when the "connect EHR" path is built, differentiate DET_OK vs PARTIAL in any gap-derived ROI rollup.
- **DRIFT-45 correction to verdict (b) (the fix-pass pre-flight caught the read-only finding's own imprecision):** the FFR figure is NOT on a `source: 'database'` endpoint. It sits on `POST /coronary-intervention/ffr-decision` - a per-patient decision-support POST whose response carries NO `source` field (the `source: 'database'` marker is on the dashboard GETs, e.g. :1810). So the "false data-provenance from a source=database endpoint" framing overstated it; the accurate defect is a hardcoded fabricated figure returned as `data` on a clinical decision endpoint, consumed by no client (grep: zero frontend references to `ffr-decision` / `ffrUtilization` / `improvementOpportunity`).
- **Backend resolution (2026-06-22, `fix/audit-187-backend-provenance`):** verdict (b) FIXED, plus a 2nd identical instance found during the fix. (1) **FFR** (`POST /coronary-intervention/ffr-decision`): dropped the fabricated `qualityMetrics` (`ffrUtilization '68.4%'` / `benchmark '75%'` / `improvementOpportunity '+$1.2M annual revenue'`) + the `revenueImplication` revenue-framing string; the decision logic was extracted to a pure, unit-tested `buildFfrDecisionSupport()` in `backend/src/routes/decisionSupport.ts` (modules.ts uses `export = router`, which blocks named exports inline) and the handler now returns only the clinical decision + a qualitative appropriate-use note (no figure). **Fix method = DROP** (compute infeasible - no persisted FFR-utilization field, and a per-patient POST is the wrong surface for a population KPI; disclosure would preserve meaningless content). (2) **WIfI** (`POST /peripheral-vascular/wifi-classification`, :1966): the SAME defect class - dropped the hardcoded `revenueImplication` block (`estimatedReimbursement '$58,000-$95,000' / '$14,500-$52,000'` + DRG-category ternary), also unconsumed. Tests: `backend/tests/api/ffrDecision.test.ts` (9 - decision-logic coverage + asserts no `qualityMetrics` / no `68.4` / no `1.2M` / no `$`-quantified figure). tsc clean; DRIFT-44 clean.
- **NEWLY SURFACED during the fix (NOT fixed here - separate scoping, likely UI-track wiring or its own finding):** `POST /heart-failure/gdmt-gaps` (`modules.ts` :374) returns a fully-mock response - hardcoded fake patient NAMES (`'Sarah Johnson'` / `'Jennifer Williams'` / `'Robert Chen'`) + fabricated `estimatedBenefit` dollars (`'$12,450/year'` etc.) + a fabricated `qualityMetrics` block (`overallOptimization: 74.3`, `improvement: '+12.1% vs last quarter'`). This is a larger whole-endpoint silent-mock (AUDIT-099 class + fabricated patient identifiers), beyond the per-patient hardcoded-$ defect resolved here; flagged for a dedicated decision -> NOW FILED as AUDIT-188 (2026-06-22, backend-track, MEDIUM/latent-HIGH per the consumption check: wired caller `careTeamConfig.tsx:94` but the handler discards the response). NOTE the `POST /valvular-disease/severity-classification` TAVR `qualityMetrics` (`deviceSuccess '>95%'`, `pacemakerRate '~7%'/'~17%'`) are legitimate clinical-literature benchmarks (PARTNER/Evolut, correctly SAPIEN-vs-Evolut varied), carry no revenue figure, and are NOT the same defect - left as-is.
- **Cross-references:** `src/components/free-tier/sections/RevenueRecoveryCalculator.tsx` (LEVERS :21-90, calcOpportunity :102, totalOpportunity :227); `src/components/free-tier/data.ts` (TOP_OPPORTUNITIES :191, PLATFORM_TOTALS); `src/data/claimsData.ts` (drgOptimization vs cdiAlerts dollar overlap); `backend/src/routes/modules.ts:1768` (backend hardcoded revenue) + the `totalOpenGaps` rollup; AUDIT-099 (silent-mock / fabricated-KPI on non-HF Executive views - same hollow lineage) / AUDIT-184 (hollow-DET_OK, the classification-integrity precedent) / AUDIT-171 (UI-chat freemium surface).

### AUDIT-188 - POST /heart-failure/gdmt-gaps is a whole-endpoint silent-mock: fabricated population stats + fabricated PATIENT IDENTIFIERS ('Sarah Johnson' + estimatedBenefit $) returned to a wired-but-stubbed frontend caller (MEDIUM now, latent HIGH)

- **Phase:** Backend-track scoping (2026-06-22, read-only; no fix). Surfaced during the AUDIT-187(b) backend fix (PR #418) when sweeping `modules.ts` for hardcoded figures; the operator scoped it here on the backend track.
- **Severity:** MEDIUM (P2) at rest, LATENT HIGH. Set by the consumption check below: the endpoint IS wired to a frontend caller, but that caller currently DISCARDS the response (stub handler), so the fabricated patient identifiers are NOT presently rendered. The moment the stub is completed to render `priorityPatients`, it becomes HIGH (fabricated patient names + dollar figures shown to a clinician/CMO as real). No PHI risk (the names are invented, not real patients) - but inventing patient identifiers is itself an AUDIT-300-class integrity defect.
- **Status:** RESOLVED 2026-06-22 (`fix/audit-188-gdmt-gaps-real-query`). Backend-track fix (the mock lived in the backend endpoint). Sibling to AUDIT-187 (same `modules.ts` sweep); lineage AUDIT-099 (silent-mock) + AUDIT-300 (fabricated-patient pure-removal precedent).
- **Resolution (2026-06-22):** `POST /heart-failure/gdmt-gaps` rewritten to a real, hospitalId-scoped `therapyGap` query (mirrors the HF dashboard GET; now returns `source: 'database'`). (1) PURE-REMOVED the fabricated payload per AUDIT-300 - the `'Sarah Johnson'`/`'Jennifer Williams'`/`'Robert Chen'` priorityPatients, `estimatedBenefit` dollars, `totalPatients: 1247`, invented `gapBreakdown` counts, and fabricated `qualityMetrics` are gone. (2) REPLACED with real per-pillar aggregation over open `MEDICATION_MISSING` + `MEDICATION_UNDERDOSED` HF gaps (GDMT 4-pillar matchers shared with the dashboard, 2022 AHA/ACC/HFSA): per-class Not-Prescribed / Under-dosed distinct-patient counts + `gapCount`; `priorityPatients` are real open gaps with INTERNAL patient UUIDs only (never names - section-10.7/PHI); `overallOptimization` = real share of HF patients with zero med gap. (3) Fields the gap engine does NOT store (per-class `eligible`/`prescribed`/`atTargetDose` denominators, external `benchmark`, historical `improvement`, `target`) return `null` EmptyState - NEVER a fabricated default (no-silent-defaults). Pure builder `buildGdmtGapAnalysis()` extracted to `backend/src/routes/decisionSupport.ts`, unit-tested (`backend/tests/api/gdmtGapAnalysis.test.ts`, 11: real aggregation + no-fabricated-names/$ + ID-only + EmptyState). tsc clean; DRIFT-44 clean; route change -> no canonical pipeline. The latent-HIGH is DEFUSED: no fabricated patient identifiers remain on the endpoint, so completing the `careTeamConfig.tsx:94` stub now renders only real, ID-referenced gap data.
- **The defect:** `backend/src/routes/modules.ts:374` `POST /heart-failure/gdmt-gaps` returns an entirely hardcoded object (comment literally: `// Simulate population-level GDMT gap analysis`): `totalPatients: 1247` + a fabricated `gapBreakdown` (aceArb/betaBlocker/mra/sglt2i eligible/prescribed/atTargetDose/gapCount counts, all invented) + `priorityPatients` with FABRICATED NAMES (`'Sarah Johnson'` / `'Jennifer Williams'` / `'Robert Chen'`) and fabricated `estimatedBenefit` dollars (`'$12,450/year'` / `'$8,200/year'` / `'$15,600/year'`) + a fabricated `qualityMetrics` block (`overallOptimization: 74.3`, `benchmark: 85.0`, `improvement: '+12.1% vs last quarter'`, `target: 90.0`). The response carries no `source` field (does not falsely claim DB provenance) but presents invented population analytics + patients as a real analysis result.
- **(a) CONSUMPTION CHECK (sets severity) - WIRED-BUT-DISCARDED -> MEDIUM (latent HIGH):** `src/services/apiService.ts:184` defines `getGDMTGapAnalysis(populationFilters)` -> POST `/modules/heart-failure/gdmt-gaps`; ONE caller exists - `src/ui/heartFailure/config/careTeamConfig.tsx:94` (a "GDMT Titration Protocol" button onClick). BUT that handler does `const analysis = response.data as any; {};` - it assigns the response and then NO-OPS it (comment: `// Action handler - implementation pending`). So the fabricated names/figures are fetched and dropped, never rendered today -> MEDIUM. The stub is a loaded gun: it is wired to render `analysis.priorityPatients` the moment someone completes it -> LATENT HIGH. (The `apiService.ts:58 overallOptimization` field is on the SEPARATE `GDMTRecommendations` type used by the gdmt-CALCULATOR endpoint - coincidental name, not this response. The fake names in CrossReferralEngine / DeviceUnderutilizationPanel / careTeamConfig staff lists are independent frontend mocks, not fed by this endpoint.)
- **(b) What it SHOULD return:** a real query against the gap engine. HF GDMT gaps ARE built + DET_OK in the live 211-DET_OK engine, so the honest source exists: query `prisma.therapyGap` (module `HEART_FAILURE`, `gapType` MEDICATION_* , `resolvedAt: null`) scoped by `req.user.hospitalId` and aggregate - exactly as the dashboard GETs already do (e.g. the VHD dashboard at :1780). `priorityPatients` must carry real INTERNAL patient UUIDs only, NEVER names (PHI rule: log/return patient IDs, never identifiers). Until the real aggregation is built, the endpoint should return an EmptyState / explicit not-implemented response, NOT fabricated data.
- **(c) Fabricated-patient-names piece -> AUDIT-300 PURE-REMOVAL applies:** per the AUDIT-300 precedent (fabricated patients / false clinical claims get PURE REMOVAL, not a demo-label), the invented names + `estimatedBenefit` dollars must be REMOVED outright, not relabeled "demo". A disclosed-demo treatment is NOT acceptable for invented patient identifiers.
- **Recommended fix (BACKEND-track):** rewrite `POST /heart-failure/gdmt-gaps` to query the real gap engine for the population aggregate (mirroring the dashboard GETs, hospitalId-scoped) returning ID-only patient references, OR return an explicit EmptyState/501-not-implemented; REMOVE the fabricated names + dollars + population counts (pure removal). The frontend stub completion (rendering the real aggregate) is UI-track, but the backend fix de-risks the latent-HIGH immediately by removing the fabricated payload the stub is wired to.
- **Cross-references:** `backend/src/routes/modules.ts:374` (endpoint) + :412-416 (fabricated `priorityPatients`) + :427-432 (fabricated `qualityMetrics`); `src/services/apiService.ts:184` (`getGDMTGapAnalysis` wrapper); `src/ui/heartFailure/config/careTeamConfig.tsx:94` (the wired-but-stubbed caller); AUDIT-099 (silent-mock / fabricated-KPI lineage) / AUDIT-300 (fabricated-patient pure-removal precedent) / AUDIT-187 (sibling `modules.ts` backend-provenance fix that surfaced this).

### AUDIT-189 - METHODOLOGY CORRECTION: the hosp-001 "211-vs-99 duplicate" reading was a coarse-(patientId,gapType,module) analysis artifact, NOT real duplication; 0 true duplicates; served counts are clinically correct

- **Phase:** Re-seed investigation (2026-06-25, read-only premise-check before a proposed dedup mutation). Self-correction caught by a pre-mutation verification - the section-18 "verify against count invariants, exit 0 is NOT verification" discipline applied to the agent's OWN earlier conclusion.
- **Severity:** METHODOLOGY-CORRECTION (no production defect; the correction PREVENTED a wrong mutation). No snapshot, no mutation, no --execute occurred.
- **Status:** CLOSED 2026-06-25 (no action required; hosp-001 data verified clinically correct).
- **The false premise (now corrected):** an earlier read-only dry-run deduped hosp-001's stored therapyGaps on the 3-tuple `(patientId, gapType, module)` and reported "211 rows -> 99 distinct keys -> 112 duplicates," concluding the served dashboards over-reported (211 vs 99). A pre-mutation verification at FULL discriminator granularity disproved this.
- **Verified facts (read-only ECS task on :325, full-discriminator analysis):** storedRows=211; distinct on `(patientId,gapType,module)`=99; distinct on `(+medication+device)`=143; distinct on the FULL set `(patientId,gapType,module,medication,device,currentStatus,targetStatus)`=211; trueDupGroups=0; trueDupExcessRows=0. EVERY one of the 211 rows is distinct; not a single identical-row pair exists.
- **Root cause of the artifact:** the schema's ONLY `@@unique` on TherapyGap is `(id, hospitalId)` - `(patientId,gapType,module)` is DELIBERATELY NOT a uniqueness key. TherapyGap carries `medication` / `device` / `currentStatus` / `targetStatus` discriminators, so multiple gaps per 3-tuple are BY DESIGN: one `CORONARY_INTERVENTION / MEDICATION_MISSING` 3-tuple held 6 rows = 6 DIFFERENT missing drugs (P2Y12 inhibitor, atorvastatin/rosuvastatin, apixaban/rivaroxaban, aspirin 81mg, metoprolol/bisoprolol, + a null) - 6 real gaps, not 6 copies. `gapDetectionRunner.ts` creates one row per distinct detected gap; its `existingKey` is `(patientId,gapType,module)` - a deliberately COARSE create-vs-update key, NOT a gap-identity key. `prisma/seed.ts` inserts 0 therapyGaps. The 211 are the legitimate output of one detection pass over 10 gap-rich patients; there is NO duplicate accumulation to explain.
- **WITHDRAWN prior readings:** (1) the "served dashboards over-report 211 vs 99 / CAD 98 vs 28" finding is WITHDRAWN - counting raw rows is CLINICALLY CORRECT (a patient missing 6 drugs has 6 medication gaps); 99 was an UNDER-count from coarse-key collapsing. (2) the earlier "engine matches stored, zero symmetric difference" held ONLY at the 3-tuple level; at the row/medication level the :325 engine produces 197 vs the stored 211 (split out as AUDIT-190).
- **METHODOLOGY RULE (forward, load-bearing):** the `(patientId, gapType, module)` 3-tuple is COARSER than gap identity and MUST NOT be used as a dedup/distinct key in stored-gap analysis. Gap identity for counting/dedup is the FULL discriminator set incl. `medication` + `device` + status. Any future "duplicate" claim must be verified at full-discriminator granularity (trueDupGroups), never the 3-tuple.
- **The coincidental 211:** hosp-001's stored ROW count (211) coincidentally equals the platform's "real-211 DET_OK" RULE-CAPABILITY count (AUDIT-184/185 lineage). These are UNRELATED: "real-211" = the count of gap-detection RULES reaching DET_OK status (engine capability); hosp-001's 211 = the count of stored gap ROWS for its 10 patients. The numeric match is coincidence.
- **Cross-references:** `backend/prisma/schema.prisma` TherapyGap (`@@unique (id, hospitalId)`; no `(patientId,gapType,module)` unique); `backend/src/ingestion/gapDetectionRunner.ts` `existingKey` (coarse create-vs-update key); AUDIT-184/185 (the UNRELATED real-211 rule-capability count); AUDIT-145 (independent prior note that the dedup key collapses ~263 rules into 14 gapType categories - same coarseness); AUDIT-190 (the engine-currency drift split out). No mutation/snapshot taken.

### AUDIT-190 - hosp-001 stored gaps (211, older engine vintage) vs :325 recompute (197): a ~14-row engine-currency drift; OPTIONAL future refresh, NOT a defect, NOT scheduled

- **Phase:** Re-seed investigation (2026-06-25, read-only). Split out from AUDIT-189.
- **Severity:** LOW (P3). NOT a defect - both vintages are distinct-gap-correct; the difference is engine VERSION, not correctness.
- **Status:** OPEN-DEFERRED 2026-06-25 (optional; explicitly NOT scheduled; no action taken).
- **The drift:** hosp-001's 211 stored gaps were generated by an OLDER engine vintage at seed time. A read-only recompute of the SAME 10 patients + current observations against the :325 engine (runner-replica, no within-run collapse) produces 197 rows - a ~14-row net difference (per-(module,gapType) examples: `CORONARY_INTERVENTION/MEDICATION_MISSING` 39 stored -> 32; `CORONARY/REFERRAL_NEEDED` 15 -> 8; `CORONARY/SCREENING_DUE` 19 -> 23; `HEART_FAILURE/MEDICATION_MISSING` 21 -> 23). Both sets are distinct-gap-correct; they differ because the rule set evolved between vintages.
- **Why NOT a defect:** stored gaps are ingestion-time + stored (CLAUDE.md section 8), reflecting the engine at detection time, not live. A drift vs the current engine is expected and harmless for synthetic demo data - no over-fire, no fabrication, no clinical error in either set.
- **Optional future refresh (NOT scheduled):** IF engine-currency on hosp-001 is ever desired, the operation is `deleteMany(therapyGap where hospitalId=hosp-001)` then `runGapDetection('hosp-001')` (verified mechanism; therapyGap is a leaf with no blocking FK children; the runner re-creates ~197), under full section-18 snapshot+verify discipline with the invariant `post == 197` (== runner-would-create), NOT 99. This is a currency REFRESH, NOT a dedup. Explicitly deferred; do not schedule without operator direction.
- **Cross-references:** AUDIT-189 (the methodology correction this splits from); `backend/src/ingestion/gapDetectionRunner.ts` `runGapDetection`; CLAUDE.md section 8 (detection is ingestion-time + stored). No mutation/snapshot taken.
