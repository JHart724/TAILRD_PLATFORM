...
- **AUDIT-116** - `audit-016-pr3-v0v1-to-v2.ts` v0/v1-to-v2 JSON discovery filter ignores the JSON `"`-quote prefix (compares `::text LIKE 'enc:%'` against a jsonb-stored `"enc:v2:..."`), so it mis-buckets JSON-v2 as plaintext AND would silently MISS any JSON-column v0/v1 envelope (never migrating it to v2). MOOT for `audit_logs.newValues` (0 v0/v1 found via the quote-aware STEP-4 census, ECS task `abd4a721`), latent across all 28 JSON targets. MEDIUM (P2) classify-up from LOW (silent correctness defect in PHI re-encryption tooling, true blast radius UNVERIFIED until the 28-target census); downgrades to LOW if that census confirms 0 v0/v1. (Tooling correctness / migration scripts, **CONFIRMED / OPEN** 2026-06-07 - sister to AUDIT-115; promoted from the `AUDIT-XXX-future-v0v1-json-discovery-filter` placeholder per branch `...-116-jsonfilter`)
...
- **Severity:** MEDIUM (P2) - classify-up from LOW per Tier-1-first discipline; downgrade path to LOW noted below pending the 28-JSON-target census. Severity is the operator's to confirm per §18.
- **Status:** CONFIRMED defect, OPEN (remediation not yet implemented; impact-verification pending the 28-JSON-target census).
...
- **Severity rationale:** MEDIUM (P2), classify-up from LOW. NOT LOW today because it is a CONFIRMED correctness defect in PHI re-encryption discovery tooling whose failure mode is SILENT (a v0/v1 JSON envelope would be skipped from migration with no error) and whose true blast radius is UNKNOWN until the 28-target census runs. **Downgrade path:** if the 28-JSON-target census confirms 0 v0/v1 envelopes across all JSON columns (as found for `audit_logs.newValues`), the realized impact collapses to a cosmetic count-display bug and this downgrades to LOW.
- **Remediation candidate (NOT yet implemented):** (1) fix the `kind: 'json'` discovery/count filters to match the quoted form `'"enc:...'` / `'"enc:v2:%'` (mirror the rekey script's already-correct json handling); (2) census all 28 JSON targets for v0/v1 envelopes to confirm zero actual victims and resolve the downgrade decision.
...
- **AUDIT-116** (was `AUDIT-XXX-future-v0v1-json-discovery-filter`) - **PROMOTED to a numbered finding 2026-06-07** (per the branch-name promise `...-116-jsonfilter`): the v0/v1-to-v2 JSON discovery filter ignores the JSON `"`-quote prefix (CONFIRMED defect; MOOT for `audit_logs.newValues` with 0 v0/v1, latent across 28 JSON targets; MEDIUM P2 classify-up, downgrades to LOW pending the 28-target census). See the **AUDIT-116** detail entry above and its severity-index row in MEDIUM (P2). No longer an unnumbered candidate.
