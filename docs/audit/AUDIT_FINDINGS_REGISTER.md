...
- **AUDIT-107** - Post-deploy Login verification regressed on prod ~2026-05-07 (70 consecutive failures); gate also non-blocking decoration (Operational maturity / production verification, **OPEN - credential-slice CLOSED 2026-06-06** (rotated-credential Login 200 + fully-green smoke run 27068065320 post-AUDIT-113-resolution); remaining scope is the SINGLE option-(B) deploy-gating posture decision - enumeration in detail; generalizes to AUDIT-111 step (2); 2026-06-03 mechanism resolved to HTTP 500 not credential - detection-gap whose predicted harm materialized as AUDIT-108)
...
- **§10.7 carryover-scope check (UNDERCLAIM - carryover beyond `patients.firstName` is UNVERIFIED):** this remediation is scoped to `patients.firstName` ONLY. The AUDIT-113 probe tested only the 4 worklist-selected fields (firstName/lastName/mrn/dateOfBirth); of those, only firstName carried the non-canonical purpose. The AUDIT-016 §10.7 carryover class can affect ANY V2 PHI column, and the rekey TARGETS inventory spans ~82 (table,column) pairs across patients (incl. phone/email/street/city/state/zipCode), encounters, observations, orders, medications, User, and the AUDIT-075-added columns (runbook §10 line ~265: the script "MUST be re-run to migrate the newly-covered columns"). Those other targets were NOT probed here, AND the now-fixed all-skip abort (AUDIT-115) would have SILENTLY no-op'd any prior rekey run on a front-loaded-canonical target - so any earlier "completed" rekey on another target is suspect. Therefore: `patients.firstName` carryover is RESOLVED and verified clean; **whether §10.7 purpose-carryover remains in other PHI columns/targets is UNKNOWN.** A full-target rekey sweep + per-target verification probe on the post-AUDIT-115 image is the recommended follow-on (filed as `AUDIT-XXX-future-full-target-rekey-sweep`). No global "§10.7 resolved" claim is made.
...
- **Status note:** 2026-06-06 OPEN at filing (read-only; no workflow change in this docs PR - the fix is an operator-gated `.github/**` edit). NOT marked RESOLVED until the notify step files an issue on a failed run (or the alert posture is otherwise made non-silent).
...
- **AUDIT-XXX-future-full-target-rekey-sweep** (MEDIUM): the AUDIT-113 rekey + verification covered `patients.firstName` only; §10.7 purpose-carryover may remain in the other ~81 rekey TARGETS (other patients PHI columns + encounters/observations/orders/medications/User + AUDIT-075-added columns). The now-fixed all-skip abort (AUDIT-115) may have silently no-op'd any prior rekey on a front-loaded-canonical target. Candidate: run the (now-fixed) rekey across all 82 targets + a per-target verification probe on the post-AUDIT-115 image. Surfaced by the AUDIT-113 §10.7 carryover-scope check.
