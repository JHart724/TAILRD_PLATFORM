...
- **AUDIT-199-B** - Durable dose-parse threading (option 2b, TRACKED FOLLOW-UP, do NOT build as a rider on AUDIT-199). Synthea's medications.csv DESCRIPTION carries the mg strength in text ("Atorvastatin 80 MG Oral Tablet") but csvParser.ts:472 drops it (doseValue hardcoded null; syntheaProofRun.ts:316 likewise). Parsing the mg out of DESCRIPTION into doseValue would make high-intensity DERIVABLE, fixing BOTH sides honestly: (i) the PV-1 / CAD-STATIN dose-unknown patients would resolve to on_high_intensity (atorva>=40 / rosuva>=20, suppressed correctly) or not_high_intensity (sub-threshold, fires correctly), and (ii) it re-activates detection for the moderate-dose-should-intensify cohort (the AUDIT-101 dose-blind false-negative), a real COVERAGE GAIN. This is the durable answer AUDIT-199 (suppression) interim-substitutes for. It is its OWN finding because it touches EVERY doseValue-dependent rule (ARNI, rivaroxaban/apixaban, digoxin, BB-target, both high-intensity statins) and so needs its own inventory + per-rule over-fire verification (a dose-parse that mis-reads a component-vs-total or BID strength could flip a fail-safe under-detect rule into an over-fire). (Threading / AUDIT-070 lineage, **OPEN - TRACKED, do not build.**) See register AUDIT-199-B. Lineage AUDIT-199 / AUDIT-101 / AUDIT-070.
