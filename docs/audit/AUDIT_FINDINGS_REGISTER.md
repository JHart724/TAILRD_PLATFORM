...
- **Fix (this build):** extended the `labFields` allow-list with the 5 numeric valve fields (thread via `valueNumeric`); added an ordinal-encode block for the coded `valve_severity` string (none/mild/moderate/severe -> 0/1/2/3, design option A - zero engine-signature ripple, mirrors `mitral_regurg_grade`); added the valve slugs to both gap runners' `IMAGING_TYPES` (365-day echo freshness, not 180-day lab). PATH 2 (FHIR): added `ECHO_LOINC_TO_SLUG` (section-16-verified NLM LOINCs) at the observationService transform.
