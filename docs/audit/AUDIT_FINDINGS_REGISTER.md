...
- **AUDIT-113** - Production HTTP 500 on `GET /api/modules/heart-failure/patients`: PHI decrypt-on-read fails fail-loud (`decryptAny` "UnknownError" KMS-wrapper-masked) on at least one Patient record in the HF cohort; data-state, sister to AUDIT-108 / AUDIT-016 §10.7. Latent across all 6 module `/patients` endpoints (identical handler code); caught by the smoke gate's first-ever real run 2026-06-06 (Production incident / data-state, OPEN; mitigated - synthetic pre-DUA data)
