...
- **Status:** OPEN (quarantine/verify-when-CPT-source-available; do NOT edit the suspect CPTs - file + scope only, per operator ruling).
- **Defect:** `backend/src/terminology/valueSets/allGapValueSets.ts` declares structural-procedure CPTs with NO section-16 verification provenance: TAVR `33361-33366` (GAP_002/066/077/078), TEER/MitraClip `33418/33419` (GAP_067), tricuspid-TEER `0544T/0545T` (GAP_068), LAAC `33340` (GAP_002-region). The SAME file carries the KNOWN-WRONG echo LOINCs `18148-8` / `77912-1` (the codes the echo-severity investigation flagged as mislabeled), so the file is suspect-provenance scaffolding, NOT section-16 product. Per section 16 (codebase trust is INSUFFICIENT), these CPTs must not be trusted without an authoritative AMA/CMS verification.
