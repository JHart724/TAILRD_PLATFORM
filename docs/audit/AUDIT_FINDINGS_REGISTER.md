...
- **Classification effect:** GAP-SH-104 proposed DET_OK -> PARTIAL_DETECTION (operator review) - a rule is mapped but detects a different scenario. SPEC_ONLY is also defensible (no rule detects post-ASA surveillance); PARTIAL chosen to match the AUDIT-122 wrong-target precedent (rule mapped -> PARTIAL floor). See `docs/audit/SH_BATCH4_PFO_ACHD_HCM.md`.
...
- **Cross-references:** `gapRuleEngine.ts:6516-6543` (SH-15); NLM ICD-10-CM I42.1 (obstructive HCM, verified); AUDIT_METHODOLOGY.md §16.6(ii); AUDIT-122 (sister wrong-target); AUDIT-106 (broad-consolidation mapping class); the SH audit Batch-4 findings doc.
