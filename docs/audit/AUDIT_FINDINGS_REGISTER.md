...
- **2026-06-25 re-verification (additive; status remains RESOLVED):** a fresh read-only blast-radius probe (`audit-113-patient-phi-decrypt-blast-radius.ts`, ECS one-off on task-def `:325`, 0 writes / `auditLogWritesEmitted=0`) confirms 0 legacy-purpose records, 6,147 / 6,147 patients canonical across all 4 PHI fields (firstName, lastName, mrn, dateOfBirth), 0 decrypt failures: `failingRecordCount=0`, `perFieldFailureMap` all-zero, `envelopeClassMap={}` (no `non-canonical-purpose:phi-migration-v0v1-to-v2`), ~24,588 canonical KMS decrypts / 0 failures, all records readable. Rekey remains complete; no action required. Context: a session premise restated the pre-rekey "5,780 / 6,147 pending" figure as if current - this re-verification (DRIFT-45 measure-do-not-assume) confirms it is stale pre-2026-06-06 state, NOT the current database.
