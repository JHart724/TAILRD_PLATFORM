...
- **AUDIT-101** - gap-cad-statin ingredient-level `STATIN_CODES` cannot encode high-intensity dose - FALSE-NEGATIVE / missed-gap (Phase 0B clinical-code, IN_PROGRESS - dose-aware fix authored, PR open, awaiting operator-gated merge)
...
- **Severity:** HIGH (P1)
- **Status:** IN_PROGRESS (2026-06-04 - dose-aware detection-set fix authored PAUSE B->D on `feat/audit-101-statin-intensity`; PR open, awaiting operator-gated merge; RESOLVED only after merge + PAUSE F on-main verification)
