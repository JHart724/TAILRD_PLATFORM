...
- **§10.7 carryover-scope check - FULLY RESOLVED 2026-06-06 (the earlier UNDERCLAIM is now closed by a full-inventory sweep):** the initial filing left the cross-target carryover UNKNOWN. A read-only full-TARGETS investigation + corrective sweep closed it:
  - **The 2026-05-16 full sweep is the source of truth (CloudWatch `auditLogger` evidence, run `targets=82`, `PHI_REKEY_RUN_COMPLETED rekeyed=495362 skipped=150 failed=0`, 2026-05-16T02:57Z).** It ran under the buggy AUDIT-115 all-skip abort. Per-target `PHI_REKEY_TARGET_COMPLETED` lines prove **27 of 29 non-zero-V2 targets were fully rekeyed to canonical** (incl. `conditions.conditionName` 225,439, `medications.medicationName` 220,552, and `patients.{lastName,dateOfBirth,phone,mrn,street,city,state,zipCode}` 6,147 each).
  ...
  - **Canonical-decrypt verification (spotcheck, 2026-06-06):** all 29 non-zero-V2 targets, 1,521 samples, **0 failures** (`audit_logs.description` 100/100, `audit_logs.newValues` 100/100) - confirms canonical-readable, no corruption.
...
- **AUDIT-XXX-future-full-target-rekey-sweep** (MEDIUM) - **COMPLETE 2026-06-06 (closed in this block, NOT outstanding):** the full-inventory investigation + corrective sweep ran. Result: 27/29 targets proven rekeyed by the 2026-05-16 full sweep; 3 abort-skipped (firstName fixed; audit_logs.{description,newValues} confirmed already-canonical); all 29 canonical-decrypt-verified (1,521 samples, 0 failures). §10.7 FULLY RESOLVED. See AUDIT-113 §10.7 note. No further sweep needed.
- **AUDIT-XXX-future-v0v1-json-discovery-filter** (LOW-MEDIUM; CONFIRMED defect, impact-verification pending): the `audit-016-pr3-v0v1-to-v2.ts` discovery/count filters for `kind: 'json'` targets do NOT account for the JSON `"`-quote prefix (a stored value is `"enc:v2:..."`). Evidence: its census reported `audit_logs.newValues v2=0 plaintext=7304`, vs the correct `v2=7101 plaintext-json=203 null=1` (quote-aware classification, STEP 4). Consequence: the count display mis-buckets JSON-v2 as plaintext, AND the candidate filter (`::text LIKE 'enc:%' AND NOT LIKE 'enc:v2:%'`) would MISS any JSON column carrying v0/v1 envelopes (`"enc:v1:...` does not match `LIKE 'enc:%'`), so they would never migrate to v2. MOOT for `audit_logs.newValues` (0 v0/v1 found there), but the defect is latent across all 28 JSON TARGETS. Candidate: fix the JSON filters to match `'"enc:...'` (mirror the rekey script's correct json handling), + census all 28 JSON targets for v0/v1 envelopes to confirm zero actual victims. Surfaced by the AUDIT-113 §10.7 STEP-4 census. (Sister tooling-correctness defect to AUDIT-115.)
