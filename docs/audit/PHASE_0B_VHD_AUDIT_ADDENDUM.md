...
**Code source:** `backend/src/ingestion/gaps/gapRuleEngine.ts` (registry=52, evaluator=49, gapsPush=47)
...
**Naming convention mismatches (20):** registry IDs not following `gap-vd-` convention.
...
- `gap-vhd-040-valve-cad-revasc` (line 4220): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-068-mech-pvt-gradient` (line 4228): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-011-bio-svd-gradient` (line 4235): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-001-subtherapeutic-inr` (line 4243): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-006-mech-asa-adjunct` (line 4250): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-057-ie-hf-surgery` (line 4258): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-059-ie-embolic-surgery` (line 4265): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-064-prior-ie-dental-prophylaxis` (line 4272): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-079-rheumatic-prophylaxis` (line 4279): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-083-rheumatic-af-warfarin` (line 4286): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-098-mech-valve-preconception` (line 4294): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-099-mech-valve-pregnancy-anticoag` (line 4301): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-091-dopamine-agonist-valve-surveillance` (line 4308): expected prefix `gap-vd-`, got `gap-vhd-`
- `gap-vhd-092-ergot-alkaloid-valve-surveillance` (line 4315): expected prefix `gap-vd-`, got `gap-vhd-`
...
|---|---:|---|---|---|
| GAP-VHD-001 | 753 | DET_OK | `gap-vhd-001-subtherapeutic-inr` (VHD-001 @8657-8681) | MANUAL OVERRIDE 2026-06-17 (v3.0 VHD chunk 3 close, AUDIT-133 RESOLVED): SPEC_ONLY -> DET_OK. The 20 |
| GAP-VHD-005 | 754 | PARTIAL_DETECTION | `gap-vd-6-doac-mechanical-valve` (VD-6 @9094-9119) | MANUAL OVERRIDE 2026-06-10 (VHD audit): DET_OK -> PARTIAL per §16.6(i) concept-match / AUDIT-123 + § |
| GAP-VHD-050 | 830 | SPEC_ONLY | — | No S. aureus + TEE rule in evaluator. \| auto-verify: No candidate evaluator block above PARTIAL_MATC |
| GAP-VHD-057 | 841 | DET_OK | `gap-vhd-057-ie-hf-surgery` (VHD-057 @8732-8755) | MANUAL OVERRIDE 2026-06-17 (v3.0 VHD chunk 4 close): newly DET_OK. Purpose-built gap-vhd-057-ie-hf-s |
...
| GAP-VHD-068 | 860 | PARTIAL_DETECTION | `gap-vhd-068-mech-pvt-gradient` (VHD-068 @8589-8612) | MANUAL OVERRIDE 2026-06-17 (v3.0 VHD chunk 2 close): RE-CITED from the broad VD-PANNUS to the purpos |
| GAP-VHD-099 | 914 | PARTIAL_DETECTION | `gap-vhd-099-mech-valve-pregnancy-anticoag` (VHD-099 @8967-8994) | MANUAL OVERRIDE 2026-06-17 (v3.0 VHD chunk 5 close): SPEC_ONLY -> PARTIAL. RE-CITED to the purpose-b |
...
### Manual classification overrides (40)
...
- **GAP-VHD-012** (T2, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL: post-TAVR HALT screening needs a CARDIAC 4D CT. No cardiac-CT code exists among the 420 distinct SNOMED procedure codes present in the substrate (chest / chest-abdomen / head CTs exist but are not cardiac gated studies). TAVR dates ARE available; the imaging modality is not. Not threaded.
- **GAP-VHD-013** (T2, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL: needs a HALT imaging FINDING (not the imaging procedure) plus an anticoagulation decision. No finding stream exists, and anticoagulant medication rows are 0 tenant-wide (warfarin 0 patients, DOAC 0 patients). Doubly blocked. Not threaded.
- **GAP-VHD-014** (T2, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL + REVERSE-HOLLOW: post-SAVR bioprosthetic 3-6 month anticoagulation needs (i) prosthesis TYPE, absent, and (ii) anticoagulant med rows - measured 0 warfarin patients and 0 DOAC patients tenant-wide. An absence-of-anticoagulation rule would fire ~100% of its cohort, the AUDIT-194 hollow signature in reverse. Not threaded.
- **GAP-VHD-015** (T2, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL + REVERSE-HOLLOW: post-bioprosthetic MVR warfarin gap. Same zero-anticoagulant substrate as GAP-VHD-014 (warfarin 0 patients), plus no mitral-prosthesis type signal. Not threaded.
...
- **GAP-VHD-018** (T2, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL: prosthesis-selection (mechanical in elderly) needs valve TYPE plus a shared-decision note. Type is not carried by the procedure code and Z95* is 0 rows tenant-wide; the decision note has no source. Age IS available. Not threaded.
- **GAP-VHD-019** (T2, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL: bioprosthetic-in-young needs valve TYPE plus a re-operation discussion record. Same type wall as GAP-VHD-018. Not threaded.
- **GAP-VHD-021** (T2, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL: patient-prosthesis mismatch needs iEOA = EOA / BSA. No valve-size, no EOA, and no BSA field exists in the substrate; the computation has no inputs. Not threaded.
- **GAP-VHD-022** (T2, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL: severe PPM management. Same missing iEOA inputs as GAP-VHD-021, plus an intervention-consideration record with no source. Not threaded.
- **GAP-VHD-023** (T3, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL: sutureless/rapid-deployment candidacy needs annulus/aortic-root dimensions - the same echo-morphometric cluster AUDIT-198 documents as real-EHR-only (standard Synthea emits only LVEF among echo signals). Not threaded.
...
- **GAP-VHD-046** (T2, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL: prosthetic-valve endocarditis FDG-PET/CT consideration needs a PET procedure code; none exists among the 420 substrate codes. Not threaded.
...
- **GAP-VHD-078** (T2, SPEC_ONLY, no cite): MANUAL OVERRIDE - TRANCHE 3 CANDIDATE B 2026-07-31 DATA WALL, measured directly: prosthesis dysfunction gates on post-op mean gradient >20. Gradient observation TYPES in the tenant: none (empty result set); valve patients with any gradient observation dated after their valve procedure: 0. The threshold has nothing to read. Not threaded.
