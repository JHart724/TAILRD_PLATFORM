...
## DRIFT-50 - Authored a gap evaluator reading a labValues slug WITHOUT verifying the slug is threaded (shipped an always-fire over-credit)

- **Date:** 2026-06-19
- **Catalyst PR / surface:** PV chunk-0 (AUDIT-178, PR #410). I tightened GAP-PV-098 (graft duplex surveillance) to `labValues['graft_duplex_months'] === undefined || > 12`, presenting the `=== undefined` branch as a "never surveilled" catch - WITHOUT verifying `graft_duplex_months` was threaded on either ingestion path. It was not (neither `observationService ECHO_LOINC_TO_SLUG` nor `patientWriter labFields`), so the `=== undefined` branch is always true and the rule shipped as an always-fire over-credit on every graft+PAD patient in production (task-def :315). Surfaced by the AUDIT-184 hollow-DET_OK threading-inventory sweep, which found PV-098 among 22 such rules (8 live over-fires).
- **Drift indicator:** A gap evaluator was authored reading an ingestion slug whose threading status was assumed, not verified. The same class as AUDIT-070 (FHIR-ingestion threading gap) and AUDIT-177 (always-fire existence-proxy) - and a recurrence even after I had personally flagged the wifi_score (PV-021) and ESR/CRP (PV-037) unthreaded-slug cases in the very same module's chunk 1. The threading inventory was applied to NEW Path-B gaps but NOT re-applied to a TIGHTENING of an existing rule.
- **Trigger:** Caught by the AUDIT-184 exhaustive cross-reference (all `labValues['x']` reads minus the threaded set, intersected with the DET_OK evaluator ranges), not at authoring time. The catch is post-hoc, not catch-and-hold - which is exactly why the mechanism update is a per-authoring gate.
- **Mechanism update:** Reading a `labValues['<slug>']` in any gap evaluator is a THREADING PREDICATE TO VERIFY before the rule is authored or tightened: the slug MUST be present in `patientWriter labFields` (CSV path) AND/OR `observationService ECHO_LOINC_TO_SLUG` (FHIR path), else the read is always-undefined and the rule is hollow (silent never-fire if the gate requires the value; always-over-fire if it is an existence-proxy). This check applies to TIGHTENINGS of existing rules, not only new builds. The AUDIT-184 hollow-DET_OK threading-inventory (62 read slugs vs 39 threaded) is the standing detector; run it before classifying any value-gated rule DET_OK.
- **Sister-cross-reference:** AUDIT-184 (the hollow-DET_OK finding + the repair); AUDIT-070 (FHIR-ingestion threading-gap parent class) + AUDIT-170 (INR both-paths) + AUDIT-181 (ApoB both-paths) + AUDIT-177 (always-fire existence-proxy class); AUDIT-178 (the PV chunk-0 tightening where the defect entered); the §16 clinical-code verification discipline (this is its threading analog - verify the data element reaches the engine, not only that the code is correct).

---

## DRIFT-51 - Selected a tenant by heuristic (largest-by-count) instead of a literal hospitalId, producing a confident but entirely false wrong-tenant narrative

- **Date:** 2026-07-12
- **Catalyst PR / surface:** AUDIT-200 hollow-gate reconciliation (post-deploy of AUDIT-148 Slice 2, task-def :359). A read-only reconciliation run-task selected the tenant by `patient.groupBy` sorted by count desc, taking the largest. TWO production tenants hold exactly 25,571 patients - `demo-synthea-proof` (the pre-threading `syntheaProofRun` baseline, LDL-absent, lvef-only, uniform-dated) and `demo-synthea-threaded` (the post-threading population the trials serve, LDL abundant, organic dates) - so the count could not distinguish them. The probe hit `demo-synthea-proof` and produced a confident, internally-coherent, entirely FALSE narrative: "a re-ingest dropped LDL and darkened the AUDIT-194-B1 / AUDIT-195 lipid gap rules platform-wide." Acting on it would have meant re-ingesting a healthy tenant to fix a regression that never existed (the never-happened AUDIT-202).
- **Drift indicator:** A measurement selected its target by a heuristic (largest / first / most-recent) rather than a literal identifier, on a surface where the heuristic was NOT a unique key. The output was internally coherent (0 LDL, uniform lvef date, a plausible re-ingest story), which is exactly what made the wrong-tenant artifact persuasive. A coherent narrative is not a correct one when the subject was mis-selected.
- **Trigger:** Caught by the operator cross-checking the tenant NAME echoed in the probe output (`demo-synthea-proof`) against the tenant the measurement CLAIMED to use (`demo-synthea-threaded`, the literal in the Slice-1 smoke's `trial_smoke_overrides.json`) - a DRIFT-45-class predicate check on a stated fact. Re-measuring on the literal `demo-synthea-threaded` refuted both discrepancies: LDL present for 18,680 raw / 5,428 evaluable; lvef-evaluable 732 (matching the Slice-1 smoke's 737); no regression.
- **Mechanism update:** NEVER select a tenant (or any scoped target) by heuristic - largest, first, most-recent - when the heuristic is not a proven unique key. ALWAYS pass the `hospitalId` as a LITERAL, and ALWAYS echo it back in the probe output so the target is verifiable against intent. A measurement that does not name its tenant in its own output is not a measurement. Corollary: two demo tenants sharing a patient count is a standing hazard - `demo-synthea-proof` (baseline) vs `demo-synthea-threaded` (population); any trials/gap population probe MUST name which.
- **Sister-cross-reference:** AUDIT-200 (the calibration whose measurement this corrected; the dated [CORRECTION 2026-07-12] carries the threaded split); AUDIT-194-B1 + AUDIT-195 (the lipid threading/consolidation falsely alleged to have regressed - intact); AUDIT-148 (trials lineage); DRIFT-45 / DRIFT-46 (the pre-state / stated-fact predicate-verification discipline that caught it); the §18 register-literal source-of-truth discipline (a datum must name its scope).

---

## DRIFT-52 - Feature branch cut from an open-PR branch, not from main - silently absorbing scope AND turning a stale docs PR into a partial feature-revert

- **Date:** 2026-07-14
- **Catalyst PR / surface:** AUDIT-204 (PR #470) + the open pointer-bump PR #469. `feat/audit-204-typecheck-coverage` was created off `docs/task-def-bump-368` (the OPEN PR #469) instead of off `main`.
- **Event / two consequences (one benign, one dangerous):** (i) BENIGN - the AUDIT-204 merge (#470) silently ABSORBED #469's pointer-bump commit (`:365`->`:368` in CLAUDE.md §9/§17 + PRODUCTION_READINESS.md), carrying scope OUTSIDE #470's stated footprint; main stayed coherent (correctly read `:368`), so no corruption. (ii) DANGEROUS - #469 thereby became STALE relative to main: its diff against its OWN base still looked like a harmless 3-file docs pointer bump, but its diff against CURRENT MAIN was a PARTIAL REVERT of AUDIT-204 (re-adding the `// @ts-nocheck` to `clinicalScenarios.test.ts`, deleting `checkTsNocheck.test.ts`). A "trivial docs-only pointer bump" PR had silently become a feature-reverting change, and nothing in its appearance signaled that. #469 was CLOSED (not merged) + its branch deleted.
- **Drift indicator:** A branch was cut from a non-`main` base (an open PR branch) rather than from a freshly-pulled `main`. The danger is invisible in the stale PR's own diff - the appearance (3-file docs bump) diverged from the effect (feature revert) precisely because the base had moved underneath it.
- **Trigger:** Caught two ways: (a) `git show --stat` on the #470 merge showed pointer-line files (`PRODUCTION_READINESS.md`, CLAUDE.md §9/§17) that were NOT in the stated AUDIT-204 scope - a scope-confirmation predicate check; (b) diffing the stale branch against CURRENT MAIN (`git diff main..origin/docs/task-def-bump-368`) BEFORE any merge decision, rather than trusting the PR's original (base-relative) diff.
- **Mechanism update:** (1) ALWAYS cut a branch from a freshly-pulled `main`; verify `git merge-base HEAD origin/main == git rev-parse origin/main` BEFORE the first commit. (2) Before merging ANY PR that has been open ACROSS other merges, diff it against CURRENT `main` (`git diff main..origin/<branch>`), NOT against its base - a stale PR's danger is invisible in its own diff. This session alone had two stale-window hazards: #465 (CAD frontend) landed between #466 and #467; #469 sat open while #470 went by and absorbed its content. (3) Confirm merge scope via `git show --stat` on the landed commit (authoritative), never the `gh` merge tail (which can print an intervening changeset).
- **Sister-cross-reference:** AUDIT-204 (the PR that absorbed the bump; its register entry + BUILD_STATE row record the branch-hygiene note); PR #469 (closed-not-merged, with the recorded rationale); DRIFT-49 (verify-cited-cross-refs) + DRIFT-45/46 (pre-state / stated-fact predicate verification - the same "verify against current reality, not the assumed snapshot" family, here applied to a branch base); CLAUDE.md §5 branch conventions (short-lived branches off `main`) + §19.4 footprint-split commit gate (scope is verified mechanically, per landed diff).

---

## DRIFT-45 RECURRENCE (2026-07-14) - Asserted a plan-gate ("SPEC_ONLY cannot be scoped until the EP/SH/VHD Phase 0B audits land") from a stale working snapshot, not from the canonical doc - and the gate did not exist

- **Date:** 2026-07-14
- **Catalyst / surface:** a read-only status assessment against the build spec. The operating premise carried into the session (and re-asserted in the assessment prompt) was that "PATH_TO_ROBUST records that SPEC_ONLY gap implementation cannot be scoped until the EP/SH/VHD Phase 0B audits land." Confirming it against the canonical doc REFUTED it.
- **Drift indicator:** A gating claim about plan state was held as a fact from a stale working memory / earlier-session snapshot, rather than verified against the current canonical doc. It is the SAME class as DRIFT-45 (pre-state predicate verification) and DRIFT-51 (a coherent narrative from a mis-selected source): the claim was internally plausible (EP/SH/VHD are the lower-coverage modules, so "un-audited" felt consistent), which is exactly what let it persist unverified.
- **The refutation (canonical-doc-literal):** (i) `grep` of `PATH_TO_ROBUST.md` v3.1 for any "cannot be scoped / un-audited / until...audit / EP-SH-VHD Phase 0B" gate returns NOTHING - no such gate is written anywhere in the live plan. (ii) All SIX Phase 0B module addenda exist on disk - `PHASE_0B_{HF,EP,SH,CAD,VHD,PV}_AUDIT_ADDENDUM.md` - and `PHASE_0B_CROSS_MODULE_SYNTHESIS.md` is generated from all six crosswalks (its per-module TOTAL rows include EP/SH/VHD). The EP/SH/VHD Phase 0B audits HAVE landed. (iii) The `293` SPEC_ONLY gaps are therefore scopable NOW (`145` of them in EP/SH/VHD: EP 29 + SH 38 + VHD 78), reconciled per-module against the live crosswalk at buildout time (PATH v3.1 §2.2). The real caps on that backlog are the THREADING ceiling (§1.4 step 1) and the AUDIT-198 test-data decision (§1.4 note) - data/threading limits, NOT an un-run audit.
- **Trigger:** Caught by verifying the stated gate against the canonical doc (grep the live plan for the gate; `ls` the addenda) instead of accepting it - a DRIFT-45 predicate check applied to a PLAN-STATE claim, the same discipline DRIFT-51 applied to a measurement's tenant.
- **Mechanism update:** A plan-state gate ("X cannot proceed until Y") is a PREDICATE TO VERIFY against the current canonical doc before it is asserted or acted on - never carried forward from a working snapshot. Two concrete checks: (a) grep the LIVE plan doc for the gate's own words - if it is not written there, it is not a gate; (b) for "un-audited / not-yet-done" claims, check the artifact on disk (the addendum, the crosswalk, the generated synthesis) - a generated aggregate that INCLUDES a module is proof that module was audited. Corollary (the reason this matters): a stale ledger seeds a false premise into the next session - which is why the same-day BUILD_STATE header + PATH_TO_ROBUST coverage-figure reconciliation (this docs pass) was necessary; source-wins, the generated synthesis is authoritative over any hand-written snapshot.
- **Sister-cross-reference:** DRIFT-45 (parent class - pre-state predicate verification) + DRIFT-46 (recurrence) + DRIFT-51 (coherent-narrative-from-mis-selected-source, the measurement analog); PATH_TO_ROBUST v3.1 §1.2 / §1.4 / §2.2 (the plan surfaces reconciled this pass); PHASE_0B_CROSS_MODULE_SYNTHESIS.md (the generated source of truth); the §18 source-wins discipline (generated synthesis over hand-written snapshot).

---

## DRIFT-45 CONSOLIDATION (2026-07-18) - Two strategic-layer false premises in one arc; both refuted pre-authoring because the execution layer verified instead of accepting

- **Catalyst / surface:** the AUDIT-203 -> AUDIT-209 arc. Across it, the strategic-review layer (the framing carried into prompts and working memory) generated TWO premises that were confidently stated, load-bearing on scope, and FALSE. Both were caught BEFORE any code or doc was authored on them, because each was treated as a predicate to verify rather than a fact to act on. This entry consolidates them as one class observation; the individual instances remain recorded above.
- **False premise 1 - the plan gate (recorded 2026-07-14 as the DRIFT-45 RECURRENCE above).** "SPEC_ONLY implementation cannot be scoped until the EP/SH/VHD Phase 0B audits land." Refuted canonical-doc-literal: no such gate appears anywhere in `PATH_TO_ROBUST.md` v3.1, and all six `PHASE_0B_{HF,EP,SH,CAD,VHD,PV}_AUDIT_ADDENDUM.md` exist on disk (the synthesis is generated from all six crosswalks). The 293 SPEC_ONLY gaps - 145 of them in EP/SH/VHD - were scopable the whole time. The real caps are the threading ceiling and the AUDIT-198 test-data decision.
- **False premise 2 - the AUDIT-204 target.** "`buildPatientEvalContext.ts` carries `@ts-nocheck`, so the shared clinical substrate is unchecked" - escalated to a HIGH-P1 filing. Refuted at STAGE 1: the file's only occurrence of the token is line 55, JSDoc PROSE explaining why the signature takes `patient: any` and NOTING that the runners are suppressed. TypeScript honors the directive only in a leading `//` line comment before any code; the first code is the import at line 12. Proof of active checking: `tsc` reports 0 errors on the file, and an injected `const x: number = "string"` was CAUGHT and attributed to `buildPatientEvalContext.ts(105,7)` TS2322, then reverted. The finding originated from `grep -rl "@ts-nocheck"` - a files-with-matches scan that matches the literal string anywhere, including prose.
- **Drift indicator (the shared shape):** a premise arrives already-framed and consequential - it names a blocker, a severity, or a scope boundary - which makes acting on it feel like execution rather than assumption. Both premises would have produced real work: premise 1 would have deferred a scopable 293-gap backlog behind an audit that had already run; premise 2 would have "fixed" a file that was never broken.
- **Trigger (what caught them):** premise 1 - grepping the live plan for the asserted gate and `ls`-ing the addenda, instead of accepting the framing. Premise 2 - reading the file's LEADING COMMENT BLOCK against TypeScript's actual directive rule, instead of accepting the grep hit. In both cases the verification was cheaper than the work the premise implied.
- **Mechanism to keep:** **a premise stated in a prompt is a PREDICATE TO VERIFY, not a fact to act on.** The verification is concrete and cheap: grep the canonical plan for the asserted gate; check the artifact on disk; read the leading comment block; run the compiler. Verify BEFORE authoring, because after authoring the sunk work biases toward accepting the premise.
- **What worked, and is the point of this entry:** both were caught pre-authoring. The discipline is not "distrust the operator" - it is that a premise and its evidence are separable, and the execution layer holds the evidence. **The correct response to a refuted premise is to RECORD THE REFUTATION WITH EVIDENCE and surface it, not to quietly proceed with the corrected understanding.** AUDIT-204 was re-scoped in the register as a documented FALSE POSITIVE with the dispositive proof retained, and the sweep it prompted found the REAL drift (an unsanctioned `@ts-nocheck` on `clinicalScenarios.test.ts`) plus produced the durable detector - a refuted premise that still yielded a correct outcome BECAUSE the refutation was recorded rather than swallowed.
- **Sister-cross-reference:** DRIFT-45 (parent class - pre-state predicate verification) + DRIFT-46 / the 2026-07-14 recurrence above (prior instances) + DRIFT-51 (coherent-narrative-from-mis-selected-source) + DRIFT-52 (stale-branch scope absorption); `AUDIT_METHODOLOGY.md` section 22.2(a) (a coarse grep is not a detector - the same root cause as premise 2) and section 23 (measurement validity); AUDIT-204 register entry (the recorded refutation).

---

## DRIFT-53 - Push bundled into an edit/amend prompt (self-caught pre-send; the outward action was split from the edit)

- **Date:** 2026-07-21
- **Class:** push-bundled-into-edit-prompt
- **Drift indicator:** a draft instruction bundled a CONDITIONAL edit (verify the BUILD_STATE supersede-not-overwrite, and IF the prior text was dropped, amend the commit to restore it; add a test and amend it in; append a drift entry and amend it in) together with a push authorization in the SAME prompt. A conditional-amend-plus-push instruction would push a just-mutated commit as an automatic consequence of the edit condition firing - the operator would authorize the push before seeing the amended result, collapsing the two gates (edit review and outward-action authorization) into one.
- **Trigger:** self-catch pre-send - the draft was inspected before issue, the bundled push was recognized as an outward/irreversible action riding on a conditional edit, and it was SPLIT OUT: the prompt was reissued with the edits + amend retained but push held ("No push in this prompt ... Push is a separately authorized step"). Caught before any bundled action executed; no correction after the fact was needed.
- **Mechanism to keep (Mechanism 2 self-watch list):** an outward or irreversible action (git push, PR merge, deploy, send) must NEVER be bundled into an edit/amend/author instruction - not even conditionally, and not even when the edit is expected to be trivial. Edits and their review are one gate; the outward action is a separate explicit gate that sees the FINISHED edit first. This extends the standing "STOP before push" / operator-merges (DRIFT-49) separation from the execution layer to the PROMPT-AUTHORING layer: the self-watch fires when a single instruction would both mutate a commit and publish it. The correct shape is always author/amend -> report -> HALT -> separately-authorized push, and a draft that fuses the last two is split before send. Sister discipline: the same reason a destructive command is never a placeholder (project instructions Destructive-Command Discipline) - the outward action is gated on its own explicit authorization, never as a rider on a benign edit.
- **What worked, and is the point of this entry:** the bundling was caught at draft time, so the two gates stayed separate by construction rather than by after-the-fact correction. The discipline is that the coupling itself is the drift - a conditional amend is fine, a push riding on it is not - and the fix is structural (split the prompt), recorded here as the pre-send self-watch that prevents a future draft from fusing them silently.
- **Sister-cross-reference:** DRIFT-49 (operator-merges gate separation; DRIFT-53 is the prompt-authoring analog at the push surface) + DRIFT-48 (`--delete-branch` outward side effect that auto-closed a dependent PR - outward-action blast radius under-scoped) + DRIFT-52 (scope absorption at branch cut); project instructions Destructive-Command Discipline (outward/irreversible actions are explicitly operator-gated, never bundled) + `<drift_prevention_mechanisms>` Mechanism 2 (self-watch list, "verify before declaring" family, extended here to "do not bundle the outward action into the edit").

---

## DRIFT-54 - Work queued from a stale state snapshot instead of the canonical register (echo-threading framed as pending when it was already complete)

- **Date:** 2026-07-22
- **Class:** status-surface-from-snapshot-not-canonical
- **Drift indicator:** the Tranche-2 echo-threading prompt framed BOTH `echo_months` (VD-ECHO-INTERVAL) and `pasp` (VD-PULMONARY-HTN) as PENDING threading work to be designed after inventory. The canonical register (`AUDIT_FINDINGS_REGISTER.md` + the gap-engine rule bodies) recorded the opposite: AUDIT-194-B3 had ALREADY restored VD-ECHO-INTERVAL on 2026-07-03 (echo_months threaded via `deriveEchoMonths`, rule LIVE / PARTIAL_DETECTION) and had ALREADY ruled VD-PULMONARY-HTN real-EHR-only (pasp absent from Synthea). The relay/work-queue layer populated the task from a STALE STATE SNAPSHOT of "what still needs threading" rather than reading the register's current disposition, so it queued completed and impossible work as pending.
- **Trigger (what caught it):** agent-inventory-surfaced - the read-only inventory's FIRST step (grep the rule bodies + register detail for the two rules' current state) returned "already restored" / "already real-EHR-only", which contradicted the prompt's pending framing before any design work began. The catch was cheap because the inventory verified the rule state against canonical instead of accepting the prompt's premise.
- **Mechanism to keep (Mechanism 2 self-watch list):** a work item's STATE ("pending" / "blocked" / "not yet done") is a canonical-register assertion, not a snapshot fact - before acting on queued work, verify the item's current disposition against the register + the code it references, exactly as a status-surface figure is verified against the live source (DRIFT-45 family). This EXTENDS the DRIFT-45 "premise is a predicate to verify" discipline from scope-sourcing (DRIFT-45/46) and cross-ref classification (DRIFT-49) to WORK-QUEUE state: the queue can carry a stale "still pending" that the register has since closed. The correct response is to surface the completed/blocked state with the canonical citation and let the operator re-scope, NOT to perform (or re-perform) the already-done work. Here the inventory correctly reported "B3 done, pasp real-EHR-only" and pivoted to the one genuinely-new finding it turned up (AUDIT-218, the procedures-table-empty persist gap) rather than re-authoring threading that already existed.
- **What worked, and is the point of this entry:** the inventory was run read-only-first and verified rule state against canonical before any authoring, so the stale-pending framing was caught at step 1 and the effort redirected to real new signal instead of duplicating AUDIT-194-B3. The discipline is that "pending" is itself a claim to check.
- **Sister-cross-reference:** DRIFT-45 (parent - pre-state predicate verification; DRIFT-54 is the work-queue-state analog) + DRIFT-45 CONSOLIDATION (premise-as-predicate) + DRIFT-46 (module-wiring-state snapshot drift) + DRIFT-49 (register content asserted in a chat instruction without register-literal verification); `AUDIT_METHODOLOGY.md` section 18 (register-literal source-of-truth) + section 23 (measurement validity); `<drift_prevention_mechanisms>` Mechanism 2 ("verify before declaring", extended to "verify a queued item's pending-state against canonical before working it"); AUDIT-194-B3 (the completed work the prompt framed as pending) + AUDIT-218 (the real new finding the verified inventory surfaced instead).

---

## DRIFT-55 - Prediction-scope-narrower-than-reality: a pre-mutation prediction modeled the marginal effect and treated an UNVERIFIED baseline as the invariant (second instance)

- **Date:** 2026-07-29
- **Class:** prediction-scope / premise-as-predicate (DRIFT-45 family)
- **Drift indicator:** the gap re-detection prediction (~416 new gaps, "every other rule zero change") modeled ONLY the marginal effect of the AUDIT-218 procedures backfill, implicitly assuming the stored 63,959-gap baseline had been computed by the CURRENT rule engine at the CURRENT clock. Both assumptions were false: the baseline was the tenant's FIRST-EVER detection, months old, so the execute landed 1,292 creates (416 predicted VD-ECHO-INTERVAL + 876 un-modeled first-re-detection catch-up: clock drift on 180-day staleness-window rules whose observations aged out, plus engine-version drift incl. the AUDIT-195 consolidation status 0 -> 178) and relabeled rows across 48 statuses via the AUDIT-222 match-key defect. Invariants (a) and (c) of the authorized battery FAILED against the too-narrow prediction even though every individual post-run figure was a genuinely-correct current-engine result (substance-checked: 185/203-day-old observations just past the 180-day window; operator KEEP ruling).
- **First instance:** the AUDIT-218 execute GO carried the precondition "procedure.count 0 on ALL tenants" - a relay-layer extrapolation beyond the finding's measured scope (only the two Synthea tenants were ever measured), caught by step-1 baseline capture (demo-medical-city-dallas at 971,135 via the FHIR path, expected architecture).
- **Trigger (what caught it):** the invariant battery itself - baseline capture (instance 1) and the post-run per-status diff (instance 2) both caught the divergence BEFORE any wrong conclusion shipped; the halt-report-await-ruling protocol held both times.
- **Mechanism to keep (Mechanism 2 self-watch list):** any pre-mutation prediction MUST state its baseline assumptions as explicit PREDICATES and VERIFY them before the delta is treated as the invariant - for a re-detection: first-run-since-WHEN (has the stored baseline ever been recomputed?), engine-version-since-WHEN (what rule changes landed since the baseline was materialized?), clock-sensitive-rules-since-WHEN (which staleness windows moved?). A prediction whose baseline predicates are unverified is a HYPOTHESIS, not an invariant; batteries built on it will fail on scope, not on substance.
- **Sister-cross-reference:** DRIFT-45 (parent - verify-not-assume; this is the prediction-baseline analog) / DRIFT-54 (state-snapshot staleness, the work-queue analog of the same premise rot) / AUDIT-218 (instance-1 arc) / AUDIT-222 (the match-key relabel mechanism the battery diff exposed) / AUDIT-195 (the engine-version drift exemplar).

## DRIFT-56 - Working tree MUTATED to read a baseline, reverting in-flight edits

- **Date:** 2026-07-29
- **Class:** worktree-mutated-for-comparison
- **Drift indicator:** mid-authoring of AUDIT-222 PR-A, a question arose that was purely a READ - were the 4 rogue-`new PrismaClient` and 3 `var` hits from the section-18 3-check gate pre-existing on main, or introduced by this PR? The check chosen was `git stash -q -u` followed by `git checkout origin/main -q -- backend/src`, then a restore. That is a WRITE to the working tree to answer a read-only question. The stash pop did not fully restore (the entry was kept), and three in-flight source edits were reverted: the 368-site `ruleId` codemod on `gapRuleEngine.ts` plus the match-key changes in both gap runners.
- **Trigger (what caught it):** self-caught immediately - the same command printed the post-restore verification line `my new files intact: ... ruleIds=0`, and `ruleIds=0` contradicted the just-applied 368-site codemod. The contradiction was visible because the command asserted a concrete expected number rather than reporting success narratively.
- **Recovery:** the stash entry was intact (`stash@{0}`), so the three files were restored with a targeted `git checkout stash@{0} -- <three paths>`, then re-verified from first principles rather than assumed: 368 `gaps.push` / 368 `ruleId` lines, the `DetectedGap.ruleId` declaration present, both runners keyed on ruleId, `tsc --noEmit` clean, and the AUDIT-222 suites green (29/29). No work was lost and nothing shipped in a half-reverted state.
- **Mechanism to keep (Mechanism 2 self-watch list):** a baseline is READ, never staged. Compare against a committed ref in place - `git show origin/main:<path>` piped to the check, or `git diff origin/main -- <path>` - and NEVER `git stash` / `git checkout <ref> -- <path>` while edits are in flight. Mutating the worktree to answer a read-only question puts unpushed authoring at risk for zero benefit, and a partially-applied stash pop can leave a subtly inconsistent tree that still compiles. Corollary that made the catch possible and is worth keeping: verification lines should assert an EXPECTED NUMBER (`ruleIds=368`), not narrate success - a bare "restored" would have hidden this.
- **Sister-cross-reference:** DRIFT-45 (parent - verify-don't-assume; this is its worktree-safety corollary) / DRIFT-53 (push bundled into an edit prompt - same family of "a read-only intent executed a state-changing command") / AUDIT-222 (the PR-A authoring this occurred during).

## DRIFT-57 - Register bucket MISFILE: severity label and severity-section placement diverged

- **Date:** 2026-07-29
- **Class:** register-bucket-misfile
- **Drift indicator:** while filing AUDIT-225, the bullet was labelled **MEDIUM (P2)** in its own status text but INSERTED into the **LOW (P3)** section of the severity index - it was appended immediately before the AUDIT-224 bullet, which lives under LOW. The label and the placement disagreed, so a register-literal severity read (per `AUDIT_METHODOLOGY.md` section 18, which copies severity verbatim from the register) would have reported a MEDIUM finding as LOW.
- **Recurrence, not a novel error:** this is the SAME class as the 2026-07-18 correction where AUDIT-203..209 had all been appended into the LOW section regardless of label, leaving two HIGH P1 findings (AUDIT-208, AUDIT-209) filed under LOW. That entry is recorded in the BUILD_STATE CURRENT row as a dated severity-index re-bucketing. The class therefore has two known instances and is officially recurrence-prone: appending near a recent neighbour is ergonomic and silently ignores which section that neighbour sits in.
- **Trigger (what caught it):** the `registerOpenCount.ts` split, pre-commit. The total was right (90 -> 91) but the SPLIT was wrong - MEDIUM 41 / LOW 30 where a MEDIUM filing should have produced MEDIUM 42 / LOW 29. The total alone would have passed; only the per-severity split exposed the misfile. Corrected by relocating the bullet into the MEDIUM section before any commit, then re-running the script to confirm 18/42/29/2.
- **Mechanism to keep (Mechanism 2 self-watch list):** the severity LABEL and the severity-SECTION placement are verified TOGETHER, via the count script's per-severity split, BEFORE any register commit - never by eyeballing the neighbouring bullet. Concretely: after filing, run `registerOpenCount.ts` and assert the expected DELTA IN THE CORRECT BUCKET, not merely the expected total. A correct total with a wrong split is exactly what this class looks like, so a total-only check cannot detect it. When inserting a bullet, locate the target `### <SEVERITY>` header first and insert relative to THAT, not relative to whichever finding happens to be numerically adjacent.
- **Sister-cross-reference:** the 2026-07-18 AUDIT-203..209 re-bucketing (instance 1 of this class) / `AUDIT_METHODOLOGY.md` section 18 (register-literal severity: the discipline this defect would have corrupted at the read side) / section 23 (measurement validity - the count script as source of truth) / DRIFT-54 (status-surface-from-snapshot, sibling status-surface-integrity class).

## DRIFT-58 - A load-bearing deploy ORDERING was asserted in a comment and treated as if it were a mechanism

- **Date:** 2026-07-30
- **Class:** documented-deploy-order-treated-as-mechanism
- **Drift indicator:** PR-B (AUDIT-223) shipped a partial-unique-index migration whose SQL header read "DEPLOY ORDER (load-bearing): this migration MUST be applied only AFTER the shadow-dedupe execute has run". Nothing enforced that. The file sat in `prisma/migrations/`, and the container CMD is `npx prisma migrate deploy && node dist/server.js`, so `migrate deploy` would have applied it on the VERY NEXT deploy - there is no directory exclusion, guard clause, or feature flag anywhere in the Dockerfile or the deploy workflow. Against the 185 still-duplicated open pairs in production the index raises `23505 unique_violation`.
- **The compounding error - a safety property reasoned about at the wrong layer:** the migration's own comment described the in-transaction abort as a VIRTUE ("a loud, non-destructive abort, by design"), and the PR body repeated it. That is true of the DATA (the table is left untouched) and false of the DEPLOY: the `&&` means a failed migration prevents the server from starting, the ECS task exits non-zero, the rollout fails, and it keeps failing for every subsequent deploy until the dedupe runs. Non-destructive-to-data was silently generalised into safe-to-ship. Two different properties; only one was checked.
- **Trigger (what caught it):** the OPERATOR, in a pre-merge verification question that asked mechanically how the migration is prevented from running - naming the only three real answers (excluded from the auto-migration path, written idempotent-and-deferred, or "the deploy fails safe") and pre-committing to HALT on the third. The agent had already merged four PRs in this arc without anyone asking that question; it was not caught by any test, by CI (Migration Validation runs against an EMPTY ephemeral postgres, where the index succeeds), or by the agent's own review.
- **Why CI could not catch it:** Migration Validation applies migrations to a fresh empty database. A constraint that only fails against PRODUCTION data passes CI by construction. Green CI is not evidence about a data-dependent migration.
- **Mechanism to keep (Mechanism 2 self-watch list):** **any load-bearing ordering between a migration and a data operation must be enforced BY CONSTRUCTION - a separate PR sequenced behind the data gate - never by a comment.** A comment is documentation; `prisma migrate deploy` does not read documentation. Corollaries: (a) when a migration depends on the STATE of production data, state that dependency as a gate and ask what enforces it, not what documents it; (b) never let "non-destructive to data" stand in for "safe to deploy" - name which layer the safety claim covers; (c) a self-guarding migration that skips silently is WORSE than a loud failure, because `_prisma_migrations` records it as applied and nothing ever re-attempts it, converting a recoverable failure into a permanent invisible gap in the invariant (this alternative was considered and rejected on exactly that ground).
- **Sister-cross-reference:** DRIFT-45 (verify-don't-assume; this is its enforcement-vs-documentation corollary) / DRIFT-55 (prediction-scope: assumptions stated but unverified) / AUDIT-024 (the in-transaction-DDL lesson this migration otherwise honoured) / AUDIT-223 (the PR re-scoped as a result) / CLAUDE.md section 15 RULE 2 (schema and code deploy together via the Dockerfile CMD - the exact coupling that makes a failed migration a failed rollout).

## DRIFT-59 - Section 9.2 full-pipeline rule violated by a CAD-only reconcile; caught by CI Gate 4

- **Date:** 2026-07-31
- **Class:** section-9.2-partial-pipeline-run
- **Drift indicator:** the Tranche 3 Slice 1 PR (#513) added two evaluator blocks + two registry entries to `gapRuleEngine.ts`, shifting engine line numbers under EVERY module's canonical cites. The local pipeline run correctly ran `refreshCites --all` after the first `validateCanonical` surfaced cross-module line mismatches in EP/SH - but ran `reconcile` for CAD ONLY. The other five modules' `reconciliation.json` (and the four addenda whose reconciliation blocks embed engine line numbers) were committed stale against the fresh extracts. Section 9.2 says every source-changing PR runs the FULL pipeline, no partial runs; a partial run was performed anyway, reasoning per-module instead of per-artifact-dependency.
- **Trigger (what caught it):** CI **Gate 4** (`auditCanonical.yml`), which regenerates `reconcile --all` + `renderAddendum --all` + `renderSynthesis` and diffs the result against the commit - it REJECTED with `derived-artifacts-divergent`, naming the nine stale files. The deterministic gate did exactly what it exists to do; no clinical content was at risk (the divergence was line-cite churn only), and the completion commit (`48adab5`) was churn-only with `validateCanonical` 6/6 before and after.
- **Mechanism to keep (Mechanism 2 self-watch list):** when a shared source file shifts line numbers, the blast radius of the canonical pipeline is ALL modules, not the module being edited - run every `--all`-capable stage as `--all` (extractCode, reconcile, refreshCites, renderAddendum) rather than scoping any stage to the edited module. The per-module scoping instinct is exactly what section 9.2's no-partial-runs rule exists to override; `validateCanonical` passing locally is NOT sufficient (it validates crosswalk cites, which refreshCites had fixed, but does not diff reconciliation.json staleness - only Gate 4 does).
- **Sister-cross-reference:** DRIFT-45 (verify-don't-assume: the local green was narrower than the CI gate) / catch #89 + section 19.4 PARSED-CANONICAL-DOC (the same parsed-artifact coupling class) / AUDIT_METHODOLOGY.md section 9.2 (the rule violated) / PR #513 commit 48adab5 (the churn-only completion).

## DRIFT-60 - Override entries added as DUPLICATE object-literal keys, silently dead; caught only by an unchanged applied-count

- **Date:** 2026-07-31
- **Class:** duplicate-key-silent-overwrite
- **Drift indicator:** the Tranche 3 closure PR added three new `applyOverrides.ts` entries (`GAP-CAD-031`, `GAP-CAD-048`, `GAP-CAD-089`) carrying the Candidate A measured record. All three ids ALREADY had override entries LATER in the same `CAD: { ... }` object literal (AUDIT-184 at lines 566/567, AUDIT-197 at 594). In a JS/TS object literal the later key wins, so the three new entries were dead on arrival - the auditNotes would never have reached the crosswalk or the rendered addendum, and the closure record the operator asked for would have been silently absent from the surface it was supposed to land on.
- **Why every mechanical scan missed it:** `tsc --noEmit` did NOT flag it (empirically verified this session - no TS1117, no error of any kind). The pipeline ran without error. `validateCanonical` would have passed 6/6, because the crosswalk was internally consistent - it simply carried the OLD note. DRIFT-44, the 3-check gate, and the full test suite are all silent on it. Nothing in the toolchain treats a duplicate override key as an error.
- **Trigger (what caught it):** the `applyOverrides` summary line reading **`CAD: applied=37 demoted=0 (out of 37 configured)`** - unchanged from the previous run despite three entries being added. The count was the only signal. This is the DRIFT-57 pattern exactly (a register bucket misfile caught by a count SPLIT, not by the label): a per-category count that does not move when you added something is evidence, and it must be read every run, not skimmed.
- **Mechanism to keep (Mechanism 2 self-watch list):** **before adding an entry to any keyed configuration map, grep for the key first** - `grep -n "'GAP-XXX-NNN'" applyOverrides.ts` - and if it exists, AMEND the existing entry (retaining its prior text verbatim per supersede-not-overwrite, appending the dated record after a `||` separator) rather than adding a second key. Corollaries: (a) after any config-map edit, read the applied/configured COUNT and confirm it moved by exactly the number of entries added - an unchanged count is a failed edit, not a no-op; (b) do not infer "the edit landed" from "the pipeline ran clean" - a silently-overwritten key produces a perfectly clean run; (c) this generalises to every id-keyed literal in the repo (`OVERRIDES`, `RUNTIME_GAP_REGISTRY`, value-set objects), not just applyOverrides.
- **Sister-cross-reference:** DRIFT-57 (register bucket misfile caught by a count split - same "read the count, not the label" mechanism) / DRIFT-59 (the sibling section-9.2 lapse in the immediately preceding PR; both are "the local run looked clean") / DRIFT-45 (verify-don't-assume) / CLAUDE.md section 18 supersede-not-overwrite (the amend-don't-replace discipline the fix follows).

## DRIFT-61 - Task framing asserted a status ("VHD/SH Phase 0B audits pending") that disk contradicted

- **Date:** 2026-07-31
- **Class:** status-surface-from-snapshot (recurrence)
- **Drift indicator:** the Tranche 3 closure task was framed with "the VHD/SH addenda note is deferred until those modules' Phase 0B audits run (do NOT create partial addenda)". On disk, all six Phase 0B addenda already existed and were fully generated: `PHASE_0B_VHD_AUDIT_ADDENDUM.md` (27KB, 19 rendered gap rows) and `PHASE_0B_SH_AUDIT_ADDENDUM.md` (14KB, 12 rows), both regenerated by the immediately-preceding PR, with `BUILD_STATE.md` line 16 recording in plain text that "all six PHASE_0B_{HF,EP,SH,CAD,VHD,PV}_AUDIT_ADDENDUM.md exist". The premise was a stale snapshot of module-audit status carried forward in the framing, not a live read.
- **Why this is the SAME class as the recurring ledger staleness, not a new one:** it is the BUILD_STATE `CURRENT` row failure mode moved up a layer - a point-in-time status assertion re-used as if it were live state. The canonical warning at the top of that row ("READ THIS FIRST, THEN VERIFY IT ... this row is a POINT-IN-TIME SNAPSHOT, not a live reading") applies to any status assertion in any channel, including a task brief. The same session also carried a second instance in the same brief - "the 6 absent api.ts endpoints, re-confirmed" - where the six named functions were present in `src/services/api.ts:413-444` and a DIFFERENT two (`submitRegistryCase`, `rejectRegistryCase`) were the actual absences: the count was right, the direction was inverted.
- **What went right (the mechanism that worked):** both divergences were caught by verifying the premise against disk BEFORE acting on it, and were SURFACED to the operator rather than silently followed or silently deviated from. The closure record was still placed per the ruling (the plan doc), because placement was a legitimate operator call independent of the stale premise; the correction was reported alongside, and the operator ruled the VHD/SH notes into this follow-up. Neither a silent workaround nor a blocked task.
- **Mechanism to keep (Mechanism 2 self-watch list):** **a status assertion arriving in a task brief is an input to verify, not a fact to build on** - exactly like a status assertion read out of BUILD_STATE. Corollaries: (a) when a brief says a thing does not exist / has not run / is absent, `ls` or `grep` it before designing around its absence, and design around the ANSWER; (b) when the verified state contradicts the brief, surface it and proceed on the part of the ruling that is premise-independent, rather than either following the stale premise or unilaterally re-scoping; (c) a count in a brief can be right while its direction is inverted - check which side of the gap the number is on.
- **Sister-cross-reference:** DRIFT-45 (verify-don't-assume - this is its inbound-premise face) / DRIFT-55 (prediction-scope: assumptions stated but unverified) / DRIFT-59 + DRIFT-60 (the same session's two self-caught lapses; all three share "the plausible thing was not the true thing") / CLAUDE.md section 18 + the BUILD_STATE CURRENT-row warning (the canonical statement of the class) / section 19.5 canonical-grep-first discipline.
