...
- **Last known working task def:** `tailrd-backend:396` (2026-07-28, AUDIT-218 closeout - the #500 docs auto-deploy, backend identical to `:395`, the revision the AUDIT-218 backfill executed on; kept in sync with section 9 / `PRODUCTION_READINESS.md`). Prior `:395` (PR #499 AUDIT-219 fix), `:393`, `:391`, read `:373`; see the section 9 note for evidence. Retained per supersede-not-overwrite.
