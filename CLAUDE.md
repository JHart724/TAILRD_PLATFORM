...
**Last known working task definition:** `tailrd-backend:315` (2026-06-18, /health healthy + login smoke PASS on Aurora at main HEAD `dab6afe`, PV module close - 6th + FINAL clinical module, PR #410. NOTE: the 6/6-module dashboard source=database probe is PENDING for this revision - health + login verified only). Prior: `:314` (`9c67bde`, AUDIT-300 UI clinical-content-leak), `:313` (`0ae6144`, CAD chunk-1 close, 6/6 smoke), `:312` (`4e0ae70`, CAD chunk-0), `:311` (`2c6f32d`, VHD module-complete), `:305` (2026-06-16, `8fdaff9`). Update after every deploy.
...
- **Last known working task def:** `tailrd-backend:315` (kept in sync with section 9 / `PRODUCTION_READINESS.md`).
