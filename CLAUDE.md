...
**Last known working task definition:** `tailrd-backend:319` (2026-06-22, /health healthy + login + 6/6-module dashboard source=database smoke PASS on Aurora at main HEAD `6469796`, AUDIT-186 T1-broader LVESD batch - +4 net-new DET_OK in CODE (real 207 -> 211): the AR LVESD>50 Class-2a COR fork (GAP-VHD-103 PARTIAL -> DET_OK) + SH-024 (TR RV-dysfunction) + VHD-060 (IE large vegetation) + VHD-100 (mech-valve-pregnancy anti-Xa), PR #414. NOTE: the 4 new + 2 enhanced rules realize in PROD DATA only after a re-seed - detection is ingestion-time + stored, so the existing demo gaps are stale until re-seed). Prior: `:317` (`5f42e05`, AUDIT-184 hollow-DET_OK repair, 12 slugs + 8 over-fires, #412), `:316` (`577a20c`, docs task-def bump #411), `:315` (`dab6afe`, PV module close #410, 6/6 smoke), `:314` (`9c67bde`, AUDIT-300 UI clinical-content-leak), `:313` (`0ae6144`, CAD chunk-1 close), `:312` (`4e0ae70`, CAD chunk-0). Update after every deploy.
...
- **Last known working task def:** `tailrd-backend:319` (kept in sync with section 9 / `PRODUCTION_READINESS.md`).
