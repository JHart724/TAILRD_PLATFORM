...
- **Last known working task def:** `tailrd-backend:442` (2026-08-11, BAA_GUARD_MODE=strict flip - AUDIT-214/215 fail-closed BAA guard LIVE; env-only derivation from `:441`, added `BAA_GUARD_MODE=strict`, SAME image `d96df987`, no rebuild and no deploy dispatch; rollout 1/1 HEALTHY, /health healthy. Enforcement proven live: positive probe 6 tenants 0 throws + 0 block events across 57,289 rows; negative control on a non-existent tenant threw `BAANotExecutedError` per §164.308(b)(1) + 1 block event. Rollback = `update-service` to `:441` (audit), env-only, ~1-2 min. Prior: `tailrd-backend:441` (2026-08-11, 99d9aab PR #551 Build & Deploy dispatch, image `d96df987`, applied the `isSyntheticData` migration). Prior: `tailrd-backend:440` (2026-08-09, in arrears, pre-AUDIT-213 deploy, image `1a585562`). Kept in sync with section 9 / PRODUCTION_READINESS.md. Prior: `tailrd-backend:439` (2026-08-09, MFA_ENFORCED activation, AUDIT-009/240 runbook step 3: env-only derivation from `:438`, `MFA_ENFORCED=true` added to the container env, SAME image digest `d06dcc4a`, rollout 1/1 HEALTHY, runtime `isMfaEnforced()=true`; `:280` enrollment-forcing now active for SUPER_ADMIN + HOSPITAL_ADMIN (non-privileged tracked-not-forced), one privileged account enrolled (SUPER_ADMIN JHart) and unaffected so any OTHER privileged account is forced to enroll on next request; durable - deploy.yml derives each task-def from the live revision and carries env forward; rollback = register a revision without the var. Kept in sync with section 9 / `PRODUCTION_READINESS.md`.) Prior: `tailrd-backend:438` (2026-08-09, PR #543 AUDIT-242 RESOLVED closeout docs auto-deploy). Prior: `tailrd-backend:437` (2026-08-08, PR #542 AUDIT-242 real-TOTP fix, digest `c367d916`). Prior: `tailrd-backend:425` (2026-08-05, deploy-bearing merge PR #529 AUDIT-230 supersede ordering (retire-then-insert, atomic), DIGEST-PINNED to verified `c60be4c`; the live proof ran the fixed path on the exact 3 rows that broke it - 3 clock-discriminated supersessions ELIGIBLE -> INDETERMINATE with the criteria hash identical on both sides, zero duplicate current pairs, no dangling `supersededBy`, and the surfaced stale indicator cleared to `stale: false` with no axis firing; kept in sync with section 9 / `PRODUCTION_READINESS.md`). Prior `:424` (PR #528 phase directive + AUDIT-148 ruling, in arrears), `:423` (PR #527 trials-module closeout, in arrears), `:422` (PR #526 trials PR 3), `:421` (PR #525 AUDIT-229, in arrears), `:420` (PR #524 AUDIT-228 closeout, in arrears), `:419` (PR #523 AUDIT-228), `:418` (PR #522 identity implementation, in arrears), `:417` (PR #521 identity design docs, in arrears), `:416` (PR #520 AUDIT-227 closeout docs, in arrears), `:415` (PR #519 AUDIT-227), `:413` (PR #517 AUDIT-226), `:409` (PR #513 Tranche 3 Slice 1), `:407` (PR #511 AUDIT-223 index), `:404` (PR #508 AUDIT-222), `:402` (PR #506 AUDIT-225), `:399` (PR #503 AUDIT-221), `:396` (AUDIT-218 closeout), `:395`, `:393`, `:391`, read `:373`; see the section 9 note for evidence. Retained per supersede-not-overwrite.
...
- **PARSED-CANONICAL-DOC (operator-gated despite the `docs/**` / repo-root `*.md` glob):** a `docs/**` or repo-root `*.md` path that is machine-parsed by ANY CI gate, test, or build script is NOT auto-merge-eligible; it routes to ALWAYS-OPERATOR-GATED, the same tier as `backend/**`, because parsed docs carry code-equivalent blast radius. Catch #89: a 6-line annotation to `docs/clinical/CLINICAL_KNOWLEDGE_BASE_v4.0.md` shifted the hardcoded line ranges in `extractSpec.ts`, failing Gate 3 (spec.json staleness) + `extractSpec.test.ts`, yet the `docs/**` glob had classified it auto-eligible.
  - **Fast-path snapshot** (known parsed-canonical set at this commit; non-exhaustive, may rot): `docs/clinical/CLINICAL_KNOWLEDGE_BASE_v4.0.md` (extractSpec line-range parse; Gate 3 + Gate 4 + Gate 6 + `extractSpec.test.ts`); `docs/audit/canonical/**` (crosswalk / spec / code / reconciliation JSON + `audit_runs.jsonl`; Gates 2/3/4/5 + reconcile/validate/render tests); `docs/audit/PHASE_0B_*_AUDIT_ADDENDUM.md` + `docs/audit/PHASE_0B_CROSS_MODULE_SYNTHESIS.md` (generated and line-parsed by `parseExistingAddendum.ts`; Gates 1/4); **`docs/audit/AUDIT_FINDINGS_REGISTER.md`** (read live and asserted against by `backend/tests/scripts/registerOpenCount.test.ts:123`, which runs under the Jest Tests job - `ci.yml` `npm test`, not the auditCanonical workflow, which is why the auditCanonical path-trigger check alone does not surface it); **`docs/audit/AUDIT_222_RULEID_ASSIGNMENT.md`** (read live and asserted row-for-row against the engine's `gaps.push` order by `backend/tests/ingestion/audit222RuleIdFreeze.test.ts:19`, same Jest Tests job); **`docs/PATH_TO_ROBUST.md`** (every `<!--@checked KEY-->` figure asserted against its derived source - the synthesis TOTAL row, `gapRuleEngine.ts`, `registerOpenCount.ts` and this section-9 task-def pointer - by `backend/tests/docs/pathToRobustFigures.test.ts`, same Jest Tests job; added 2026-08-05 IN THE SAME PR that made it parsed, because adding a parsed doc without listing it is precisely the AUDIT-229 defect). **`docs/audit/canonical/.manifests/*.manifest.json`** (the AUDIT-328 (iii) per-stage run manifests: read by `verifyManifest.ts` from auditCanonical Gate 5 UNCONDITIONALLY, and asserted by `backend/tests/scripts/auditCanonical/manifestVerify.test.ts`; added 2026-08-20 IN THE SAME PR that created them, for the same reason the PATH_TO_ROBUST entry above was. They are DERIVED OUTPUT and must never be hand-edited - on a merge conflict, delete and regenerate per AUDIT_METHODOLOGY section 24). Both were added 2026-08-04 under AUDIT-229 after the snapshot was found to have rotted exactly as its own "non-exhaustive, may rot" caveat predicted - see that finding for why the caveat is not a substitute for a mechanical backstop.
