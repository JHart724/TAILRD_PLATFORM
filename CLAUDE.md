...
**Last known working task definition:** `tailrd-backend:317` (2026-06-19, /health healthy + login + 6/6-module dashboard source=database smoke PASS on Aurora at main HEAD `5f42e05`, AUDIT-184 hollow-DET_OK repair - threaded 12 slugs + closed 8 over-fires in CODE, PR #412. NOTE: the 8 over-fire repairs realize in PROD DATA only after a re-seed - the existing demo gaps were computed pre-fix at ingestion-time; the code fix is unit-proven + deployed but the stored gaps are stale until re-seed). Prior: `:316` (`577a20c`, docs task-def bump #411), `:315` (`dab6afe`, PV module close #410, 6/6 smoke), `:314` (`9c67bde`, AUDIT-300 UI clinical-content-leak), `:313` (`0ae6144`, CAD chunk-1 close), `:312` (`4e0ae70`, CAD chunk-0). Update after every deploy.
...
- **Last known working task def:** `tailrd-backend:317` (kept in sync with section 9 / `PRODUCTION_READINESS.md`).
