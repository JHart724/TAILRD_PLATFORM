...
export interface SummaryMetric {
  ...
  colorScheme?: 'porsche' | 'green' | 'amber' | 'crimson' | 'arterial' | 'teal' | 'titanium';
}
...
function BaseDetailModal<T extends Record<string, any>>({
 ...
 return (
 <div key={metric.label} className={`${scheme.bg} rounded-xl p-6 border ${scheme.border} shadow-chrome-card`}>
 ...
 <div key={`chart-section-${i}`} className="bg-white rounded-xl p-6 border border-titanium-200 shadow-chrome-card">{section}</div>
 ))}
 </div>
 )}
 ...
 <tr key={columns[0] ? String(row[columns[0].key]) : rowIdx} className="border-b border-titanium-200 hover:bg-chrome-50 transition-colors ease-chrome">
 {columns.map(col => (
