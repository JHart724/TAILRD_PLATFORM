...
const HFMonthDetailModal: React.FC<HFMonthDetailModalProps> = ({
 ...
 <span className={`font-medium ml-1 ${data.realized >= data.projected ? 'text-teal-700' : 'text-amber-600'}`}>
 ...
 <div className={`rounded-lg p-6 text-center ${isPositive ? 'bg-chrome-50' : 'bg-amber-50'}`}>
 <div className="flex items-center justify-center mb-3">
 {isPositive ? (
 <TrendingUp className="w-8 h-8 text-teal-700" />
 ) : (
 ...
 <div className={`text-2xl font-bold mb-1 ${isPositive ? 'text-teal-700' : 'text-amber-900'}`}>
 {formatMoney(Math.abs(variance))}
 </div>
 <div className={`text-sm ${isPositive ? 'text-teal-700' : 'text-amber-700'}`}>
 {isPositive ? 'Over Target' : 'Under Target'}
 </div>
 <div className={`text-xs mt-1 font-medium ${isPositive ? 'text-teal-700' : 'text-amber-600'}`}>
