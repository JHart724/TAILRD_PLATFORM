...
const DeviceUnderutilizationPanel: React.FC = () => {
 ...
 {filteredPatients.length} patients • ${toFixed(totalRevenue / 1000, 0)}K potential revenue
 </div>
 </div>
 </div>
 ...
 <span key={comorbidity} className="px-2 py-0.5 bg-titanium-100 text-titanium-700 text-xs rounded">
 {comorbidity}
 </span>
 ))}
 </div>
 ...
 <div key={barrier} className="text-xs text-medical-red-700 bg-medical-red-50 p-2 rounded border border-medical-red-200 mb-1">
 {barrier}
 </div>
 ))}
 </div>
