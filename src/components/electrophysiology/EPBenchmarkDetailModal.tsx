...
const EPBenchmarkDetailModal: React.FC<EPBenchmarkDetailModalProps> = ({
  ...
  const delta = ourValue - nationalValue;
 ...
 {delta > 0 ? '+' : ''}{delta}{unit} vs national ({trendPercentage.toFixed(1)}%)
 </div>
 </div>
 </div>
 <button
 ...
 <div className="text-2xl font-bold text-green-900">{percentile}th</div>
 <div className="text-sm text-green-700">Percentile</div>
 </div>
 <div className="bg-purple-50 rounded-lg p-4 text-center">
 <div className="text-2xl font-bold text-purple-900">{comparisonData.top10}{unit}</div>
 <div className="text-sm text-purple-700">Top 10% Target</div>
 </div>
 </div>
