...
import {
  ckdSGLT2iPatients, compassPatients, pcsk9Patients, cardiacRehabPatientsCAD,
  lpaPatients, sglt2iUnderdosingPatients, stableAnginaPatients, ranolazinePatients,
...
function daptUrgencyLabel(kv: Record<string, string | number>): { label: string; color: string } {
  ...
  return { label: 'HIGH — DES 90-365 days', color: 'text-amber-600 bg-amber-50' };
}
...
function renderDigoxinAlert(pt: CADGapPatient): React.ReactNode {
  ...
  return (
    <div className={`${beersFlag ? 'bg-red-50 border-red-200' : 'bg-chrome-50 border-titanium-300'} border-2 rounded-xl p-4 space-y-2`}>
      <h5 className={`font-bold flex items-center gap-2 text-sm ${beersFlag ? 'text-red-800' : 'text-gray-500'}`}>
        <AlertTriangle className={`w-4 h-4 ${beersFlag ? 'text-red-600' : 'text-gray-500'}`} />
      ...
      <p className={`text-xs flex items-center gap-1 ${beersFlag ? 'text-red-500' : 'text-gray-500'}`}>
        <Zap className={`w-3 h-3 flex-shrink-0 ${beersFlag ? 'text-red-400' : 'text-gray-500'}`} />
...
function renderBBDeprescribing(pt: CADGapPatient): React.ReactNode {
  return (
    <div className="bg-chrome-50 border-2 border-titanium-300 rounded-xl p-4 space-y-2">
      <h5 className="font-bold text-gray-500 flex items-center gap-2 text-sm">
...
function renderSAQTrend(pt: CADGapPatient): React.ReactNode {
  ...
  const arrowColor = prior != null ? (score > prior ? 'text-teal-700' : score < prior ? 'text-red-600' : 'text-titanium-500') : '';
...
function renderSYNTAXDisplay(pt: CADGapPatient): React.ReactNode {
  ...
  const tierColor =
    ...
    syntaxResult.tier === 'Intermediate' ? 'text-gray-500' :
    'text-teal-700';
...
function renderCADPredictedEvent(gapId: string, pt: CADGapPatient): React.ReactNode {
  ...
  if ((gapId.includes('52') || gapId.includes('pcsk9') || gapId.includes('post-acs')) && pt.keyValues['ACS Date'] && pt.keyValues['PCSK9i Window Remaining']) {
    return (
      <div className="mt-2 px-3 py-2 bg-chrome-50 border border-titanium-300 rounded-lg">
        <div className="text-xs font-semibold text-gray-500 mb-1">Treatment Window Countdown</div>
        ...
        <div className="text-xs text-gray-500 mt-0.5">ODYSSEY OUTCOMES: Greatest absolute benefit in first 90 days post-ACS</div>
      </div>
    );
  }
  ...
  if (gapId.includes('60') || gapId.includes('cabg')) {
    ...
    if (typeof cabgDate === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(cabgDate)) {
      ...
      const riskColor = svg.riskCategory === 'very_high' ? 'text-red-700' : svg.riskCategory === 'high' ? 'text-red-600' : svg.riskCategory === 'moderate' ? 'text-gray-500' : 'text-teal-700';
...
const CADClinicalGapDetectionDashboard: React.FC = () => {
  ...
  const { data: moduleDashboard, loading: dashboardLoading } = useModuleDashboard('coronary-intervention');
  ...
  const tierColor = (tier?: string) => {
    ...
    if (tier.includes('B')) return 'bg-amber-50 text-amber-600';
      ...
      <div className="metal-card bg-white border border-titanium-200 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-red-50 border border-red-200 rounded-xl p-4">
            ...
            <div className="text-2xl font-bold text-red-800">{displayPatients.toLocaleString()}</div>
          </div>
                ...
                <div className="flex-1">
                  ...
                  {gap.category === 'Discovery' && (
                    <div className="text-xs font-semibold text-slate-700 mt-1 flex items-center gap-1">
                      <span className="text-teal-700">&#x2B21;</span> Discovery — Net new patients
                    </div>
                  )}
                ...
                <div className="border-t border-titanium-200 p-5 space-y-5">
                  ...
                  {gap.whyMissed && (
                    <div className="bg-chrome-50 border border-titanium-300 rounded-xl p-4">
                      <h4 className="font-semibold text-gray-500 mb-1 flex items-center gap-2">
                        <Search className="w-4 h-4 text-gray-500" />
                        Why standard systems miss this
                      </h4>
                      <p className="text-sm text-gray-500">{gap.whyMissed}</p>
                    </div>
                  )}
