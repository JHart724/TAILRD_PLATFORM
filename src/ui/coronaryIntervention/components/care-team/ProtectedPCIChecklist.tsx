...
const ProtectedPCIChecklist: React.FC = () => {
 ...
 <div className={`w-3 h-3 rounded-full ${getCompletionPercentage(items) === 100 ? 'bg-chrome-50' : 'bg-chrome-50'}`} />
 ...
 {items.map(item => (
 <div key={item.id} className="flex items-start gap-3 p-3 rounded-lg bg-titanium-50">
 <button
 ...
 item.completed
 ? 'bg-chrome-50 border-titanium-300 text-white'
 : 'border-titanium-300 hover:border-titanium-300'
 }`}
 >
 ...
 <div className="flex items-center gap-2">
 <span className={`text-sm ${item.completed ? 'text-titanium-600 line-through' : 'text-titanium-800'}`}>
 {item.label}
 </span>
 ...
 <div className="flex items-center justify-between mb-4">
 <div>
 <h2 className="text-2xl font-bold text-titanium-800 mb-2">Protected PCI Checklist</h2>
 <div className="flex items-center gap-6 text-sm text-titanium-600">
 ...
 pciState.status === 'active' ? 'bg-chrome-50 text-teal-700' :
 pciState.status === 'paused' ? 'bg-amber-50 text-amber-600' :
 ...
 className="flex items-center gap-2 px-6 py-3 bg-chrome-50 text-white rounded-lg hover:bg-chrome-50 transition-colors font-medium"
 >
 ...
 className="flex items-center gap-2 px-4 py-2 bg-chrome-50 text-white rounded-lg hover:bg-chrome-50 transition-colors"
 >
 ...
 <div key={goal.parameter} className="p-4 rounded-lg bg-titanium-50 border border-titanium-200">
 <div className="text-sm font-medium text-titanium-700 mb-2">{goal.parameter}</div>
 <div className="text-xs text-titanium-500 mb-2">Target: {goal.target}</div>
 <div className="flex items-center gap-2">
 <input
 type="text"
 placeholder="Current"
 ...
 />
 <div className={`w-3 h-3 rounded-full ${goal.achieved ? 'bg-chrome-50' : 'bg-titanium-300'}`} />
 </div>
 </div>
 ))}
 </div>
 ...
 <Clock className="w-5 h-5 text-porsche-600" />
 Procedure Phases
 </h3>
 ...
 phase.status === 'completed' ? 'bg-chrome-50 text-white' :
 phase.status === 'active' ? 'bg-porsche-500 text-white' :
 'bg-titanium-300 text-titanium-600'
 }`}>
 {index + 1}
 </div>
 <h4 className="font-medium text-titanium-800">{phase.name}</h4>
 </div>
 ...
 <div key={item.id} className="flex items-center gap-2 p-2 rounded bg-titanium-50">
 <button
