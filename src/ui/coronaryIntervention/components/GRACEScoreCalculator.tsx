...
const GRACEScoreCalculator: React.FC<{ patientData?: PatientContext }> = ({ patientData }) => {
  const [inputs, setInputs] = useState<GRACEInputs>({
 age: patientData?.age ?? 65,
 heartRate: patientData?.heartRate ?? 75,
 systolicBP: patientData?.systolicBP ?? 120,
 creatinine: patientData?.creatinine ?? 1.0,
 killipClass: 1,
 cardiacArrest: false,
 stElevation: false,
 ...
 <div>
 <h2 className="text-2xl font-bold text-titanium-900 font-sf">GRACE Score Calculator</h2>
 <p className="text-titanium-600">Global Registry of Acute Coronary Events Risk Assessment</p>
 </div>
 </div>
 ...
 />
 <span className="text-sm font-medium text-titanium-700">Cardiac Arrest at Admission</span>
 </label>
 ...
 className="rounded text-medical-red-600"
 />
 <span className="text-sm font-medium text-titanium-700">ST Elevation on ECG</span>
 </label>
