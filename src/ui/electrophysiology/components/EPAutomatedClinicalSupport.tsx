...
const EPAutomatedClinicalSupport: React.FC = () => {
 ...
 <div key={factor.key} className={`flex items-center gap-2 ${factor.value ? 'text-medical-purple-700' : 'text-titanium-500'}`}>
 <div className={`w-2 h-2 rounded-full ${factor.value ? 'bg-medical-purple-500' : 'bg-titanium-300'}`}></div>
 <span>{factor.label}</span>
 </div>
 ))}
 </div>
