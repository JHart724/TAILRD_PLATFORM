...
const EPAutomatedClinicalSupport: React.FC = () => {
 ...
 <div className="text-2xl font-bold text-titanium-900 font-sf">{stats.laacEligible}</div>
 <div className="text-xs text-porsche-700">+{stats.laacConsider} to consider</div>
 </div>
 ...
 <div className="text-2xl font-bold text-titanium-900 font-sf">{stats.ablationEligible}</div>
 <div className="text-xs text-medical-red-700">+{stats.ablationConsider} to consider</div>
 </div>

 <div className="bg-gradient-to-r from-crimson-50 to-crimson-100 rounded-xl p-4 border border-crimson-200">
 <div className="text-sm font-semibold text-titanium-600 uppercase tracking-wider mb-1">
 Avg Risk Scores
 </div>
 <div className="text-xl font-bold text-titanium-900 font-sf">{stats.avgCha2ds2vasc} / {stats.avgHasbled}</div>
 <div className="text-xs text-crimson-700">CHA₂DS₂-VASc / HAS-BLED</div>
 </div>
 </div>
 ...
 <div className="flex items-center gap-3">
 <div className="w-3 h-3 bg-crimson-500 rounded-full"></div>
 <span className="font-medium">Consider LAAC</span>
 </div>
 <span className="font-bold text-crimson-700">{stats.laacConsider} patients</span>
 </div>
 ...
 <span className="font-bold text-crimson-700">{stats.ablationConsider} patients</span>
 </div>
 <div className="flex items-center justify-between p-3 bg-medical-red-50 rounded-lg border border-medical-red-200">
 <div className="flex items-center gap-3">
 <div className="w-3 h-3 bg-medical-red-500 rounded-full"></div>
 ...
 <div key={assessment.id} className="flex items-center justify-between p-4 bg-titanium-50 rounded-lg">
 <div className="flex items-center gap-4">
 <div>
 <div className="font-semibold text-titanium-900">{assessment.name}</div>
 ...
 <div className="text-xs text-titanium-600">CHA₂DS₂-VASc</div>
 <div className="text-sm font-bold text-titanium-900">{assessment.cha2ds2vasc}</div>
 </div>
 <div className="text-center">
 <div className="text-xs text-titanium-600">HAS-BLED</div>
 <div className="text-sm font-bold text-titanium-900">{assessment.hasbled}</div>
 </div>
 ...
 filterEligibility === filter.id
 ? 'bg-white text-white'
 : 'bg-titanium-200 text-titanium-600'
 }`}>
 {filter.count}
 </span>
 </button>
 ...
 <div
 key={assessment.id}
 className="metal-card p-6 border-l-4 border-l-[#4A6880] cursor-pointer transition-all duration-300 hover:shadow-chrome-elevated"
 onClick={() => setSelectedAssessment(selectedAssessment === assessment.id ? null : assessment.id)}
 >
 <div className="flex items-start justify-between mb-4">
 <div className="flex-1">
 <div className="flex items-center gap-2">
 <h3 className="text-lg font-bold text-titanium-900">{assessment.name}</h3>
 <button
 onClick={(e) => {
 e.stopPropagation();
 togglePatientDetails(assessment.id);
 }}
 className="p-1 rounded-full hover:bg-titanium-100 transition-colors"
 >
 {expandedPatients[assessment.id] ? (
 <ChevronUp className="w-4 h-4 text-titanium-600" />
 ) : (
 <Plus className="w-4 h-4 text-titanium-600" />
 )}
 </button>
 </div>
 <p className="text-sm text-titanium-600">{assessment.mrn} • Age {assessment.age} • {assessment.gender}</p>
 <p className="text-sm text-titanium-600">{assessment.provider} • Scanned: {assessment.lastScan}</p>
 </div>
 ...
 <div className="text-sm text-titanium-600 mb-1">CHA₂DS₂-VASc</div>
 ...
 <div className="text-sm text-titanium-600 mb-1">HAS-BLED</div>
 <div className="text-xl font-bold text-crimson-600">{assessment.hasbled}</div>
 </div>
 </div>
 ...
 <span className="text-sm text-titanium-600">LAAC Eligibility:</span>
 <div className={`px-2 py-1 rounded text-xs font-semibold ${getEligibilityColor(assessment.laacEligibility)}`}>
 {assessment.laacEligibility.replace('-', ' ').toUpperCase()}
 </div>
 </div>
 <div className="flex items-center justify-between">
 <span className="text-sm text-titanium-600">Confidence Score:</span>
 ...
 <div className="space-y-3">
 <div className="flex items-center justify-between">
 <span className="text-sm text-titanium-600">Ablation Eligibility:</span>
 <div className={`px-2 py-1 rounded text-xs font-semibold ${getEligibilityColor(assessment.ablationEligibility)}`}>
 ...
 <div className="flex items-center justify-between">
 <span className="text-sm text-titanium-600">AF Type:</span>
 <span className="font-semibold text-titanium-900 capitalize">{assessment.afType}</span>
 </div>
 </div>
 )}
 ...
 <div key={factor.key} className={`flex items-center gap-2 ${factor.value ? 'text-medical-red-700' : 'text-titanium-500'}`}>
 <div className={`w-2 h-2 rounded-full ${factor.value ? 'bg-medical-red-500' : 'bg-titanium-300'}`}></div>
 ...
 <div key={factor.key} className={`flex items-center gap-2 ${factor.value ? 'text-crimson-700' : 'text-titanium-500'}`}>
 <div className={`w-2 h-2 rounded-full ${factor.value ? 'bg-crimson-500' : 'bg-titanium-300'}`}></div>
 <span>{factor.label}</span>
 </div>
 ))}
 </div>
 ...
 {/* LAAC Specific Factors */}
 {activeTab === 'laac-candidates' && (
 <div className="bg-porsche-50/50 rounded-lg p-4">
 <h4 className="font-semibold text-titanium-900 mb-3 flex items-center gap-2">
 <Shield className="w-4 h-4 text-porsche-600" />
 LAAC Specific Factors
 </h4>
 <div className="grid grid-cols-2 gap-2 text-sm">
 {[
 ...
 <div key={factor.key} className={`flex items-center gap-2 ${factor.value ? 'text-porsche-700' : 'text-titanium-500'}`}>
 <div className={`w-2 h-2 rounded-full ${factor.value ? 'bg-porsche-500' : 'bg-titanium-300'}`}></div>
 <span>{factor.label}</span>
 </div>
 ))}
 </div>
 ...
 <span className="text-titanium-600">AF Type:</span>
 <span className="font-semibold text-titanium-900 capitalize">{assessment.afType}</span>
 </div>
 <div className="flex justify-between">
 <span className="text-titanium-600">Medication Failures:</span>
 <span className="font-semibold text-titanium-900">{assessment.medicationFailures}</span>
 </div>
 <div>
 <span className="text-titanium-600">Symptoms:</span>
 <div className="flex flex-wrap gap-1 mt-1">
 {assessment.symptoms.map((symptom, idx) => (
 <span key={symptom} className="px-2 py-1 bg-medical-red-100 text-medical-red-700 text-xs rounded">
 {symptom}
 </span>
 ))}
 </div>
 ...
 <div className="bg-gradient-to-br from-crimson-50 to-crimson-100 rounded-xl p-6">
 <h4 className="font-semibold text-crimson-700 mb-4">High Bleeding Risk</h4>
 <div className="text-3xl font-bold text-crimson-700 mb-2">
 {automatedAssessments.filter(p => p.hasbled >= 3).length}
 </div>
 <div className="text-sm text-crimson-600">HAS-BLED ≥3</div>
 <div className="text-xs text-crimson-500 mt-2">
 {Math.round((automatedAssessments.filter(p => p.hasbled >= 3).length / automatedAssessments.length) * 100)}% of total population
 </div>
 </div>

 <div className="bg-gradient-to-br from-porsche-50 to-porsche-100 rounded-xl p-6">
 <h4 className="font-semibold text-porsche-900 mb-4">Optimal LAAC Candidates</h4>
 <div className="text-3xl font-bold text-porsche-700 mb-2">
 {automatedAssessments.filter(p => p.cha2ds2vasc >= 3 && p.anticoagContraindicated).length}
 </div>
 <div className="text-sm text-porsche-600">High risk + contraindicated</div>
 <div className="text-xs text-porsche-500 mt-2">Clear LAAC indication</div>
 </div>
 </div>
 </div>
 </div>
