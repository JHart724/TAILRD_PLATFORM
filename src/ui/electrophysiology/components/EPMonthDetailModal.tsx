...
interface EPMonthDetailModalProps {
  month: string;
  projected: number;
  realized: number;
...
const formatPercent = (value: number): string => {
  const sign = value >= 0 ? '+' : '';
  return `${sign}${toFixed(value, 1)}%`;
};


const EPMonthDetailModal: React.FC<EPMonthDetailModalProps> = ({
  month,
  projected,
  realized,
  breakdown,
 ...
 <p className="text-sm">
 <span className="text-chrome-600">Projected:</span> 
 <span className="font-medium ml-1">{formatMoney(data.projected)}</span>
 </p>
 <p className="text-sm">
 <span className="text-teal-700">Realized:</span> 
 <span className="font-medium ml-1">{formatMoney(data.realized)}</span>
 </p>
 <p className="text-sm">
 <span className="text-gray-600">Variance:</span> 
 <span className={`font-medium ml-1 ${data.realized >= data.projected ? 'text-teal-700' : 'text-red-600'}`}>
 ...
 <Calendar className="w-6 h-6 text-chrome-600 mr-3" />
 <div>
 <h2 className="text-2xl font-bold text-gray-900">
 {month} 2025 Performance
 </h2>
 <p className="text-gray-600 mt-1">Electrophysiology Revenue Analysis</p>
 </div>
 </div>
 <button
 onClick={onClose}
 ...
 <div className={`rounded-lg p-6 text-center ${isPositive ? 'bg-chrome-50' : 'bg-red-50'}`}>
 <div className="flex items-center justify-center mb-3">
 {isPositive ? (
 <TrendingUp className="w-8 h-8 text-teal-700" />
 ) : (
 ...
 <div className={`text-2xl font-bold mb-1 ${isPositive ? 'text-teal-700' : 'text-red-900'}`}>
 {formatMoney(Math.abs(variance))}
 </div>
 <div className={`text-sm ${isPositive ? 'text-teal-700' : 'text-red-700'}`}>
 {isPositive ? 'Over Target' : 'Under Target'}
 </div>
 <div className={`text-xs mt-1 font-medium ${isPositive ? 'text-teal-700' : 'text-red-600'}`}>
...
export default EPMonthDetailModal;