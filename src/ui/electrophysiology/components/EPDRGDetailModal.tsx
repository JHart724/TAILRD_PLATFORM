...
const EPDRGDetailModal: React.FC<EPDRGDetailModalProps> = ({
 ...
 {toFixed((cases.filter(c => c.marginPercent >= 20).length / cases.length) * 100, 1)}%
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="text-sm text-titanium-500">
 {toFixed((cases.filter(c => c.los <= targetLos).length / cases.length) * 100, 1)}%
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="text-sm text-titanium-500">
 {toFixed(cases.reduce((sum, c) => sum + c.marginPercent, 0) / cases.length, 1)}%
 </div>
 </div>
 </div>
 </div>
