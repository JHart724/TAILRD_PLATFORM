...
interface EPDRGDetailModalProps {
  drgCode: string;
  description: string;
  volume: number;
...
const EPDRGDetailModal: React.FC<EPDRGDetailModalProps> = ({
  drgCode,
  description,
  volume,
  avgReimbursement,
  ...
  // Robustness (AUDIT-304 EP DRG drill-down fix): the DRG feed is demo and may not
  // carry a case-level array. NEVER spread/iterate a non-array (the prior crash was
  // `[...cases]` where `cases` was the numeric case COUNT, throwing "cases is not
  // iterable" and taking the whole module to the error boundary). Coerce to a safe
  // array; an empty list renders an honest empty state instead of throwing.
...
export default EPDRGDetailModal;