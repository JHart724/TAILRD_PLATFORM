...
interface EPFacilityDetailModalProps {
  facilityName: string;
  location: string;
  totalRevenue: number;
  patientCount: number;
...
const EPFacilityDetailModal: React.FC<EPFacilityDetailModalProps> = ({
  facilityName,
  location,
  totalRevenue,
  patientCount,
 ...
 <p className="text-sm text-gray-600">
 Share: <span className="font-medium">{toFixed((data.revenue / totalRevenue) * 100, 1)}%</span>
 </p>
 </div>
 );
 }
 ...
 <Building2 className="w-6 h-6 text-chrome-600 mr-3" />
 <div>
 ...
 <div className="text-sm text-chrome-700">EP Patient Population</div>
 </div>
 ...
 <div className="text-sm text-teal-700">Ablation Therapy Optimization Rate</div>
 </div>
 ...
 <div className="text-sm text-arterial-700">Opportunity Capture Rate</div>
 </div>
 </div>
 ...
 {toFixed((item.revenue / totalRevenue) * 100, 1)}% of total
 </div>
 </div>
 </div>
 ))}
 ...
 <th className="pb-3 font-medium text-right">Ablation Therapy Rate</th>
 ...
 provider.ablationRate >= 80 ? 'text-emerald-600' : 
 provider.ablationRate >= 70 ? 'text-chrome-600' : 
 provider.ablationRate >= 60 ? 'text-amber-600' : 'text-red-600'
 }`}>
 {provider.ablationRate}%
 </span>
 </td>
 ...
 <div className="mt-6 p-4 bg-chrome-50 rounded-lg">
 <h4 className="font-semibold text-chrome-900 mb-3">Facility Performance Summary</h4>
 <div className="grid grid-cols-2 gap-4 text-sm">
 <div>
 <div className="flex justify-between mb-2">
 <span className="text-chrome-700">Total Providers:</span>
 <span className="font-medium text-chrome-900">{providers.length}</span>
 </div>
 <div className="flex justify-between mb-2">
 <span className="text-chrome-700">Avg Revenue/Patient:</span>
 <span className="font-medium text-chrome-900">{formatMoney(avgRevenuePerPatient)}</span>
 </div>
 </div>
 <div>
 <div className="flex justify-between mb-2">
 <span className="text-chrome-700">Top Performer:</span>
 <span className="font-medium text-chrome-900">{providers[0]?.name || 'N/A'}</span>
 </div>
 <div className="flex justify-between mb-2">
 <span className="text-chrome-700">Avg Ablation Therapy Rate:</span>
 <span className="font-medium text-chrome-900">{ablationRate}%</span>
 </div>
 </div>
 </div>
 </div>
...
export default EPFacilityDetailModal;