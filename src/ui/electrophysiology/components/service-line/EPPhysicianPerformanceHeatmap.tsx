...
const EPPhysicianPerformanceHeatmap: React.FC = () => {
 ...
 {sortedPhysicians.sort((a, b) => b.oacs_rx_rate - a.oacs_rx_rate)[0]?.oacs_rx_rate.toFixed(1)}% Prescription Rate
 </div>
 </div>
 ...
 {(sortedPhysicians.reduce((sum, p) => sum + p.inr_control_pct, 0) / sortedPhysicians.length).toFixed(1)}%
 </div>
 <div className="text-sm text-titanium-600">
 ...
 <div className="text-2xl font-bold text-chrome-800">{selectedPhysicianData.oacs_rx_rate.toFixed(1)}%</div>
 </div>
 ...
 }`}>{selectedPhysicianData.inr_control_pct.toFixed(1)}%</div>
 </div>
 ...
 <div className="text-2xl font-bold text-purple-800">{selectedPhysicianData.ablation_referral_rate.toFixed(1)}%</div>
 </div>
 </div>
 ...
 <div key={idx} className="text-xs bg-porsche-100 text-porsche-700 px-2 py-1 rounded">
 {therapy}
 </div>
 ))}
 </div>
