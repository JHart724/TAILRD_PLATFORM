...
const EPEquityAnalysis: React.FC = () => {
 ...
 if (isInverted) {
 if (delta <= -2) return 'bg-medical-green-500';
 if (delta <= 0) return 'bg-medical-amber-500';
 ...
 {selectedMetricData.overallAverage.toFixed(1)}{selectedMetricData.unit}
 </div>
 ...
 {group.value.toFixed(1)}{selectedMetricData.unit}
 </div>
 <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold ${
 (isInvertedMetric ? group.delta <= 0 : group.delta >= 0) 
 ? 'text-medical-green-700 bg-medical-green-100' 
 ...
 {group.delta > 0 ? '+' : ''}{group.delta.toFixed(1)}{selectedMetricData.unit}
 </div>
 ...
 {selectedSegmentData.groups.reduce((prev, curr) => 
 Math.abs(prev.delta) > Math.abs(curr.delta) ? prev : curr
 ).delta.toFixed(1)}{selectedMetricData.unit} gap
 </div>
 </div>
 ...
 {selectedSegmentData.groups.reduce((prev, curr) => 
 (isInvertedMetric ? prev.value < curr.value : prev.value > curr.value) ? prev : curr
 ).value.toFixed(1)}{selectedMetricData.unit}
 </div>
 </div>
 </div>
 ...
 .slice(0, 3)
 .map((group, index) => (
 <div key={index} className="p-3 bg-white rounded-lg border border-porsche-100">
 <div className="font-medium text-titanium-900 mb-1">{group.group} Cohort</div>
 <div className="text-sm text-titanium-600 mb-2">
 {group.count.toLocaleString()} patients • {Math.abs(group.delta).toFixed(1)}{selectedMetricData.unit} below average
 </div>
 <button
 onClick={() => {
 ...
 {selectedSegmentData.groups.find(g => g.group === selectedGroup)?.value.toFixed(1)}{selectedMetricData.unit}
 </div>
 <div className="text-sm text-porsche-600">
 vs {selectedMetricData.overallAverage.toFixed(1)}{selectedMetricData.unit} overall
 </div>
 </div>
 ...
 {selectedSegmentData.groups.find(g => g.group === selectedGroup)?.delta.toFixed(1)}{selectedMetricData.unit}
 </div>
