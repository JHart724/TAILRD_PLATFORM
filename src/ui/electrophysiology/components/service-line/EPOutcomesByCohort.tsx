...
const EPOutcomesByCohort: React.FC = () => {
 ...
 if (unit === 'events/100pt-yr') return `${value.toFixed(1)}`;
 ...
 {cohort.trend !== 'stable' && (cohort.trendValue > 0 ? '+' : '')}{cohort.trendValue.toFixed(1)}
 </span>
 </div>
 </div>
 ...
 <span key={idx} className="px-2 py-1 bg-porsche-100 text-porsche-700 text-xs rounded">
 {driver}
 </span>
 ))}
 ...
 Performance gap: {Math.abs(cohort.currentValue - cohort.targetValue).toFixed(1)}{selectedOutcomeData.unit} from target
 </div>
