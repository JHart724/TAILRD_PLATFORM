...
interface EPPatientDetailPanelProps {
  patient: EPWorklistPatient;
  onClose: () => void;
}

const EPPatientDetailPanel: React.FC<EPPatientDetailPanelProps> = ({ patient, onClose }) => {
 ...
 <span className="text-2xl font-bold text-gray-900">{patient.cha2ds2vascScore}%</span>
 </div>
 <p className="text-sm text-gray-600">CHA₂DS₂-VASc</p>
 </div>
 ...
 <div className="text-2xl font-bold text-amber-600 mb-2">{patient.treatmentGaps.length}</div>
 ...
 {/* Anticoagulation Optimization Dashboard */}
 {patient.fullChart.anticoagulation && (
 ...
 {/* Overall Anticoagulation Score */}
 <div className="mb-4 p-3 bg-white rounded-lg border">
 <div className="flex items-center justify-between mb-2">
 <span className="text-sm font-medium text-gray-700">Overall Anticoagulation Score</span>
 <span className={`px-3 py-1 rounded-full text-lg font-bold ${getAnticoagulationScoreColor(patient.fullChart.anticoagulation.overallScore)}`}>
 {patient.fullChart.anticoagulation.overallScore}%
 </span>
 </div>
 <div className="w-full bg-gray-200 rounded-full h-2">
 <div 
 className={`h-2 rounded-full transition-all duration-500 ${
 patient.fullChart.anticoagulation.overallScore >= 80 ? 'bg-green-500' :
 patient.fullChart.anticoagulation.overallScore >= 60 ? 'bg-yellow-500' : 'bg-red-500'
 }`}
 ...
 {/* Barriers */}
 {patient.fullChart.anticoagulation.barriers.length > 0 && (
 <div className="mb-4">
 <h4 className="font-semibold text-gray-900 mb-2 text-sm">Current Barriers</h4>
 <div className="flex flex-wrap gap-2">
 {patient.fullChart.anticoagulation.barriers.map((barrier, idx) => (
 ...
 <span>Last Review: {new Date(patient.fullChart.anticoagulation.lastReview).toLocaleDateString()}</span>
 <span>Next Review: {new Date(patient.fullChart.anticoagulation.nextReview).toLocaleDateString()}</span>
 </div>
 </div>
 )}
 ...
 Ablation Therapy Eligibility
 </h3>
 <div className="bg-white p-3 rounded border border-chrome-200">
 <div className="flex items-center gap-2 mb-2">
 <div className="w-3 h-3 bg-chrome-500 rounded-full"></div>
 <span className="font-medium text-chrome-800">Eligible for Ablation Therapy</span>
 </div>
 <p className="text-sm text-gray-600">
 Patient meets criteria for ablation evaluation based on current CHA₂DS₂-VASc and clinical status.
 ...
 <button 
 onClick={() => {
 console.log('Beginning care plan for patient:', patient.name, patient.mrn, 'Anticoagulation gaps:', patient.treatmentGaps);
...
export default EPPatientDetailPanel;