...
interface EPWorklistPatient {
  ...
  actionItems: {
 category: 'Anticoagulation' | 'Device' | 'Ablation' | 'Follow-up';
 description: string;
 dueDate: string;
 urgent: boolean;
  }[];
...
const EPPatientWorklistEnhanced: React.FC = () => {
  ...
  const [filterAnticoagulation, setFilterAnticoagulation] = useState<boolean>(false);
  const [filterDevice, setFilterDevice] = useState<boolean>(false);
  const [sortBy, setSortBy] = useState<keyof EPWorklistPatient>('riskScore');
  const [expandedAnticoagulation, setExpandedAnticoagulation] = useState<string | null>(null);
 ...
 riskCategory: 'III',
 priority: 'high',
 treatmentGaps: ['Ablation Referral not started', 'Rhythm Control suboptimal dose'],
 ...
 phenotype: 'AF/AFL',
 actionItems: [
 { category: 'Anticoagulation', description: 'Initiate Ablation Referral therapy', dueDate: '2025-10-20', urgent: true },
 { category: 'Device', description: 'Ablation-D evaluation', dueDate: '2025-10-22', urgent: true },
 { category: 'Ablation', description: 'BNP trending', dueDate: '2025-10-18', urgent: false },
 ],
 fullChart: {
 vitals: {
 bp: '138/82',
 ...
 'Recent 30-day readmission for arrhythmia exacerbation'
 ],
 allergies: ['Sulfa drugs', 'Shellfish'],
 recentHospitalizations: [
 { date: '2025-09-15', reason: 'arrhythmia exacerbation', los: 4 },
 { date: '2025-07-22', reason: 'Acute decompensated arrhythmia', los: 6 }
 ],
 gdmt: {
 overallScore: 62,
 pillars: {
 ...
 sglt2i: {
 status: 'not_started',
 reason: 'Not initiated despite DM and AF/AFL indication',
 ...
 opportunities: [
 {
 pillar: 'Ablation Referral',
 action: 'Initiate dapagliflozin for dual diabetes and arrhythmia benefit',
 priority: 'high',
 timeframe: '1-2 weeks'
 },
 {
 pillar: 'Rhythm Control',
 ...
 {
 pillar: 'ARNi',
 action: 'Transition OAC Therapy to ARNi for mortality benefit',
 priority: 'medium',
 timeframe: '4-6 weeks'
 }
 ]
 ...
 riskCategory: 'II',
 priority: 'medium',
 treatmentGaps: ['Rate Control not at target dose'],
 ...
 phenotype: 'AF/AFL',
 actionItems: [
 { category: 'Anticoagulation', description: 'Uptitrate metoprolol', dueDate: '2025-10-28', urgent: false },
 ...
 notes: [
 'Stable arrhythmia with good functional capacity',
 'Rate Control dose can be further optimized',
 ...
 riskCategory: 'IV',
 priority: 'high',
 treatmentGaps: ['ARNi not started', 'Rhythm Control contraindicated'],
 ablationEligible: true,
 lastVisit: '2025-10-15',
 assignedProvider: 'Dr. Martinez',
 recentAdmission: true,
 riskScore: 9.4,
 phenotype: 'Advanced arrhythmia',
 actionItems: [
 { category: 'Anticoagulation', description: 'ARNi transition plan', dueDate: '2025-10-17', urgent: true },
 { category: 'Device', description: 'Watchman evaluation', dueDate: '2025-10-20', urgent: true },
 ...
 notes: [
 'Advanced arrhythmia with frequent hospitalizations',
 'Refractory to standard medical therapy',
 'Candidate for advanced arrhythmia interventions',
 'Family meeting scheduled to discuss options'
 ],
 allergies: ['Penicillin'],
 recentHospitalizations: [
 { date: '2025-10-05', reason: 'Cardiogenic shock', los: 8 },
 { date: '2025-08-18', reason: 'Acute decompensated arrhythmia', los: 12 },
 { date: '2025-06-10', reason: 'arrhythmia exacerbation', los: 5 }
 ]
 }
 },
 {
 ...
 recentAdmission: false,
 riskScore: 4.1,
 phenotype: 'Bradycardia',
 actionItems: [
 { category: 'Ablation', description: 'Annual lab panel', dueDate: '2025-11-15', urgent: false },
 ...
 notes: [
 'Stable arrhythmia with preserved stroke risk score',
 ...
 riskCategory: 'III',
 priority: 'medium',
 treatmentGaps: ['Ablation Referral age consideration', 'Rate Control intolerance'],
 ablationEligible: true,
 ...
 phenotype: 'AF/AFL',
 actionItems: [
 { category: 'Anticoagulation', description: 'Review Ablation Referral risks/benefits', dueDate: '2025-10-25', urgent: false },
 { category: 'Device', description: 'Pacemaker consideration', dueDate: '2025-11-10', urgent: false },
 ],
 fullChart: {
 vitals: {
 bp: '108/68',
 ...
 'Rate Control intolerance due to bradycardia',
 'Cautious approach to Anticoagulation optimization',
 'Fall risk assessment completed'
 ],
 allergies: ['Rate Control', 'NSAIDs'],
 recentHospitalizations: [
 { date: '2025-08-05', reason: 'arrhythmia exacerbation', los: 3 }
 ]
 }
 },
  ];
 ...
 if (filterAnticoagulation && patient.treatmentGaps.length === 0) return false;
 if (filterDevice && !patient.ablationEligible) return false;
 if (debouncedSearch) {
   const q = debouncedSearch.toLowerCase();
   const searchable = [
     patient.name, patient.mrn, patient.priority,
     patient.assignedProvider, patient.phenotype || '',
     patient.riskCategory, ...patient.treatmentGaps
   ].join(' ').toLowerCase();
 ...
 <input
 type="checkbox"
 checked={filterAnticoagulation}
 onChange={(e) => setFilterAnticoagulation(e.target.checked)}
 className="rounded"
 />
 <span className="text-sm text-titanium-800">Anticoagulation Gaps Only</span>
 </label>
 ...
 <select
 value={sortBy}
 onChange={(e) => setSortBy(e.target.value as keyof EPWorklistPatient)}
 className="px-3 py-2 text-sm border border-titanium-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-porsche-500"
 >
 <option value="riskScore">Sort by Risk Score</option>
 <option value="lastVisit">Sort by Last Visit</option>
 <option value="age">Sort by Age</option>
 <option value="lvef">Sort by Stroke Risk Score</option>
 </select>
 </div>
 ...
 <span className="text-2xl font-bold text-titanium-900">{patient.cha2ds2vascScore}%</span>
 </div>
 <div className="text-xs text-titanium-600">Stroke Risk Score</div>
 </div>
 <div className="text-center p-3 bg-white rounded-lg border border-titanium-200">
 <div className={`text-lg font-bold px-2 py-1 rounded ${getCHA2DS2VAScColor(patient.riskCategory)}`}>
 CHA₂DS₂-VASc {patient.riskCategory}
 </div>
 <div className="text-xs text-titanium-600 mt-1">Functional Class</div>
 </div>
 <div className="text-center p-3 bg-white rounded-lg border border-titanium-200">
 <div className="text-2xl font-bold text-titanium-900">{patient.treatmentGaps.length}</div>
 <div className="text-xs text-titanium-600">Anticoagulation Gaps</div>
 </div>
 <div className="text-center p-3 bg-white rounded-lg border border-titanium-200">
 ...
 {/* Anticoagulation 4-Pillar Status */}
 {patient.fullChart?.gdmt && (
 <div className="mb-4">
 <button 
 onClick={(e) => {
 e.stopPropagation();
 setExpandedAnticoagulation(expandedAnticoagulation === patient.id ? null : patient.id);
 }}
 ...
 Anticoagulation 4-Pillar Status
 {expandedAnticoagulation === patient.id ? (
 <ChevronUp className="w-4 h-4 text-titanium-500" />
 ) : (
 <ChevronRight className="w-4 h-4 text-titanium-500" />
 )}
 </div>
 <span className={`px-2 py-1 rounded text-sm font-medium ${getAnticoagulationScoreColor(patient.fullChart.gdmt.overallScore)}`}>
 {patient.fullChart.gdmt.overallScore}%
 </span>
 </button>
 <div className="grid grid-cols-4 gap-2">
 {Object.entries(patient.fullChart.gdmt.pillars).map(([pillar, data]) => (
 <div key={pillar} className={`p-2 rounded border text-center ${getAnticoagulationStatusColor(data.status)}`}>
 <div className="flex items-center justify-center mb-1">
 {getAnticoagulationStatusIcon(data.status)}
 </div>
 <div className="text-xs font-medium">
 {pillar === 'arni' ? 'ARNi' : 
 pillar === 'betaBlocker' ? 'Rate Control' :
 pillar === 'sglt2i' ? 'Ablation Referral' :
 pillar === 'mra' ? 'Rhythm Control' : pillar}
 </div>
 </div>
 ))}
 </div>

 {/* Expanded Anticoagulation Details */}
 {expandedAnticoagulation === patient.id && (
 ...
 {pillar === 'arni' ? 'ARNi/OAC Therapy/ARB' : 
 pillar === 'betaBlocker' ? 'Rate Control' :
 pillar === 'sglt2i' ? 'Ablation Referral Inhibitor' :
 pillar === 'mra' ? 'Rhythm Control Receptor Antagonist' : pillar}
 </h4>
 </div>
 ...
 {/* Anticoagulation Optimization Opportunities */}
 ...
 <div className="mb-4">
 <div className="text-sm font-semibold text-titanium-700 mb-2">Anticoagulation Gaps:</div>
...
export default EPPatientWorklistEnhanced;