...
const EPWorklist: React.FC = () => {
 ...
 case 'Urgent': return 'bg-amber-50 text-amber-600 border-titanium-300';
 default: return 'bg-green-50 text-green-600 border-teal-700';
 }
  };

  const getStatusColor = (status: string) => {
 switch (status) {
 case 'Completed': return 'bg-chrome-50 text-teal-700';
 case 'In Progress': return 'bg-chrome-100 text-chrome-700';
 case 'Pre-procedure': return 'bg-amber-50 text-amber-600';
 ...
 <Shield className="w-4 h-4 text-chrome-500" />
 Risk Assessment
 </h4>
 ...
 patient.cha2ds2vasc >= 2 ? 'text-gray-500' : 'text-teal-700'
 }`}>
 {patient.cha2ds2vasc}
 </div>
 ...
 patient.hasbled >= 2 ? 'text-gray-500' : 'text-teal-700'
 }`}>
 {patient.hasbled}
 </div>
 <div className="text-xs text-titanium-500">
 ...
 <div>
 <h4 className="font-semibold text-titanium-800 mb-3 flex items-center gap-2">
 <Cpu className="w-4 h-4 text-arterial-500" />
 Device Status
 </h4>
 <div className="space-y-2">
 <div className="p-3 bg-white rounded-lg border border-titanium-200">
 <div className="font-medium text-titanium-800">{patient.deviceStatus.type}</div>
 ...
 <div className="text-xs text-titanium-600">Battery</div>
 <div className={`font-medium ${getBatteryColor(patient.deviceStatus.batteryStatus)}`}>
 {patient.deviceStatus.batteryStatus}
 </div>
 </div>
 <div className="p-2 bg-white rounded border border-titanium-200">
 <div className="text-xs text-titanium-600">Last Check</div>
 <div className="text-sm text-titanium-800">
 {new Date(patient.deviceStatus.lastInterrogation).toLocaleDateString()}
 </div>
 </div>
 </div>
 {patient.remoteMonitoring && (
 <div className="flex items-center gap-2 p-2 bg-titanium-300 rounded border border-teal-700">
 <Wifi className="w-4 h-4 text-teal-700" />
 <span className="text-sm text-teal-700">Remote monitoring active</span>
 </div>
 )}
 </div>
 </div>
 ...
 <div className="space-y-2">
 <div className="p-2 bg-white rounded border border-titanium-200">
 <div className="text-xs text-titanium-600">Antiarrhythmic</div>
 <div className="text-sm text-titanium-800">
 ...
 <div className="p-2 bg-white rounded border border-titanium-200">
 <div className="text-xs text-titanium-600">Anticoagulation</div>
 <div className="text-sm text-titanium-800">{patient.medications.anticoagulation}</div>
 </div>
 <div className="p-2 bg-white rounded border border-titanium-200">
 <div className="text-xs text-titanium-600">Beta Blocker</div>
 <div className="text-sm text-titanium-800">{patient.medications.betaBlocker || 'None'}</div>
 </div>
 </div>
 </div>
 ...
 patient.lastEcho.ef >= 50 ? 'text-teal-700' :
 patient.lastEcho.ef >= 35 ? 'text-gray-500' : 'text-red-600'
 }`}>
 {patient.lastEcho.ef}%
 </div>
 </div>
 ...
 <div className={`text-lg font-bold ${
 patient.lastEcho.laSize <= 40 ? 'text-teal-700' :
 patient.lastEcho.laSize <= 45 ? 'text-gray-500' : 'text-red-600'
 }`}>
 {patient.lastEcho.laSize}mm
 </div>
 </div>
 </div>
 <div className="text-xs text-titanium-500 mt-2">
 ...
 <div key={`${ablation.type}-${ablation.date}`} className="p-2 bg-white rounded border border-titanium-200">
 <div className="flex justify-between items-start">
 <div>
 <div className="font-medium text-titanium-800">{ablation.type}</div>
 <div className="text-xs text-titanium-500">
 {new Date(ablation.date).toLocaleDateString()}
 </div>
 </div>
 <span className={`text-xs px-2 py-1 rounded font-medium ${
 ablation.outcome === 'Success' ? 'bg-green-50 text-green-600' :
 ablation.outcome === 'Partial' ? 'bg-amber-50 text-amber-600' :
 ...
 <div>
 <h4 className="font-semibold text-titanium-800 mb-3 flex items-center gap-2">
 <Calendar className="w-4 h-4 text-chrome-500" />
 Follow-up
 </h4>
 <div className="p-3 bg-white rounded-lg border border-titanium-200">
 <div className="text-sm text-titanium-600 mb-1">Next Appointment</div>
 <div className="font-medium text-titanium-800 mb-2">
 {new Date(patient.nextFollowUp).toLocaleDateString()}
 </div>
 {patient.deviceStatus && (
 <div>
 <div className="text-xs text-titanium-600">Device Follow-up</div>
 <div className="text-sm text-titanium-800">
 ...
 {patient.notes && (
 <div className="mt-4">
 <h4 className="font-semibold text-titanium-800 mb-2">Notes</h4>
 <div className="p-3 bg-white rounded-lg border border-titanium-200">
 <div className="text-sm text-titanium-700">{patient.notes}</div>
 </div>
 </div>
 )}
 </div>
 ...
 <Search className="w-4 h-4 text-titanium-400" />
 <input
 type="text"
 placeholder="Search by name, MRN, or rhythm..."
 value={searchTerm}
 onChange={(e) => setSearchTerm(e.target.value)}
 className="flex-1 px-3 py-2 border border-titanium-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-porsche-500"
 />
 </div>
 ...
 <button
 onClick={exportToCSV}
 className="flex items-center gap-2 px-4 py-2 bg-titanium-600 text-white rounded-lg hover:bg-titanium-700 transition-colors"
 >
 ...
 <th className="p-3 text-left text-sm font-semibold text-titanium-700">Status</th>
 <th 
 className="p-3 text-left text-sm font-semibold text-titanium-700 cursor-pointer hover:text-titanium-900"
 ...
 <td className="p-3">
 <div>
 <div className="font-medium text-titanium-800">{patient.name}</div>
 <div className="text-sm text-titanium-500">Age {patient.age} • {patient.mrn}</div>
 </div>
 </td>
 <td className="p-3">
 ...
 <td className="p-3">
 <span className="font-medium text-titanium-800">{patient.procedureType}</span>
 </td>
 <td className="p-3">
 <div className="text-sm text-titanium-800">
