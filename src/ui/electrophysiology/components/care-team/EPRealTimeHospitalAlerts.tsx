...
const EPRealTimeHospitalAlerts: React.FC = () => {
  const [alerts, setAlerts] = useState<EPHospitalAlert[]>([
 {
 id: 'ALERT001',
 ...
 gdmtGaps: ['ARNi not initiated', 'Rhythm Control not started', 'Ablation Referral not prescribed'],
 contraindications: [],
 alertLevel: 'critical',
 ...
 'Order Loop Recorder evaluation'
 ],
 ...
 'ECHO pending to assess current Stroke Risk Score'
 ],
 allergies: ['NKDA']
 }
 },
 ...
 location: 'Floor',
 admitTime: '09:15',
 chiefComplaint: 'arrhythmia exacerbation',
 lvef: 28,
 bnp: 2340,
 currentGDMT: {
 betaBlocker: true,
 ...
 gdmtGaps: ['Rhythm Control not started', 'Ablation Referral not prescribed'],
 contraindications: ['Mild hyperkalemia (K+ 5.1)'],
 alertLevel: 'high',
 timeToIntervene: '< 24 hours',
 recommendations: [
 'Monitor K+ and consider Rhythm Control when < 5.0',
 'Start dapagliflozin 10mg daily',
 'Consider Ablation-D evaluation (QRS 142ms)',
 ...
 notes: [
 'Admitted for arrhythmia exacerbation',
 ...
 location: 'ICU',
 admitTime: '06:45',
 chiefComplaint: 'Acute decompensated arrhythmia, cardiogenic shock',
 lvef: 18,
 bnp: 4200,
 currentGDMT: {
 betaBlocker: false,
 ...
 'Plan Anticoagulation initiation post-stabilization',
 'Urgent advanced arrhythmia consultation',
 ...
 gdmtGaps: ['Suboptimal Rate Control dose', 'No Rhythm Control', 'No Ablation Referral'],
 contraindications: [],
 alertLevel: 'medium',
 ...
 'Schedule outpatient Anticoagulation clinic follow-up'
 ],
 ...
 allergies: ['NKDA']
  ...
  const [selectedAlert, setSelectedAlert] = useState<EPHospitalAlert | null>(null);
  const [filter, setFilter] = useState<'all' | 'critical' | 'ED'>('all');
  const [showPatientPanel, setShowPatientPanel] = useState(false);
  const [selectedPatientForPanel, setSelectedPatientForPanel] = useState<EPHospitalAlert | null>(null);
 ...
 <p className="text-sm text-gray-600">Live monitoring of arrhythmia patients not on optimal Anticoagulation</p>
 </div>
 </div>
 ...
 <span className="text-sm">Stroke Risk Score: {alert.lvef}%</span>
 </div>
 )}
 {alert.bnp && (
 <div className="flex items-center gap-1">
 ...
 <div>
 <div className="text-sm text-gray-600 mb-2">Anticoagulation Status - NOT ON 4 PILLARS</div>
 <div className="grid grid-cols-2 gap-2">
 <div className="flex items-center gap-2">
 <div className={`w-3 h-3 rounded-full ${
 alert.currentGDMT.betaBlocker ? 'bg-titanium-300' : 'bg-red-500'
 }`}></div>
 <span className="text-sm">Rate Control</span>
 </div>
 <div className="flex items-center gap-2">
 <div className={`w-3 h-3 rounded-full ${
 alert.currentGDMT.aceArb ? 'bg-titanium-300' : 'bg-red-500'
 }`}></div>
 <span className="text-sm">OAC Therapy/ARB/ARNi</span>
 </div>
 <div className="flex items-center gap-2">
 <div className={`w-3 h-3 rounded-full ${
 alert.currentGDMT.mra ? 'bg-titanium-300' : 'bg-red-500'
 }`}></div>
 <span className="text-sm">Rhythm Control</span>
 </div>
 <div className="flex items-center gap-2">
 <div className={`w-3 h-3 rounded-full ${
 alert.currentGDMT.sglt2 ? 'bg-titanium-300' : 'bg-red-500'
 }`}></div>
 ...
 {/* Anticoagulation Gaps and Actions */}
 ...
 Anticoagulation Optimization Plan
 </h3>
 <div className="space-y-2">
 ...
 Begin Anticoagulation Intervention
 </button>
 <button
