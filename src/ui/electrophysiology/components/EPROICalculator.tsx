...
const EPROICalculator: React.FC = () => {
 ...
 : 'bg-chrome-50 text-white hover:bg-chrome-50'
 } transition-colors`}
 >
 ...
 className="px-4 py-2 bg-teal-700 text-white rounded-lg hover:bg-teal-700 transition-colors flex items-center gap-2"
 >
 ...
 <div className="bg-gradient-to-br from-[#EFF3F7] to-[#E4EDF5] p-6 rounded-xl border border-teal-700">
 <div className="flex items-center gap-3 mb-2">
 <DollarSign className="w-8 h-8 text-teal-700" />
 <div>
 <div className="text-sm text-teal-700 font-medium">EP ROI</div>
 <div className="text-3xl font-bold text-teal-700">{toFixed(calculateEPROI.roiPercentage, 0)}%</div>
 </div>
 </div>
 <div className="text-sm text-teal-700">Over {selectedTimeframe}</div>
 </div>
 ...
 <div>
 <div className="text-sm text-chrome-700 font-medium">Net Benefit</div>
 <div className="text-3xl font-bold text-chrome-800">${toFixed(calculateEPROI.netBenefit / 1000000, 1)}M</div>
 </div>
 </div>
 <div className="text-sm text-chrome-600">Total savings minus costs</div>
 </div>

 <div className="bg-gradient-to-br from-arterial-50 to-arterial-100 p-6 rounded-xl border border-arterial-200">
 <div className="flex items-center gap-3 mb-2">
 <Calendar className="w-8 h-8 text-arterial-600" />
 <div>
 <div className="text-sm text-arterial-700 font-medium">Payback Period</div>
 <div className="text-3xl font-bold text-arterial-800">{toFixed(calculateEPROI.paybackPeriod, 1)} yr</div>
 </div>
 </div>
 <div className="text-sm text-arterial-600">Time to recover investment</div>
 </div>

 <div className="bg-gradient-to-br from-slate-50 to-slate-100 p-6 rounded-xl border border-titanium-300">
 <div className="flex items-center gap-3 mb-2">
 <Target className="w-8 h-8 text-gray-500" />
 <div>
 <div className="text-sm text-gray-500 font-medium">Annual Savings</div>
 <div className="text-3xl font-bold text-gray-500">${toFixed(calculateEPROI.annualSavings / 1000000, 1)}M</div>
 </div>
 </div>
 <div className="text-sm text-gray-500">Per year potential</div>
 </div>
 </div>
 ...
 <div className="text-2xl font-bold text-red-800">${toFixed(calculateEPROI.ablationImpact / 1000, 0)}K</div>
 </div>
 </div>
 ...
 <div>
 <div className="text-sm text-chrome-700 font-medium">LAAC Revenue Impact</div>
 <div className="text-2xl font-bold text-chrome-800">${toFixed(calculateEPROI.laacImpact / 1000, 0)}K</div>
 </div>
 </div>
 <div className="text-sm text-chrome-600">Annual additional revenue</div>
 </div>

 <div className="bg-gradient-to-br from-[#EFF3F7] to-[#E4EDF5] p-6 rounded-xl border border-titanium-300">
 <div className="flex items-center gap-3 mb-2">
 <Heart className="w-6 h-6 text-teal-700" />
 <div>
 <div className="text-sm text-teal-700 font-medium">Device Revenue Impact</div>
 <div className="text-2xl font-bold text-teal-700">${toFixed(calculateEPROI.deviceImpact / 1000, 0)}K</div>
 </div>
 </div>
 <div className="text-sm text-teal-700">Annual additional revenue</div>
 </div>
 </div>
 ...
 <div className="bg-white p-6 rounded-xl border border-titanium-200">
 <h3 className="text-xl font-bold text-titanium-900 mb-4">EP Scenario Parameters</h3>
 
 <div className="space-y-4">
 <div>
 <label className="block text-sm font-medium text-titanium-700 mb-2">Analysis Timeframe</label>
 <select 
 value={selectedTimeframe} 
 onChange={(e) => setSelectedTimeframe(e.target.value as any)}
 className="w-full px-3 py-2 border border-titanium-300 rounded-lg"
 >
 ...
 <div>
 <label className="block text-sm font-medium text-titanium-700 mb-2">AFib Population</label>
 <input 
 type="number" 
 ...
 <div>
 <label className="block text-sm font-medium text-titanium-700 mb-2">Current Screening Rate (%)</label>
 <input 
 type="number" 
 step="0.1"
 ...
 <div>
 <label className="block text-sm font-medium text-titanium-700 mb-2">Target Screening Rate (%)</label>
 <input 
 type="number" 
 step="0.1"
 ...
 <div>
 <label className="block text-sm font-medium text-titanium-700 mb-2">Current Ablation Success (%)</label>
 <input 
 type="number" 
 step="0.1"
 ...
 <div>
 <label className="block text-sm font-medium text-titanium-700 mb-2">Target Ablation Success (%)</label>
 <input 
 type="number" 
 step="0.1"
 ...
 <div className="bg-white p-6 rounded-xl border border-titanium-200">
 <h3 className="text-xl font-bold text-titanium-900 mb-4">Annual EP Cost Savings</h3>
 
 <div className="space-y-3 max-h-96 overflow-y-auto">
 {epCostSavings.map((saving, index) => (
 <div key={`${saving.category}-${saving.description}`} className="flex items-center justify-between p-3 bg-titanium-50 rounded-lg">
 <div className="flex-1">
 <div className="font-medium text-titanium-900 text-sm">{saving.description}</div>
 <div className="text-xs text-titanium-600 flex items-center gap-2">
 <span>{toFixed(saving.probability * 100, 0)}% confidence</span>
 <span>•</span>
 ...
 ${toFixed(saving.annualSaving / 1000, 0)}K max
 </div>
 </div>
 </div>
 ))}
 ...
 {/* EP Investment Costs */}
 <div className="bg-white p-6 rounded-xl border border-titanium-200">
 <h3 className="text-xl font-bold text-titanium-900 mb-4">EP Investment Costs</h3>
 
 <div className="space-y-3">
 {epInvestmentCosts.map((cost, index) => (
 <div key={`${cost.category}-${cost.description}`} className="flex items-center justify-between p-3 bg-titanium-50 rounded-lg">
 <div className="flex-1">
 <div className="font-medium text-titanium-900 text-sm">{cost.description}</div>
 <div className="text-xs text-titanium-600 flex items-center gap-2">
 ...
 ${toFixed(cost.amount / 1000, 0)}K
 </div>
 </div>
 </div>
 ))}
 
 <div className="pt-3 border-t border-titanium-200">
 <div className="flex justify-between">
 <span className="font-medium text-titanium-700">Annual Total:</span>
 <span className="font-bold text-red-600">
 ${toFixed(calculateEPROI.annualCosts / 1000, 0)}K
 </span>
 </div>
 </div>
 </div>
 ...
 <div className="bg-white p-6 rounded-xl border border-titanium-200">
 <h3 className="text-xl font-bold text-titanium-900 mb-4">EP Procedure Volume & Revenue Forecasting</h3>
 ...
 {proc.type === 'device-implant' && <Heart className="w-5 h-5 text-teal-700" />}
 {proc.type === 'laac' && <Shield className="w-5 h-5 text-chrome-500" />}
 {proc.type === 'afib-screening' && <Activity className="w-5 h-5 text-gray-500" />}
 ...
 <span className="text-titanium-600">Current Volume:</span>
 <span className="font-medium">{Math.round(proc.currentVolume)}</span>
 </div>
 <div className="flex justify-between text-sm">
 <span className="text-titanium-600">Projected Volume:</span>
 <span className="font-medium text-chrome-600">{Math.round(proc.projectedVolume)}</span>
 </div>
 <div className="flex justify-between text-sm">
 <span className="text-titanium-600">Success Rate:</span>
 <span className="font-medium">{toFixed(proc.successRate, 1)}%</span>
 </div>
 <div className="flex justify-between text-sm">
 ...
 {/* EP ROI Metrics */}
 <div className="bg-white p-6 rounded-xl border border-titanium-200">
 <h3 className="text-xl font-bold text-titanium-900 mb-4">Expected EP Clinical & Operational Improvements</h3>
 
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
 {epRoiMetrics.map((metric, index) => (
 <div key={`${metric.category}-${metric.timeframe}`} className="p-4 border border-titanium-200 rounded-lg">
 <div className="flex items-center justify-between mb-2">
 <span className="font-medium text-titanium-900 text-sm">{metric.category}</span>
 <span className="text-xs text-titanium-500">{metric.timeframe}</span>
 </div>
 ...
 <div className="text-xs text-titanium-500">Baseline</div>
 </div>
 ...
 <span className={`text-sm font-medium ${metric.impact > 0 ? 'text-teal-700' : 'text-red-600'}`}>
 ...
 <div className="bg-gradient-to-r from-slate-50 to-[#EFF3F7] p-6 rounded-xl border border-titanium-300">
 <div className="flex items-center gap-4">
 ...
 <div className="flex-1">
 <h3 className="text-xl font-bold text-titanium-900 mb-2">EP Investment Recommendation</h3>
 <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
 <div>
 ...
 <div>
 <div className="text-sm text-titanium-600">Risk Level</div>
 <div className="text-lg font-bold text-chrome-600">Low</div>
 </div>
 <div>
 <div className="text-sm text-titanium-600">Strategic Priority</div>
 <div className="text-lg font-bold text-arterial-600">High</div>
 </div>
 <div>
 <div className="text-sm text-titanium-600">Implementation Focus</div>
 <div className="text-lg font-bold text-gray-500">AFib & LAAC</div>
 </div>
 </div>
