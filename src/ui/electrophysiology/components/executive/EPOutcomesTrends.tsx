...
const EPOutcomesTrends: React.FC = () => {
 ...
 strokePrevention: 'bg-green-600', 
 deviceImplants: 'bg-purple-600',
 satisfaction: 'bg-amber-600',
 ...
 {trend.trend === 'up' ? '+' : ''}{trend.change.toFixed(1)}%
 </div>
 </div>
 ))}
 </div>
 ...
 onClick={() => console.log('Average performance drill-down:', selectedMetric, (chartData.reduce((sum, d) => sum + d.value, 0) / chartData.length).toFixed(1))}
 >
 <div className="text-xs text-green-800 font-medium mb-1">Average Performance</div>
 ...
 onClick={() => console.log('Improvement trend drill-down:', selectedMetric, ((chartData[chartData.length - 1]?.value - chartData[0]?.value) / chartData[0]?.value * 100).toFixed(1))}
 >
 <div className="text-xs text-purple-800 font-medium mb-1">Improvement</div>
