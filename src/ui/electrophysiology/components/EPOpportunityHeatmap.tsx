...
interface EPOpportunityHeatmapProps {
  data: Array<{
 site_id: string;
 opp_revenue: number;
 rank: number;
...
const EPOpportunityHeatmap: React.FC<EPOpportunityHeatmapProps> = ({ data, onFacilityClick }) => {
  if (!data || data.length === 0) return (
    <div className="bg-white rounded-2xl border border-titanium-200 p-8 shadow-glass">
      <ChartEmptyState message="No EP revenue opportunity data available by facility" />
    </div>
  );
 ...
 <p className="text-sm text-titanium-600">Electrophysiology Anticoagulation optimization potential across health system sites</p>
 </div>
 ...
 Annual revenue potential from Anticoagulation optimization across all facilities
 </div>
 </div>
 </div>
 </div>
...
export default EPOpportunityHeatmap;