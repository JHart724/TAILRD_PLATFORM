...
interface EPAnticoagulationHeatmapProps {
  data: Array<{
 site_id: string;
 score: number;
...
const EPAnticoagulationHeatmap: React.FC<EPAnticoagulationHeatmapProps> = ({ data }) => {
  if (!data) return null;
 ...
 <div className="text-2xl font-bold text-titanium-900">{site.score.toFixed(1)}%</div>
 </div>
 <div>
 <div className="text-xs text-titanium-500 mb-1">Quadruple Therapy</div>
 <div className="text-2xl font-bold text-titanium-900">{site.pct_quadruple.toFixed(1)}%</div>
 </div>
 </div>
...
export default EPAnticoagulationHeatmap;