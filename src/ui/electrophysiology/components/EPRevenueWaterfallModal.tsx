...
interface EPRevenueWaterfallModalProps {
  category: 'Ablation Therapy' | 'Devices' | 'Phenotypes' | '340B';
  totalRevenue: number;
  patientCount: number;
  onClose: () => void;
}
...
const getSubcategoryData = (category: string): SubcategoryData[] => {
 ...
 Devices: [
 { name: 'CRT-D Implantation', revenue: 1000000, patientCount: 40, color: '#4A6880' },
 { name: 'ICD Implantation', revenue: 600000, patientCount: 30, color: '#3A5870' },
 { name: 'CardioMEMS', revenue: 200000, patientCount: 10, color: '#2A4860' }
 ],
 Phenotypes: [
...
const getCategoryTitle = (category: string): string => {
 ...
 'Ablation Therapy': 'Guideline-Directed Medical Therapy',
...
const getCategoryDescription = (category: string): string => {
 ...
 'Ablation Therapy': 'Revenue from optimizing electrophysiology medications according to current guidelines',
 Devices: 'Revenue from cardiac device implantations and remote monitoring',
 Phenotypes: 'Revenue from identifying and treating specific electrophysiology phenotypes',
 '340B': 'Cost savings from 340B drug pricing program for eligible medications'
  };
  return descriptions[category] || '';
};

export const EPRevenueWaterfallModal: React.FC<EPRevenueWaterfallModalProps> = ({
  category,
  totalRevenue,
  patientCount,
  onClose
...
export default EPRevenueWaterfallModal;