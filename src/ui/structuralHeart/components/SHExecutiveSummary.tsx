 ...
 * endpoint GET /structural-heart/dashboard emits gap/patient counts only - NO GDMT
 * (that is HF-specific). Typed here so StructuralExecutiveView can drop `data: any`.
 */
export interface SHDashboardData {
  summary: {
...
/**
 * SH Executive summary KPI row (AUDIT-304 SH convergence; mirrors EPExecutiveSummary).
 *
 * The SH tier previously had NO rendered KPI summary (its live totalPatients /
 * deviceCandidates were never displayed; the SHExecutiveKPICard copies in
 * components/executive are AUDIT-152 dead code, unimported). LIVE cards (patients /
 * open gaps / device candidates) read the real SH dashboard contract; DEMO cards
 * (revenue / at-risk / captured) come from the single shDemoFinancials model and each
 * carry a Demo pill. No fabricated trend on live cards (no trend-history endpoint exists).
 */
...
const KPI_DEFS: KPICardDef[] = [
  { id: 'total-patients', label: 'Total SH Patients', icon: Users, color: 'blue', live: true },
  ...
  { id: 'device-candidates', label: 'Device Therapy Candidates', icon: Activity, color: 'orange', live: true },
  {
    ...
    color: 'green',
    live: false,
    demoValue: `$${SH_DEMO_ANNUAL_OPPORTUNITY_M.toFixed(1)}M`,
    ...
    trendData: [
      ...
      { month: 'Oct', value: 10.7 },
      { month: 'Nov', value: SH_DEMO_ANNUAL_OPPORTUNITY_M },
    ],
  },
  {
    id: 'at-risk',
  ...
  },
  {
    ...
    color: 'copper',
    live: false,
    demoValue: `$${SH_DEMO_YTD_CAPTURED_M.toFixed(1)}M`,
    demoSubtext: '33% of opportunity (demo model)',
    trend: 13,
    trendData: [
      { month: 'Jun', value: 2.4 },
      { month: 'Jul', value: 2.7 },
      ...
      { month: 'Sep', value: 3.2 },
      { month: 'Oct', value: 3.4 },
      { month: 'Nov', value: SH_DEMO_YTD_CAPTURED_M },
    ],
  },
];

interface SHExecutiveSummaryProps {
  dashboard: SHDashboardData | null;
  loading?: boolean;
  error?: string | null;
}

export const SHExecutiveSummary: React.FC<SHExecutiveSummaryProps> = ({ dashboard, loading = false, error = null }) => {
  const [selectedKPI, setSelectedKPI] = useState<KPICardDef | null>(null);

  // White-card treatment (SH Gap-Intelligence reference): white surface + titanium
  ...
  // Live values from the SH dashboard contract; '-' while unavailable, never a
  // fabricated placeholder number (closes the SH-side AUDIT-099 dead-error defect).
  const liveValues = (id: string): { value: string; subtext: string } => {
    ...
    switch (id) {
      case 'total-patients':
        return { value: s.totalPatients.toLocaleString(), subtext: 'Active SH panel (database)' };
      case 'open-gaps':
    ...
    <>
      <div className="metal-card border-2 border-titanium-200 p-6 mb-6">
        <h2 className="text-xl font-bold mb-6 flex items-center">
          ...
          Structural Heart Executive Summary
        </h2>
              ...
              <>
                ...
                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                  <p className="text-sm text-gray-700">
                    Illustrative demo trend from the SH demo financial model - not derived from
