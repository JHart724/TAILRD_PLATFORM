...
const SHDRGDetailModal: React.FC<SHDRGDetailModalProps> = ({
 ...
 {(cases?.length ? ((cases.filter(c => c.marginPercent >= 20).length / cases.length) * 100).toFixed(1) : 0)}%
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="text-sm text-titanium-500">
 {(cases?.length ? ((cases.filter(c => c.los <= targetLos).length / cases.length) * 100).toFixed(1) : 0)}%
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="text-sm text-titanium-500">
 {(cases?.length ? (cases.reduce((sum, c) => sum + c.marginPercent, 0) / cases.length).toFixed(1) : 0)}%
 </div>
 </div>
 </div>
 </div>
