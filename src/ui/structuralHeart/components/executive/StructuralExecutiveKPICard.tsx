...
interface StructuralExecutiveKPICardProps {
  label: string;
  value: string;
  subvalue?: string;
  trend?: {
...
const StructuralExecutiveKPICard: React.FC<StructuralExecutiveKPICardProps> = ({
  label,
  value,
  subvalue,
  trend,
 ...
 return 'text-deep-red-600';
  ...
  return (
 <div className={`metal-card p-6 border-l-4 ${getStatusClasses()}`}>
 ...
 {subvalue && (
 <div className="text-sm text-titanium-600">{subvalue}</div>
 )}
 </div>
 {Icon && (
 <div className="ml-4 p-3 rounded-xl bg-white icon-float">
 <Icon className="w-6 h-6 text-titanium-600 drop-shadow-sm" />
 </div>
 )}
 </div>
...
export default StructuralExecutiveKPICard;