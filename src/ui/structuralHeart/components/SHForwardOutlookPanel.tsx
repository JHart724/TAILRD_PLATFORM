...
/**
 * Consolidated forward-outlook panel (AUDIT-304 SH convergence; mirrors the HF/EP
 * ForwardOutlookPanel). Replaces the three overlapping SH cards (RevenuePipelineCard
 ...
 * shDemoFinancials model (sums derived in code) - demo-badged, white-card treatment.
 * SH-local: the shared components stay untouched for the other modules.
 */
const box = 'border rounded-xl p-4 bg-white border-titanium-200';

const SHForwardOutlookPanel: React.FC = () => {
  const acceleration = SH_DEMO_PREDICTIVE.projectedRevenueSystematic - SH_DEMO_PREDICTIVE.projectedRevenueCurrentRate;
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          <h4 className="text-sm font-semibold text-titanium-700 mb-3 flex items-center gap-1.5">
            ...
            Revenue Pipeline - {m(SH_DEMO_PIPELINE.totalProjected12Month)} projected over 12 months
          </h4>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {SH_DEMO_PIPELINE.quarters.map((q) => (
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className={box}>
              ...
              <div className="text-2xl font-bold" style={{ color: '#9B2438' }}>{m(SH_DEMO_AT_RISK.immediateRevenue)}</div>
              <div className="text-xs mt-1" style={{ color: '#9B2438' }}>
                {SH_DEMO_AT_RISK.immediatePatients} patients in immediate time horizon = the YTD projected-realized gap
              </div>
            </div>
            <div className={box}>
              <div className="text-xs font-semibold uppercase tracking-wide" style={{ color: '#8B6914' }}>If Deferred Past Q2</div>
              <div className="text-2xl font-bold" style={{ color: '#8B6914' }}>+{m(SH_DEMO_AT_RISK.deferralRevenue)}</div>
            ...
            <div className={box}>
              ...
              <div className="text-2xl font-bold" style={{ color: '#4A6880' }}>{m(SH_DEMO_AT_RISK.cumulativeRisk12Month)}</div>
              <div className="text-xs mt-1" style={{ color: '#4A6880' }}>
                ${(SH_DEMO_AT_RISK.deferralCostPerMonth / 1_000).toFixed(0)}K per month of deferral
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className={box}>
              ...
              <div className="text-xl font-bold" style={{ color: '#4A6880' }}>{m(SH_DEMO_PREDICTIVE.projectedRevenueCurrentRate)}</div>
            ...
            <div className={box}>
              ...
              <div className="text-xl font-bold" style={{ color: '#2C4A60' }}>{m(SH_DEMO_PREDICTIVE.projectedRevenueSystematic)}</div>
...
export default SHForwardOutlookPanel;
