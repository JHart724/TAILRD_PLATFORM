...
interface SHPatientInfo {
 ...
 allergies: string[];
 valveTherapyStatus: {
 medicalManagement: boolean;
 anticoagulation: boolean;
 diuretics: boolean;
 tavrReferral: boolean;
 };
  };
}

const SHTeamCollaborationPanel: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'messages' | 'team' | 'alerts'>('messages');
  const [selectedMessage, setSelectedMessage] = useState<SHMessage | null>(null);
  ...
  const teamMembers: SHTeamMember[] = [
 {
 id: 'TM001',
 ...
 role: 'Structural Heart Specialist',
 specialty: 'Advanced Structural Heart',
 status: 'online',
 phone: '(555) 234-5678',
 pager: '23456',
 ...
 content: 'Yes, please start IV furosemide 40mg BID. Also check echo for interval change and consider TAVR evaluation.',
 ...
 senderRole: 'Interventional Cardiologist',
 content: 'Team: We have a new TAVR activation coming to the cath lab in 10 minutes. Patient is 67M with severe AS, cardiogenic shock. Fellow please prepare for IABP.',
 ...
 content: 'Handoff update: Williams, Robert (MRN: 987654321) is stable post-TAVR. Moving to step-down unit. Continue current meds, follow-up echo in AM.',
 ...
 mrn: '123456789',
 message: 'Upcoming TAVR evaluation appointment tomorrow at 2 PM - patient has transportation concerns',
 ...
 'Post-TAVR stable, moving to step-down unit',
 ...
 <div className="text-sm text-titanium-600 mb-1">Unread Messages</div>
 ...
 activeTab === tab.id
 ? 'border-porsche-400 bg-porsche-50 shadow-chrome-elevated'
 : 'border-titanium-200 hover:border-titanium-300 bg-white'
 }`}
 >
 <IconComponent className={`w-5 h-5 ${
 activeTab === tab.id ? 'text-porsche-600' : 'text-titanium-600'
 }`} />
 <span className={`font-semibold ${
 activeTab === tab.id ? 'text-porsche-900' : 'text-titanium-900'
 }`}>
 {tab.label}
 </span>
 {tab.count > 0 && (
 ...
 activeTab === tab.id 
 ? 'bg-porsche-100 text-porsche-700' 
 ...
 <div>
 {/* Message Filters */}
 <div className="flex items-center justify-between mb-4 p-3 bg-titanium-50 rounded-lg">
 ...
 : 'text-titanium-600 hover:bg-titanium-100'
 }`}
 >
 ...
 !message.read 
 ? 'border-porsche-200 bg-porsche-50/30' 
 : 'border-titanium-200 bg-white hover:border-titanium-300'
 ...
 <div>
 <div className="font-semibold text-titanium-900">{message.senderName}</div>
 <div className="text-sm text-titanium-600">{message.senderRole}</div>
 </div>
 </div>
 ...
 {!message.read && (
 <div className="w-3 h-3 bg-porsche-500 rounded-full"></div>
 )}
 ...
 <div 
 className="mb-2 p-2 bg-titanium-100 rounded text-sm cursor-pointer hover:bg-titanium-200 transition-colors flex items-center justify-between"
 onClick={(e) => {
 e.stopPropagation();
 ...
 <span className="text-titanium-600 ml-2">MRN: {message.patientContext.mrn}</span>
 </div>
 <ExternalLink className="w-4 h-4 text-titanium-600" />
 </div>
 )}
 
 <div className="text-titanium-800 mb-2">{message.content}</div>
 
 {message.responses && message.responses.length > 0 && (
 <div className="mt-3 pl-4 border-l-2 border-titanium-200">
 {message.responses.map((response) => (
 <div key={response.id} className="mb-2 last:mb-0">
 <div className="text-sm font-medium text-titanium-700">{response.senderName}</div>
 <div className="text-sm text-titanium-600">{response.content}</div>
 ...
 <div key={member.id} className="p-4 bg-white rounded-xl border border-titanium-200">
 <div className="flex items-start justify-between mb-3">
 <div className="flex items-center gap-3">
 <div className="relative">
 <div className="w-12 h-12 bg-porsche-100 rounded-full flex items-center justify-center">
 <User className="w-6 h-6 text-porsche-600" />
 </div>
 <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${getStatusColor(member.status)}`}></div>
 </div>
 <div>
 <div className="font-semibold text-titanium-900">{member.name}</div>
 <div className="text-sm text-titanium-600">{member.role}</div>
 {member.specialty && (
 ...
 : 'border-titanium-200 bg-white'
 }`}
 >
 <div className="flex items-start justify-between mb-3">
 <div className="flex items-center gap-3">
 <AlertTriangle className={`w-5 h-5 ${
 !alert.acknowledged ? 'text-medical-red-600' : 'text-titanium-400'
 }`} />
 <div>
 ...
 <div 
 className="text-sm text-titanium-600 cursor-pointer hover:text-porsche-600 flex items-center gap-1"
 onClick={(e) => {
 e.stopPropagation();
 ...
 <span className="text-2xl font-bold text-gray-900">{selectedPatient.diseaseStage}%</span>
 </div>
 <p className="text-sm text-gray-600">Disease Stage</p>
 </div>
 ...
 {/* Valve Therapy Status */}
 <div className="bg-porsche-50 rounded-lg p-4">
 <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
 ...
 selectedPatient.fullChart.valveTherapyStatus.medicalManagement ? 'bg-titanium-300' : 'bg-red-500'
 }`}></div>
 <span className="text-sm">Medical Management</span>
 </div>
 <div className="flex items-center gap-2">
 <div className={`w-3 h-3 rounded-full ${
 selectedPatient.fullChart.valveTherapyStatus.anticoagulation ? 'bg-titanium-300' : 'bg-red-500'
 }`}></div>
 <span className="text-sm">Anticoagulation</span>
 </div>
 <div className="flex items-center gap-2">
 <div className={`w-3 h-3 rounded-full ${
 selectedPatient.fullChart.valveTherapyStatus.diuretics ? 'bg-titanium-300' : 'bg-red-500'
 }`}></div>
 <span className="text-sm">Diuretics</span>
 </div>
 <div className="flex items-center gap-2">
 <div className={`w-3 h-3 rounded-full ${
 selectedPatient.fullChart.valveTherapyStatus.tavrReferral ? 'bg-titanium-300' : 'bg-red-500'
 }`}></div>
...
export default SHTeamCollaborationPanel;