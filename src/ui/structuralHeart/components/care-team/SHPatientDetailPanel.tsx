...
interface SHPatientDetailPanelProps {
  patient: SHWorklistPatient;
  onClose: () => void;
}

const SHPatientDetailPanel: React.FC<SHPatientDetailPanelProps> = ({ patient, onClose }) => {
  const [carePlanFeedback, setCarePlanFeedback] = useState(false);
 ...
 if (riskScore >= 6) return 'text-amber-600 bg-amber-100';
 ...
 return 'text-red-700 bg-red-100';
  ...
  // Valve Therapy Contraindication Checking Functions
  const checkMedicalManagementContraindications = (patient: SHWorklistPatient) => {
 ...
 return alerts;
  ...
  const checkAnticoagulationContraindications = (patient: SHWorklistPatient) => {
 const vitals = patient.fullChart?.vitals;
 const alerts = [];
 ...
 message: 'Severe bradycardia (HR <50) - avoid Medical Management' 
 });
 }
 ...
 return alerts;
  ...
  const checkTavrReferralContraindications = (patient: SHWorklistPatient) => {
 const labs = patient.fullChart?.labs;
 const alerts = [];
 ...
 return alerts;
  ...
  const checkDiureticsContraindications = (patient: SHWorklistPatient) => {
 const labs = patient.fullChart?.labs;
 const alerts = [];
 ...
 switch(pillar) {
 case 'medicalManagement': return checkMedicalManagementContraindications(patient);
 case 'anticoagulation': return checkAnticoagulationContraindications(patient);
 case 'tavrReferral': return checkTavrReferralContraindications(patient);
 case 'diuretics': return checkDiureticsContraindications(patient);
 default: return [];
 }
  };
 ...
 Risk Score: {toFixed(patient.riskScore, 1)}
 </span>
 ...
 <span className="text-2xl font-bold text-gray-900">{patient.diseaseStage}%</span>
 </div>
 <p className="text-sm text-gray-600">Disease Stage</p>
 </div>
 <div className="bg-arterial-50 rounded-lg p-4 text-center">
 <div className="text-xl font-bold text-arterial-600 mb-2">Valve Stage {patient.valveStage}</div>
 <p className="text-sm text-gray-600">Functional Class</p>
 </div>
 <div className="bg-amber-50 rounded-lg p-4 text-center">
 <div className="text-2xl font-bold text-amber-600 mb-2">{patient.therapyGaps.length}</div>
 ...
 {/* Valve Therapy Optimization Dashboard */}
 ...
 {/* Overall Valve Therapy Score */}
 <div className="mb-4 p-3 bg-white rounded-lg border">
 <div className="flex items-center justify-between mb-2">
 <span className="text-sm font-medium text-gray-700">Overall Valve Therapy Score</span>
 <span className={`px-3 py-1 rounded-full text-lg font-bold ${getValveTherapyScoreColor(patient.fullChart.valveTherapy.overallScore)}`}>
 ...
 patient.fullChart.valveTherapy.overallScore >= 80 ? 'bg-green-500' :
 patient.fullChart.valveTherapy.overallScore >= 60 ? 'bg-yellow-500' : 'bg-red-500'
 }`}
 style={{ width: `${patient.fullChart.valveTherapy.overallScore}%` }}
 ></div>
 </div>
 </div>
 ...
 <div key={pillar} className={`p-3 rounded-lg border-2 ${getValveTherapyStatusColor(data.status)}`}>
 ...
 {patient.fullChart.valveTherapy.opportunities.length > 0 && (
 <div className="mb-4">
 <h4 className="font-semibold text-gray-900 mb-2 text-sm">Optimization Opportunities</h4>
 <div className="space-y-2">
 {patient.fullChart.valveTherapy.opportunities.map((opp, idx) => (
 ...
 {/* Barriers */}
 {patient.fullChart.valveTherapy.barriers.length > 0 && (
 <div className="mb-4">
 <h4 className="font-semibold text-gray-900 mb-2 text-sm">Current Barriers</h4>
 <div className="flex flex-wrap gap-2">
 {patient.fullChart.valveTherapy.barriers.map((barrier, idx) => (
 <span key={barrier} className="px-2 py-1 bg-red-100 text-red-800 rounded text-xs">
 {barrier}
 </span>
 ))}
 </div>
 ...
 <span>Last Review: {new Date(patient.fullChart.valveTherapy.lastReview).toLocaleDateString()}</span>
 <span>Next Review: {new Date(patient.fullChart.valveTherapy.nextReview).toLocaleDateString()}</span>
 </div>
 </div>
 )}
 ...
 <div key={`${hosp.reason}-${hosp.date}`} className="flex justify-between items-center bg-white p-2 rounded">
 <div>
 ...
 Valve Therapy Optimization Gaps
 </h3>
 <div className="grid gap-2">
 {patient.therapyGaps.map((gap, idx) => (
 ...
 {patient.procedureEligible && (
 <div className="bg-chrome-50 rounded-lg p-4">
 <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
 <TrendingUp className="w-5 h-5 text-chrome-600" />
 Procedure Therapy Eligibility
 </h3>
 <div className="bg-white p-3 rounded border border-chrome-200">
 <div className="flex items-center gap-2 mb-2">
 <div className="w-3 h-3 bg-chrome-500 rounded-full"></div>
 <span className="font-medium text-chrome-800">Eligible for Procedure Therapy</span>
 </div>
 <p className="text-sm text-gray-600">
 Patient meets criteria for procedure evaluation based on current Disease Stage and clinical status.
 ...
 <button 
 onClick={() => {
 // Initiating comprehensive care plan for patient
 setCarePlanFeedback(true);
 setTimeout(() => setCarePlanFeedback(false), 2500);
 }}
 className="flex-1 px-4 py-3 bg-porsche-600 text-white rounded-lg font-medium hover:bg-porsche-700 transition-colors"
 >
...
export default SHPatientDetailPanel;