...
const SHQualityMetricsDashboard: React.FC = () => {
 ...
 {
 id: 'valve-therapy-4pillar',
 name: 'Valve Therapy 4-Pillar Optimization',
 category: 'Core',
 currentValue: 68.4,
 targetValue: 85.0,
 benchmark: 72.1,
 ...
 description: 'Percentage of eligible Aortic patients on optimal Valve Therapy across all 4 pillars',
 opportunities: [
 ...
 patientCohorts: [
 { name: 'Aortic patients on optimal ARNi', count: 1042, percentage: 41.8, riskLevel: 'low' },
 { name: 'Aortic patients missing SGLT2i', count: 734, percentage: 29.4, riskLevel: 'high' },
 ...
 patientCount: 1784,
 lastUpdated: '2024-01-15',
 description: 'Risk-adjusted 30-day all-cause readmission rate for valve disease patients',
 opportunities: [
 ...
 patientCount: 4268,
 lastUpdated: '2024-01-15',
 description: 'Risk-adjusted 1-year mortality rate for valve disease patients',
 opportunities: [
 { description: 'Advanced valve disease therapy evaluation', impact: 15, difficulty: 'High' },
 ...
 {
 id: 'disease-stage-improvement',
 name: 'Disease Stage Improvement ≥5%',
 ...
 patientCount: 1752,
 lastUpdated: '2024-01-15',
 description: 'Percentage of patients achieving ≥5% disease stage improvement at 6 months',
 opportunities: [
 { description: 'Optimal Valve Therapy titration protocols', impact: 18, difficulty: 'Medium' },
 ...
 {
 id: 'composite-score',
 name: 'Valve Disease Care Composite Score',
 category: 'Composite',
 currentValue: 78.2,
 targetValue: 85.0,
 benchmark: 74.6,
 ...
 opportunities: [
 { description: 'Focus on Valve Therapy optimization', impact: 25, difficulty: 'Medium' },
 ...
 if (performance >= 90) return 'text-teal-700 bg-chrome-50';
 ...
 Valve disease care quality indicators and improvement opportunities
 </p>
 </div>
 ...
 ? `border-${colorClass}-400 bg-${colorClass}-50 shadow-chrome-elevated`
 ...
 ? `border-${getCategoryColor(metric.category)}-400 shadow-chrome-elevated bg-${getCategoryColor(metric.category)}-50/30`
 ...
 (metric.currentValue / metric.targetValue) >= 0.9 ? 'bg-teal-700' :
 ...
 <Users className="w-4 h-4 text-porsche-600" />
 <h4 className="font-semibold text-titanium-900">Patient Cohorts</h4>
 </div>
 <div className="space-y-2">
 {metric.patientCohorts.map((cohort) => (
 <div key={cohort.name} className="flex items-center justify-between p-2 bg-white rounded border">
 ...
 <div className="space-y-2">
 {metric.trendData.map((trend) => (
 <div key={trend.period} className="flex items-center justify-between p-2 bg-white rounded border">
 ...
 <button 
 onClick={() => {
 // Opening patient list view
 {}
 }}
 className="flex items-center gap-2 px-3 py-2 bg-porsche-600 text-white text-xs rounded-lg hover:bg-porsche-700 transition-colors"
 >
 ...
 className="flex items-center gap-2 px-3 py-2 bg-teal-700 text-white text-xs rounded-lg hover:bg-teal-700 transition-colors"
 >
 ...
 {selectedMetricData.opportunities.map((opportunity) => (
 ...
 <span className="text-sm font-bold text-porsche-600">
 ...
 <div className="text-sm text-titanium-600">
 {(() => {
 const sorted = filteredMetrics.sort((a, b) => (a.currentValue / a.targetValue) - (b.currentValue / b.targetValue));
 const val = (sorted[0]?.currentValue || 0) / (sorted[0]?.targetValue || 1) * 100;
 return `${toFixed(val, 0)}% to target`;
 })()}
 </div>
 </div>
...
export default SHQualityMetricsDashboard;