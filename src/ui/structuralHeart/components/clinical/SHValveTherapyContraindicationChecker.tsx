...
const SHValveTherapyContraindicationChecker: React.FC<{ patientData?: PatientContext }> = ({ patientData: ctx }) => {
 ...
 patient[key] ? (key === 'hasAdequateVascularAccess' ? 'bg-emerald-500 border-emerald-500' : 'bg-red-500 border-red-500') : 'bg-white border-titanium-200 hover:border-titanium-200'
 }`}>
 ...
 {results.map((r, i) => (
 <div key={i} className={`flex items-center justify-between p-2.5 rounded-lg border cursor-pointer ${
