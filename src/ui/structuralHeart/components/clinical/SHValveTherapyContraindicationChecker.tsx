...
const SHValveTherapyContraindicationChecker: React.FC<{ patientData?: PatientContext }> = ({ patientData: ctx }) => {
 ...
 switch (status) {
 case 'safe': return <CheckCircle className="w-5 h-5 text-teal-700" />;
 case 'caution': return <AlertTriangle className="w-5 h-5 text-gray-500" />;
 ...
 switch (status) {
 case 'safe': return 'bg-chrome-50 border-titanium-300';
 case 'caution': return 'bg-chrome-50/10 border-titanium-300/30';
 ...
 switch (status) {
 case 'safe': return 'text-teal-700';
 case 'caution': return 'text-gray-500';
 ...
 patient[key] ? (key === 'hasAdequateVascularAccess' ? 'bg-chrome-50 border-titanium-300' : 'bg-red-500 border-red-500') : 'bg-white border-titanium-200 hover:border-titanium-200'
 }`}>
 ...
 <AlertTriangle className="w-3 h-3 text-gray-500 mt-0.5 flex-shrink-0" /> {r}
 </li>
 ))}
 </ul>
 </div>
 ...
 <li key={a} className="text-xs text-teal-700 flex items-center gap-2">
