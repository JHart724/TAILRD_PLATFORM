...
const SHValvePhenotypeClassification: React.FC<{ patientData?: PatientContext }> = ({ patientData: ctx }) => {
 ...
 {results.stenosisSeverity.criteria.map((c, i) => (
 ...
 <ul className="space-y-1">
 {results.regurgitationSeverity.criteria.map((c, i) => (
 <li key={i} className="text-xs text-titanium-300 flex items-start gap-2">
 <span className="text-porsche-400 mt-0.5">\u2022</span> {c}
 </li>
 ))}
 ...
 {results.morphologyProfile.features.map((f, i) => (
 <p key={i} className="text-xs text-titanium-300">\u2022 {f}</p>
 ))}
 </div>
 <div>
 <h5 className="text-xs font-semibold text-titanium-400 mb-1">Implications</h5>
 {results.morphologyProfile.implications.map((imp, i) => (
 <p key={i} className="text-xs text-titanium-300">\u2022 {imp}</p>
 ))}
 </div>
 </div>
 </div>
