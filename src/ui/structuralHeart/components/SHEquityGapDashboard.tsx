...
interface SHEquityGapDashboardProps {
  data: {
 metric: string;
 segment: string;
 stratified: Array<{
...
const SHEquityGapDashboard: React.FC<SHEquityGapDashboardProps> = ({ data, segment }) => {
 ...
 <div className="text-2xl font-bold text-titanium-900">Average: {avgValue.toFixed(1)}%</div>
 </div>
 ...
 <span className="text-sm font-bold text-titanium-900">{group.value.toFixed(1)}%</span>
 <span className={`text-xs font-bold ${isAboveAvg ? "text-emerald-600" : "text-arterial-600"}`}>
...
export default SHEquityGapDashboard;