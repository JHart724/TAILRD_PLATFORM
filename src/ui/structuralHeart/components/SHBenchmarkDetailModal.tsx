...
import { getOrdinalSuffix, roundTo, formatDelta } from '../../../utils/formatters';
...
interface SHBenchmarkDetailModalProps {
...
const getPercentileColor = (percentile: number): string => {
  if (percentile >= 90) return 'bg-chrome-50 text-teal-700 border-titanium-300';
  if (percentile >= 75) return 'bg-chrome-100 text-chrome-800 border-chrome-300';
  if (percentile >= 50) return 'bg-amber-50 text-amber-600 border-titanium-300';
...
const SHBenchmarkDetailModal: React.FC<SHBenchmarkDetailModalProps> = ({
  benchmarkName,
  description,
  ourValue,
  nationalValue,
  ...
  const variance = roundTo(ourValue - nationalValue);
 ...
 <Target className="w-6 h-6 text-teal-700 mr-3" />
 <div>
 ...
 <div className={`text-sm font-medium ${isPositive ? 'text-teal-700' : 'text-red-600'}`}>
 ...
 {/* Performance Summary */}
 <div className="mt-8 bg-chrome-50 rounded-lg p-6">
 <h4 className="text-lg font-semibold text-chrome-900 mb-4">Performance Summary</h4>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
 <div>
 <h5 className="font-medium text-chrome-800 mb-2">Current Position</h5>
 <div className="space-y-2 text-sm text-chrome-700">
 ...
 <span className="font-medium">{percentile}{getOrdinalSuffix(percentile)}</span>
 </div>
 </div>
 </div>
 <div>
 <h5 className="font-medium text-chrome-800 mb-2">Benchmark Comparison</h5>
 <div className="space-y-2 text-sm text-chrome-700">
 <div className="flex justify-between">
 <span>Top 10%:</span>
...
export default SHBenchmarkDetailModal;