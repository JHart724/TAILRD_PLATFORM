...
const padWaterfallCategories: WaterfallCategory[] = [
  ...
  { label: 'WIfI Classification', value: 3800000, color: '#7c3aed' },
];
...
const PeripheralExecutiveView: React.FC = () => {
 ...
 { label: 'Avg Revenue / Patient', value: config.kpiData.avgRoi, sub: config.kpiData.avgRoiSub, icon: <TrendUpIcon />, color: 'text-green-600' },
  ...
  return (
 <div className="min-h-screen bg-gradient-to-br from-teal-50 via-titanium-50 to-porsche-50 p-6 relative overflow-hidden">
 {/* Web 3.0 Background */}
 <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-teal-100 via-transparent to-transparent" />
 <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-teal-400 to-porsche-400 rounded-full blur-3xl" />
 <div className="absolute bottom-0 left-0 w-96 h-96 bg-gradient-to-tr from-porsche-400 to-green-400 rounded-full blur-3xl" />
 ...
 Financial performance and population health insights for PAD &bull; Critical Limb Ischemia &bull; Endovascular Intervention
 </p>
 </div>
 <ExportButton
 ...
 { label: 'Share of Total', value: cat ? `${((cat.value / 42300000) * 100).toFixed(1)}%` : 'N/A' },
 { label: 'Realized', value: `${((cat?.value || 0) * 0.70 / 1000000).toFixed(1)}M`, colorScheme: 'green' },
 { label: 'Gap', value: `${((cat?.value || 0) * 0.30 / 1000000).toFixed(1)}M`, colorScheme: 'amber' },
 ]}
 onClose={() => setActiveModal(null)}
 />
 );
