...
const LimbSalvageScreening: React.FC = () => {
 ...
 switch (risk) {
 case 'Low': return 'bg-chrome-50 text-teal-700 border-titanium-300';
 case 'Moderate': return 'bg-amber-50 text-amber-600 border-titanium-300';
 ...
 case 'Suburban': return 'bg-amber-50 text-amber-600';
 case 'Urban': return 'bg-chrome-50 text-teal-700';
 ...
 <button className="px-4 py-2 bg-teal-700 text-white rounded-lg hover:bg-teal-700 transition-colors">
 Export List
 </button>
 </div>
 </div>
 ...
 <div className="p-4 rounded-lg bg-chrome-50 border border-titanium-300">
 <div className="text-2xl font-bold text-gray-500">67</div>
 <div className="text-sm text-gray-500">High Risk</div>
 <div className="text-xs text-gray-500 mt-1">Urgent evaluation required</div>
 </div>
 <div className="p-4 rounded-lg bg-chrome-50 border border-titanium-300">
 <div className="text-2xl font-bold text-gray-500">91</div>
 <div className="text-sm text-gray-500">Moderate Risk</div>
 <div className="text-xs text-gray-500 mt-1">Enhanced monitoring</div>
 </div>
 <div className="p-4 rounded-lg bg-chrome-50 border border-titanium-300">
 <div className="text-2xl font-bold text-teal-700">89</div>
 <div className="text-sm text-teal-700">Care Gaps</div>
 <div className="text-xs text-teal-700 mt-1">Overdue for screening</div>
 </div>
 </div>
 ...
 <div key={patient.id} className="p-6 border border-titanium-200 rounded-xl hover:shadow-lg transition-all duration-300">
 <div className="flex items-start justify-between mb-4">
 <div className="flex items-start gap-4">
 <div>
 <h4 className="text-lg font-semibold text-titanium-900">{patient.name}</h4>
 <div className="text-sm text-titanium-600">{patient.mrn} • Age {patient.age} • {patient.race}</div>
 <div className="flex items-center gap-2 mt-1">
 <MapPin className="w-4 h-4 text-titanium-400" />
 <span className="text-sm text-titanium-600">{patient.zipCode}</span>
 ...
 {/* Disparity Flags */}
 <div>
 <h5 className="text-sm font-semibold text-titanium-700 mb-2">Disparity Risk Factors</h5>
 ...
 {/* Clinical Indicators */}
 <div>
 <h5 className="text-sm font-semibold text-titanium-700 mb-2">Clinical Indicators</h5>
 <div className="space-y-1">
 {patient.clinicalIndicators.map((indicator) => (
 <div key={indicator} className="flex items-center gap-2">
 <Stethoscope className="w-3 h-3 text-porsche-500" />
 <span className="text-xs text-titanium-600">{indicator}</span>
 </div>
 ))}
 </div>
 {patient.abi && (
 <div className="mt-2 p-2 bg-titanium-50 rounded">
 <div className="text-xs text-titanium-600">ABI: <span className="font-semibold">{patient.abi}</span></div>
 </div>
 )}
 </div>
 ...
 {/* Next Actions */}
 <div>
 <h5 className="text-sm font-semibold text-titanium-700 mb-2">Recommended Action</h5>
 <div className="p-3 bg-chrome-50 border border-titanium-300 rounded-lg">
 <div className="text-sm font-medium text-teal-700">{patient.nextAction}</div>
 <div className="text-xs text-teal-700 mt-1">Last visit: {patient.lastVisit}</div>
 </div>
 <button className="w-full mt-3 px-4 py-2 bg-teal-700 text-white rounded-lg hover:bg-teal-700 transition-colors text-sm">
 ...
 <div className="space-y-6">
 <div className="metal-card p-6">
 <h3 className="text-xl font-bold text-titanium-900 mb-6 flex items-center gap-2">
 ...
 <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
 {/* By Race/Ethnicity */}
 <div className="p-6 bg-white border border-titanium-200 rounded-xl">
 <h4 className="font-semibold text-titanium-900 mb-4">High-Risk by Race/Ethnicity</h4>
 <div className="space-y-3">
 {Object.entries(disparityMetrics.byRace).map(([race, count]) => (
 <div key={race} className="flex items-center justify-between">
 <span className="text-sm text-titanium-700">{race}</span>
 <div className="flex items-center gap-2">
 ...
 <div className="p-6 bg-white border border-titanium-200 rounded-xl">
 <h4 className="font-semibold text-titanium-900 mb-4">Geographic Distribution</h4>
 <div className="space-y-3">
 {Object.entries(disparityMetrics.byRurality).map(([location, count]) => (
 <div key={location} className="flex items-center justify-between">
 <span className="text-sm text-titanium-700">{location}</span>
 <div className="flex items-center gap-2">
 <span className="font-semibold text-titanium-900">{count}</span>
 <div className="w-16 bg-titanium-200 rounded-full h-2">
 <div 
 className="h-2 bg-chrome-50 rounded-full"
 ...
 <div className="p-6 bg-white border border-titanium-200 rounded-xl">
 <h4 className="font-semibold text-titanium-900 mb-4">Amputation Rates (%)</h4>
 <div className="space-y-3">
 ...
 <div className="flex items-center gap-2">
 <span className={`font-semibold ${rate > 8 ? 'text-red-600' : rate > 6 ? 'text-gray-500' : 'text-teal-700'}`}>
 {rate}%
 </span>
 <div className="w-16 bg-titanium-200 rounded-full h-2">
 <div 
 className={`h-2 rounded-full ${rate > 8 ? 'bg-red-500' : rate > 6 ? 'bg-chrome-50' : 'bg-chrome-50'}`}
 ...
 <div className="text-2xl font-bold text-gray-500">{disparityMetrics.averageTimeToDiagnosis}</div>
 <div className="text-sm text-gray-500">Average Days to Diagnosis</div>
 <div className="text-xs text-gray-500 mt-1">Higher in rural/minority populations</div>
 </div>
 ...
 <div className="p-4 bg-chrome-50 border border-titanium-300 rounded-lg">
 <div className="text-2xl font-bold text-teal-700">2.8x</div>
 <div className="text-sm text-teal-700">Higher Risk in Minorities</div>
 <div className="text-xs text-teal-700 mt-1">Compared to overall population</div>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="space-y-4">
 <h4 className="font-semibold text-titanium-900">Immediate Actions Needed</h4>
 <div className="space-y-3">
 ...
 <div className="p-4 border-l-4 border-titanium-300 bg-chrome-50">
 <div className="font-medium text-gray-500">Overdue ABI Screening (67)</div>
 <div className="text-sm text-gray-500 mt-1">High-risk patients without recent screening</div>
 <button className="mt-2 px-3 py-1 bg-chrome-50 text-white rounded text-xs hover:bg-chrome-50">
 Schedule Screening
 </button>
 </div>
 <div className="p-4 border-l-4 border-teal-700 bg-chrome-50">
 <div className="font-medium text-teal-700">Rural Access Initiative (124)</div>
 <div className="text-sm text-teal-700 mt-1">Mobile screening unit deployment recommended</div>
 <button className="mt-2 px-3 py-1 bg-teal-700 text-white rounded text-xs hover:bg-teal-700">
 Plan Mobile Unit
 </button>
 </div>
 </div>
 ...
 <div className="space-y-4">
 <h4 className="font-semibold text-titanium-900">Community Health Initiatives</h4>
 <div className="space-y-3">
 ...
 <div className="text-xs text-teal-700 mt-2">Expected impact: 30% reduction in late-stage PAD</div>
 </div>
 <div className="p-4 bg-white border border-titanium-200 rounded-lg">
 ...
 <div className="text-xs text-teal-700 mt-2">Scheduled: 3 events next quarter</div>
 </div>
 <div className="p-4 bg-white border border-titanium-200 rounded-lg">
 ...
 <div className="text-xs text-teal-700 mt-2">Goal: Reduce time to diagnosis by 40%</div>
 </div>
 </div>
 </div>
 </div>
 ...
 {/* Outcome Projections */}
 <div className="mt-8 p-6 bg-chrome-50 border border-titanium-300 rounded-xl">
 <h4 className="font-semibold text-teal-700 mb-4">Projected Impact of Interventions</h4>
 <div className="grid grid-cols-3 gap-6">
 <div className="text-center">
 <div className="text-2xl font-bold text-teal-700">35%</div>
 <div className="text-sm text-teal-700">Reduction in Late-Stage PAD</div>
 </div>
 <div className="text-center">
 <div className="text-2xl font-bold text-teal-700">28%</div>
 <div className="text-sm text-teal-700">Decrease in Amputation Rate</div>
 </div>
 <div className="text-center">
 <div className="text-2xl font-bold text-teal-700">$2.8M</div>
 <div className="text-sm text-teal-700">Annual Cost Savings</div>
 </div>
 </div>
 </div>
 </div>
