...
const InterventionContraindicationChecker: React.FC<{ patientData?: PatientContext }> = ({ patientData: ctx }) => {
 ...
 <input type="checkbox" checked={patientData[c.k as keyof PatientData] as boolean} onChange={e => updatePatientData(c.k as keyof PatientData, e.target.checked)} className="rounded text-amber-600" />
 ...
 <ul className="text-sm space-y-1">{result.reasons.map((reason, idx) => (<li key={idx} className="flex items-start gap-1"><div className="w-1 h-1 bg-current rounded-full mt-2 flex-shrink-0" />{reason}</li>))}</ul>
 </div>
 )}
 {result.precautions.length > 0 && (
 <div className="mb-3">
 <div className="text-sm font-semibold mb-1">Precautions:</div>
 <ul className="text-sm space-y-1">{result.precautions.map((p, idx) => (<li key={idx} className="flex items-start gap-1"><div className="w-1 h-1 bg-current rounded-full mt-2 flex-shrink-0" />{p}</li>))}</ul>
 </div>
 )}
 ...
 <h3 className="text-lg font-semibold text-amber-800 mb-4">Alternative Approaches</h3>
 <div className="space-y-4">
 {results.filter(r => r.alternatives.length > 0).map((result) => (
 <div key={result.therapy}>
 <div className="font-semibold text-amber-800 mb-2">{result.therapy}:</div>
 <ul className="text-sm text-amber-700 space-y-1">{result.alternatives.map((alt, idx) => (<li key={idx} className="flex items-start gap-1"><div className="w-1 h-1 bg-amber-600 rounded-full mt-2 flex-shrink-0" />{alt}</li>))}</ul>
 </div>
 ))}
 </div>
 </div>
