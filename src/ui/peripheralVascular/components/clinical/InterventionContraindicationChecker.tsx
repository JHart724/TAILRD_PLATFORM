...
const InterventionContraindicationChecker: React.FC<{ patientData?: PatientContext }> = ({ patientData: ctx }) => {
 ...
 <ul className="text-sm text-amber-700 space-y-1">{result.alternatives.map((alt) => (<li key={alt} className="flex items-start gap-1"><div className="w-1 h-1 bg-amber-600 rounded-full mt-2 flex-shrink-0" />{alt}</li>))}</ul>
 </div>
 ))}
 </div>
 </div>
