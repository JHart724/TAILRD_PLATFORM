...
const PADRiskScoreCalculator: React.FC<{ patientData?: PatientContext }> = ({ patientData }) => {
 ...
 <ul className="space-y-1">{result.managementRecommendations.map((r,i) => (<li key={i} className="text-xs text-titanium-700 flex items-start gap-1"><span className="text-porsche-500 mt-0.5">{BULLET}</span>{r}</li>))}</ul>
 </div>
 </>) : (
