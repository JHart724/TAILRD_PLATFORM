...
const PADSpecialtyPhenotypesDashboard: React.FC = () => {
 ...
 <ul className="space-y-0.5">{p.clinicalFeatures.map((f,i) => (<li key={i} className="text-xs text-titanium-700 flex items-start gap-1"><span className="text-porsche-500 mt-0.5">{BULLET}</span>{f}</li>))}</ul></div>
 <div><div className="text-xs font-semibold text-titanium-600 mb-1">Diagnostic Criteria</div>
 <ul className="space-y-0.5">{p.diagnosticCriteria.map((d,i) => (<li key={i} className="text-xs text-titanium-700 flex items-start gap-1"><span className="text-porsche-500 mt-0.5">{BULLET}</span>{d}</li>))}</ul></div>
 </div>
 </div>
 ))}
 </div>
