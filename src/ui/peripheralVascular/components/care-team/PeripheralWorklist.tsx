...
const PeripheralWorklist: React.FC = () => {
 ...
 const searchable = [
   p.name, p.mrn, p.diseaseSeverity,
   p.lesionLocation, p.procedure, p.status,
   p.smokingStatus, p.diabetesControl,
   p.woundStatus || ''
 ].join(' ').toLowerCase();
 ...
 return filtered;
  }, [mockPatients, activeTab, debouncedSearch, filterSeverity, sortField, sortDirection]);
 ...
 switch (severity) {
 case 'Claudication': return 'bg-[#F0F7F4] text-[#2D6147]';
 case 'CLI': return 'bg-[#FAF6E8] text-[#8B6914]';
 ...
 case 'Post-op': return 'bg-[#F0F7F4] text-[#2D6147]';
 case 'Follow-up': return 'bg-[#F0F7F4] text-[#2D6147]';
 ...
 if (abi < 0.7) return 'text-[#8B6914]';
 if (abi < 0.9) return 'text-[#8B6914]';
 return 'text-chrome-600';
  };

  const getWoundStatusColor = (status?: string) => {
 switch (status) {
 case 'Healing': return 'bg-[#F0F7F4] text-[#2D6147]';
 ...
 <div className="text-xs text-titanium-600">Right ABI</div>
 <div className={`font-bold ${getABIColor(patient.abi.right)}`}>
 {toFixed(patient.abi.right, 2)}
 </div>
 </div>
 <div className="p-2 bg-white rounded border border-titanium-200">
 <div className="text-xs text-titanium-600">Left ABI</div>
 <div className={`font-bold ${getABIColor(patient.abi.left)}`}>
 {toFixed(patient.abi.left, 2)}
 </div>
 </div>
 </div>
 ...
 patient.wifiStage <= 2 ? 'text-[#2D6147]' :
 patient.wifiStage === 3 ? 'text-[#8B6914]' : 'text-red-600'
 }`}>
 ...
 <div className={`font-bold ${
 patient.tascGrade === 'A' ? 'text-[#2D6147]' :
 patient.tascGrade === 'B' ? 'text-[#8B6914]' :
 ...
 <div className="flex items-center gap-2 p-2 bg-white rounded border border-titanium-200">
 <Cigarette className="w-4 h-4 text-titanium-500" />
 <div className="text-sm">
 <span className="text-titanium-600">Smoking:</span>
 <span className={`ml-2 font-medium ${
 patient.smokingStatus === 'Current' ? 'text-red-600' :
 patient.smokingStatus === 'Former' ? 'text-[#8B6914]' : 'text-[#2D6147]'
 }`}>
 ...
 <div className="flex items-center gap-2 p-2 bg-white rounded border border-titanium-200">
 <Droplets className="w-4 h-4 text-titanium-500" />
 <div className="text-sm">
 <span className="text-titanium-600">Diabetes:</span>
 <span className={`ml-2 font-medium ${
 patient.diabetesControl === 'Good' ? 'text-[#2D6147]' :
 patient.diabetesControl === 'Fair' ? 'text-[#8B6914]' :
 ...
 <div className="flex items-center gap-2 p-2 bg-white rounded border border-titanium-200">
 <Target className="w-4 h-4 text-titanium-500" />
 <div className="text-sm">
 <span className="text-titanium-600">Wound:</span>
 ...
 <div className="space-y-2">
 <div className="p-2 bg-white rounded border border-titanium-200">
 <div className="text-xs text-titanium-600">Approach</div>
 <div className="text-sm font-medium text-titanium-800">{patient.procedurePlan.approach}</div>
 </div>
 
 <div className="p-2 bg-white rounded border border-titanium-200">
 <div className="text-xs text-titanium-600">Timeline</div>
 <div className="text-sm font-medium text-titanium-800">{patient.procedurePlan.timeline}</div>
 </div>
 ...
 patient.procedurePlan.urgency === 'Urgent' ? 'bg-[#FAF6E8] text-[#8B6914]' :
 ...
 <div key={complication} className="p-2 bg-red-50 border border-red-200 rounded text-sm text-red-800">
 {complication}
 </div>
 ))}
 </div>
 ...
 className="px-3 py-2 border border-titanium-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-medical-red-500"
 >
 ...
 <td className="p-3">
 <span className="font-medium text-titanium-800">{patient.lesionLocation}</span>
 {patient.tascGrade && (
 <div className="text-xs text-titanium-500">TASC {patient.tascGrade}</div>
 )}
 </td>
 <td className="p-3">
 <span className="font-medium text-titanium-800">{patient.procedure}</span>
 </td>
 <td className="p-3">
