...
const PADInterventionPathwayFunnel: React.FC = () => {
 ...
 <div className="text-sm text-green-700 font-medium">Avg ABI</div>
 <div className="text-2xl font-bold text-green-800">
 {getFilteredPatients().length ? 
 (getFilteredPatients().reduce((sum, p) => sum + p.abiScore, 0) / getFilteredPatients().length).toFixed(2) : '0.00'
 }
 </div>
 </div>
 </div>
 ...
 <div className="text-sm text-titanium-600">
 Age {patient.age} • ABI {patient.abiScore.toFixed(2)} • Rutherford {patient.rutherfordClass}
 </div>
 </div>
 </div>
