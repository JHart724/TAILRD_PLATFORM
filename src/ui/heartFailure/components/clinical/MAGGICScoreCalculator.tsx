...
const MAGGICScoreCalculator: React.FC<{ patientData?: PatientContext }> = ({ patientData }) => {
  const [inputs, setInputs] = useState<MAGGICInputs>({
 age: patientData?.age ?? 65,
 gender: patientData?.gender ?? 'male',
 ef: patientData?.lvef ?? 35,
 nyhaClass: patientData?.nyhaClass ?? 2,
 creatinine: patientData?.creatinine ?? 1.2,
 diabetes: patientData?.diabetes ?? false,
 copd: patientData?.copd ?? false,
 hfDuration: 12,
 systolicBP: patientData?.systolicBP ?? 120,
 bmi: patientData?.bmi ?? 25,
 smoker: false,
 aceiOrArb: (patientData?.onACEiARB || patientData?.onARNi) ?? true,
 betaBlocker: patientData?.onBetaBlocker ?? true
  });
 ...
 <div>
 <h2 className="text-2xl font-bold text-titanium-900 font-sf">MAGGIC Score Calculator</h2>
 <p className="text-titanium-600">Meta-Analysis Global Group in Chronic Heart Failure Risk Score</p>
 </div>
 </div>
 ...
 <div>
 <label className="block text-sm font-medium text-titanium-700 mb-2">HF Duration (months)</label>
 <input
 type="number"
 ...
 />
 <span className="text-sm font-medium text-titanium-700">Diabetes Mellitus</span>
 </label>
 ...
 <div>&bull; Low: &lt;10% 1-year mortality</div>
 <div>&bull; Intermediate: 10-20% 1-year mortality</div>
 <div>&bull; High: 20-40% 1-year mortality</div>
 <div>&bull; Very High: &gt;40% 1-year mortality</div>
 </div>
 </div>
 </div>
 </div>
