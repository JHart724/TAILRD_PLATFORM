...
const HFPhenotypeClassification: React.FC<{ patientData?: PatientContext }> = ({ patientData: ctx }) => {
  const [inputs, setInputs] = useState<PhenotypeInputs>({
 hemoglobin: ctx?.hemoglobin ?? 12.0,
 ferritin: ctx?.ferritin ?? 50,
 transferrinSaturation: ctx?.transferrinSaturation ?? 18,
 ahiScore: 0,
 sleepStudyDone: false,
 snoring: true,
 obesityBMI: ctx?.bmi ?? 28,
 ...
 <div>
 <h2 className="text-2xl font-bold text-titanium-900 font-sf">HF Phenotype Classification</h2>
 <p className="text-titanium-600">Iron Deficiency & Sleep Apnea Assessment</p>
 </div>
 </div>
 ...
 <div>
 <label className="block text-sm font-medium text-titanium-700 mb-2">Ferritin (ng/mL)</label>
 <input
 type="number"
 ...
 <label className="block text-sm font-medium text-titanium-700 mb-2">Transferrin Saturation (%)</label>
 <input
 type="number"
 ...
 <div>
 <label className="block text-sm font-medium text-titanium-700 mb-2">AHI Score</label>
 <input
 type="number"
 ...
 <label className="flex items-center space-x-3 p-3 bg-titanium-50 rounded-lg cursor-pointer">
 <input
 type="checkbox"
 ...
 <Heart className="w-5 h-5 text-porsche-600 mt-0.5 flex-shrink-0" />
