...
const ProviderScorecard: React.FC = () => {
 ...
 return <div className="w-4 h-4 bg-titanium-400 rounded-full"></div>;
  ...
  // Also show expanded metrics for the panel provider when side panel is open
  const displayProviderData = selectedProviderData || (showPatientPanel ? providerData.find(p => p.id === selectedProviderForPanel) : null);
 ...
 className={`p-4 rounded-xl border-2 transition-all duration-300 text-left group relative cursor-pointer ${
 ...
 {/* Hover hint */}
 <div className="flex items-center justify-between text-xs text-porsche-600 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-200 mb-1">
 <span>Click to view patients</span>
 ...
 <div className="text-2xl font-bold">{toFixed(provider.gdmtRate, 1)}%</div>
 ...
 <div className="mt-3 pt-3 border-t border-titanium-200 flex items-center justify-between">
 <span className="text-sm text-titanium-600">
 <Users className="w-4 h-4 inline mr-1" />
 {providerPatients[provider.id]?.length || 0} patients on file
 </span>
 <ChevronRight className="w-4 h-4 text-titanium-400 group-hover:text-porsche-500 transition-colors" />
 </div>
 </button>
 ))}
 </div>
 ...
 {displayProviderData.name}
 </h3>
 <p className="text-titanium-600">{displayProviderData.title}</p>
 </div>
 <div className="flex items-center gap-2">
 <Award className="w-5 h-5 text-porsche-600" />
 <span className="text-lg font-bold text-porsche-600">
 Rank #{sortedProviders.findIndex(p => p.id === displayProviderData.id) + 1}
 </span>
 </div>
 </div>
 ...
 <div className="text-3xl font-bold text-titanium-900">{displayProviderData.patients}</div>
 <div className="text-sm text-titanium-600 mt-1">
 {displayProviderData.gdmtOptimal} on optimal GDMT
 </div>
 </div>

 <div className="p-4 bg-white rounded-lg">
 <div className="text-sm text-titanium-600 mb-1">GDMT Adoption</div>
 <div className="text-3xl font-bold text-porsche-600">
 {toFixed(displayProviderData.gdmtRate, 1)}%
 </div>
 <div className="text-sm text-titanium-600 mt-1">
 vs {toFixed(sortedProviders.reduce((sum, p) => sum + p.gdmtRate, 0) / sortedProviders.length, 1)}% avg
 </div>
 </div>
 ...
 <div>
 <div className="text-sm text-titanium-600 mb-1">Average GDMT Rate</div>
 <div className="text-lg font-bold text-titanium-900">
 {toFixed(sortedProviders.reduce((sum, p) => sum + p.gdmtRate, 0) / sortedProviders.length, 1)}%
 </div>
 <div className="text-sm text-titanium-600">
