 ...
 * GET /valvular-disease/dashboard emits gap/patient counts only. Typed here so
 * ValvularExecutiveView can drop `data: any`.
 */
export interface VHDDashboardData {
  summary: {
...
/**
 * VHD Executive summary KPI row (AUDIT-304 VHD convergence; mirrors CADExecutiveSummary).
 *
 ...
 * device candidates) and DROPS the config.kpiData fallback (a fabricated number behind
 * a live field is the AUDIT-099 core defect). Built fresh - NOT the unrendered AUDIT-152
 * ValvularExecutiveKPICard copy. DEMO cards (revenue / at-risk / captured) come from the
 * single vhdDemoFinancials model and each carry a Demo pill. Live reads are guarded
 * (?./?? '-') so a partial summary renders honest dashes, never crashes.
 */
...
const KPI_DEFS: KPICardDef[] = [
  { id: 'total-patients', label: 'Total VHD Patients', icon: Users, color: 'blue', live: true },
  ...
  { id: 'device-candidates', label: 'Device Therapy Candidates', icon: Activity, color: 'orange', live: true },
  {
    ...
    color: 'green',
    live: false,
    demoValue: `$${VHD_DEMO_ANNUAL_OPPORTUNITY_M.toFixed(1)}M`,
    ...
    trendData: [
      ...
      { month: 'Sep', value: 5.1 },
      { month: 'Oct', value: 5.2 },
      { month: 'Nov', value: VHD_DEMO_ANNUAL_OPPORTUNITY_M },
    ],
  },
  {
    id: 'at-risk',
  ...
  },
  {
    ...
    color: 'copper',
    live: false,
    demoValue: `$${VHD_DEMO_YTD_CAPTURED_M.toFixed(1)}M`,
    demoSubtext: '36% of opportunity (demo model)',
    trend: 9,
    trendData: [
      { month: 'Jun', value: 1.4 },
      { month: 'Jul', value: 1.5 },
      ...
      { month: 'Sep', value: 1.7 },
      { month: 'Oct', value: 1.8 },
      { month: 'Nov', value: VHD_DEMO_YTD_CAPTURED_M },
    ],
  },
];

interface VHDExecutiveSummaryProps {
  dashboard: VHDDashboardData | null;
  loading?: boolean;
  error?: string | null;
}

export const VHDExecutiveSummary: React.FC<VHDExecutiveSummaryProps> = ({ dashboard, loading = false, error = null }) => {
  ...
  // Live values from the VHD dashboard contract; '-' while unavailable, never a
  // fabricated placeholder number (closes the VHD-side AUDIT-099 dead-error defect).
  const liveValues = (id: string): { value: string; subtext: string } => {
    ...
    switch (id) {
      case 'total-patients':
        return { value: s.totalPatients?.toLocaleString() ?? '-', subtext: 'Active valve panel (database)' };
      case 'open-gaps':
    ...
    <>
      <div className="metal-card border-2 border-titanium-200 p-6 mb-6">
        <h2 className="text-xl font-bold mb-6 flex items-center">
          ...
          Valvular Disease Executive Summary
        </h2>
              ...
              <>
                ...
                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                  <p className="text-sm text-gray-700">
                    Illustrative demo trend from the VHD demo financial model - not derived from
