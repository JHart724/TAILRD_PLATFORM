...
const ValvePhenotypeClassification: React.FC<{ patientData?: PatientContext }> = ({ patientData }) => {
 ...
 : risk === 'Moderate' ? 'text-amber-600 bg-amber-50 border-amber-200'
 ...
 <ul className="space-y-1">{d.recommendations.map((r,idx)=>(<li key={idx} className="text-sm text-titanium-700 flex items-start gap-2"><span className="text-porsche-500 mt-1">{'\u2022'}</span>{r}</li>))}</ul>
 </div>))}
 </div>
 )}
 </div>
