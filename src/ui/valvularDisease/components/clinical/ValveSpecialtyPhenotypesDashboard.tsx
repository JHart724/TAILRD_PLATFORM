...
const ValveSpecialtyPhenotypesDashboard: React.FC = () => {
 ...
 <span key={index} className="px-2 py-1 bg-porsche-100 text-porsche-700 text-xs rounded">{trigger}</span>
 ))}
 ...
 <div className="flex flex-wrap gap-1">
 {phenotype.interventions.slice(0, 3).map((intervention, index) => (
 <span key={index} className="px-2 py-1 bg-green-100 text-green-700 text-xs rounded">{intervention}</span>
 ))}
 </div>
 </div>
 </div>
 ...
 ${phenotype.revenueGap >= 1000000 ? `${(phenotype.revenueGap / 1000000).toFixed(1)}M` : `${(phenotype.revenueGap / 1000).toFixed(0)}K`}
 </div>
 ...
 <button className="p-4 border-2 border-amber-200 rounded-lg hover:bg-amber-50 transition-colors"
