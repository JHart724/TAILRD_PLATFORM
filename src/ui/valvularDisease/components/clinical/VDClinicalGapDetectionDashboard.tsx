...
const renderHALTAlert = (pt: VDGapPatient) => {
  ...
  return (
    <div className="mt-3 bg-chrome-50 border-2 border-titanium-300 rounded-xl p-3 space-y-2">
...
// COMPONENT
// ============================================================
interface VDCategoryFilter {
  label: string;
  keywords: string[];
}

interface VDClinicalGapDetectionDashboardProps {
  categoryFilter?: VDCategoryFilter;
}

const VDClinicalGapDetectionDashboard: React.FC<VDClinicalGapDetectionDashboardProps> = ({ categoryFilter }) => {
  const [expandedGap, setExpandedGap] = useState<string | null>(null);
  const { trackGapView, gapActions } = useGapActions('VALVULAR_DISEASE');
  ...
  const { data: moduleDashboard, loading: dashboardLoading } = useModuleDashboard('valvular-disease');

  // Try to fetch real gap data from backend; fall back to mock data
  useEffect(() => {
    let cancelled = false;
    fetchModuleGapsFromApi('valvular-disease').then(gaps => {
  ...
  const activeGaps: VDClinicalGap[] = dataSource === 'api' && apiGaps
    ? apiGaps.map(g => ({
        ...g,
  ...
  const safetyGaps = activeGaps.filter(g => g.category === 'Safety');
      ...
      <div className="metal-card bg-white border border-titanium-200 rounded-2xl p-6">
        <h3 className="text-lg font-semibold text-titanium-900 mb-1 flex items-center gap-2">
          ...
          {categoryFilter
            ...
            : 'Clinical Gap Detection \u2014 Valvular Disease Module'}
        </h3>
        {!categoryFilter && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
            ...
            <div className="text-2xl font-bold text-blue-800">{categoryFilter ? filteredGaps.length : activeGaps.length}</div>
          </div>
