...
const ValvularSurgicalNetworkVisualization: React.FC = () => {
 ...
 case 'echo-evaluation': return '#B8922E';
 case 'heart-team-decision': return '#8B6914';
 case 'surgical-planning': return '#8B6914';
 case 'tavr-pathway': return '#6B4A1A';
 ...
 <div>
 <h3 className="text-2xl font-bold text-titanium-900">Valvular Disease Surgical Planning Network</h3>
 <p className="text-titanium-600">Echo → Cath → Surgery decision pathways and Heart Team coordination</p>
 ...
 <Users className={`w-6 h-6 ${metrics.heartTeamUtilization >= 0.85 ? 'text-teal-700' : 'text-gray-500'}`} />
 <div>
 ...
 <div className="bg-white p-4 rounded-xl border border-titanium-200">
 <div className="flex items-center gap-3">
 <Timer className="w-6 h-6 text-chrome-600" />
 <div>
 <div className="text-lg font-bold text-titanium-900">{metrics.avgTimeToIntervention}d</div>
 <div className="text-xs text-titanium-600">Time to Procedure</div>
 </div>
 </div>
 </div>
 ...
 <div>
 <div className="text-lg font-bold text-titanium-900">{toFixed(metrics.surgicalVsTranscatheterRate, 1)}:1</div>
 <div className="text-xs text-titanium-600">SAVR:TAVR Ratio</div>
 </div>
 </div>
 </div>
 ...
 <div>
 <div className="text-lg font-bold text-titanium-900">{toFixed(metrics.riskAssessmentAccuracy * 100, 0)}%</div>
 <div className="text-xs text-titanium-600">Risk Accuracy</div>
 </div>
 </div>
 </div>
 ...
 <div className="text-xs text-titanium-600">Total Patients</div>
 </div>
 </div>
 </div>
 </div>
 ...
 width="700"
 height="520"
 className="border border-titanium-100 rounded-lg bg-gradient-to-br from-gold-50 to-white"
 >
 {/* Render connections */}
 ...
 textAnchor="middle"
 className="text-xs fill-gold-700 font-bold"
 style={{ fontSize: '10px' }}
 >
 ...
 style={{ fontSize: '8px' }}
 >
 {node.patientVolume ? node.patientVolume : `${toFixed(node.conversionRate! * 100, 0)}%`}
 </text>
 )}
 </g>
 );
 ...
 <div className="flex items-center gap-2">
 <div className="w-4 h-1 bg-gold-600"></div>
 ...
 <div className="space-y-2">
 <div className="flex items-center gap-2">
 <div className="w-4 h-1 bg-gold-700"></div>
 ...
 <div className="text-lg font-bold text-gold-600">{selectedNode.patientVolume}</div>
 <div className="text-xs text-gold-700">Patient Volume</div>
 </div>
 )}
 {selectedNode.conversionRate && (
 <div className={`p-3 rounded-lg ${selectedNode.conversionRate >= 0.8 ? 'bg-titanium-300' : 'bg-chrome-50'}`}>
 <div className={`text-lg font-bold ${selectedNode.conversionRate >= 0.8 ? 'text-teal-700' : 'text-gray-500'}`}>
 {toFixed(selectedNode.conversionRate * 100, 0)}%
 </div>
 <div className={`text-xs ${selectedNode.conversionRate >= 0.8 ? 'text-teal-700' : 'text-gray-500'}`}>
 Conversion Rate
 </div>
 </div>
 )}
 ...
 <div className="text-lg font-bold text-chrome-600">{selectedNode.timeToEvaluation}d</div>
 <div className="text-xs text-chrome-700">Time to Eval</div>
 </div>
 )}
 {selectedNode.timeToIntervention && (
 <div className="bg-chrome-50 p-3 rounded-lg">
 <div className="text-lg font-bold text-chrome-600">{selectedNode.timeToIntervention}d</div>
 <div className="text-xs text-chrome-700">Time to Procedure</div>
 </div>
 )}
 {selectedNode.successRate && (
 <div className="bg-chrome-50 p-3 rounded-lg">
 <div className="text-lg font-bold text-teal-700">{toFixed(selectedNode.successRate * 100, 1)}%</div>
 <div className="text-xs text-teal-700">Success Rate</div>
 </div>
 )}
 {selectedNode.avgStsScore && (
 <div className="bg-red-50 p-3 rounded-lg">
 <div className="text-lg font-bold text-red-600">{toFixed(selectedNode.avgStsScore, 1)}%</div>
 ...
 <div className="text-lg font-bold text-teal-700">{toFixed(selectedNode.complianceRate * 100, 0)}%</div>
 <div className="text-xs text-teal-700">Compliance</div>
 </div>
 )}
 {selectedNode.complicationRate && (
 <div className="bg-chrome-50 p-3 rounded-lg">
 <div className="text-lg font-bold text-gray-500">{toFixed(selectedNode.complicationRate * 100, 1)}%</div>
 <div className="text-xs text-gray-500">Complications</div>
 </div>
 )}
 </div>
 ...
 {conn.avgTimeToNext && (
 <span className={`text-xs px-1 rounded ${conn.avgTimeToNext <= 7 ? 'bg-green-50 text-green-600' : 'bg-amber-50 text-amber-600'}`}>
 ...
 <div className="bg-white p-6 rounded-xl border border-titanium-200">
 <h4 className="font-semibold text-titanium-900 mb-4 flex items-center gap-2">
 <Gauge className="w-5 h-5 text-gold-600" />
 Surgical Planning Efficiency
 </h4>
 <div className="space-y-3">
 <div className="flex justify-between items-center">
 <span className="text-titanium-600">Heart Team Utilization</span>
 <span className={`font-bold ${metrics.heartTeamUtilization >= 0.85 ? 'text-teal-700' : 'text-gray-500'}`}>
 {toFixed(metrics.heartTeamUtilization * 100, 1)}%
 </span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-titanium-600">Time to Intervention</span>
 <span className="font-bold text-chrome-600">{metrics.avgTimeToIntervention} days</span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-titanium-600">Risk Assessment Accuracy</span>
 <span className="font-bold text-gold-600">{toFixed(metrics.riskAssessmentAccuracy * 100, 0)}%</span>
 </div>
 </div>
 </div>
 ...
 <div className="space-y-3">
 <div className="flex justify-between items-center">
 <span className="text-titanium-600">SAVR Procedures</span>
 <span className="font-bold text-gold-600">{metrics.savrVolume}</span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-titanium-600">TAVR Procedures</span>
 <span className="font-bold text-chrome-600">{metrics.tavrVolume}</span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-titanium-600">Overall Success Rate</span>
 <span className="font-bold text-teal-700">{toFixed(metrics.overallSurgicalSuccessRate * 100, 1)}%</span>
 </div>
 </div>
 </div>
 ...
 <span className="font-bold text-teal-700">{toFixed(metrics.postOpSurveillanceCompliance * 100, 1)}%</span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-titanium-600">Long-term Patients</span>
 <span className="font-bold text-chrome-600">{metrics.longTermSurveillance}</span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-titanium-600">SAVR:TAVR Ratio</span>
 <span className="font-bold text-gold-600">{toFixed(metrics.surgicalVsTranscatheterRate, 1)}:1</span>
 </div>
 </div>
 </div>
 </div>
