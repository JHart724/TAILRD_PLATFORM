...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const hasModuleAccess = useCallback((moduleKey: string): boolean => {
    if (isDemoMode) return true;
    if (!state.user?.backendPermissions) return false;
    return !!(state.user.backendPermissions.modules as any)[moduleKey];
  }, [state.user]);

  const hasViewAccess = useCallback((viewKey: string): boolean => {
    if (isDemoMode) return true;
    if (!state.user?.backendPermissions) return false;
    return !!(state.user.backendPermissions.views as any)[viewKey];
  }, [state.user]);
