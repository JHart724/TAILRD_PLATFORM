...
export const usePerformanceOptimization = () => {
 ...
 style.textContent = `
 .transition-all duration-200 hover:shadow-chrome-card-hover hover:-translate-y-0.5,
 .floating-screen,
 .retina-card,
 .animate-pulse,
 .animate-float,
 ...
 if (style.textContent?.includes('transition-all duration-200 hover:shadow-chrome-card-hover hover:-translate-y-0.5')) {
 style.remove();
 }
 });
 };
 ...
 const cards = document.querySelectorAll('.retina-card, .transition-all duration-200 hover:shadow-chrome-card-hover hover:-translate-y-0.5');
 cards.forEach(card => {
