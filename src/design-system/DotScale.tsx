...
const DotScale: React.FC<DotScaleProps> = ({
  ...
  const formatDisplayValue = (v: number): string => {
    ...
    if (unit === 'percentile') return `${v.toFixed(0)}th`;
    return v.toFixed(1);
  };
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm font-body font-medium text-chrome-300">{label}</span>
        <span className="text-sm font-data font-semibold text-chrome-100">
