...
- [/] Phase 2 — Security posture (Tier S 3.5 of 4 closed; AUDIT-011 Phase a-pre RESOLVED; AUDIT-022 RESOLVED 2026-05-07; **AUDIT-016 RESOLVED 2026-05-07** — full implementation arc complete (PR 1 #255 envelope schema + V1 emission + AUDIT-017 bundle; PR 2 #258 V2 emission + kmsService wiring + per-record EncryptionContext + 27 net new tests + AUDIT-076 partial closure +2 promotions; PR 3 this PR — migrateRecord + V0/V1 → V2 migration script + 39 net new tests + 9-section operator runbook + 8th §17.1 architectural-precedent (raw SQL bypass to prevent middleware double-encrypt)); **AUDIT-071 RESOLVED 2026-05-07** + AUDIT-073 bundled per §17.3 + AUDIT-076 partial closure (4 HIPAA-grade promotions); AUDIT-024 LOW + AUDIT-025 MEDIUM + AUDIT-077 LOW remain)
...
**Phase 0B clinical-code verification arc materially COMPLETE (2026-05-06).** **HIPAA-foundations arc COMPLETE (2026-05-07).** **Phase 3 data-layer audit COMPLETE (2026-05-07; CONDITIONAL PASS — production posture NOT production-ready today).** **AUDIT-071 RESOLVED (2026-05-07; first production-readiness gate item closed).** **AUDIT-016 RESOLVED (2026-05-07; full PHI key rotation arc complete — PR 1 #255 + PR 2 #258 + PR 3 this PR — V0/V1/V2 envelope schema + V1 + V2 emission paths + V0/V1 → V2 migration script + 9-section operator runbook).**

**Top 3 priorities — production-readiness immediate-remediation arc** (data-state-independent; PHI may arrive any day):

1. **AUDIT-011 Phase b/c/d (Layer 3 Prisma extension; ~9-11h)** — IMMEDIATE next work block now that AUDIT-016 RESOLVED. AUDIT-071 demonstrated app-layer `where: { hospitalId }` discipline alone is structurally insufficient; Layer 3 is the structural backstop.
2. **AUDIT-075 PHI encryption coverage (errorMessage / description / notes; ~4-8h)** — production-readiness gate item; sister to AUDIT-018/019. Note: AUDIT-075 PR will trigger AUDIT-016 PR 3 migration script re-run on newly-covered columns (sister-run requirement documented in `docs/runbooks/AUDIT_016_PR_3_MIGRATION_RUNBOOK.md` §6.2).
3. **AUDIT-078 Aurora backup IaC + restore-test (~6-10h)** — production-readiness gate item; operator-side ops PR.
