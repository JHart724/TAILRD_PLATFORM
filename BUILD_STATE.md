...
1. **AUDIT-075 PHI encryption coverage (errorMessage / description / notes; ~4-8h)** — IMMEDIATE next work block now that AUDIT-011 Phase b/c shipped. Production-readiness gate item; sister to AUDIT-018/019. Note: AUDIT-075 PR will trigger AUDIT-016 PR 3 migration script re-run on newly-covered columns (sister-run requirement documented in `docs/runbooks/AUDIT_016_PR_3_MIGRATION_RUNBOOK.md` §6.2).
2. **AUDIT-078 Aurora backup IaC + restore-test (~6-10h)** — production-readiness gate item; operator-side ops PR.
3. **AUDIT-080 Zod validation coverage (~12-20h)** — phased rollout; production-readiness gate item.
